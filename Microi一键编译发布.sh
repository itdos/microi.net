...
ENCRYPTED_PROJECTS=("Microi.net" "Microi.AI" "Microi.WorkFlow" "Microi.Vision")
#
...
if [ -d "Microi.Server/Microi.net" ] && [ -d "Microi.Server/Microi.AI" ] && [ -d "Microi.Server/Microi.WorkFlow" ] && [ -d "Microi.Server/Microi.Vision" ]; then
    if [ -f "$ENCRYPT_SCRIPT" ]; then
        HAS_ENCRYPT=true
        print_success "DLL 加密: 可用（检测到 Microi.net + Microi.AI + Microi.WorkFlow + Microi.Vision 源码）"
    else
    ...
    while IFS= read -r csproj_file; do
    ...
    done < <(
        {
            ...
            for closed_source_project in \
                ...
                "Microi.Server/Microi.WorkFlow/Microi.WorkFlow.csproj" \
                "Microi.Server/Microi.Vision/Microi.Vision.csproj"; do
...
if [ "$HAS_ENCRYPT" = true ]; then
    print_divider
    print_step "加密 DLL（Microi.net.dll + Microi.AI.dll + Microi.WorkFlow.dll + Microi.Vision.dll）..."
...
if [ "$PUSH_NUGET" = true ]; then
    ...
    if [ "$HAS_ENCRYPT" = true ] && [ "$DLL_ENCRYPTED" != true ]; then
        print_fail "检测到 Microi.net/Microi.AI/Microi.WorkFlow/Microi.Vision 源码但 DLL 未加密，禁止推送未加密的 NuGet 包！"
    fi
...
if [ ${#SELECTED_API_PLANS[@]} -gt 0 ] || [ ${#SELECTED_CLIENT_PLANS[@]} -gt 0 ]; then
    ...
    if [ ${#SELECTED_API_PLANS[@]} -gt 0 ] && [ "$HAS_ENCRYPT" = true ] && [ "$DLL_ENCRYPTED" != true ]; then
        print_fail "检测到 Microi.net/Microi.AI/Microi.WorkFlow/Microi.Vision 源码但 DLL 未加密，禁止推送未加密的 Docker 镜像！"
    fi
