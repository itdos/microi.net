...
- 根因：插件只读取进程首次初始化的静态授权状态；首次未找到授权文件后，后续请求不再重新查找和验签，形成永久的假未授权状态。只修共享静态缓存仍不充分：插件与宿主可能因发布产物或程序集上下文不同继续观察到分叉状态。
- 通用规则：所有付费能力必须优先复用统一授权入口。缓存为未授权时，应限频重新读取本机 License 文件并完整执行 HID、有效期和 RSA 签名验证；如果宿主授权接口与插件判断不一致，插件必须在自身执行上下文内复核同一签名文件，并在服务端日志输出宿主/插件程序集版本。只能由有效签名把状态恢复为个人版或企业版，禁止信任前端展示值或数据库中的产品类型文本直接放行。
- 自动化检查：先让宿主与插件在 License 文件不存在时完成初始化，再写入或挂载有效签名文件；即使共享状态仍处于限频未授权窗口，插件自身也应验签恢复付费能力。无效签名、错误 HID、过期文件必须继续拒绝，并验证线上授权接口与真实付费功能，而不是只检查授权页面。
