...
- 根因：插件只读取进程首次初始化的静态授权状态；首次未找到授权文件后，后续请求不再重新查找和验签，形成永久的假未授权状态。
- 通用规则：所有付费能力必须复用统一授权入口。缓存为未授权时，应限频重新读取本机 License 文件并完整执行 HID、有效期和 RSA 签名验证；只能由有效签名把状态恢复为个人版或企业版，禁止信任前端展示值或数据库中的产品类型文本直接放行。
- 自动化检查：先让进程在 License 文件不存在时完成初始化，再写入或挂载有效签名文件并调用付费能力；无需重启即可恢复授权。无效签名、错误 HID、过期文件必须继续拒绝。
