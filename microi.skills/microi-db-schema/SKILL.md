...
- 子表外键必须建立与物理租户隔离方式一致的回查索引：物理表存在 `OsClient` 时通常为
  `(OsClient, ParentId)`，独立租户物理表没有 `OsClient` 时为 `(ParentId)`。结构或索引读取失败
  必须拒绝关系配置，不能为了通过校验新增不存在业务语义的 `OsClient` 列。子表还要有绑定同一子表的隐藏菜单，
