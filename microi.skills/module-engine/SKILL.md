...
顶层 PC 数据列表默认使用紧凑的新模块标题样式；即使未启用 ViewSchema，也不能退回无标题的旧外观。无指标头部固定 `44px`、含指标头部固定 `58px`，连同间距总纵向占用约 `50px / 64px`。子表、关联表、嵌入表不重复显示，移动端由固定导航栏承载标题。`ViewSchema` 只负责个性化标题、指标、复合列和卡片，不改变 Detail/Edit 的显式启用语义。
