...
相机扫描、收银称台、扫码、签到等高频工具页必须另设“一屏操作”断点：在宿主下发的可用高度内同时显示输入/取景、主操作、处理状态和核心结果，完成主流程不得依赖纵向滚动；竖屏优先上下紧凑分区，短横屏切换左右分区。可折叠次要说明、历史记录和低优先级明细，但不能隐藏错误、当前结果、暂停/继续/重试等关键动作。真实浏览器验收至少覆盖一个常见手机竖屏和一个短横屏，并断言根容器不溢出、取景区与结果区同时在视口内；桌面断点必须回归，不能为移动端压缩破坏 PC 布局。
