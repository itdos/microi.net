...
- MCP 对应工具为 `microi_generate_minimax_image`、`microi_get_minimax_image_task`、`microi_recover_minimax_image_task`。生成工具未提供 `confirmExecution=requestId` 时只返回 dry-run；确认后接受 `Code=2` 排队结果。状态工具只读同一 TaskId；恢复工具仅在 `CanRecoverResult=true` 时重新落盘既有结果，禁止重新生成。工具不接收供应商密钥、Endpoint、Authorization 或租户覆盖，宽高必须成对出现，平台发布前仍须回读当前租户 HDFS 地址和文件摘要。
...
- 图片与音乐都使用稳定 `RequestId` 和共享幂等；相同参数回放、参数冲突拒绝，上游结果不确定时禁止换 Id 盲重试。音乐当前仅向平台管理员开放且固定无人声；托管结果为 MP3，开源回退为 WAV、10～60 秒；采样率、声道、码率和时长以实际文件头为准。成功内容由服务端校验后直接写入当前租户公有 HDFS。
- `GenerateMiniMaxMusic`（HTTP 与 V8）持久入队后返回 `Code=2 + TaskId`；使用 `GET /api/Ai/GetMiniMaxMusicTask?taskId=...` 或 `V8.AI.GetMiniMaxMusicTask(taskId)` 查询，只有 `Code=1 + Permanent=true + FileUrl` 是成功。任务复用共享数据库唯一键、租约和原生音乐 Worker；HTTP 断连不取消生成，前端按当前 API、租户、用户保存原任务并恢复查询。图片与音乐的中转等待任务合并计算容量，必须为供应商执行任务保留槽位。
- 音乐中转使用 `POST /v1/microi/music_tasks`、`GET /v1/microi/music_tasks/{taskId}` 和平台 API Key，提交携带稳定 `Idempotency-Key`；客户端节点和中转节点均需更新。官方 Music3 事件编号必须在读取 SSE 前写入共享 Redis，完成文件地址在下载前保存，整个事件流受总时限约束。存在有效原回执时才允许 `POST /api/Ai/RecoverMiniMaxMusicTask?taskId=...`（V8 同名方法）或中转 `/v1/microi/music_tasks/{taskId}/recover` 恢复；不得接受新提示词或再提交供应商生成。没有原回执的旧 `Uncertain` 不能自动修复为成功。应用包更新不替代前后端和中转二进制升级。
