...
- 生成或修改 V8 事件代码时必须是可读的多行格式，禁止把整段事件逻辑压成一行、禁止生成 minified 风格代码。
- 中文业务提示、返回 `Msg`、`V8.Tips`、注释和表单文案必须直接写正常中文，禁止输出 `\u64cd\u4f5c\u6210\u529f` 这类 Unicode 转义串。
- 保存前必须做 JS 语法检查；如果使用脚本生成事件代码，必须先格式化再写入本地文件/数据库。
- 代码头只写当前事件功能说明；修改历史写到平台记录或外部 ChangeHistory，不能塞进代码头。
