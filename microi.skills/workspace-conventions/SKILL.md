...
- 必须为重任务保留至少 `max(6 GB, 物理内存的 20%)` 给 VS Code、Codex 和操作系统。机器总内存占用达到 75% 或可用内存低于该保留值时，立即终止 AI 启动的重任务及其子进程，不得等待 OOM。
