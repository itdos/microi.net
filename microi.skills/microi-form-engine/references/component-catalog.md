...
- `Limit=true` 表示字段配置为私有桶，`false` 表示配置为公有桶。普通用户有当前表单新增/编辑权限时也按此权威配置执行；运行时提交表、字段、菜单、记录上下文，后端回读 `diy_field.Config`，不得信任客户端 `Limit/Path`，也不得按用户等级统一强制私有。
...
- 普通用户经当前表单新增/编辑授权后可以发布公有 RichText；后端按字段权威配置选择桶。缺少 `FormEngineKey/FieldId` 等可验证上下文的普通上传仍保持私有，不能用裸 `Limit=false` 代替授权。
