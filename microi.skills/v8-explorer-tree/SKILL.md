...
`microi-v8-engine/` 下的接口引擎、表单 V8 事件、字段 V8 事件、模块按钮代码不是普通源码文件，而是远端数据库代码的本地镜像。任何 AI 或插件改动必须闭环到远端，不能只改本地文件。

**AI 修改 V8 文件时必须执行：**
1. 改动前确认目标 server / OsClient 与当前 MCP 或插件连接一致，不能用一个租户的 MCP 写另一个租户。
2. 改动前优先拉取或比对远端当前代码；如果已有本地改动，先判断是否为用户改动，不可盲目覆盖。
3. 本地修改并通过语法 / 单元 / 接口级校验后，立即调用对应保存能力（接口引擎用 `microi_save_engine_code`，事件和按钮用插件/MCP对应保存接口）写回远端。
4. 保存成功后重新查看同步状态；目标文件不得继续显示“本地代码与远端不一致”。如果同步状态仍不一致，必须继续处理或明确说明阻塞原因。
5. 任务完成回复中要说明哪些引擎/事件已同步；不能只说“本地已修改”。

**编码回读校验（2026-05 补充）：**
- VS Code 插件同步状态以远端 `ApiV8Code` 回读结果为准；保存接口返回成功不等于同步完成。
- 若 `UpdateApiEngineCode/CreateApiEngine` 保存后，远端回读把中文变成 `���` 导致仍显示不一致，不要反复盲推中文源码。对该引擎代码做 ASCII Unicode 转义（如字符串中的中文转成 `\u4e2d\u6587`），再保存远端并写回本地文件、更新 `.microi-meta.json.updateTime`、设置本地文件 `mtime` 为远端 `UpdateTime`。
- Unicode 转义后的字符串运行时仍会还原为中文，但本地与远端均为 ASCII，可避开旧引擎/历史字段的编码回读差异。
- 最终必须用等价于插件“查看同步状态”的检查确认 `localModifiedCount=0` 且 `remoteDiffCount=0`。

**禁止行为：**
- 禁止把大量未知差异一次性本地覆盖远端；必须按业务相关文件逐个确认。
- 禁止只运行构建、不保存远端就结束 V8 任务。
- 禁止用旧本地文件覆盖远端新增逻辑；冲突时先保留远端并做最小合并。
- 禁止在未验证同步状态的情况下要求用户“再点同步试试”。
...
| I8 | 本地 V8 文件改动必须同步远端且复查同步状态 | 本地/远端代码分叉，运行库仍执行旧逻辑 | 保存后调用“查看同步状态”或等价 MCP 校验，目标文件必须为 clean |
...
- AI 修改 `microi-v8-engine/` 后必须同步远端并复查同步状态；没有同步成功就不能标记任务完成
