...
note: 排查长任务时，严禁把“等待或执行超过1分钟”直接判定为失败；接口引擎、V8、表单写入、导入导出、批处理等合法长任务默认应支持10分钟以上，同步链路推荐10分钟，排队窗口推荐30分钟。
...
- 在队列有未完成工作、锁已取得、写库成功但响应未返回等时间点分别终止一个节点，再启动同一稳定 `MICROI_NODE_ID` 的节点，验证持久 spool/outbox 自动恢复、共享状态不损坏、其余节点持续服务。
