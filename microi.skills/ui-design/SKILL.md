...
<!-- microi-progressive:chunk id=ui-design-000 sha256=ba3ee4e932e517b3ad893abb399b2d64322434e0581c06a31ec2032e47abb427 -->
## 整体风格定义
...
- 主按钮、下载按钮、CTA、标签式链接必须在组件自己的命名空间内显式声明默认、hover、focus-visible、active、disabled 的文字色、背景色和边框色；不得依赖父级文字色、全局 `a` 样式、浏览器默认色或透明背景上的偶然对比度。图标的 `color` / `stroke` / `fill` 必须跟随同一语义前景色。
- 普通文字与背景的对比度至少达到 4.5:1；大号文字至少达到 3:1；有意义的图标、边框和焦点指示至少达到 3:1。渐变、透明、毛玻璃和背景图片上的按钮，要按实际可出现的最差背景位置验收，不能只检查 CSS 变量名或设计稿色值。
- 每个关键按钮必须在真实渲染页逐一核对“按钮存在、完整文案可见、前景色和背景色可区分、hover/focus 后仍可读、点击区域未被遮挡”。自动化至少读取最终 computed style 并断言颜色不是透明或同色；视觉验收至少覆盖 1440px 桌面与约 390px 手机宽度的浅色、暗色截图。
