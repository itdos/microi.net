...
- `microi.app` 是 5+ 在线壳，不能假设存在 `uni` 或 Web Bluetooth。BLE 连接、发现和写入必须统一使用 `plus.bluetooth`，禁止用 `uni.writeBLECharacteristicValue` 写入由 `plus.bluetooth` 建立的连接。5+ `V8.Print.getConnectionState()` 的 `mtu/maxWriteBytes/recommendedPacketSize/writeType/packetIntervalMs` 描述本次连接能力。Android GP-M322 在特征支持时优先 `writeNoResponse`，即使协商出 180 字节有效载荷也只采用 100 字节连续发送稳定档，并保留约 8ms 包间保护；不支持无响应写时兼容 `write`，空 MTU 成功、失败、超时或旧壳只用 20 字节。详见蓝牙参考。
