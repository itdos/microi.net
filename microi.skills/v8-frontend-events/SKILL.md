...
- `microi.app` 是 5+ 在线壳，不能假设存在 `uni` 或 Web Bluetooth。BLE 连接、发现和写入必须统一使用 `plus.bluetooth`，禁止用 `uni.writeBLECharacteristicValue` 写入由 `plus.bluetooth` 建立的连接。Android 佳博确认写入逐包等待原生回调，不再追加人工等待；5+ `V8.Print.getConnectionState()` 的 `mtu/maxWriteBytes/recommendedPacketSize/writeType/packetIntervalMs` 描述本次连接能力。GP-M322 在本次连接 MTU 允许时默认采用 180 字节档；空 MTU 成功、失败、超时或旧壳只用 20 字节。详见蓝牙参考。
