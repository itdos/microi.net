  ...
  <mci-page-shell class="detail-page" :style="mciTokenStyle" :title="isPublic ? '处理公示' : '投诉详情'" :subtitle="isPublic ? '内容已脱敏并通过审核' : '处理进度与反馈时限'" @back="goBack">
    ...
    <scroll-view v-else class="detail-scroll" scroll-y>
      ...
      <view v-if="isPublic" class="metric-card">
        ...
        <view><text>{{ Number(complaint.IsOverdue) === 1 ? '是' : '否' }}</text><text>是否逾期</text></view>
      </view>
      ...
      <view v-if="!isPublic && Number(complaint.PublicConsent) === 1" class="section-card publication-card">
        <view class="section-head"><text>脱敏公示</text><text :class="publicStatusTone">{{ publicStatusLabel }}</text></view>
        <text class="publication-note">{{ publicStatusNote }}</text>
      </view>
      ...
      <view v-if="!isPublic" class="section-card timeline-card">
        ...
        <view v-for="(item,index) in actions" :key="item.Id || index" class="timeline-item">
          ...
          <view class="timeline-copy">
            <view><text>{{ item.ActionName }}</text><text>{{ item.CreateTime }}</text></view>
            <text>{{ item.Content }}</text>
            <text v-if="item.NextFeedbackDeadline">下次反馈截止：{{ item.NextFeedbackDeadline }}</text>
            <view v-if="actionImages(item).length || actionVideos(item).length || actionFiles(item).length" class="timeline-attachments">
              <text>本次附件</text>
              <mci-media-uploader v-if="actionImages(item).length" :model-value="actionImages(item)" media-type="image" readonly :file-context="detailFileContext" />
              <mci-media-uploader v-if="actionVideos(item).length" :model-value="actionVideos(item)" media-type="video" readonly :file-context="detailFileContext" />
              <mci-media-uploader v-if="actionFiles(item).length" :model-value="actionFiles(item)" media-type="file" readonly :file-context="detailFileContext" />
            </view>
          </view>
        </view>
...
export default{
  ...
  computed:{
    ...
    publicStatusLabel(){return{Pending:'待平台脱敏审核',Published:'已发布',Rejected:'公示审核未通过',None:'尚未进入审核'}[this.complaint.PublicStatus]||'尚未进入审核'},
    publicStatusTone(){return this.complaint.PublicStatus==='Published'?'publication-status publication-status--success':this.complaint.PublicStatus==='Rejected'?'publication-status publication-status--muted':'publication-status publication-status--pending'},
    publicStatusNote(){return this.complaint.PublicStatus==='Published'?'平台已完成脱敏审核，公示内容可在“处理公示”中查看。':this.complaint.PublicStatus==='Rejected'?'本次内容未通过公示审核，不会对外展示。':'同意脱敏公示表示授权平台在办结后进行审核；审核通过并发布前，不会出现在公开列表中。'}
  },
  ...
  methods:{
    ...
    actionAttachmentItems(item){return Array.isArray(item&&item.Attachments)?item.Attachments:[]},
    attachmentKind(item){const type=String(item&&item.ContentType||'').toLowerCase();const path=String(item&&item.Path||'').toLowerCase();if(type.startsWith('image/')||/\.(png|jpe?g|gif|webp|bmp|svg)$/.test(path))return'image';if(type.startsWith('video/')||/\.(mp4|mov|m4v|avi|mkv|webm)$/.test(path))return'video';return'file'},
    actionImages(item){return this.actionAttachmentItems(item).filter(file=>this.attachmentKind(file)==='image')},
    actionVideos(item){return this.actionAttachmentItems(item).filter(file=>this.attachmentKind(file)==='video')},
    actionFiles(item){return this.actionAttachmentItems(item).filter(file=>this.attachmentKind(file)==='file')},
