<template>
  <mci-page-shell class="device-page" :style="mciTokenStyle" title="设备处理" :subtitle="device.ShebeiBH || device.ShebeiXH || ''" @back="goBack">
    <template #right><view v-if="!isCompleted" class="draft-action" hover-class="draft-action--pressed" @tap="saveDraft"><text>存草稿</text></view></template>
    ...
    <scroll-view v-else class="device-scroll" :class="{ 'device-scroll--readonly': isCompleted }" scroll-y>
      <view v-if="!isCompleted && draftRestored" class="draft-tip"><view><text class="draft-tip__title">已恢复未提交草稿</text><text class="draft-tip__time">{{ draftTime }}</text></view><view @tap="discardDraft"><text>放弃草稿</text></view></view>
      ...
      <view class="device-summary">
        ...
        <text class="device-summary__status" :class="{ complete: isCompleted }">{{ device.FuwuZT || (isCompleted ? '已完成' : '未完成') }}</text>
      </view>
      ...
      <view class="section-band">
        ...
        <view v-show="sectionOpen.result" class="section-body">
          <textarea v-model="form.ChuliJG" class="result-textarea" :disabled="isCompleted" maxlength="1500" placeholder="请记录故障现象、处理过程、处理结果和后续建议" />
      ...
      <view class="section-band">
        ...
        <view v-show="sectionOpen.resultPhotos" class="section-body">
          <view class="upload-block"><mci-media-uploader v-model="form.JieguoTP" :max-count="9" :upload-path="uploadPath('result')" :file-context="photoFileContext('JieguoTP')" :readonly="isCompleted" /></view>
          <view v-if="!isCompleted" class="watermark-command" hover-class="watermark-command--pressed" @tap="openWatermark('JieguoTP')"><image src="/static/xjy/watermarkCamera/camera.png" mode="aspectFit" /><text>拍摄带时间、客户和位置水印的照片</text><text>›</text></view>
        </view>
      </view>
      ...
      <view v-if="isInstall" class="section-band">
        ...
        <view v-show="sectionOpen.installPhotos" class="section-body">
          <view v-for="photo in installPhotoFields" :key="photo.name" class="photo-category">
            ...
            <mci-media-uploader v-model="form[photo.name]" :max-count="photo.max" :upload-path="uploadPath(photo.name)" :file-context="photoFileContext(photo.name)" :readonly="isCompleted" />
          </view>
        </view>
      </view>
        ...
        <view v-show="sectionOpen.scene" class="section-body">
          <view class="site-field">
            <view class="site-field__heading">
              <text>安装位置</text>
              <view v-if="!isCompleted" class="site-field__location" :class="{ disabled: locating }" hover-class="site-field__location--pressed" @tap="locateDevice">
                <image src="/static/xjy/business/dw.png" mode="aspectFit" />
                <text>{{ locating ? '定位中…' : '现场定位' }}</text>
              </view>
            </view>
            <input v-model.trim="form.AnzhuangWZ" :disabled="isCompleted" maxlength="200" confirm-type="done" placeholder="定位后可补充楼层、房间或点位" />
            <text v-if="hasDeviceCoordinates" class="site-field__location-tip">已保存坐标，可继续修改补充安装位置文字</text>
          </view>
          <view class="photo-category"><view class="photo-category__heading"><text>现场环境照片</text><text>展示设备周边与施工环境</text></view><mci-media-uploader v-model="form.XianchangZP" :max-count="6" :upload-path="uploadPath('scene')" :file-context="photoFileContext('XianchangZP')" :readonly="isCompleted" /></view>
          <view class="photo-category"><view class="photo-category__heading"><text>服务视频</text><text>建议控制时长，降低上传等待</text></view><mci-media-uploader v-model="form.ShipinSC" media-type="video" :max-count="3" :upload-path="uploadPath('video')" :file-context="photoFileContext('ShipinSC')" :readonly="isCompleted" /></view>
        </view>
      </view>
      ...
      <view class="quality-note"><view></view><text>{{ isCompleted ? '该设备任务已完成，当前仅展示已提交的处理结果和现场资料。' : '提交后该设备将标记为已完成，并同步结果照片和安装分类照片到客户设备档案。' }}</text></view>
      <view class="device-spacer"></view>
    </scroll-view>

    <view v-if="!loading && !error && !isCompleted" class="bottom-bar"><view class="bottom-button bottom-button--plain" hover-class="bottom-button--pressed" @tap="saveDraft"><text>保存草稿</text></view><view class="bottom-button bottom-button--primary" :class="{ disabled: submitting }" hover-class="bottom-button--pressed" @tap="submit"><text>{{ submitting ? '正在提交' : '完成并提交' }}</text></view></view>
  </mci-page-shell>
</template>
...
const FILE_FIELDS = FORM_FIELDS.filter((field) => !TEXT_FIELDS.includes(field))
...
const EMPTY_FILE_CONTEXT = Object.freeze({})

function isTaskDeviceCompleted(device = {}) {
  return String(device.FuwuZTZ ?? '').trim() === '1' || String(device.FuwuZT || '').trim() === '已完成'
}
...
export default {
  ...
  data() {
    return {
      ...
      locationUpdated: false, fileContexts: {}, emptyFileContext: EMPTY_FILE_CONTEXT,
  ...
  computed: {
    ...
    isCompleted() { return isTaskDeviceCompleted(this.device) },
  ...
  methods: {
    ...
    async prepareFileContexts(device) {
      const taskId = String(this.taskId || device.ShouhouDDID || '').trim()
      if (!taskId) return
      const parentMenu = await findMenu(['售后任务', '售后订单'], TASK_TABLE)
      if (!parentMenu || !parentMenu.Id) throw new Error('未找到售后任务菜单权限')
      const parentDefinition = await loadNativeFormDefinition(TASK_TABLE, false, { menuId: parentMenu.Id })
      ...
      const childMenuId = String(childConfig.TableChildSysMenuId || '').trim()
      if (!childMenuId) throw new Error('售后任务设备菜单配置不完整')
      ...
      const childDefinition = await loadNativeFormDefinition(TASK_DEVICE_TABLE, false, {
        menuId: childMenuId,
        tableChildAuth
      })
      const baseContext = {
        formEngineKey: TASK_DEVICE_TABLE,
        formDataId: this.id,
        sysMenuId: childMenuId,
        tableChildAuth
      }
      const contexts = {}
      ;(childDefinition.fields || []).forEach((field) => {
        if (FILE_FIELDS.includes(field.Name) && field.Id) contexts[field.Name] = { ...baseContext, fieldId: field.Id }
      })
      this.fileContexts = contexts
    },
    photoFileContext(fieldName) { return this.fileContexts[fieldName] || this.emptyFileContext },
    ...
    async locateDevice() {
      if (this.isCompleted || this.locating) return
      this.locating = true
      try {
    ...
    async submit() {
      if (this.isCompleted || this.submitting || this.locating) return
