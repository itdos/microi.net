  ...
  <mci-page-shell class="device-page" :style="mciTokenStyle" title="设备处理" :subtitle="device.ShebeiBH || device.ShebeiXH || ''" @back="goBack">
    ...
    <scroll-view v-else class="device-scroll" scroll-y>
      ...
      <view class="device-summary">
        ...
        <view class="device-summary__copy"><text class="device-summary__name">{{ device.ShebeiMC || device.ShangpinMC || '服务设备' }}</text><text class="device-summary__meta">{{ [device.ShebeiXH, device.ShebeiBH].filter(Boolean).join(' · ') || '暂无型号和编号' }}</text><text class="device-summary__position">{{ device.AnzhuangWZ || '暂未维护安装位置' }}</text></view>
        <text class="device-summary__status" :class="{ complete: device.FuwuZT === '已完成' }">{{ device.FuwuZT || '未完成' }}</text>
      </view>

      <view class="section-band">
        <view class="section-heading"><view></view><text>处理结果</text><text class="section-heading__required">必填</text></view>
      ...
      <view class="section-band">
        <view class="section-heading"><view></view><text>结果照片</text><text class="section-heading__hint">支持水印相机和相册</text></view>
      ...
      <view v-if="isInstall" class="section-band">
        <view class="section-heading"><view></view><text>安装验收照片</text><text class="section-heading__hint">按类别留档</text></view>
      ...
      <view class="section-band">
        <view class="section-heading"><view></view><text>现场补充</text><text class="section-heading__hint">图片与视频</text></view>
      ...
      <view class="section-band">
        <view class="section-heading"><view></view><text>设备与耗材</text></view>
...
const FORM_FIELDS = ['ChuliJG', 'JieguoTP', 'ShipinSC', 'ZhengmianZP', 'farZP', 'LvxinZP', 'GuanluZP', 'MingpaiCSZP', 'TiaoxingMZP', 'JixieBHZP', 'XianchangZP']

export default {
  mixins: [themeMixin],
  data() {
    return {
      id: '', taskId: '', taskType: '', device: {}, form: {}, loading: true, submitting: false,
  ...
  methods: {
    initializeForm() { const form = {}; FORM_FIELDS.forEach((field) => { form[field] = field === 'ChuliJG' ? '' : '[]' }); this.form = form },
    async loadDevice() {
      ...
      try {
        ...
        this.device = device
        const base = {}; FORM_FIELDS.forEach((field) => { base[field] = device[field] || (field === 'ChuliJG' ? '' : '[]') })
    ...
    openWatermark(field) {
      ...
      const query = `customer=${encodeURIComponent(this.device.KehuMC || '')}&address=${encodeURIComponent(this.device.AnzhuangWZ || '')}`
      uni.navigateTo({
    ...
    saveDraft(showToast = true) {
      writeTaskDraft(`device:${this.id}`, { form: { ...this.form } })
    ...
    async submit() {
      ...
      if (!this.form.ChuliJG.trim()) { uni.showToast({ title: '请填写处理结果', icon: 'none' }); return }
      ...
      try {
        await saveTaskDevice(this.id, this.taskType, { ...this.form }, this.device)
