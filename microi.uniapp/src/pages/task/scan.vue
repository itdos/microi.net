<template>
  <mci-page-shell class="scan-page" :style="mciTokenStyle" title="扫码做任务" subtitle="识别设备并进入未完成任务" @back="goBack">
    <template #right>
          ...
          <view class="task-card__bottom">
            ...
            <view class="card-actions">
              ...
              <view
                ...
                @tap="openDevice(item)"
              >
                <image class="process-icon" src="/static/xjy/business/sh.png" mode="aspectFit" />
                <text>处理设备</text>
              </view>
  ...
  methods:{
    ...
    async loadTasks(){
      ...
      try{
        ...
        while(true){
          const result=await callApiEngine('getrenwu-by-shebeiid',{Id:this.deviceId.trim(),PageIndex:pageIndex,PageSize:pageSize})
          if(requestId!==this.loadRequestId)return
          if(result&&Number(result.Code)===0)throw new Error(result.Msg||'任务查询失败')
          const pageRows=Array.isArray(result)?result:(result&&Array.isArray(result.Data)?result.Data:[])
          let appended=0
          pageRows.forEach((item,index)=>{
            const key=String(item&&item.AID||'')+'|'+String(item&&item.BID||'')
            const stableKey=key==='|'?'page-'+pageIndex+'-row-'+index:key
            if(rowKeys.has(stableKey))return
            rowKeys.add(stableKey)
            rows.push(item)
            appended+=1
          })
          const totalCount=Number(result&&result.TotalCount!==undefined?result.TotalCount:(result&&result.DataCount))
          if(!pageRows.length||!appended||(Number.isFinite(totalCount)&&rows.length>=totalCount)||pageRows.length<pageSize)break
          pageIndex+=1
        }
