<template>
  <mci-page-shell class="feedback-page" :style="mciTokenStyle" title="完成服务" :subtitle="taskNo" @back="goBack">
    ...
    <scroll-view v-else class="feedback-scroll" scroll-y>
      ...
      <view class="section-band">
        <view class="section-heading"><view></view><text>服务汇总</text><text class="section-heading__hint">选填</text></view>
      ...
      <view class="section-band">
        ...
        <view class="upload-block"><mci-media-uploader v-model="form.photos" :max-count="9" :upload-path="`xjy/task-result/${taskId}/photos`" :file-context="taskFileContext('JieguoTP')" /></view>
      ...
      <view class="section-band">
        ...
        <view class="upload-block"><mci-media-uploader v-model="form.videos" media-type="video" :max-count="3" :upload-path="`xjy/task-result/${taskId}/videos`" :file-context="taskFileContext('ShipinSC')" /></view>
      </view>
...
const TASK_FILE_FIELDS = ['JieguoTP', 'ShipinSC']
...
export default {
  ...
  methods:{
    async loadData(){
      ...
      this.loading=true
      try{
        const [taskResult]=await Promise.all([loadTask(this.taskId,true),this.loadDevices(true)])
        this.task=taskResult.task
        this.taskNo=this.taskNo||this.task.no
        this.customer=this.customer||this.task.customer
        this.taskType=this.taskType||this.task.type
        await this.prepareTaskFileContexts()
        this.form.amount=this.task.ShouhouFY||''
        this.form.result=this.task.result||''
        this.form.photos=this.task.JieguoTP||'[]'
        this.form.videos=this.task.ShipinSC||'[]'
        const draft=readTaskDraft(`finish:${this.taskId}`)
        if(draft&&draft.savedAt){this.form={...this.form,...(draft.form||{})};this.draftRestored=true;this.draftSavedAt=draft.savedAt}
      }catch(error){uni.showToast({title:error.message||'任务加载失败',icon:'none'})}finally{this.loading=false}
    },
    async prepareTaskFileContexts(){
      this.fileContexts={}
    ...
    async submit(){if(this.submitting)return;await this.loadDevices(true);if(!this.devicesCompleted){uni.showModal({title:'设备任务未完成',content:`还有 ${this.devices.length-this.completedDeviceCount} 台设备未完成，请先逐台处理。`,showCancel:false});return}this.submitting=true;uni.showLoading({title:'正在提交',mask:true});try{await runTaskAction('finish',this.task,{ShouhouFY:Number(this.form.amount||0),Jieguo:String(this.form.result||'').trim(),JieguoTP:this.form.photos||'[]',ShipinSC:this.form.videos||'[]',GenjinFS:this.form.followType||'',GenjinFSZ:'',Neirong:this.taskType.includes('回访')?this.task.content:undefined});removeTaskDraft(`finish:${this.taskId}`);const channel=this.getOpenerEventChannel&&this.getOpenerEventChannel();if(channel&&channel.emit)channel.emit('taskFinished',{taskId:this.taskId});uni.showToast({title:'服务已提交验收',icon:'success'});setTimeout(()=>this.goBack(),700)}catch(error){this.saveDraft(false);uni.showToast({title:error.message||error.Msg||'提交失败，草稿已保留',icon:'none'})}finally{uni.hideLoading();this.submitting=false}},
...
.private-media-notice{margin:14rpx 24rpx 0;padding:16rpx 18rpx;border:1rpx solid #f0d9b5;border-radius:6px;color:#8b6428;background:#fff9ec;font-size:20rpx;line-height:1.55}
</style>
