<template>
  <mci-page-shell class="stats-page" :style="mciTokenStyle" title="业绩统计" :subtitle="`${periodLabel}业绩概览`" @back="goBack">
...
export default {
  ...
  methods:{
    money:formatMoney,
    async loadStats(refresh=false){this.loading=true;const common={pageIndex:1,pageSize:1,period:this.period,customRange:this.customRange,refresh};try{const user=getUser()||{};const modules=['customers','orders','visits','devices','opportunities'];const promises=modules.map((key)=>loadModuleRows(getBusinessModule(key),buildPerformanceModuleOptions(key,user,common)));const taskFilters=buildPerformanceTaskOptions(user,common);const [rows,taskResult,types,states]=await Promise.all([Promise.all(promises),loadTasks(taskFilters),loadTaskCounts(taskFilters),loadTaskStateCounts(taskFilters)]);modules.forEach((key,index)=>{this.metrics[key]=rows[index].count});this.metrics.tasks=taskResult.count;const orderStats=rows[1].append&&rows[1].append.StatisticsFields;this.metrics.orderAmount=Number(orderStats&&(orderStats.DingdanJE??orderStats.dingdanje)||0);this.taskTypes=types;this.taskStates=states}catch(error){uni.showToast({title:error.message||'统计加载失败',icon:'none'})}finally{this.loading=false}},
    ...
    openMetric(item){try{const key=item.route==='tasks'?'tasks':item.key;const url=buildPerformanceMetricRoute(key,getUser()||{},{period:this.period,customStart:this.customStart,customEnd:this.customEnd},item.state||'');uni.navigateTo({url})}catch(error){uni.showToast({title:error.message||'暂时无法打开列表',icon:'none'})}},
