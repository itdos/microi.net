...
const xjyBusinessFile = path.join(projectRoot, 'src/tenants/xjy/business.js')
...
const xjyBusinessSource = fs.readFileSync(xjyBusinessFile, 'utf8')
...
assert.match(permissionSource, /filter\(\(item\) => String\(item\.FkId \|\| ''\) === String\(menuId\)\)[\s\S]*?\.some\(\(row\)/,
  '同一用户多个角色的菜单权限必须取并集')
assert.match(xjyBusinessSource, /attendanceRecords:\s*native\(\{[\s\S]*?menuAliases:\s*\[[^\]]*'人员定位'[^\]]*\]/,
  '打卡记录必须使用线上真实“人员定位”菜单解析新增权限')
