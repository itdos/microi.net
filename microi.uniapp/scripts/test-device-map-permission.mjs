...
assert.match(mapSource, /<template v-if="mode === 'device'">[\s\S]*?<view class="entity-sheet__row"><text>客户名称<\/text><text>\{\{ selected\.KehuMC \|\| '-' \}\}<\/text><\/view>\s*<view class="entity-sheet__row"><text>设备型号<\/text><text>\{\{ selected\.ShebeiXH \|\| selected\.ShangpinXH \|\| '-' \}\}<\/text><\/view>\s*<view class="entity-sheet__row"><text>安装位置<\/text>/,
  '设备地图坐标信息必须依次展示客户名称、设备型号和安装位置')
