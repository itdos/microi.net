...
- **移动端入口显隐**：系统设置“界面风格 → 登录界面与入口”中的 `DisableMessageTabBar` 和 `DisableInviteEntry` 分别控制消息 tabBar 与“我的”页邀请入口；两者仅在明确为 `1/true` 时隐藏，旧租户缺少字段或请求失败时继续显示。
...
2. 普通列表退回上一级后再进入，视为新会话；售后任务和任务设备属于显式保留会话的长列表，会按“当前账号 + 入口条件”恢复筛选、已加载记录和原位置。
...
7. 长列表会话只保存在当前应用进程内，最长 12 小时、最多 24 个入口；退出登录或 Token 失效时统一清空，避免跨账号展示旧快照。
