...
export async function generateMiniMaxImage(options) {
    ...
    if (!body && !resumedTaskId) throw new Error("缺少图片请求或已有任务号。");
    ...
    async function call(path, method, payload) {
        ...
        try {
            ...
            if (response.status >= 500) throw new Error(`图片状态服务暂不可用（HTTP ${response.status}）`);
            let current;
            try { current = await response.json(); }
            catch { throw new Error(`图片状态服务响应无法解析（HTTP ${response.status}）`); }
            for (let i = 0; i < 3; i += 1) {
    ...
    while (Date.now() < deadline) {
        ...
        try {
            current = recover && taskId
                ? await call(`/api/Ai/RecoverMiniMaxImageTask?taskId=${encodeURIComponent(taskId)}`, "POST", null)
                : taskId
                ? await call(`/api/Ai/GetMiniMaxImageTask?taskId=${encodeURIComponent(taskId)}`, "GET", null)
                : await call("/api/Ai/GenerateMiniMaxImage", "POST", body);
            recover = false;
        } catch (error) {
            if (signal?.aborted) throw new DOMException("已停止等待，后台任务继续执行。", "AbortError");
            if (++failures >= 3) {
        ...
        if (code === 1 && Array.isArray(data.Images) && data.Images.length) return data;
        if (data.Status === "Unavailable" && ++failures < 3) {
            await pause();
            continue;
        }
        if (code !== 2 || !taskId) {
            const error = new Error(current?.Msg || current?.msg || "图片任务没有返回可确认的结果。");
            error.taskId = taskId;
            error.imageStatus = data.Status || "";
    ...
    const error = new Error("已停止等待图片结果，后台任务状态已保留；可回到本页继续查询。");
    error.taskId = taskId;
    throw error;
}
