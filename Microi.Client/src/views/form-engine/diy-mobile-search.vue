<template>
    <section class="diy-search-container">
      <!--移动端  Dropdown 版本（将复选框收进下拉面板）v-if="GetSearchFieldList('Checkbox', SearchType).length > 0" -->
      <div class="select-menu" v-if="GetSearchFieldList('Checkbox', SearchType).length > 0">
        <template v-for="(field, index) in GetSearchFieldList('Checkbox', SearchType)" :key="'search_dd_' + field.Id + '_' ">
          <div  v-if="Array.isArray(field.Data) && field.Data.length > 0" class="select-menu-item">
            <el-dropdown
              trigger="click"
              v-model:visible="DropdownVisible[field.Name]"
              @visible-change="(v) => DropdownVisible[field.Name] = v"
            >
            <div class="select-title">
              ...
              <template #dropdown>
                <div class="dropdown-checkbox-panel" style="padding:8px; max-width:320px;">
                  <el-checkbox-group v-model="SearchCheckbox[field.AsName || field.Name]" @change="GetDiyTableRow({ _PageIndex: 1 })" class="checkbox-group" style="display:block;">
                    <el-checkbox
                      v-for="(fieldData, fieldDatIndex) in field.Data"
                      :key="'dd_fieldData_' + field.Name + fieldDatIndex"
...
.select-menu-item:nth-child(n+2){
  margin-left: 20px;
}
...
export default {
    ...
    watch: {
      // 监听 props 变化，触发计算属性重新计算,解决Vue 响应式系统追踪问题。当计算属性无法正确响应 props 变化时，通常是响应式依赖链被破坏了
      SearchFieldIds: {
        handler() {
          this.computedDepsVersion++;
        },
        deep: true,
        immediate: true
      },
    ...
    computed: {
        ...
        GetSearchFieldList: function () {
            const self = this;
            // 依赖 computedDepsVersion 确保重新计算
            const version = this.computedDepsVersion;
            return (type, InOrOut) => {
    ...
    data() {
        return {
            ...
            DropdownVisible: {}, // 动态保存每个字段下拉展示状态
             computedDepsVersion: 0,
        };
    },
