<template>
  <view class="tn-safe-area-inset-bottom">
    ...
    <view style="padding: 30px 15px;">
		<tn-button backgroundColor="#3668FC" 
			@click="Logout()"
			padding="40rpx 40rpx" width="100%" fontBold>
  ...
  export default {
    ...
    methods: {
		Logout(){
			var self = this;
			self.Microi.Logout();
			self.$MicroiStore.commit('SetCurrentUser', {});
			self.$MicroiStore.commit('SetIsLogin', false);
			setTimeout(function() {
				self.Microi.Tips('退出登录成功！', true, {
					MustTips : true,
					Time : 2000
				})
			}, 500);
