        ...
        <div v-for="(field, index) in GetSearchFieldList('Text', SearchType)"
            ...
            <div 
                ...
                <el-select
                    ...
                    :remote-method="(query) => { return SelectRemoteMethod(query, field)}"
                    :placeholder="GetFieldPlaceholder(field)"
                    :value-key="GetSelectValueKey(field)"
                    @change="(item) => {return SearchSelectChange(item, field)}"
                    >
                    <!--注意：field.Data数据是在变的，
            ...
            <div 
                ...
                <el-cascader
                    ...
                    @change="(item) => {return CascaderChange(item, field)}"
                    :collapse-tags="true"
                    >
                </el-cascader>
            </div>
            <div 
                ...
                <el-select 
                    ...
                    v-model="SearchModel[field.AsName || field.Name]"
                    :value-key="GetSelectValueKey(field)"
                    @change="(item) => {return SelectChange(item, field)}"
                    >
                    <el-option v-for="item in formatData(field.Data, field)"
                    ...
                    @node-click="(item) => {return handleNodeClick(item, field)}"
            ...
            <div 
                v-else-if="field.Type && (field.Type == 'int' || field.Type.indexOf('decimal') > -1)"
                class="block">
...
export default {
                ...
                self.SearchFieldIds.forEach(id => {
                    ...
                    self.DiyFieldList.forEach(field => {
                        ...
                        if (
                            ...
                            //初始化SearchNumber
                            if (
                                (field.Type && (field.Type == 'int' || field.Type.indexOf('decimal') > -1 ))
    ...
    methods: {
        ...
        GetDepartmentProps(field){
            var self = this;
            var result = { value: 'Id', label:'Name', children:'_Child',checkStrictly : true };
        ...
        GetCascaderProps(field){
            var self = this;
            if(self.DiyCommon.IsNull(field.Config.SelectSaveField) ){//|| self.DiyCommon.IsNull(field.Config.Cascader.Children)
            ...
            var result = {
                            ...
                            label: !self.DiyCommon.IsNull(field.Config.SelectLabel) 
                                    ? field.Config.SelectLabel : field.Config.SelectSaveField, 
        ...
        GetSelectTreeProps(field){
            var self = this;
            if(self.DiyCommon.IsNull(field.Config.SelectSaveField) ){//|| self.DiyCommon.IsNull(field.Config.SelectTree.Children)
                self.DiyCommon.Tips(field.Label + field.Name + ' 存在必填属性[存储字段]未填写！' , false);//, 子级字段
            }
            //checkStrictly:是否严格的遵守父子节点不互相关联，
            var result = {
        ...
        SelectRemoteMethod(query, field) {
            var self = this;
            if (field.Config.DataSourceSqlRemote == true) { //query !== ''
...
.more-search-item{
    float:left;margin-right: 15px;margin-bottom: 10px;height: 28px;
}
