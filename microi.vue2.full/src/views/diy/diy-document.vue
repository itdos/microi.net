        ...
        <div class="pull-right content-container">
            <div class="add"
                ...
                <el-button
                    ...
                    type="success"
                    icon="el-icon-edit"
                    @click="OpenDiyForm(CurrentDiyTableRowModel)">{{ $t('Msg.Edit') }}</el-button>
                <el-button
                    type="primary"
                    icon="el-icon-plus"
                    @click="OpenDiyForm()">{{ $t('Msg.Add') }}</el-button>
            </div>
            <div class="title">{{CurrentDiyTableRowModel.Title}}</div>
            <div class="desc">{{CurrentDiyTableRowModel.UpdateTime || CurrentDiyTableRowModel.CreateTime}}</div>
            <div v-if="!CurrentDiyTableRowModel.IframeUrl" class="content" v-html="CurrentDiyTableRowModel.Content"></div>
            <iframe v-else :src="CurrentDiyTableRowModel.IframeUrl" ref="iframe" id="iframepage" name="mainIFrame" 
...
export default {
    ...
    methods: {
        TreeClick(data){
            ...
            self.DiyCommon.Post('https://api-china.itdos.com/api/diytable/getDiyTableRowModelAnonymous', {
        ...
        GetDiyTableRowTree(){
            var self = this;
            self.DiyCommon.Post('https://api-china.itdos.com/api/diytable/getDiyDocumentTree', { 
		...
		SaveDiyForm(){
            ...
            self.$refs.refDiyForm.FormSubmit({},
...
.diy-document{
    ...
    .right-document{
        ...
        .menu-container{
            ...
            overflow-y: auto;
            width:280px;padding:15px;background: #F7F8FA;
        }
        .title{
            font-size: 28px;
            color: #1F2D3D;
