...
<div>
    ...
    <div class="workflow-history">
        <el-timeline 
            ...
            <div :style="{color: (history.ApprovalType == 'Disagree' || history.ApprovalType == 'Recall' || history.ApprovalType == 'Cancel') ? 'red' : '#303133'}">
                ...
                <div style="margin-top:5px;" v-if="history.Receivers.length > 0">
                    ...
                    <div>
                        <div v-for="(copyUser, index2) in history.Receivers"
                ...
                <div style="margin-top:5px;" v-if="history.CopyUsers.length > 0">
                    <div>
                        <el-divider content-position="left"><el-tag>抄送人</el-tag></el-divider>
                    </div>
                    <div>
                        ...
                        <div v-for="(copyUser, index2) in history.CopyUsers"
                            :key="'receiveUser_' + copyUser.Id"
                            style="text-align:center;margin-bottom:5px;margin-right:5px;float:left;">
                            <!-- <el-tooltip class="item" effect="dark" :content="copyUser.Name" placement="bottom" style="height:28px;"> -->
                                <el-tag class="user-item" type="danger">
    ...
    methods: {
		...
		Init(param){
            ...
            if(!self.DiyCommon.IsNull(self.CurrentFlowId)){
                ...
                self.DiyCommon.Post('/api/WorkFlow/getWFHistory', { FlowId : self.CurrentFlowId}, function(result2){
                    if(self.DiyCommon.Result(result2)){
                        ...
                        self.DiyCommon.Post('/api/SysUser/getSysUserPublicInfo', { Ids : userIds}, function(result3){
                            if (self.DiyCommon.Result(result3)) {
                                result2.Data.forEach(history => {
                                    var searchUser = _.where(result3.Data, { Id : history.SenderId});
                                    ...
                                    history.CopyUsers.forEach(copyUser => {
                                        var searchUser = _.where(result3.Data, { Id : copyUser.Id});
                                        if (searchUser && searchUser.length > 0) {
                                            if (!self.DiyCommon.IsNull(searchUser[0].Avatar)) {
...
.workflow-history{
    ...
    .user-item{
        ...
        height: 24px;
        line-height: 24px;
        .user-item-avatar,.user-item-name
        {
            float: left;
        }
        .user-item-avatar{
