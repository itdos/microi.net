...
<div class="wf-work-handler">
    <template v-if="OpenFormMode == 'Edit'">
        ...
        <div style="margin-top:10px;"
            v-if="CurrentNodeModel.NodeType != 'Business' && CurrentStartType != 'StartWork'">
            <!--如果是业务节点，不需要显示同意或不同意-->
            <el-radio-group 
        ...
        <!--请选择退回节点-->
        <div style="margin-top:10px;" 
            v-if="CurrentApprovalType == 'Disagree'">
            <el-select v-model="CurrentBackNodeId" filterable 
                clearable
                placeholder="请选择退回节点"
    ...
    <template v-if="OpenFormMode == 'Edit' || OpenFormMode == 'Add'">
        <!--选择审批人：-->
        <div style="margin-top:10px;"
            v-if="NextNodeConfirmUsersData.AllowSelectUsers === true && NextNodeConfirmUsersData.SelectUsers.length">
            <div style="margin-bottom:5px;">选择审批人：</div>
            <el-checkbox-group 
                v-model="CurrentSelectUsers"
                :max="20">
                <el-checkbox v-for="user in NextNodeConfirmUsersData.SelectUsers" 
                            :label="user.Id" :key="user.Id">{{user.Name}}</el-checkbox>
            </el-checkbox-group>
        </div>
        <!--添加审批人：-->
        <div style="margin-top:10px;"
            v-if="NextNodeConfirmUsersData.AllowAddUsers === true && CurrentApprovalType != 'HandOver'">
            <div style="margin-bottom:5px;">添加审批人：</div>
            <el-select v-model="CurrentAddUsers" 
                    filterable
                    clearable
        ...
        <!--指定移交人：-->
        <div style="margin-top:10px;"
            v-if="CurrentApprovalType === 'HandOver' && !CurrentNodeModel.HideHandOverSelect">
            <div style="margin-bottom:5px;">指定移交人：</div>
            <el-select v-model="CurrentAddHandOverUsers" 
                    filterable
                    clearable
                    multiple placeholder="请选择"
                    style="width:100%;"
...
export default {
    ...
    methods: {
        ...
        SubmitHandOverWork(){
            ...
            self.$emit("CallbackHandOverWork", {
        ...
        HandOverWork(param, callback){
            ...
            self.DiyCommon.Post('/api/WorkFlow/handOverWork', {
		...
		SubmitRecallOrCancelWork(){
            ...
            self.BtnLoading = true;
            //这里的5个值，要和CallbackSendWork接收到的param赋值数量对应
            self.$emit("CallbackRecallOrCancelWork", {
                CurrentNodeModel : self.CurrentNodeModel,
                CurrentFlowDesign : self.CurrentFlowDesign,
                CurrentFlowDesignId: self.CurrentFlowDesignId,
                CurrentApprovalType: self.CurrentApprovalType,
        ...
        GetNextNodeConfirmUsers(){
            ...
            self.$nextTick(function(){
                ...
                self.DiyCommon.Post('/api/WorkFlow/getNextNodeConfirmUsers', {
        ...
        InitStartWork(param, callback){
            ...
            self.DiyCommon.Post('/api/diytable/NewGuid',{}, function(result){
                if (self.DiyCommon.Result(result)) {
                    ...
                    //获取字段设置
                    var startNode = self.DiyCommon.Post('/api/WorkFlow/getStartWFNode', {
		...
		InitSendWork(param, callback){
            ...
            self.DiyCommon.Post('/api/WorkFlow/getWFNodeModel', {NodeId : self.CurrentNodeId}, function(result){
                if (self.DiyCommon.Result(result)) {
                    ...
                    self.$emit('CallbackInit', {CurrentShowFields : currentShowFields, CurrentReadonlyFields: currentReadonlyFields});
                    self.$nextTick(function(){
                        if (callback) {
                            callback({
                                CurrentShowFields : self.CurrentShowFields,
        ...
        SetV8DefaultValue(V8, formData){
            ...
            V8.FormSet = (fieldName, value) => {return self.FormSet(fieldName, value, formData)};
