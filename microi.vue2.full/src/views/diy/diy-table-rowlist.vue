...
<div
    ...
    <el-tabs
        ...
        :class="!IsPageTabs()
                            ? 'table-rowlist-tabs tab-pane-hide'
                            : 'table-rowlist-tabs box-card-top-tabs'"
        >
        <!-- 之前是使用GetPageTabs()，使用改成了预渲染  -->
        <template v-for="(tab, tabIndex) in SysMenuModel.PageTabs">
            <el-tab-pane
                v-if="tab.IsVisible"
                :key="TypeFieldName + 'page_tabs_' + tab.Id + tabIndex"
                :name="tab.Id"
                :lazy="true">
                <span slot="label"
                    :style="{color: TableRowListActiveTab !== tab.Id ? ' #171717 !important' : ''}">
                    <i :class="DiyCommon.IsNull(tab.Icon) ? 'fas fa-list-ol marginRight5' : tab.Icon + ' marginRight5'"
                        :style="{color: TableRowListActiveTab !== tab.Id ? ' #171717 !important' : ''}"
                    />
        ...
        <el-row>
            <el-col :span="24">
                ...
                <el-card class="box-card box-card-table-row-list">
                    <div v-if="(IsTableChild()) && TableChildField.Label"
                        slot="header" class="clearfix">
                        <span style="font-weight:bold;">
                            <i class="mr-2 fas fa-table"></i>{{TableChildField.Label}}</span>
                    </div>
                    <div v-if="(PropsIsJoinTable) && JoinTableField.Label"
                        slot="header" class="clearfix">
                        <span style="font-weight:bold;">
                            <i class="mr-2 fas fa-table"></i>{{JoinTableField.Label}}</span>
                    </div>
                    <!--DIY功能按钮 新版-->
                    <div
                        ...
                        <div class="pull-left item-in" style="margin-right:0px;">
                            <el-button
                                ...
                                {{ !DiyCommon.IsNull(SysMenuModel.DiyConfig) && !DiyCommon.IsNull(SysMenuModel.DiyConfig.AddBtnText)
                                    ? SysMenuModel.DiyConfig.AddBtnText : $t('Msg.Add') }}
                            </el-button>
                            <template v-if="!DiyCommon.IsNull(SysMenuModel.DiyConfig)
                                    && !DiyCommon.IsNull(SysMenuModel.PageBtns)
                            ...
                            <template
                                v-if="!IsTableChild()
                                        || (IsTableChild()  && !(TableChildField.Readonly))">
                                <el-button
                                    v-if="LimitImport() && TableChildFormMode != 'View'"
                                    icon="el-icon-upload2"
                                    @click="ImportDiyTableRow()">{{ $t('Msg.Import') }}</el-button>
                                <el-button
                                    v-if="LimitExport()
                                        && (DiyCommon.IsNull(SysMenuModel.ExportMoreBtns) || SysMenuModel.ExportMoreBtns.length == 0)"
                                    icon="el-icon-download"
                                    :loading="BtnExportLoading"
                                    @click="ExportDiyTableRow()">{{ $t('Msg.Export') }}</el-button>
                                        <!-- @click="ExportDiyTableRow()" -->
                                        <!-- split-button -->
                                <el-dropdown
                                        v-if="LimitExport()
                                        && !DiyCommon.IsNull(SysMenuModel.ExportMoreBtns)
                                        && SysMenuModel.ExportMoreBtns.length > 0"
                                        size="mini"
                                        trigger="click"
                                        ...
                                        <el-button class="mr-10">
                                            {{ $t('Msg.Export') }} <i class="el-icon-arrow-down el-icon--right" />
                                        </el-button>
                                        <el-dropdown-menu
                                            slot="dropdown"
                                            class="table-more-btn">
                                            ...
                                            <template v-if="!DiyCommon.IsNull(SysMenuModel.DiyConfig)
                                                ...
                                                <template v-for="(btn, btnIndex) in SysMenuModel.ExportMoreBtns"
                                                    ...
                                                    <el-dropdown-item
                                                        v-if="btn.IsVisible"
                                                        :key="TypeFieldName + 'more_btn_export_' + btnIndex"
                                                        @click.native="ExportDiyTableRow(btn)">
                                                        <i :class="'more-btn mr-1 ' + (DiyCommon.IsNull(btn.Icon) ? 'far fa-check-circle' : btn.Icon)"></i> {{ btn.Name }}
                                                    </el-dropdown-item>
                                                </template>
                            ...
                            <el-button
                                v-if="!DiyCommon.IsNull(SysMenuModel.ImportTemplate)"
                                icon="el-icon-document"
                                @click="DownloadTemplate()">{{ $t('Msg.DownloadTemplate') }}</el-button>
                        </div>

                        <div class="pull-left item-in"
                        ...
                        <div class="pull-left item-in"
                            ...
                            <el-button
                                ...
                                @click="InitSearch();GetDiyTableRow({_PageIndex : 1});">
                                {{ $t('Msg.ClearSearch') }}
                            </el-button>
                        </div>

                        <div class="pull-left item-in search-in"
                            v-if="GetSearchFieldList().length > 0 && IsPermission('NoSearch')"
                            >
                            <!-- 更多搜索 弹出层  【内部】搜索-->
                            <el-popover
                                placement="bottom"
                        ...
                        <div class="pull-left item-in"
                            ...
                            <el-button
                                ...
                                class=""
                                icon="el-icon-s-help"
                                @click="$router.push(`/diy/diy-design/${TableId}?PageType=${CurrentDiyTableModel.ReportId ? 'Report' : ''}`)">{{ '表单设计' }}</el-button>
                        </div>
                        <div class="pull-left item-in"
                            v-if="GetCurrentUser._IsAdmin"
                            >
                            <el-button
                                ...
                                class=""
                                icon="el-icon-s-help"
                                @click="OpenMenuForm()">{{ '模块设计' }}</el-button>
                        </div>
                    </div>
                                ...
                                <el-table-column
                                    ...
                                    <template slot-scope="scope">
                                        ...
                                        <template v-if="!DiyCommon.IsNull(field.V8TmpEngineTable) && scope.row[field.Name + '_TmpEngineResult'] !== undefined">
                                            ...
                                            <span v-html="scope.row[field.Name + '_TmpEngineResult']"></span>
                                        </template>
                                        ...
                                        <template v-else-if="NeedDiyTemplateFieldLst.indexOf(field.Component) > -1">
                                            ...
                                            <template v-else-if="field.Component == 'Map'">
                                                <el-tag
                                                    v-if="DiyCommon.IsNull(scope.row[field.Name+'_Lng'])"
                                            ...
                                            <template v-else-if="field.Component == 'MapArea'">
                                                <el-tag
                                                    v-if="DiyCommon.IsNull(scope.row[field.Name])"
                                                    @click="OpenDetail(scope.row, 'Edit')"
                                                    type="info"
                                                    class="hand">
                                                    <i class="el-icon-location-outline"></i>
                                            ...
                                            <template v-else-if="field.Component == 'ImgUpload'">
                                                <template>
                                                    <i class="far fa-images hand"
                                                        :style="{color: !DiyCommon.IsNull(scope.row[(DiyCommon.IsNull(field.AsName) ? field.Name : field.AsName)]) ? '#ff6c04' : '#ccc'}"></i>
                                                </template>
                                            </template>
                                            <!-- <template v-else>
                                                <el-tag
                                        ...
                                        <template v-else>
                                            <!--如果是表内编辑-->
                                            <template v-if="SysMenuModel.InTableEdit
                                                            && SysMenuModel.InTableEditFields.indexOf(field.Id) > -1
                                                            ">
                                                <template v-if="field.Component == 'Text'">
                                                    ...
                                                    <DiyInput
                                                        ...
                                                        @CallbackRunV8Code="(field, thisValue) => {return RunV8Code(field, thisValue, scope.row)}"
                                                        @CallbakOnKeyup="(event, field) => {return FieldOnKeyup(event, field, scope)}"
                                                        />
                                                </template>
                        ...
                        <el-table-column
                            ...
                            <template slot-scope="scope">
                                ...
                                <template>
                                    ...
                                    <template
                                        v-for="(btn, btnIndex) in scope.row._RowMoreBtnsOut">
                                        <!-- <el-dropdown-item
                                    ...
                                    <el-dropdown
                                        v-if="TableChildFormMode != 'View' && !(TableChildField.Readonly)"
                                        trigger="click">
                                        <el-button>
                                        ...
                                        <el-dropdown-menu
                                            ...
                                            class="table-more-btn">
                                            <el-dropdown-item
                                                v-if="LimitEdit() && TableChildFormMode != 'View'  && scope.row._IsInTableAdd !== true"
                                                icon="el-icon-edit"
                                            ...
                                            <template v-if="scope.row._RowMoreBtnsIn.length > 0">
                                                <template v-for="(btn, btnIndex) in scope.row._RowMoreBtnsIn"
                                                    ...
                                                    <el-dropdown-item
                                                        v-if="btn.IsVisible"
                                                        :key="TypeFieldName + 'more_btn_' + scope.row.Id + btnIndex"
                                                        @click.native="RunMoreBtn(btn, scope.row)">
                                                        <i :class="'more-btn mr-1 ' + (DiyCommon.IsNull(btn.Icon) ? 'far fa-check-circle' : btn.Icon)"></i> {{ btn.Name }}
                                                    </el-dropdown-item>
                                                </template>
    ...
    <el-dialog
        ...
        <div slot="title">
            ...
            <div class="pull-right">
                <el-dropdown
                    ...
                    <el-dropdown-menu slot="dropdown" class="form-submit-btns">
                        <el-dropdown-item
                            ...
                            :disabled="BtnLoading"
                            @click.native="SaveDiyTableCommon(false, 'Insert')">{{ (FormMode == 'Add' || FormMode == 'Insert') ?  $t('Msg.AddAdd') :  $t('Msg.UptAdd')}}</el-dropdown-item>
                        <el-dropdown-item
                            v-if="ShowFormBottomBtns.SaveUpdate"
                            :icon="BtnLoading ? 'el-icon-loading' : 'el-icon-s-help'"
                            :disabled="BtnLoading"
                            @click.native="SaveDiyTableCommon(false, 'Update')">{{ (FormMode == 'Add' || FormMode == 'Insert') ? $t('Msg.AddUpdate') : $t('Msg.UptUpdate') }}</el-dropdown-item>
                        <el-dropdown-item
                            v-if="ShowFormBottomBtns.SaveView"
                            :icon="BtnLoading ? 'el-icon-loading' : 'el-icon-s-help'"
                            :disabled="BtnLoading"
                            @click.native="SaveDiyTableCommon(false, 'View')">{{ (FormMode == 'Add' || FormMode == 'Insert') ? $t('Msg.AddView') : $t('Msg.UptView') }}</el-dropdown-item>
                    </el-dropdown-menu>
                </el-dropdown>
                <el-button
                    v-if="FormMode == 'View' && LimitEdit()
                            && TableChildFormMode !== 'View'
                            && !(TableChildField.Readonly)
                            && ShowUpdateBtn
                            && OpenDiyFormWorkFlowType.WorkType != 'StartWork'
                    ...
                    size="mini"
                    icon="el-icon-edit"
                    @click="OpenDetail({Id : TableRowId}, 'Edit', true)">{{ $t('Msg.Edit') }}</el-button>
                <template v-if="!DiyCommon.IsNull(SysMenuModel.DiyConfig)
                ...
                <el-button
                    v-if="LimitDel()
                            && TableChildFormMode !== 'View' && FormMode != 'Add'
                            && !(TableChildField.Readonly)
                            && ShowDeleteBtn
                            && OpenDiyFormWorkFlowType.WorkType != 'StartWork'
                            "
                ...
                <el-button
                    size="mini"
                    icon="el-icon-close"
                    @click="CloseFieldForm('ShowFieldForm', 'Close', TableRowId)">{{ $t('Msg.Close') }}</el-button>
            </div>
        </div>
        <div class="clear">
            <div :class="ShowFormRight() ? 'pull-left' : ''" :style="{width: ShowFormRight() ? 'calc(100% - 380px)' : '100%'}">
                <DiyForm
                    ref="fieldForm"
                    :form-wf="FormWF"
                    :load-mode="''"
            ...
            <div v-if="ShowFormRight()"
                class="pull-right"
                style="width:360px;background-color:#F5F7FA;height:100%;padding-left:15px;padding-right:15px;">
                <el-tabs v-model="FormRightType">
                    <el-tab-pane v-if="OpenDiyFormWorkFlow" label="流程信息" name="WorkFlow">
                        ...
                        <WFWorkHandler v-if="OpenDiyFormWorkFlowType.WorkType == 'StartWork'" ref="refWfWorkHandler_2"
                            @CallbackStartWork="CallbackStartWork"
                            ></WFWorkHandler>
                    </el-tab-pane>
                    <el-tab-pane v-if="CurrentDiyTableModel.EnableDataLog" label="数据日志" name="DataLog">
                        <div class="datalog-timeline">
                            <el-timeline style="padding-left: 5px;">
                                <el-timeline-item
                                    ...
                                    <div>{{item.Title}}</div>
                                    <div v-for="(log, i2) in item.Content" :key="'datalog_content_' + log.Name"
                                        style="background-color: #e8f4ff;margin-bottom:5px;margin-top:5px;"
                                    >
                                        <!-- <el-tag> -->
                                            <span style="color:red">{{ log.Label }}</span>：
                                            由 <span style="color:red">{{ log.OVal }}</span>
                                            修改为 <span style="color:red">{{ log.NVal }}</span>
                                        <!-- </el-tag>     -->
                                    </div>
                                </el-timeline-item>
                            </el-timeline>
    ...
    <el-drawer
            ...
            <div class="pull-right">
                <el-dropdown
                    ...
                    {{ ((FormMode == 'Add' || FormMode == 'Insert') || FormMode == 'Insert') && !DiyCommon.IsNull(SysMenuModel.DiyConfig) && !DiyCommon.IsNull(SysMenuModel.DiyConfig.SaveBtnText)
                        ? SysMenuModel.DiyConfig.SaveBtnText : $t('Msg.Save') }}
                ...
                <el-button
                    v-if="LimitDel() && TableChildFormMode !== 'View' && FormMode != 'Add' && ShowDeleteBtn
                            && OpenDiyFormWorkFlowType.WorkType != 'StartWork'
                            "
                    :loading="BtnLoading"
                    type="danger"
                    size="mini"
                    icon="el-icon-delete"
                    @click="DelDiyTableRow(CurrentRowModel, 'ShowFieldFormDrawer')">{{ $t('Msg.Delete') }}</el-button>
                <el-button
                    size="mini"
                    icon="el-icon-close"
                    @click="CloseFieldForm('ShowFieldFormDrawer', 'Close', TableRowId)">{{ $t('Msg.Close') }}</el-button>
        ...
        <div class="clear">
            <div :class="ShowFormRight() ? 'pull-left' : ''" :style="{width: ShowFormRight() ? 'calc(100% - 380px)' : '100%'}">
             ...
             <div v-if="ShowFormRight()" class="pull-right" style="width:360px;background-color:#F5F7FA;height:100%;padding-left:15px;padding-right:15px;">
                <el-tabs v-model="FormRightType">
                    <el-tab-pane v-if="OpenDiyFormWorkFlow" label="流程信息" name="WorkFlow">
                        <WFHistory v-if="OpenDiyFormWorkFlowType.WorkType == 'ViewWork'" ref="refWFHistory"></WFHistory>
                        <WFWorkHandler v-if="OpenDiyFormWorkFlowType.WorkType == 'StartWork'" ref="refWfWorkHandler_2"
                    ...
                    <el-tab-pane v-if="CurrentDiyTableModel.EnableDataLog" label="数据日志" name="DataLog">
                        <div class="datalog-timeline">
                            ...
                            <div v-if="DataLogListLoading" style="height:50px;line-hegiht:50px;">
                                <i class="el-icon-loading"></i> {{ $t('Msg.Loading') }}
                            </div>
    ...
    <el-dialog
        ...
        <el-upload
            ...
            :on-success="(result, file, fileList) => {return ImportUploadSuccess(result, file, fileList)}"
            ...
            <div
                slot="tip"
                class="el-upload__tip">只能上传.xls/.xlsx文件</div>
        ...
        <div class="marginTop10 marginBottom10">
            <el-button
                ...
                @click="GetImportDiyTableRowStep">
                查看进度（可多次点击查看）
            </el-button>
            <el-tooltip
                v-if="GetCurrentUser._IsAdmin"
                class="item" effect="dark" content="清除后可强制重新开始导入，此功能谨慎使用，只适合在长时间进度卡住不动的情况下使用。" placement="top">
                <el-button
                    icon="el-icon-warning"
                    @click="DelImportDiyTableRowStep">
                    清除导入进度缓存
                </el-button>
            </el-tooltip>
        </div>
        <div class="">
            <div
                v-for="(m, index) in ImportStepList"
                :key="TypeFieldName + 'importStep_' + index"
                style="color:red;">
                {{ m }}
            </div>
            <div
                v-if="ImportStepList.length == 0"
                style="color:red;">暂无进度</div>
        </div>
        ...
        <span slot="footer" class="dialog-footer">
            ...
            <el-button
                size="mini"
                icon="el-icon-close"
                @click="ShowImport = false;">{{ $t('Msg.Close') }}</el-button>
        </span>
    </el-dialog>
    <DiyModule
        :modal="!IsTableChild()"
    ...
    <DiyCustomDialog
        ...
        ref="refDiyCustomDialog"></DiyCustomDialog>
    ...
    >
        <div slot="title">
            ...
            <div class="pull-right">
                ...
                @click="RunOpenAnyTableSubmitEvent()"
                >
                {{ $t("Msg.Submit") }}
                </el-button>
                <el-button
...
export default {
            ...
            return function(type, InOrOut){
                ...
                self.SearchFieldIds.forEach(id => {
                    self.DiyFieldList.forEach(field => {
                        ...
                        if (
                            (field.Id == id || field.Id == id.Id)
                            && id.Hide !== true
                            ) {
                            //初始化SearchNumber
                            if (
                                (field.Type && (field.Type && (field.Type == 'int' || field.Type.indexOf('decimal') > -1 )))
    ...
    methods: {
        ...
        ShowFormRight(){
            ...
            if(!self.OpenDiyFormWorkFlow && self.CurrentDiyTableModel.EnableDataLog && self.FormMode != 'Add'){
                return true;
            }
            return false;
        },
        ...
        async Init(parentFormModel, v8){
            ...
            if(self.PropsModuleEngineKey){
                var sysMenuResult = await self.DiyCommon.FormEngine.GetFormData({
                    ...
                    _Where: [{ Name : 'ModuleEngineKey', Value : self.PropsModuleEngineKey, Type : '=' }]
                });
        ...
        GetUploadData(){
            var self = this;
            var result = {Limit : true, TableId : self.TableId, UserId : self.GetCurrentUser.Id};
        ...
        GetCommonV8(){
            ...
            V8.FormSet = (fieldName, value) => { return self.FormSet(fieldName, value, self.CurrentSelectedRowModel)}, // 给Form表单其它字段赋值
        ...
        async CallbackStartWork(param, callback){
            ...
            try {
                ...
                //-------第2步：提交表单
                self.$refs.fieldForm.FormSubmit(formParam,
        ...
        async DiyTableRowClick(row, column, event){
            ...
            var V8 = {};
            try {
                if (!self.DiyCommon.IsNull(self.TableChildField)
                    ...
                    self.SetV8DefaultValue(V8);
                    await self.DiyCommon.InitV8Code(V8, self.$router);
                    V8.RefreshChildTable = (field, parentFormModel) => { return self.RefreshChildTable(field, parentFormModel, V8) };
                    // eval(btn.V8Code)
        ...
        GetAllData(){
            ...
            self.DiyCommon.PostAll(params, async function (results) {
                if (self.DiyCommon.Result(results[0]) && self.DiyCommon.Result(results[1])) {// && self.DiyCommon.Result(results[2])
        ...
        GetOpenFormWidth(){
            ...
            var result = self.DiyCommon.IsNull(self.CurrentDiyTableModel.FormOpenWidth)
                            ? '768px'
        ...
        async RunV8Code(field, thisValue, row) {
            ...
            var V8 = {};
            try {
                if (!self.DiyCommon.IsNull(field)
        ...
        IsPageTabs(){
            var self = this;
            if (!self.DiyCommon.IsNull(self.SysMenuModel.DiyConfig)
                && !self.DiyCommon.IsNull(self.SysMenuModel.PageTabs)
                ) {
                if(self.SysMenuModel.PageTabs.length > 1 ||
                    (self.SysMenuModel.PageTabs.length == 1 && self.SysMenuModel.PageTabs[0].Id != 'none' && self.SysMenuModel.PageTabs[0].Name != '')
                    ){
                    return true;
                }
            }
        ...
        async RunFieldTemplateEngine(field, row){
            ...
            try {
                ...
                if (self.DiyCommon.IsNull(V8.Result) && V8.Result != '') {//注意有时候确实是在v8中设置返回了空字符串
        ...
        GetOpenTitle(){
            ...
            if (self.DiyCommon.IsNull(self.CurrentRowModel) || self.DiyCommon.IsNull(self.CurrentRowModel.Id)) {
                ...
                // }
                if (fieldModel &&
                    (fieldModel.Component == 'Text'
                     || fieldModel.Component == 'NumberText'
                     || fieldModel.Component == 'Textarea'
                     || fieldModel.Component == 'AutoNumber'
                     )) {
                        firstValue = self.CurrentRowModel[fieldModel.Name];
                }
            ...
            var title2 = '';
            var title3 = (self.DiyCommon.IsNull(self.CurrentDiyTableModel) || self.DiyCommon.IsNull(self.CurrentDiyTableModel.Description))
        ...
        GetDiyField() {
            ...
            self.DiyCommon.Post(self.DiyApi.GetDiyFieldByDiyTables, {
                TableIds: tableIds,
            }, function (result) {
                ...
                if (
                    ...
                    try {
                        ...
                        if(self.DiyCommon.IsNull(self.DevComponents[field.Config.DevComponentName])){
                            self.DevComponents[field.Config.DevComponentName] = {Name:'',Path:''};
                        }
            ...
            if (!self.DiyCommon.IsNull(field.Config)) {
                ...
                if (!self.DiyCommon.IsNull(field.Config.SelectLabel)) {
                    try {
                        ...
                        if (Array.isArray(tObj)) { //if (field.Component == 'MultipleSelect')
        ...
        GetImportDiyTableRowStep() {
            var self = this
            self.DiyCommon.Post(self.GetImportProgressApi(), {
                // OsClient: self.OsClient,
        ...
        DelImportDiyTableRowStep() {
            var self = this
            self.DiyCommon.Post('/api/diytable/delImportDiyTableRowStep', {
                TableId : self.TableId
            }, function (result) {
                if (self.DiyCommon.Result(result)) {
        ...
        ExportDiyTableRow(btn) {
            ...
            self.DiyCommon.FormExportFileV2(url, param, function(){
        ...
        async OpenDetailHandler(tableRowModel, formMode, isDefaultOpen, isOpenWorkFlowForm, wfParam){
            ...
            if (self.CurrentDiyTableModel.FormOpenType == 'Dialog'
        ...
        GetSysMenuModel() {
            var self = this;
            self.DiyCommon.Post(self.DiyApi.GetSysMenuModel, {
                Id : self.SysMenuId,
            }, function(result){
                if(self.DiyCommon.Result(result)){
        ...
        async GetSysMenuModelAfter(result) {
            ...
            //-------GetPageTabs()提前预生成
            if (!self.DiyCommon.IsNull(result.Data.DiyConfig)
        ...
        SortShowHideFieldsList(tempArr){
            ...
            if (self.ShowHideFieldsList.length > 0) {
                for (let index = 1; index <= self.ShowHideFieldsList.length; index++) {
                    //先查询到上一个字段所在位置
                    var firstIndex = _.findIndex(tempArr, {Name : self.ShowHideFieldsList[index - 1]});
                    if (firstIndex != -1) {
                        //如果下一个位置的值和现在这个不相等
                        if (tempArr[firstIndex + 1] && self.ShowHideFieldsList[index] != tempArr[firstIndex + 1].Name) {
                            //获取老位置
                            var currentIndex = _.findIndex(tempArr, {Name : self.ShowHideFieldsList[index]});
                            if(currentIndex != -1){
                                //缓存用于替换的值
                                ...
                                //重新获取老位置
                                firstIndex = _.findIndex(tempArr, {Name : self.ShowHideFieldsList[index - 1]});
                                //插入新位置
        ...
        async DiguiDiyTableRowDataList(firsrtData){
            ...
            for (let index = 0; index < firsrtData.length; index++) {//result.Data
                let row = firsrtData[index];//result.Data
                if(!row.Id && (row.id || row.ID)){
                    row.Id = row.id || row.ID;
                }
                let _rowMoreBtnsOut = _.where(self.SysMenuModel.MoreBtns, { ShowRow : true});
                ...
                let _rowMoreBtnsIn = _.where(self.SysMenuModel.MoreBtns, { ShowRow : false});
        ...
        async FormSubmitAction(actionType, tableRowId, rowModel) {
            ...
            if (!self.DiyCommon.IsNull(self.CurrentDiyTableModel.SubmitFormV8)) {
                var V8 = {
                    Form: rowModel,
                    FormSet: (fieldName, value) => { return self.FormSet(fieldName, value, rowModel)}, // 给Form表单其它字段赋值
                    FormSubmitAction: actionType,
                    GetDiyTableRow: self.GetDiyTableRow,
