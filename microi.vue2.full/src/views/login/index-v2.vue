<template>
    <div>
        <div
            id="divLogin"
            :style="{backgroundImage:'url('+ DiyCommon.GetServerPath(DesktopBg.LockImgUrl, false) + ')', '--LockBgCss': 'url(' + DiyCommon.GetServerPath(DesktopBg.LockImgUrl, false) + ')'}">
            ...
            <div
                ...
                <video id="videoLogin" class="video" :poster="DiyCommon.GetServerPath(DesktopBg.LockImgUrl)" :muted="!DesktopBg.LockVideoVoice" :src="DiyCommon.GetServerPath(DesktopBg.LockVideoUrl)" autoplay="autoplay" data-autoplay="true" preload="auto" loop="loop" webkit-playsinline="true" playsinline="true" x5-video-player-type="h5">
                    <source
            ...
            <div style="">
                <p style="font-size: 16px;font-weight: bold;margin-top: 50px;">{{ DiyCommon.Months[CurrentTime.getMonth()] + DiyCommon.GetLanDate(CurrentTime.getDate()) + ' ' + DiyCommon.Weeks[CurrentTime.getDay()] }}</p>
                <p style="font-size: 36px;font-weight: bold;margin-top: 10px;">{{ CurrentTime.Format('HH:mm:ss') }}</p>
            </div>
            <!-- :style="{opacity: (LoginCover ? '0':'1')}" -->
            <div
                class="divLoginCenter"
                >
            ...
            <!-- <div
                class="divLoginTime"
                :style="{bottom: (LoginCover ? '7.5%':'100%'), opacity: (LoginCover ? '1':'0')}">
            ...
            <el-dialog
                ...
                <span slot="footer" class="dialog-footer">
                    <el-button
                        type="primary"
                        size="mini"
                        @click="GotoSystem">立即进入</el-button>
                    <el-button
                        size="mini"
                        @click="$store.commit('DiyStore/SetShowGotoWebOS', false)">取 消</el-button>
                </span>
            </el-dialog>
            ...
            <el-dialog
                ...
                :close-on-click-modal="false">
                <div v-html="SysConfig.PrivacyPolicy"
    ...
    export default {
        ...
        mounted() {
            ...
            try {
                self.DiyCommon.PostAsync('/api/diyTable/getSysConfig', {
                    _SearchEqual:{
                        IsEnable : 1
                    },
        ...
        methods: {
            ...
            TokenLogin(){
                ...
                //取所有单点登录
                var diySsoList = self.DiyCommon.Post('/api/FormEngine/getTableDataAnonymous', {
                    ...
                    if(result.Code == 1 && Array.isArray(result.Data) && result.Data.length > 0){
                        ...
                        for (let index = 0; index < result.Data.length; index++) {
                            ...
                            if(!self.DiyCommon.IsNull(token) && token != '$V8.CurrentToken$'){
                                ...
                                self.DiyCommon.Post(diySso.ClientSsoApi, {//'/api/SysUser/SsoPengrui'
                                    _token : token,
                                    Token : token,
            ...
            Login() {
                ...
                self.DiyCommon.Post(loginApi, loginParam, function (result) {
                    ...
                    if (self.DiyCommon.Result(result)) {
                        self.LoginResult = result;
                        if (false) {//self.OsClient == 'Tdx' || self.OsClient == 'Nbgysh'
                            self.$store.commit('DiyStore/SetState', {
                                key: 'SystemStyle',
                                value: 'WebOS'
                            });
