<template>
  <div id="divOsSystemSetting" class="microi-desktop-secmenu" style="height: 100%;display:none;">
    ...
    <div class="container-fluid">
      ...
      <div class="row items">
        <div v-for="(m,i) in SecondMenuList" :key="m.Id" class="col-lg-3 col-md-4 col-sm-6 col-xs-12">
          <div class="item" @click.stop="OpenDesktopItem(m)">
            <div class="float-left" style="width:40px;">
              <!-- fas fa-laptop -->
              <i :class="DiyCommon.IsNull(m.IconClass) ? 'fas fa-bars' : m.IconClass" />
            </div>
            <div class="rightContent">
              <div class="title">{{ DiyCommon.GetLangValue(m, 'Name') }}</div>
              <div class="child">{{ DiyCommon.GetLangValue(m, 'Description') }}</div>
            </div>
          </div>
        </div>
      </div>
