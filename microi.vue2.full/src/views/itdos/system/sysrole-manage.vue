            ...
            <el-card 
                ...
                <div class="text item"  style="max-height: calc(100vh - 220px);overflow-y: scroll;">
                    ...
                    <el-tree 
                        ...
                        @node-click="TreeDeptClick"></el-tree>
                </div>
            </el-card>
        </el-col>
            ...
            <el-card class="box-card no-padding-body">
                <el-form
                    ...
                    <el-form-item size="mini">
                        <el-button
                            icon="el-icon-search"
                            @click="GetSysRole(true)">{{ $t('Msg.Search') }}</el-button>
                        <el-button
                            type="primary"
                            icon="el-icon-plus"
                            @click="OpenSysRole()">{{ $t('Msg.Add') }}</el-button>
                    </el-form-item>
                </el-form>
                <el-table
                    ...
                    <el-table-column
                        ...
                        <template slot-scope="scope">
                            <el-button
                                ...
                                class="marginRight5"
                                icon="el-icon-s-help"
                                @click="OpenSysRole(scope.row)">{{ $t('Msg.Edit') }}</el-button>
                            <el-button
                                type="danger"
                                size="mini"
                                class="marginRight5"
                                icon="el-icon-delete"
                                @click="DelSysRole(scope.row)">{{ $t('Msg.Del') }}</el-button>
                        </template>
                    </el-table-column>
                </el-table>
    ...
    <el-dialog
                ...
                <el-col
                    ...
                    <el-form-item
                        ...
                        <el-cascader
                            ...
                            :props="{ value: 'Id', label:'Name', children:'_Child',multiple:true ,checkStrictly : true}"
                            ></el-cascader>
                    </el-form-item>
                </el-col>
                <el-col
                    ...
                    <el-form-item
                        :label="$t('Msg.Remark')"
                        size="mini">
                        <el-input v-model="CurrentSysRoleModel.Remark" 
                            type="textarea"
                            :rows="2"/>
                    </el-form-item>
                </el-col>
                            ...
                            <el-table-column
                                ...
                                <template slot-scope="scope">
                                    <span :style="{marginLeft: 
                                                            ((DiyCommon.IsNull(scope.row._Child) || scope.row._Child.length == 0)
                                                                &&  scope.row.ParentId == DiyCommon.GuidEmpty) 
                                                            ? '23px' : '0px'}">
                                        <el-checkbox v-model="scope.row._Check"
                                            @change="(val) => { return NameChange(val, scope.row);}">
                            ...
                            <el-table-column label="权限">
                                <template slot-scope="scope">
                                    <el-checkbox-group v-model="scope.row.Permission"
                                        >
                                        <el-checkbox label="Add" @change="(val) => { return BtnChange(val, scope.row, 'Add');}">{{$t('Msg.Add')}}</el-checkbox>
                                        <el-checkbox label="Edit" @change="(val) => { return BtnChange(val, scope.row, 'Edit');}">{{$t('Msg.Edit')}}</el-checkbox>
                                        <el-checkbox label="Del" @change="(val) => { return BtnChange(val, scope.row, 'Del');}">{{$t('Msg.Del')}}</el-checkbox>
                                        <el-checkbox label="Import" @change="(val) => { return BtnChange(val, scope.row, 'Import');}">{{$t('Msg.Import')}}</el-checkbox>
                                        <el-checkbox label="Export" @change="(val) => { return BtnChange(val, scope.row, 'Export');}">{{$t('Msg.Export')}}</el-checkbox>
                                        <el-checkbox label="NoDetail" @change="(val) => { return BtnChange(val, scope.row, 'NoDetail');}">无{{$t('Msg.Detail')}}</el-checkbox>
                                        <el-checkbox label="NoSearch" @change="(val) => { return BtnChange(val, scope.row, 'NoSearch');}">无{{$t('Msg.Search')}}</el-checkbox>

                                        <el-checkbox v-for="(btn, btnI) in scope.row.MoreBtns" 
                                                    :key="'btni' + btnI + btn.Id"
                                                    :label="btn.Id">{{btn.Name}}</el-checkbox>
                                        <el-checkbox v-for="(btn, btnI) in scope.row.ExportMoreBtns" 
                                                    :key="'btni' + btnI + btn.Id"
                                                    :label="btn.Id">{{btn.Name}}</el-checkbox>
                                        <el-checkbox v-for="(btn, btnI) in scope.row.BatchSelectMoreBtns" 
                                                    :key="'btni' + btnI + btn.Id"
                                                    :label="btn.Id">{{btn.Name}}</el-checkbox>
                                        <el-checkbox v-for="(btn, btnI) in scope.row.PageBtns" 
                                                    :key="'btni' + btnI + btn.Id"
                                                    :label="btn.Id">{{btn.Name}}</el-checkbox>
                                        <el-checkbox v-for="(btn, btnI) in scope.row.PageTabs" 
                                                    :key="'btni' + btnI + btn.Id"
                                                    :label="btn.Id">{{btn.Name}}</el-checkbox>
                                        <el-checkbox v-for="(btn, btnI) in scope.row.FormBtns" 
                                                    :key="'btni' + btnI + btn.Id"
                                                    :label="btn.Id">{{btn.Name}}</el-checkbox>
                                    </el-checkbox-group>
                                </template>
        ...
        <span slot="footer" class="dialog-footer">
            <el-button
                ...
                size="mini"
                icon="el-icon-s-help"
                @click="SaveSysRole">{{ $t('Msg.Save') }}</el-button>
            <el-button
                size="mini"
                icon="el-icon-close"
...
export default {
    ...
    'DiyTableId', 'ParentId', 'Sort', "MoreBtns", "FormBtns", "ExportMoreBtns", "BatchSelectMoreBtns", "PageBtns", "PageTabs"],
        ...
        DelSysRole(m) {
            ...
            self.DiyCommon.OsConfirm(self.$t('Msg.ConfirmDelTo') + '【' + m.Name + '】？', function () {
                self.DiyCommon.Post(self.DiyApi.DelSysRole(), {
                        Id: m.Id
                    },
                    function (result) {
                        if (self.DiyCommon.Result(result)) {
        ...
        ForGetSysMenuListCheck(sysMenuList) {
            ...
            for (let index = 0; index < sysMenuList.length; index++) {
                ...
                if (sysMenu._Check == true) {
                    ...
                    result.push({Id : sysMenu.Id, Permission : JSON.stringify(_permission)});
                }
                if (!self.DiyCommon.IsNull(sysMenu._Child) && sysMenu._Child.length > 0) {
                    ...
                    tmpResult.forEach(tmpSysMenu => {
                        result.push({Id : tmpSysMenu.Id, Permission : tmpSysMenu.Permission});
                    });
                }
            }
            return result;
        ...
        OpenSysRole(m) {
            ...
            if (self.DiyCommon.IsNull(m)) {
                ...
                // self.LoadingCount++;
                self.DiyCommon.Post(self.DiyApi.GetSysRoleModel(), {
                    Id: m.Id
                }, function (result1) {
        ...
        GetSysRole(initPageIndex) {
            ...
            self.tableLoading = true;
            self.DiyCommon.Post(self.DiyApi.GetSysRole(), {
