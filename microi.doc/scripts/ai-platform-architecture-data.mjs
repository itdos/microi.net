...
export const officialSystemEngines = [
  'AI 引擎', 'V8引擎', '表单引擎', '模块引擎', '接口引擎', '界面引擎', '打印引擎',
  '报表引擎', '工作流引擎 v4', 'SaaS 引擎', '缓存引擎', '搜索引擎', '采集引擎', '任务调度', 'MQ 消息队列',
  'MQTT 引擎', '消息通知', '翻译引擎（多语言）', 'OCR 引擎', '视觉引擎', '图片处理引擎', '分布式存储 / HDFS', '文件柜', '模板引擎', 'Office 引擎',
...
export const panels = [
  ...
  },
  {
    ...
    items: [
      ...
      '40+ 表单控件', '主子表 / 关联表', '字段 / 表单 V8', '可写报表 CRUD',
      '权限 / 数据范围', '左右树表', '移动卡片', 'ECharts / 地图',
      'Office 引擎', '蓝牙打印', '多表 / 多库编排', '定制组件',
  ...
  },
  {
    ...
    items: [
      ...
      'L1 / L2 多级缓存', 'Excel / CSV 导入', 'Word / PPT 导出', '邮件 / 短信 / 微信',
      '分布式存储 / HDFS', '文件柜', '公众号 / 小程序', 'OpenAPI / SDK', 'gRPC / 多语言'
    ]
  },
  {
...
export const v8Capabilities = [
  ['接口引擎', '保存即生效'], ['FormEngine', 'CRUD / _Where'], ['接口数据源', 'V8 / SQL / JSON'], ['Db / Dos.ORM', '多数据库'],
  ['HTTP', 'GET / POST / PATCH'], ['Redis Cache', 'L1 / L2 / TTL'], ['MongoDB', '文档数据'], ['Search Engine', '索引 / 检索'],
  ['Job / Quartz', '可靠后台任务'], ['Spider Engine', '采集 / 浏览器'], ['MQ / RabbitMQ', 'Outbox / Inbox'], ['MQTT / IoT', '设备事件'],
  ['Files / HDFS', '流式资产'], ['Office', 'Excel / Word / PPT'], ['OCR / Vision / Image', '识别 / 图像处理'], ['Translate', '翻译 / 多语言'],
...
export const architectureLayers = [
  ...
  },
  {
    ...
    capabilities: ['接口引擎', 'FormEngine', 'V8 / SQL / JSON', 'Dos.ORM', 'HTTP', 'Redis', 'MongoDB', 'MQ / MQTT', 'Office / OCR', 'Webhook / SignalR']
  },
  {
    name: '数据、集成与自动化引擎',
    responsibility: '连接数据库、服务、文件、设备与消息通道',
    capabilities: ['缓存', '扩展数据库', '搜索', '采集', '任务调度', 'MQ / MQTT', '通知', '翻译引擎（多语言）', 'OCR / 视觉 / 图片', '分布式存储 / HDFS', '文件柜']
  },
  {
    name: '平台治理、安全与可靠运行',
