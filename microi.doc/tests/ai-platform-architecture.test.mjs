...
const versionedSvgPath = resolve(imagesRoot, `${architectureAssetBaseName}.svg`)
const versionedPngPath = resolve(imagesRoot, `${architectureAssetBaseName}-1920x1080.png`)
const versionedPng4kPath = resolve(imagesRoot, `${architectureAssetBaseName}-3840x2160.png`)
...
test('架构图以 V8引擎为唯一运行核心并覆盖平台关键能力', async () => {
  const [generator, dataSource, svg] = await Promise.all([
    ...
    readFile(versionedSvgPath, 'utf8')
  ])
  assert.doesNotMatch(`${generator}\n${dataSource}\n${svg}`, /Jint/i)
  for (const text of [
    'Microi吾码 AI平台 架构图', platformVersion, 'V8引擎', '10×+', 'Token 更省', '典型交付更快', '30+', '成熟引擎复用', '在线生效', 'V8 无需编译发布',
    '表单引擎', '模块引擎', '界面引擎', '打印引擎', '报表引擎', '工作流引擎 v4', '业务架构蓝图', '系统设置',
    '系统日志 / 监控', '采集引擎', '缓存引擎', '消息通知', '翻译引擎（多语言）', 'OCR 引擎', '视觉引擎', '应用商城', 'Microi.Code', 'MCP / Skills',
  ...
  assert.ok(officialSystemEngines.length >= 30, `系统引擎数量不足 30：${officialSystemEngines.length}`)
...
test('1080P 与 4K PNG 尺寸精确、文字无二次缩放且体积均小于 2MB', async () => {
  const assets = [
    [versionedPngPath, 1920, 1080],
    [versionedPng4kPath, 3840, 2160],
    [pngPath, 1920, 1080],
    [png4kPath, 3840, 2160]
  ]
...
test('资产清单锁定数据源哈希、尺寸和每个输出文件内容', async () => {
  ...
  assert.deepEqual(await readFile(versionedSvgPath), await readFile(svgPath))
  assert.deepEqual(await readFile(versionedPngPath), await readFile(pngPath))
  assert.deepEqual(await readFile(versionedPng4kPath), await readFile(png4kPath))
})
...
test('README 与官网首页共享同一份机器可读能力索引和架构图引用', async () => {
  ...
  assert.ok(index.includes(`](/images/${architectureAssetBaseName}.svg)`))
  assert.ok(readme.includes(`](./microi.doc/docs/public/images/${architectureAssetBaseName}-3840x2160.png)`))
  assert.doesNotMatch(index, /\]\(\/images\/microi-ai-platform-architecture\.svg\)/)
...
test('源码架构页直接展示同一份 SVG 系统架构图', async () => {
  ...
  assert.ok(sourceArchitecture.includes(`](/images/${architectureAssetBaseName}.svg)`))
  assert.doesNotMatch(sourceArchitecture, /\]\(\/images\/microi-ai-platform-architecture\.svg\)/)
  assert.match(sourceArchitecture, /30\+ 系统引擎/)
})
