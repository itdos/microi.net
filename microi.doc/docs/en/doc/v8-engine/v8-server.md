...
`FontFamily` is a preferred family. The runtime validates every Unicode character and first falls back to a matching installed font, then to Noto Sans CJK SC embedded in the `Dos.Common` assembly. Mixed-script text may therefore use multiple font runs, and basic Latin, digits, and Simplified Chinese work even in a Synology or minimal Linux container with no system fonts. If neither system nor embedded fonts contain a character, the API returns a clear error containing that character and its `U+XXXX` code point instead of rendering missing-glyph boxes. The embedded font guarantees baseline availability; it does not replace brand fonts, regional variants, special-symbol fonts, or emoji fonts. Install the required business fonts and pass `FontFamily` when typography must remain stable.
