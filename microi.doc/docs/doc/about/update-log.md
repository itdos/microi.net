...
<section class="mci-release-scope-banner" aria-labelledby="mci-release-scope-title">
  <div class="mci-release-scope-banner__glow" aria-hidden="true"></div>
  <div class="mci-release-scope-banner__mark" aria-hidden="true"><span>μ</span><i></i><i></i><i></i></div>
  <div class="mci-release-scope-banner__content">
    <p class="mci-release-scope-banner__eyebrow">MICROI FRAMEWORK RELEASES</p>
    <h2 id="mci-release-scope-title">先分清两类更新，升级才会真正生效</h2>
    <p class="mci-release-scope-banner__lead">本页只记录 <strong>Microi吾码 AI 开发框架</strong>的升级内容，不包含应用商城中各个应用的更新日志。</p>
    <div class="mci-release-scope-banner__paths">
      <article>
        <span>01</span>
        <div><b>升级平台框架</b><small>更新前端与后端源码，或拉取并部署对应版本的 Docker 镜像。</small></div>
      </article>
      <article>
        <span>02</span>
        <div><b>升级商城应用</b><small>进入平台【应用商城】，按应用查看更新日志并执行安装或更新。</small></div>
      </article>
    </div>
    <p class="mci-release-scope-banner__note"><b>提示</b>：只升级平台不会自动更新已安装应用；只更新应用也不会替代平台源码或镜像升级。</p>
  </div>
</section>

## v8.0.2 - (2026-09-04 19:09)

- **视觉引擎补齐“识别—回看—人工纠错—沉淀样本”的学习闭环**：`platform-vision-runtime` 升至 v1.2.0，新增 `Correct` 动作，可从最近识别记录选择正确对象、保留原始识别来源、纠错人、纠错时间和说明，并按需把当时的私有画面直接存入样本库；人脸模式继续要求授权同意，重复提交按来源请求和对象保持幂等。最近识别和结果详情只返回短时效私有图片地址，不暴露 HDFS 路径；旧租户仍使用默认 CreateIfMissing Hook 时，仅对明确“不支持 Correct”的旧返回做兼容，其它失败继续原样暴露。
- **视觉识别结果增加生鲜属性、预估价格和可解释来源**：对象、样本、识别记录和结果统一承载是否冰冻、新鲜度百分比、预估最低／最高价格、单位与币种；数据库命中优先使用样本或对象中的权威值，未命中再由平台 AI 补全，AI Worker 升至 v1.1.0 并使用结构化提示词和有界解析。接口结果把数据库匹配、AI 匹配和人工纠错分别标识，不把 AI 推断冒充样本库事实；人脸识别不臆测生鲜和价格属性。工作台 E2E 同时覆盖 PC 全宽、手机竖屏／横屏首屏无根滚动、底部导航无遮挡、最近识别图片预览、纠错弹窗、数据库匹配中／失败后切换 AI 等阶段状态，并移除重复的“再次识别当前画面”入口。
- **应用商城通知改为稳定应用身份与单条脏数据隔离**：官方应用商城包升至 v7.9.23，`get-microi-store` 升至 v1.5.0，前后端按 `StoreId / AppId / AppKey` 别名归并最多 5000 条安装历史并保留最新软删除状态；缺少版本号的记录标记为“数据异常”，不再让一个旧租户脏行拖垮全部平台应用更新提醒。`ai_app_publish_store` 升至 v1.9.24，发布时要求稳定应用标识、对称补齐 AppId／AppKey，并在写入后执行强回读；配套脚本可幂等修正官方商城通知身份元数据。
- **商城安装器完成 SQL Server 字段级 DDL 兼容**：`import-microi-store-package` 升至 v2.7.4，按目标租户数据库方言把 MySQL 逻辑类型转换为 SQL Server 的 `nvarchar / datetime2` 等物理类型，读取并保留长度、精度和日期精度；表／索引 DDL、字段重命名、类型修改、默认约束及相关索引的拆除与重建都使用 SQL Server 安全路径，字段只改显示名称时不做无意义物理重写。应用包继续携带数据库无关逻辑结构，安装时才投影到目标方言。
- **SQL Server 平台运行链进一步消除 MySQL 隐式假设**：Dos.ORM 将 `tinytext / text / mediumtext / longtext / ntext` 统一映射为 `nvarchar(max)`，物理字段回读保留字符长度、数值精度和 `datetime2` 精度，避免误判为 `nvarchar(1)` 或升级后截断；Upgrade25 收紧非空字段时沿用真实物理类型。Quartz 根据 `OsClientDbType` 选择 MySQL 或 SQL Server Provider、Delegate 与规范化连接串，后台任务 SQL 对保留字别名使用方言安全引用；SaaS 启动时以明确配置覆盖过期租户模型。Microi.net 的列表排序仅在主排序尚未包含 Id 时追加稳定 Id，修复 SQL Server 拒绝重复 `ORDER BY A.Id, A.Id` 的问题，并新增完整 SQL Server 测试报告 PDF 与类型兼容回归。
- **Excel 导入支持服务端事务预检与请求幂等**：菜单 `ImportV8` 可声明 `ApiEngine:<Key>`，平台会在任何行落库前把标准化行、固定值、当前用户、文件名、唯一规则和幂等键交给接口引擎，并与整批导入共享同一数据库事务；启用该能力时前端强制整批回滚策略。每次选择文件生成稳定 `_ImportIdempotencyKey`，后端按租户、表、菜单、用户和请求键记录 Pending／Running／Succeeded／Failed，阻止重复点击、网络重试或后台任务重放造成二次写入，重新选择文件才生成新请求。
- **MCP 表单建模修复局部字段更新与子表索引误判**：字段 Patch 对标量、空值和缺失值统一安全读取，未出现在 Patch 中的既有属性原样保留，并消除动态对象污染元组结果造成的运行时绑定异常；菜单 Manifest 允许保留 `ImportV8 / ImportProgressApi`。TableChild 关系校验先回读真实物理字段：共享库子表要求 `(OsClient, 外键)` 前缀索引，独立租户库没有 OsClient 时只要求 `(外键)`，避免把合法独立库结构误判为不可交付。
- **前端微服务宿主桥接返回稳定、可操作的动作结果**：打印等宿主能力失败时返回结构化错误码、原因、处理建议、是否可重试和“结果是否未知”，同时把响应固定保存在 `hostActionResult`；宿主主题、尺寸等随后批量下发的数据不再覆盖刚完成的动作回执。数据投递按 `forceSetData`、组件数据桥和全局桥逐级兼容，页面可直接向用户显示恢复建议，而不是只得到不可定位的通用失败。
- **畅捷通 V2 回调纠正为接口引擎完整承载**：兼容地址 `/api/Message/ReceiveV2?OsClient=...` 现在是 `platform-chanjet-callback-v2` 的 `ApiRoutes`，公开路由、POST 限制、HTTP 200／400／404／503 响应、事务与租户 Hook 调度均由 `ResponseType=HTTP` 的 Managed 接口引擎实现，不再新增 Controller 方法。C# 仅扩展固定该接口 Key、固定当前租户且不暴露密钥的 `V8.Method.DecodeChanjetCallbackV2` 原子，从当前租户 `mci_system_setting` 解密 AES Key 与最多 32 个 AppKey，并完成长度、重复 JSON 属性、应用标识和安全投影校验。官方 SaaS 引擎包升至 v7.8.27；`platform-chanjet-callback-v2-hook` 仍为 CreateIfMissing，旧回调、旧 Java 和客户当前接口引擎不切换。
- **租户升级从“每次启动重放历史修复”收口为一次性版本基线**：新增 Upgrade36（`7.6.14.0`），在共享租约和 fencing 检查下仅一次收敛 Upgrade23／26／28／29／30／31／33／34／35 的运行时不变量；升级协调器先读取 `ServerVersion`，已经达到目标版本的租户在取得租约、执行历史迁移和刷新缓存前直接快返，取得租约后再次读版本以处理并发。新增治理文档与源码门禁，明确业务表、字段、菜单和微服务必须走应用商城，只有商城无法表达且是运行前必需的核心物理／协议前置条件才允许新增 .NET Upgrade。
- **官网、架构图和企业培训资料同步本轮能力边界**：官网统一把平台能力表述从“20+”更新为“30+”成熟引擎；AI 平台架构图生成器升至 `2026.09.04.1`，同时输出带版本名的 SVG、1080p 和 4K 文件并保留兼容旧路径，绕过 CDN 不可变缓存导致的旧图命中。45 页企业培训课件增强亮／暗主题 PDF 按钮对比度，重新生成两套 PDF 与关键缩略图；视觉、服务端 V8、表单组件、AI 治理和源码架构文档同步补齐本轮接口与验收口径，能力断言继续区分工程合同、浏览器测试、模型准确率和生产验收。
- **Skills 与 VS Code 内置知识库保持同源并发布 v5.5.2**：应用商城 Skill 新增 CreateIfMissing 官方空实现安全门和商城不可表达性判断，表单／数据库／导入导出 Skill 补齐事务预检、幂等和方言规则，MicroService 与 Vision Skill 固化主题适配、手机首屏、最近识别、纠错和属性来源边界。`npm run publish` 已通过构建、类型检查、CLI／Codex 测试、Bundle 与 Skills 同源校验，并成功推送 npm `@microi.net/cli@5.5.2`、Visual Studio Marketplace `Microi.v8-engine v5.5.2` 和 Open VSX `Microi.v8-engine v5.5.2`；按发布响应成功即完成，不在索引传播窗口对同一版本重复推送。
- **七个 Git 边界完成历史、远端和工作树去重审计**：根仓及 Microi.net、Microi.AI、Microi.VSCode、WebOS、Microi.WorkFlow、Microi.Vision 均已抓取远端并确认 0 ahead／0 behind。v8.0.0 日志时间之后，各仓只有已经由上一节完整说明的 v8.0.0／VS Code v5.5.1 线性封版提交，没有遗漏的英文功能提交、团队自动 merge 或应重复计算的变更；因此本节以当前最终净差异为准。写入本节前，根仓为 141 个已跟踪文件（文本 3,708 行新增／862 行删除）和 21 个新增源码、测试、文档或验收文件；Microi.net 为 3 个文件（40/8），Microi.AI、Microi.WorkFlow、Microi.Vision 各 1 个版本文件（3/3），Microi.VSCode 为 18 个版本、插件 Bundle 与 Skills 文件（271/224），WebOS 保持干净且不制造空提交。`dist / bin / obj / node_modules`、机械版本号、压缩图片和 PDF 只核对版本、摘要及可打开性，不重复包装成功能项。
- **v8.0.2 全量现代版公共制品发布完成**：按 `8.0.2、4、0、2、2、0、0` 执行一键发布，官方资源同步并强回读为 Managed 145 + CreateIfMissing 10 = 155，投影 SHA-256 为 `861aa285577f3515cd03e4b9afa9d14e3ba506c41a07e779730b1591e7d1ad253`；资源安全门定向回归 68/68 通过，现代前端、Microi.net 与完整 `Microi.Anderson.sln` Release 构建成功，独立 18080 存活检查返回 HTTP 200。23 个 NuGet 均收到成功推送响应，Microi.net、Microi.AI、Microi.WorkFlow、Microi.Vision DLL 的保护前后 MD5 均不同；后端正式／开发镜像摘要统一为 `sha256:7df9c4e21615a7b900578739df689af40646fd0a8d6f68f482e08aae7724241a2`，现代前端正式／测试镜像摘要统一为 `sha256:c6c50f76af4b2490222fcf9c488f5ec9021abd19aa38616fb8b7d1dadda2691ca`，明确未生成 Chrome 49 legacy 制品。NuGet／镜像成功响应是本轮发布终点，不因索引延迟重复推送；客户节点自动更新与登录后业务验收仍是独立边界。

## v8.0.0 - (2026-09-04 11:53)

- **文件上传改为默认允许、显式关闭的负向开关**：SaaS 引擎新增“关闭文件上传”字段 `DisableFileUpload`，未配置、空值、`0` 或 `false` 均允许上传，只有 `1` 或 `true` 才禁用；滚动升级期间仅在新物理字段尚不存在时兼容旧 `FileUploadEnabled`，错误提示同步指向新开关。新增 Upgrade35 幂等创建物理字段和元数据、把旧正向字段标记为只读兼容项，并接入租户升级协调器；官方 SaaS 引擎包升至 v7.8.25，数据库字典、HDFS／安全／SaaS／V8 文档及 Skills 使用同一语义，不把历史“未启用”值反向迁移成禁用。
- **视觉引擎修复私有图片暂存失败与 AI 识别不稳定问题**：`platform-vision-runtime` 升至 v1.1.2，区分“上传被关闭、配额超限、存储失败”等可操作错误；数据库已命中但留图失败时返回识别结果并附警告，未命中且必须交给 AI 时才明确失败。连续帧稳定信息统一兼容 `VoteCount / WindowCount / AverageConfidence` 与旧字段名，前端不再显示 `undefined/undefined`；AI Worker 升至 v1.0.3，同一模型最多有界重试 3 次、多模型总尝试最多 5 次，对 408、429 与可恢复 5xx 指数退避并切换备用模型，能确认鱼类但不能可靠细分品种时至少返回“鱼／水产鱼类”，不再把明显鱼体长期停留为 0% 未匹配。
- **手机摄像头工作台收口为横竖屏首屏可操作界面**：视觉工作台在 390×844 竖屏采用紧凑上下布局，在 844×390 短横屏改为采集区与结果区并排；摄像头／上传预览、识别结果、暂停／继续／再次识别等高频按钮均保留在一屏内，关键触控区不低于 44px，根页面和嵌套容器不产生纵向滚动。AI 后台识别期间启用单飞约束，轮询终态前不再重复提交视频帧；自动化同时覆盖上传图片、连续识别、稳定投票、目标图片远端验收和移动端无滚动断言。现有 `hongdi-dev` 390×844 登录态证据中，测试图片先进入 AI Pending，最终由平台 AI 引擎识别为“针鱼”、置信度 82%、总耗时 10.957 秒，根容器、采集区和结果区均完整落在首屏；接口证据另覆盖鲤鱼样本库 100% 命中（221ms）及番茄 AI 识别 97%（5.943s）。
- **应用商城修复旧租户更新日志唯一键碰撞并升级官方包**：导入器升至 v2.7.2，在把 `sys_microistore_changelog.OsClient` 收紧为非空前，先按租户、应用和版本确定性选择保留记录，有界扫描最多 5000 个候选、按精确数量分批删除重复项并强回读；并发漂移时失败关闭，幂等重跑保持零额外改动。Upgrade13 固定最低导入器版本和修复标记，刷新脚本与商城契约同步更新，官方应用商城包升至 v7.9.20。
- **AI 助手补齐附件上下文的跨运行时兼容**：官方 AI 助手应用升至 v7.6.6、`platform-ai-runtime` 升至 v1.1.1；进入 .NET `List<T>` 绑定前把 V8 空数组归一为 null，意图识别只附加有界的文件摘要并排除聊天历史和集合对象，修复附件场景下 `IConvertible` 类型转换异常，同时保留租户隔离与受保护模型配置边界。
- **AI 工作流增加真正的首屏质量门禁**：工作台端到端测试升至 v1.0.3，在 1920×1080 和 1440×900 两种桌面视口同时校验页面无纵向滚动、Studio 与业务关系图首屏可见、内部不存在第二个纵向滚动拥有者；原有 3D 摄像机、拓扑、骨架屏和拖拽交互继续回归，避免“页面能打开但必须滚动才能工作”的伪通过。
- **集福鲤 UniApp 完成积分兑换、案例册 PDF 与 IoT 表单闭环**：积分商城增加分类筛选、实时积分余额、地址与备注，兑换写入正确积分流水并使用稳定幂等键防止重复扣减；案例册支持生成、预览和分享 PDF，兼容微信与 H5；JoinForm 可调用租户配置的快照接口并展示 IoT 状态与指标。新增静态契约、真实接口 smoke 与视觉交付检查脚本，租户 Profile、业务配置和生成配置保持一致。
- **官网与培训资料改为真实产品证据驱动**：首页主行动按钮聚焦企业培训大纲并收口为两个入口；培训第 3 页重构为“八项企业优势 + 五步交付生命周期 + 可信指标”，同步暗／亮主题、打印样式、缩略图及高质量 PDF。新增 10 张真实产品截图，分别进入 AI 创作、AI 治理、AI 工作流、应用商城、文件管理、视觉引擎和可视化文档，并统一为可放大的单图库布局；文档测试对 SVG 摘要统一换行符，消除跨平台误报。大文件恢复文档同步说明 13 步进度、有界日志／心跳、可信后台锁最长 12 小时自动续租以及 fencing 失败关闭边界。
- **Skills 固化高频微服务、锁与上传的新强制规范**：MicroService 规范新增手机竖屏紧凑堆叠、短横屏左右并排、44px 关键操作和摄像头／扫码工具首屏无滚动要求；Vision Skill 固化 AI Pending 单飞与稳定字段归一；文件上传 Skill 统一为“关闭文件上传”负向开关；V8 锁文档明确 `LockKey` 是参数字段名、`Timeout` 是租约 TTL，只有可信持久后台任务可自动续租，并必须配合 owner token、fencing 和安全检查点。能力地图、数据库表目录、多宿主渐进披露清单及 VS Code 内置副本同步更新。
- **Microi.VSCode v5.5.1 已完成三渠道发布**：CLI、扩展清单、构建元数据与内置 Skills 同步本次接口和交付规范；`npm run publish` 已成功推送 `@microi.net/cli@5.5.1`、Visual Studio Marketplace `Microi.v8-engine v5.5.1` 与 Open VSX `Microi.v8-engine v5.5.1`。发布端明确返回成功即视为完成，不因索引传播延迟在同一版本重复发布。
- **源码、历史与重复合并按七个 Git 边界完成审计**：根仓以及 Microi.net、Microi.AI、Microi.VSCode、WebOS、Microi.WorkFlow、Microi.Vision 均先抓取远端并确认 0 ahead／0 behind；v7.9.8 日志时间之后仅有根仓和四个后端闭源仓的 v7.9.8 版本封版提交，内容已由上一节覆盖，VS Code 与 WebOS 无新增历史提交，也没有需要另计的英文提交或团队自动 merge。当前工作树按上述功能归并统计，`dist / bin / obj / node_modules`、机械版本号、压缩图片和 PDF 仅核对最终版本、摘要及可打开性，不重复包装成功能项；WebOS 本轮没有源码差异。
- **回归测试覆盖源码、资源、移动端与发布边界**：应用商城／AI／Vision／租户升级等 6 组资源契约共 139/139 通过，文件上传安全、升级协调和运行时门禁等 .NET 定向测试 118/118 通过，官网定向测试 36/36 通过，集福鲤积分／PDF／IoT 契约检查通过；完整 `Microi.Anderson.sln` Release 构建 0 错误（28 条既有 xUnit 分析器建议），独立 18080 存活检查返回 HTTP 200。移动视觉 E2E 的最近结果为通过，鱼类样本数量门禁不少于 10 种，并保留竖屏、横屏和最终识别截图作为验收证据。
- **v8.0.0 公共制品完成全量发布**：按 `8.0.0、4、0、2、2、0、0` 执行一键发布，用时 9 分 38 秒并以退出码 0 完成；官方资源强回读为 Managed 144 + CreateIfMissing 9 = 153，投影 SHA-256 为 `bca63c3878f6dbd0d4ff0d15ab6b2b27331a0a88354a97e22680736925992c394f`。23 个 NuGet 包均收到成功响应，Microi.net、Microi.AI、Microi.WorkFlow 与 Microi.Vision DLL 全部经过同一保护链且保护前后 MD5 不同；后端开发镜像摘要为 `sha256:7c08a1cf5a114224d7c2cc06ffabf24417256e5a4395957675131b7eff125d4eb`，现代前端镜像摘要为 `sha256:5b6caedf1f860f530bd05caf45fcb830da2ca0d456be2ff217389730dd6367adf`，本轮按要求未生成 Chrome 49 legacy 制品。发布成功响应是本轮 NuGet／镜像交付终点，不以索引刷新延迟触发同版本重发；商城应用安装、客户节点自动更新及具体租户业务验收仍按独立边界判断。

## v7.9.8 - (2026-09-04 05:24)

- **ESA 域名就绪判断升级为控制面与数据面双重验收**：完成 ESA 控制面变更后先做强回读，再依次校验 DNS、固定解析 IP、SNI/TLS 与 `HEAD` 请求；只有真实入口返回 HTTP 100～499 才标记 `BareDomainReady=true`，避免仅凭控制面配置成功误报域名可用。
- **HTTP 522 提供只读回源保护诊断，不越权修改防火墙**：遇到 522 时只读调用 `GetOriginProtection`，返回当前与最新回源 CIDR 以及建议新增、移除的差异，不自动放宽防火墙。`hongdi-dev.microi.net` 当前仍返回 HTTP 522，裸域访问尚未通过验收；租户控制面与数据库数据回读正确，因此保留 `hongdi-dev`，不删除重建。
- **SaaS v7.8.23 与 v7.9.8 公共制品完成封版并通过分层验收**：官方 SaaS 应用资源投影共 153 项，SHA-256 为 `707e46757bc95307b4f800aa5605120e0dd265df933a00001d4f243836221738`，包 JSON SHA-256 为 `0416617661255af89e55073276049ddc1d5bc8fa2724a284e9498ee897c45afe`；不可变商城版本 `01M1MHTKR30CC0ZK04TT2V8S5F` 已由任务 `77389d71c5454a56a8387d041d61c3d6` 在目标环境用 182 个分片、0 次重试完成安装，70 个接口资源和 4 个内联运行资产强回读一致。v7.9.8 全量发布用时 9 分 55 秒并以退出码 0 完成，共发布 23 个 NuGet 包；后端构建 0 错误、28 个警告，存活检查返回 HTTP 200，未生成 Chrome 49 legacy 制品。后端镜像摘要为 `1b16b07f8b029a0684138fa495a25677314a4a03f546d385d3739f288941e1a7`，前端镜像摘要为 `976f9096c10250d6b25d5e8a85a06304ed69f890080d9dcfe5c538e7b3d16c98`。`api.itdos.com` 与 `api-dev.chongstech.com` 已自动部署并回读 `BackendVersion=v7.9.8`；本地 iTdos 和线上 chongstech 的个人设置／创建 SaaS 租户微服务页面均完成真实登录态浏览器验收，`hongdi-dev` 通过可信宿主启动地址完成新标题及 `admin` 新密码登录验收。裸域名仍因 ESA 回源 HTTP 522 单独处于未通过状态，不以其它边界成功替代。

## v7.9.7 - (2026-09-04 04:51)

- **SaaS 创建链路完成授权分层并交付官方应用 v7.8.22**：将租户创建所需的系统级授权与普通管理操作分离，修复克隆数据库场景下系统标题、管理员密码及租户元数据未按新租户覆盖的问题。官方 SaaS v7.8.22 包 SHA-256 为 `53da2a9d2df074ac41827ff9472548138cb2894d5a5cffd7317a21bc39b0880d`；目标安装任务 `559555f1eec14dd29d54ebdc03611ffd` 已成功完成并通过状态回读。
- **应用商城 524 恢复改为 Managed 资源差异化终检**：资源投影共 153 项，SHA-256 为 `d691664cc10b322a7944dc9fb720cc04977d8e6ac0896dcf8acbff1b33a3b6c91`；遭遇 524 后，发布器只对仍不一致的 `Managed` Key 做严格回读，最多执行 2 轮，避免重复扫描已一致资源，同时保持“包内声明式资源为最终事实”的覆盖语义。
- **v7.9.7 全量公共制品发布完成**：发布用时 9 分 31 秒并以退出码 0 完成，共发布 23 个 NuGet 包；后端构建 0 错误、28 个警告，端口 18080 存活检查返回 HTTP 200，未生成 Chrome 49 legacy 制品。后端镜像摘要为 `d5b94bb4419e8791a5014afb4839042908bdacf56585f9351d083a8fd8f8187d`，前端镜像摘要为 `226bb29cd99daf605ad04957724a8e0f4704e547b3c7bd9ea0fa72e9c2d45e2`。

## v7.9.6 - (2026-09-04 03:41)

- **创建 SaaS 租户的大文件上传改为平台原生断点续传**：普通文件的单文件与单次总量默认上限由 100 MB 提升为 500 MB，可在 SaaS 引擎的 `FileUploadMaxFileMB / FileUploadMaxRequestMB` 中配置为 1024 MB 或 2048 MB，平台 HTTP／Multipart 绝对硬顶保持 2048 MB；数据库 ZIP 不再整包提交 `/api/HDFS/UniappUpload`，而是使用默认 16 MB、单片最大 32 MB 的原始二进制分片，每片校验 SHA-256 并把会话、缺片清单和已收字节持久化 7 天。上传页实时展示速度、已上传容量和百分比，支持暂停、刷新后重新选择同一文件继续补传、合并前后整包摘要强校验；单片请求不超过 64 MB，因此逻辑上的 500 MB／1 GB／2 GB 文件不再被历史 100 MB 代理限制直接拦成 413，CDN/WAF 仍需允许单片大小和空闲超时。
- **大 ZIP 与数十万行 SQL 改为全程流式、分批、可观测导入**：平台从 HDFS 流式落盘，再由 `ZipArchive` 和 `TextReader` 逐段解压解析，不把最大 2 GB ZIP 或解压后最多 8 GB 的 SQL 整体装入内存；同时限制 250 倍压缩比，兼容引号、注释、`DELIMITER` 和会话事务状态。连续 `INSERT / REPLACE` 最多 500 条或 1 MB 合并为一个事务批次，DDL／SET 等边界语句单独执行；13 个开户步骤会展示 ZIP／SQL 已读容量、批次数、已执行语句数、平均吞吐和预计剩余时间，状态最多每 2 秒或每新增 16 MB 更新一次，持久执行记录仅在阶段切换或跨越 5% 时追加，后端约二十余条、前端最多保留 200 条，避免几十万行日志拖死浏览器。
- **SaaS 长任务补齐真实进度、锁续租与终态一致性**：租户开户共享租约采用短 TTL 持续续租，数据库分片合并和可信持久后台接口最长可续到 12 小时；接口引擎原有 Timeout 只决定单次租约，不再让仍在正常执行的大库导入因旧 20 分钟 TTL 到期而在业务已提交后被误判失败。任务进度保持单调，取消、失败、补偿、运行时重新加载、`ServerVersion` 升级和精确域名绑定各有独立阶段，只有 13 步全部完成才返回成功；C# 开户原子、Managed 接口和前端进度卡共用同一个总步数事实源，不再在导入期间发生 13→12→13 的分母回退。这同时修复了 1 GB 级数据库长时间停在“第 2 步 16%”、没有执行记录，以及实际完成后仍显示过期锁失败的问题。
- **复制数据库创建租户时原位恢复系统标题、超级管理员与访问入口**：开户服务不再向复制库盲目新增第二个 admin，而是确定性选取模板中的规范管理员并原位写入本次指定的新密码、租户、启用状态和 `Level=9999`；`sys_config` 同样只保留一条活动配置，将 `OsClient / SysTitle / SysShortTitle / CompanyName / PeizhiMC` 统一为用户输入的系统名称并清理缓存。`hongdi-dev` 已按唯一 `Product / Internal` 控制面记录回读，API 侧管理员登录、系统标题和 `ApiBase` 均恢复正确；创建结果先返回当前可信宿主 `WebBase + ApiBase + OsClient` 组成的立即可用启动地址，再由 Managed 域名接口自动绑定并强回读用户填写的精确裸域。只有完整回读为 `Verified` 才把裸域标记为可用；DNS、证书或网关尚未完成时明确返回 `PendingExternalBinding`，并可按 TenantKey／Id 幂等补偿，不再把未绑定域名冒充成功入口。
- **一键安装与空数据库启动消除旧 `iTdos` 模板残留**：官方空数据库发布门禁要求且只允许一条 `iTdos / Product / Internal` 主租户模板，并清除连接、运行态与平台资源指针残留；一键安装器在配置的新主租户不存在时，仅对官方空库把这条唯一模板原位认领为目标 `OsClient`，停用旧包遗留的其它活动空模板并强回读为零，自定义恢复库的既有子租户继续保持不变。新服务器按同一脚本安装后不再因活动主租户与模板不一致而出现平台微服务入口 404；MySQL、SQL Server、Oracle 与 PostgreSQL 使用同一失败关闭规则。
- **平台内置微服务 404 从源头修复并升级至 v1.9.16／CurrentVersion 58**：空数据库制作会中和复制来的外部 v3 运行指针；运行时仅在应用明确为 `DatabaseOnly`、数据库内联入口及每个资产真实字节摘要都可验证时，才从失效外部指针恢复到本租户内联运行包，其它异常继续失败关闭。宿主把 `window.location.origin` 作为可信 `WebBase` 注入已挂载实例，弹层和页面共用权威运行上下文，`/personal-settings`、`/create-empty-tenant` 等 10 条页面路由不再因 `entryUrl` 为空请求到 404。最终包含 42 个源码文件（888,210 bytes）和 4 个运行资产（642,660 bytes），源码清单、运行清单、路由摘要分别为 `dc556beea82a3ca2b7022bd3eca9b73d5699bfe85eec248c23ebc75815cabf15`、`5f254e731ee4e275d33f97bdb5ef7886b3b8792605204d461b6f5d3d3021f140`、`dc1a2f1bcbe13916ac8791de762cde27c472c7b4449fbc8b2d55c8bbf56fc5b3`。
- **应用商城 Managed 资源真正执行“存在即覆盖”**：导入器升至 v2.7.1，包内声明为 Managed 的接口引擎始终以本次已选且已校验的包正文为最终事实，Base／Local／Incoming 摘要只作审计，不再因本地改码、版本漂移、软删除、历史 Ownership、稳定 Id 或路由占用整包回滚。步骤 7 会按大小写无关 `ApiEngineKey` 物理查询全部记录，优先复用包 Id 或最早稳定记录，恢复规范记录、退役其它活动重复项、清理缓存别名并强回读唯一后再原位更新；新 Key 只有包 Id 被无关接口占用时才重映射。由此 `mqtt_event_copy` 等“唯一值已存在”会自动收敛而不是要求人工删数据；MQTT v6.2.2 已覆盖该历史重复路径并回读为唯一活动接口。CreateIfMissing 的租户 Hook（包括禁用或软删除状态）仍保持租户所有，后台任务幂等键历史重复也只在全部终态时归档修复，在途冲突继续失败关闭。
- **官方应用与平台运行包同步封版并完成目标租户重放**：SaaS 引擎最终升至 v7.8.20，应用商城升至 v7.9.19，两包都嵌入同一 `microi-platform-service v1.9.16 / CurrentVersion 58` DatabaseOnly 产物；工作区官方升级资源 JSON 的 SHA-256 分别为 `58e32aa44479c28ab96aca2e6d5a9a67423d283f822866bc3edb7d173705f3bd` 和 `4b960a585dd0a47d5c052849f878d9eb5629bee101910231923146765e78b9fa`。平台微服务不可变商城快照 `mcipkg-492617eaa52d9b733747d0386ca89` 大小 1,236,736 bytes、包 SHA-256 为 `b08a008c1e0bddb7ea9f56295d7016d2a9e2c31d69c88201048e14d1ceb770ab`；目标环境已分别重放应用商城 v7.9.19、平台微服务 v1.9.16 与 MQTT v6.2.2。源码资源、不可变安装快照、目标租户安装和框架镜像仍作为四个独立验收边界，任一边界成功都不替代其它边界的回读。
- **新增阿里云 ESA 精确 CNAME 的最小可信原子能力并接入开户闭环**：SaaS 开户在数据库、管理员和升级均成功后自动调用 Managed 域名接口，绑定成功强回读为 `Verified` 并返回裸域，失败不回滚已经导入的大库，而是返回可按 TenantKey／Id 幂等补偿的明确状态。`V8.Method.AuthorizeAdminTenantDomainBinding` 只接受可信 Managed 执行上下文或真实 DiyToken，固定主租户与精确接口 Key，拒绝访问密钥会话，并在主库重新验证平台管理员，禁止信任 `V8.Param` 或伪造 `_CurrentUser`；随后 `V8.Alidns` 才可幂等创建或更新确定的站点主机记录，固定 CNAME、Web 代理、自动 TTL、80／443 回源端口以及跟随源域名的 Host／SNI。调用方不能覆盖安全策略，通配主机、IP、带协议／凭据／路径的值、站点外主机、回源环路、非唯一站点／记录都会被拒绝；创建响应丢失和并发创建只按同一 SiteId 与精确主机回查，更新不确定只按 RecordId 回读，最终必须完整强回读才返回成功，日志不写访问密钥。SaaS v7.8.19 只兼容并严格校验运行时自动注入的 `_RequireApiRoleAuthorization=true/1` 与合法 W3C `traceparent`；v7.8.20 再兼容并丢弃 MCP 调试运行时自动追加的 `TestParam1`，且仅允许 null、undefined 或不超过 200 字符的字符串／数字／布尔标量，对象、数组、超长值和其它未知字段仍在权威读取与 ESA 写入前失败关闭。MCP 启动包装器同时在保持严格 TLS 校验的前提下转发 `NODE_EXTRA_CA_CERTS / NODE_USE_SYSTEM_CA`，便于使用私有或明确固定 CA 的部署，而不是关闭证书验证。
- **视觉引擎形成独立、可保护、可扩展的 `Microi.Vision` 内核**：新增受控图片解码、ONNX 模型描述与 SHA-256／许可证校验、DINOv2／DINOv3／SigLIP2 向量、YOLO／YOLOX／RT-DETR 检测与分割、SCRFD 人脸检测对齐、ArcFace 特征、精确余弦与 HNSW 近邻检索、IoU 跟踪和连续帧投票；支持 CPU、CUDA、TensorRT、OpenVINO 与 DirectML 提供程序选择，模型权重保持在源码仓和 NuGet 之外。公共内核增加 `IMicroiVision`、租户隔离的 `V8.Vision`、插件发现与五个 Managed／CreateIfMissing 编排入口；发布脚本、保护钩子和公开仓扫描把 Vision 纳入第六个闭源边界，DLL 与 Microi.net、Microi.AI、Microi.WorkFlow 使用同一混淆保护链。
- **AI 工作流和复杂引擎收敛到应用所有的单一运行源**：业务图谱有界抽样先为每种资源保留语义锚点，再按关系类型成对保留边的两端，最后均衡补齐节点，修复大租户被截成“240 个节点、0 条关系”的伪图；紧凑节点补齐表、流程、接口等关系标识。PC 端删除重复维护的 AI 工作流、业务蓝图、3D、流程挖掘和状态机静态实现，旧路由转到权威 MicroService，避免两套代码继续漂移；AI 音乐允许既有 `music-2.6` 配置继续解析并优先使用 `music-3.0`，只有供应商明确返回 HTTP 410 退役语义时才切换开源 Music3，而不是把普通网络或业务错误伪装成降级成功。
- **SQL Server、FormEngine 与种子导入的并发边界加固**：SQL Server 动态加字段按表进入进程内门闩，先查元数据、对锁超时有界重试，并把并发造成的“列已存在”收敛为幂等成功；FormEngine 批量新增可延迟统一清缓存，批量删除在调用方事务内读取删除前快照，避免另开连接等待自己尚未提交的删除而自阻塞。MySQL 5.7 种子导入器新增流式语句阅读与批处理 API，和 SaaS 大库恢复复用同一解析事实源。
- **系统工具与高频表格体验补齐真实状态**：`platform-schedule-job v1.0.1` 用可信请求路径映射 `/api/Job/GetAllJob`、详情、新增、修改、暂停、恢复和删除七条旧路由，限制为 POST，并以真实“正常”状态统计启用任务；Redis 管理器使用当前 DiyToken 主动恢复 `CurrentUser`，只有明确鉴权失败才进入匿名模式，不再把数据库或网络异常误显示为“未登录”。Element Plus 表格只在真实横向溢出时常显 12 px 可拖动滚动条与底部留白，补齐语义色、悬停、抓取光标和减少动效；旧首页图标字体增加离线兜底，避免新装环境出现空白图标。
- **官网文档与培训资料覆盖新增能力**：企业培训交互式 HTML PPT 从 18 页扩展到 45 页，以 36 个逐项功能页覆盖表单、模块、V8、接口、数据源、AI 数据分析、AI 图片／视频／音乐创作、流程、缓存、集成、治理和可视化，并同步暗色／浅色 45 页 PDF 与完整缩略图；新增 AI 数据分析、AI 创作工作室、OCR、Vision 文档，更新 HDFS 大文件／断点续传、Docker 2 GB 代理边界与完整证书链说明。平台架构和能力地图同步澄清接口数据源、可写报表、L1／L2 缓存等事实，编译生成的 dist／PDF 不作为功能源码重复统计。
- **合并团队提交时按最终净差异去重**：v7.8.9 之后根仓上游六个实质提交修复了 PC 服务记录生成无数据与档案快照、UniApp 集福鲤邀请入口后台显隐、Microi.SSO 公开源码编译、案例册新增／编辑／图片／刷新／按钮层级，以及新数据库启动资源校验；两个仅包装这些提交的 merge 记录未重复记账。与本地升级协调器、应用商城升级和 SSO 工程重叠的文件按最终源码差异归并，闭源 WebOS 本轮无变更，Microi.WorkFlow 仅同步 v7.9.6 版本元数据，不创建虚构功能项。
- **Microi.VSCode v5.5.0 与多宿主插件完成发布**：编辑器、CLI 与插件内置类型声明新增 `V8.Vision.Extract / ExtractBatch / Compare / CompareBatch / GetCapabilities`，同步 Vision Skill、能力地图、源码规范和 Codex／WorkBuddy／CodeBuddy 清单。`npm run publish` 已完成 npm、Visual Studio Marketplace 与 Open VSX 推送；VSIX SHA-256 为 `a1c797d03d7a52779cef0e40a573a034b726a9964ca95b8bba4d6a2c86b57301`，npm tgz SHA-256 为 `6d00360774fa43e1986100028e906c2c9a3d418ea3d39c0344786ab537bc6522`，同版本索引传播延迟不触发无意义的再次升版。
- **v7.9.6 完成最终封版、公共制品发布和开发环境自动部署**：v7.9.4 全量发布后，增量门禁发现 Managed 接口已扩为 13 步，而 C# 开户原子仍会把同一任务总步数写回 12，实际 UI 会出现 13→12→13 的分母回退；该中间制品因此未被当作最终封版。修复为单一 13 步事实源并补齐 SaaS v7.8.19／v7.8.20 的 MCP 域名补偿兼容后，`Microi一键编译发布.sh` 按 `7.9.6、4、0、2、2、0、0` 从头完成现代前端与后端发布，耗时 11 分 25 秒、退出码 0；23 个 NuGet 全部推送，后端 Release 为 0 错误／28 条分析器建议，独立 18080 存活检查返回 HTTP 200，本轮未生成 Chrome 49 legacy。后端 `microi-api:latest / microi-api:v7.9.6 / microi-api-dev:latest` 三个远端标签强回读摘要统一为 `sha256:83e6baeeb0cdce2577385ef507f03cff2e5b1105a65a2d193bf322a93ccfb3d5`；现代前端 `microi-web-dev:latest / microi-web-dev:v7.9.6 / microi-client-dev:latest / microi-client-dev:v7.9.6` 四个远端标签统一为 `sha256:c855b1dbfb54836e4dcfe7040b04a14386a28cd3a5a3107a1dba407266d78bf0`，7／7 标签均由远端 manifest 独立回读。`api-dev.chongstech.com` 已自动部署并通过 liveness 回读 `BackendVersion=v7.9.6`，`dev.chongstech.com` 的入口资产清单与本次现代版构建逐项一致；公共包、镜像和开发节点部署仍不替代裸域绑定、目标租户安装及登录后真实业务验收，后三项继续以各自回读为准。

## v7.8.9 - (2026-09-03 10:31)

- **AI 助手首页改为“对话先行、能力全览”的统一入口**：移除占据首屏的五功能固定侧栏，仅在用户主动查看时展示会话历史；首屏直接提供与完整对话页同源的 AI 输入框、附件入口和快捷提示，并用“智能助手、视觉创作、声音创作、模型与扩展”四个大类承载对话、数据分析、AI 绘图、AI 音乐、AI 视频、模型列表与扩展入口。图片能力目录不再藏在二级工作台内，29 项工具按生成创作、智能编辑、人像商品、精确处理完整平铺，可一键进入文生图、图生图、重绘、高清放大、消除、扩图、授权素材去水印、证件照、抠图、多图合成、换背景、黑白上色、老照片修复等功能；1920×1080、1366×768 和移动端均保持首屏可理解、全页可达。
- **平台首页用界面引擎完成可复用的三版融合设计**：官方 `PAGE5` 首页以大号 AI 对话框为首要交互，并复用 AI 助手组件直接进入完整工作台；新增 `homeoverview` 内置组件，按当前用户真实菜单访问统计展示四项概览、近 7 日趋势图和六个常用应用，再组合我的工作、日历与系统通知。页面内容不硬编码“吾码”品牌名，自动跟随租户主题与明暗模式；统计接口只保存菜单 Id、次数和日期，按当前角色、菜单显示状态和数据权限重新过滤，不收集提示词、表单内容或业务数据，旧租户缺少新资源时安静降级。
- **官方应用商城和启动闭包同步交付首页能力**：系统账号包升至 v7.6.5，新增隐藏字段 `HomeUsageStats` 并纳入 `platform-home-overview`；AI 助手包升至 v7.6.5，仍严格保持 `mci_ai_data_assistant`、`platform-ai-account`、`platform-ai-runtime` 三个 Managed 与 `platform-ai-custom-hook` 一个 CreateIfMissing，不把首页接口误塞进 AI 包；SaaS 引擎升至 v7.8.12，应用商城升至 v7.9.8，官方资源控制面升至 v1.3.5。四包已通过 `microi_itdos` 发布和二次回读，首页 PAGE5 CAS 历史为 `01M1JDT7MAP2VGFDP3TSR1CN00`、SHA-256 `23fe05d0a1a7fdf9367d6352a593c87695b1990547936d36dfaf9c1b54ac7c28`；live 投影为 Managed 142 + CreateIfMissing 9 = 151，SHA-256 `794e6c7f3c663045a7dd710c951448c2ee3320e1ff6dea203cdce5f3d8ab325e`。
- **源码、真实租户、浏览器与公共制品完成分层验收**：服务端最终资源契约 64/64、Vue SFC 门禁 3/3、真实 Edge 登录态 AI／首页／图片／音乐共 5/5 通过，另有 2 个会产生真实供应商费用的用例按测试开关明确跳过；亮色、暗色、1920×1080、1366×768 与全页截图均完成视觉核验。现代前端生产构建和 `Microi.Anderson.sln` Release 均成功，后端 0 错误（仅 10 条既有 xUnit 分析器建议），独立 18080 liveness 返回 HTTP 200。v7.8.9 完成 22 个 NuGet 发布；Docker Desktop 的 Windows AF_UNIX 陈旧 socket 通过原位保留运行时目录恢复后，只以模式 5 续跑镜像阶段并 exit 0，未重复推送 NuGet。API 正式／测试标签统一为 `sha256:d7d472b9b41d8f323f0203a6912136f8e790365fe55ed3d2cf6f53ad373324fb`，现代 Web／Client 四标签统一为 `sha256:7f302de696aad654db182424cac14b1be96acc51f032262997077e5bbc20983d`，本轮未生成 Chrome 49 legacy；商城安装、框架升级、客户节点拉取部署与登录后业务验收继续作为独立边界。
...
- **租户数据库升级形成“启动、开户、人工补跑”共用的一条可靠生命周期**：新增 `TenantUpgradeCoordinator` 与 `IMicroiUpgrade.UpgradeTenantAsync`，API 启动、普通／ZIP 新租户创建和主租户管理员发起的人工升级统一执行物理兼容、运行时不变量及历史版本链；每个阶段受共享租约和 fencing 约束，后台任务持续记录步骤、进度、日志与取消状态，只有全部成功才推进 `ServerVersion` 并在安全点刷新语言缓存。新租户在返回成功前必须重新加载真实运行时并完成升级，失败执行补偿；连接异常按主机解析、登录拒绝、数据库不存在、容量耗尽和端点不可达输出脱敏中文诊断。官方发布源启动时不会用程序集旧副本反向覆盖官网 Managed 源；人工入口 `admin_upgrade_saas_tenant_database` 升至 v1.0.8，只接受可信当前用户、平台管理员、目标租户与持久任务 fence，SaaS 引擎包升至 v7.8.11，内置 `microi-platform-service` 升至 v1.9.12／CurrentVersion 54 并提供进度页面。
- **应用商城安装升级为面向 AI 应用、多端运行时和历史租户的可恢复交付器**：`import-microi-store-package` 升至 v2.6.8，安装前只返回不泄露包正文／秘密的资源摘要；根菜单可挂到顶层、既有目录或事务内新建目录，阻止自循环并把 ModuleEngineKey 收口到 46 字符，管理员菜单权限与资源同事务落库。Web／UniApp 独立应用自动生成或重绑可启动菜单，公开入口由当前租户 FileServer 与真实对象 Key 重建并剥离内部／私有签名地址；包版本与运行版本分离，源码字节保持原样，仅编译运行资产注入目标租户上下文，ZIP 重复路径采用最后条目且不把历史 `upload/*` 还原成源码，数据库与对象存储均按最终原始字节 SHA-256 强回读。大包继续用有界检查点分片恢复，Jint 无 CLR 密码学类型时也可计算兼容 SHA-256；`get-microi-store-model` v1.4.0 公开安全 `PackageSummary`，`get-microi-store-list` v1.4.9 统一预览地址，批量安装工作器升至 v1.4.1。
- **商城前端、后台任务与动态菜单在安装完成后即时闭环**：应用商城列表合并两类应用视图、展示包摘要并允许选择目标父级；平台维护任务首次进入终态后，SignalR／轮询会刷新商城状态、角色权限、动态路由与左侧菜单，不再要求整页刷新。新增统一动态菜单路由工具，语言切换与商城完成共用同一注册入口；旧 AI 应用和业务蓝图静态别名路由移除，在线 MicroService 成为唯一事实源，Redis 管理等平台页面补齐标准 Layout 宿主。业务失败、权限错误与网络故障继续分层处理，不用兼容回退伪装成功。
- **表单计数 SQL 与角色权限树同时修复正确性和首屏负载**：Microi.net 新增只替换开头 `SELECT` 投影的 `ReplaceLeadingSelectProjection`，避免 `_SelectFields` 只有 Id 时把 WHERE 中同名表达式也改成 `COUNT(...)` 并触发 MySQL `Invalid use of group function`，分组／HAVING 计数仍保留子查询语义。官方模块接口新增固定窄投影 `GetRolePermissionTree`，服务端缓存的 `GetSysMenuStep` 以线性树构建返回角色所需菜单；两套角色管理界面优先调用新动作，仅在精确识别旧服务不支持时回退，鉴权、网络或数据库错误一律失败关闭，并补齐加载／保存 E2E 与 SQL 回归。
- **AI 工作流业务图谱改为有界、均衡且更省传输的运行时**：Microi.AI 的 Overview 支持 `MaxGraphNodes` 50～600、默认 240，按菜单、表、接口、流程、字段、事件、页面和打印等类型轮询取样，边数最多为节点上限的 4 倍；轻量模式可省略节点详情与完整 Inventory，并从最终渲染图重建精简清单，同时分别返回可用／已渲染节点边数和截断状态。字段匹配由重复扫描改为预构建字典，空关键词结果真正返回空图而不是原图；客户端取消自动加载全量图谱，改为用户显式加载、二次限流，并同时提示“可用／当前展示”规模，避免大租户首屏卡顿、浏览器内存飙升和无效 token 上下文。
- **GoView、Three.js 与平台配置回退修复竞态和运行时兼容**：GoView 为路由切换增加最新请求门，旧项目响应在切换或卸载后直接丢弃；组件恢复改为深拷贝输入、串行创建、全部成功后原子替换画布，保存会等待恢复队列、复用同一快照给预览并拒绝跨分组重复组件 Id，失败时不再留下半张画布。Three.js r182 的 TransformControls 改为挂载／移除 `getHelper()` 返回的 Object3D 并完整释放。平台系统设置回退只接受精确 `NoExistData` 且 ApiAddress／ApiEngineKey 与目标一致的缺失场景，不再吞掉鉴权、网络和数据库错误。
- **WebOS 全面采用平台图片优先的 iOS 拟物图标体系**：新增 `ThemeMenuIcon` 图片解析与 `menu-icon-assets` 目录解析器，兼容对象／字符串图标，按“远端平台图标 → 27 个内置透明 WebP → currentColor SVG／FaIcon”降级；桌面、Dock、文件夹和窗口标题统一使用，透明素材不再出现深色底板。27 张 320×320、带 alpha、单张 23～39KB 的资源同时进入 WebOS 闭源仓与官方升级资源，SaaS 数据包为 99 条 `microi_icon` 记录执行幂等 upsert；窗口弹层拖拽、居中、权限隔离和多层文件夹行为继续由测试锁定。九个官方基础包同步升版：AI 引擎 v7.6.3、表单引擎 v7.7.0、消息通知 v1.0.15、模块引擎 v7.6.3、SaaS 引擎 v7.8.11、SSO v7.5.10、应用商城 v7.9.6、系统设置 v6.3.12、系统账号 v7.6.4。
- **官网培训大纲重写为 18 页可演示、可下载的 HTML 技术 PPT**：`/doc/about/microi-training-syllabus.html` 采用 Microi UI 深色科技视觉，左侧常驻真实缩略图导航，支持鼠标按钮、滚轮、键盘、触摸滑动、URL hash、全屏与页码跳转；4K 屏放大排版，手机和低分辨率自适应收口，并为每页提供独立图形、粒子／光带／浮层等动画以及 `prefers-reduced-motion`／打印捕获模式。内容以少量文字和架构图重点讲解 20+ 平台引擎、MCP、表单／模块／V8／工作流／AI／SaaS／多端交付，以及“减少重复上下文、复用成熟能力、结构化生成与自动验收”带来的 10× 方法论优势；下载按钮直接指向预生成的 18 页静态 PDF，不在用户点击时临时生成，公开压缩版 377,239 字节、SHA-256 `494b6a08a086c41dcf03cf86ee7390459d17981fbfebe45f4c4bf960b07355e5`，并保留 4,708,618 字节的高质量制作用 PDF；根仓同时把 `*.pdf` 固定为 binary，防止 Windows 换行转换破坏 PDF 字节偏移表。
- **MicroService 打印、AI Schema 与中文文档／Skills 单一事实源同步完善**：普通 Web／MicroService 通过 `openPlatformPrint` 调用宿主打印桥，与蓝牙设备 `V8.Print` 明确分层；3D 引擎改为官方 MicroService 交付并移除重复静态路由。AI 引擎、AI 工作流、可视化、打印、蓝牙打印、微应用、数据库字典和能力覆盖文档补齐 Schema 来源／缓存生命周期、运行入口、安全边界与验收说明；`microi.skills` 及 Microi.VSCode 的 Codex、CodeBuddy、WorkBuddy、DeepSeek Harness 多宿主插件副本同源更新。Microi.VSCode／npm CLI 已升至 v5.4.9，构建、类型检查、测试、VSIX／tgz 校验通过，npm、Visual Studio Marketplace 与 Open VSX 均明确返回发布成功，按发布规则不在同版本上等待公共索引刷新。
- **v7.8.7 全量现代版公共制品与分层回归完成验收**：`Microi一键编译发布.sh` 按输入 `7.8.7、4、0、2、2、0、0` 完成 Managed 141 + CreateIfMissing 9 = 150 项官方资源强回读，投影 SHA-256 为 `71bda47025f8626905b96f078a906185977b5f71f06fc532bb3f4b5ade4f5b8f`；现代前端压缩、`Microi.Anderson.sln` Release、独立 18080 liveness HTTP 200、三项闭源 DLL 保护、22 个 NuGet 包和 7 个 Docker 标签全部成功，明确未生成 Chrome 49 legacy。后端 `microi-api:v7.8.7/latest` 与 `microi-api-dev:latest` 摘要统一为 `sha256:4bc3f125527b025540ffede4f2c69f853fbf31067e659d279dd6b82c3565cb27`，现代 Web／Client 四标签统一为 `sha256:1616333c7a8b4b427a162d062e3741f9bd2e87082e07536970901cbc52c57aa0`。客户端定向测试 43/43、服务端资源契约 164/164、WebOS 19/19、.NET 定向测试 147/147、官网 PPT／MicroService 文档 14/14 通过；完整构建 0 错误，仅保留 10 条既有 xUnit 分析器建议。公共包、官方应用源、源码测试、客户生产部署与登录后业务验收仍是独立边界，公开发布成功不代表客户节点已拉取、重启或完成生产验收。

## v7.8.6 - (2026-09-02 06:46)

- **六个 Git 边界完成 v7.8.5 之后的远端、提交、工作树与重复合并审计**：根仓、Microi.net、Microi.AI、Microi.VSCode、WebOS 与 Microi.WorkFlow 重新抓取远端后均为 0 ahead／0 behind，没有远端独有提交、英文提交说明或团队自动 merge。根仓 `b38890984`、Microi.net `4e1eb582d`、Microi.AI `2d4821a64`、Microi.VSCode `355e090fa` 与 Microi.WorkFlow `eefb9ddcd` 是 v7.8.5 日志写入后形成的线性封版提交，内容已由上一版完整说明，本版不重复包装；WebOS 仍没有新增提交或待提交差异。写入本节前共有 56 个已跟踪差异文件（983 行新增／359 行删除）和 3 个新增源码／测试文件：根仓 42 个（777/153）及 3 个新增文件，Microi.VSCode 11 个版本与多宿主插件生成文件（197/197），Microi.net、Microi.AI、Microi.WorkFlow 各 1 个版本文件（均为 3/3），WebOS 为 0；长应用包 JSON、插件 Skills 清单、`dist/bin/obj/node_modules` 与机械版本文件只核对最终版本、摘要和同源性，不冒充独立功能。
- **租户数据库故障从模糊英文退避升级为安全、可操作的中文诊断**：Dos.ORM 在连接保护期保留不含主机名、账号和连接串的失败分类，将非法或不可解析主机、凭据拒绝、数据库不存在、连接容量耗尽及端点不可达分别映射为 `DatabaseHostInvalid`、`DatabaseCredentialsRejected`、`DatabaseNotFound`、`DatabaseCapacityExceeded` 与 `DatabaseEndpointUnreachable`，后续请求仍携带安全错误码和剩余重试秒数。平台系统设置兼容入口只用原始异常做服务端分类，对浏览器统一返回中文原因、主租户 SaaS 配置入口、写库／只读库检查项、容器可解析地址与网络排查步骤，且不会回传原主机名、密码或完整连接串。PC 启动页识别数据库故障后明确显示“租户数据库连接失败”，不再把 `Database connection is temporarily unavailable` 当成普通网络错误或让页面停在无提示空白状态。
- **子租户数据库修复支持精确收口复制残留的只读库连接**：可信宿主原子和官方 Managed 修复接口新增 `ExpectedStaleReadDatabaseName`，只有主租户超级管理员先执行无副作用预检、再提交包含旧读库名的精确确认串，并且当前只读连接实际解析出的数据库与该旧值完全一致、同时不同于目标租户数据库时，才允许把读写连接一并切换；真实读副本、空值、未知库名、并发漂移和目标库相同的情况继续失败关闭。切换仍使用旧连接 CAS、一次性影子账号、最小目标库授权、缓存刷新及写库／读库双回读，并只向响应公开 `StaleReadConnectionReplaced` 等非秘密状态。SaaS 引擎包升至 v7.7.23，本地包与官网回读共同基线 SHA-256 均为 `1b95afec5c05c338c5d697c06b738af8b2829f785100717b2b06564c321b04c8`。
- **应用商城为旧租户补齐私有 AI 源码包的响应期签名桥**：`get-microi-store-model` 升至 v1.3.1；旧版安装器不会读取 `DataAppend.ApplicationAssetDownloadUrls` 时，商城详情仅在本次普通对象响应副本中，把已通过应用身份、版本、路径归属和私有存储范围校验的 Source ZIP 临时签名地址装饰到 `FullPath/Url`，公有 Build ZIP 保持原值。HDFS 原包、`sys_microistore`、不可变 `mic_data_version` 快照、稳定文件路径与摘要均不写入签名 URL，跨应用路径继续拒绝，解决旧租户把官方源私有路径交给本租户签名后误报源码 ZIP SHA-256 校验失败。应用商城包升至 v7.8.5，本地包与官网回读共同基线 SHA-256 均为 `6a058495646de58d04f048c5666ba3f153483949bfdab4c7ed4d4516a817e3c5`。
- **发布脚本、框架与多宿主插件完成 v7.8.6 封版同步**：一键发布脚本在 MSYS `find` 不穿越 Windows 目录联接时，显式补齐 Microi.net、Microi.AI、Microi.WorkFlow 三个闭源项目并去重，避免源码存在但闭源包仍保留上一版本。客户端、`Directory.Build.props`、19 个公开后端包及三个闭源包统一为 v7.8.6；本轮根据已成功发布的 v7.8.6 继续整理日志，没有再制造或重复发布 v7.8.7。Microi.VSCode、npm CLI、Codex、CodeBuddy、WorkBuddy、DeepSeek Harness 与同步 Skills 升至 v5.4.8：npm 和 Open VSX 已公开回读为 5.4.8，Visual Studio Marketplace 上传端明确返回 `Microi.v8-engine v5.4.8 already exists`，按发布规则视为服务端已接收，不在同一版本上继续等待公共索引刷新或重复上传。
- **v7.8.6 公共制品与定向回归完成分层验收**：19 个公开 NuGet 包均已从 NuGet 平坦容器回读 HTTP 200；后端 `microi-api:v7.8.6/latest` 与 `microi-api-dev:latest` 三个标签摘要统一为 `sha256:3eb964db9a536ecdd472a30f2c92cea85bc8660e2548416e30cfac3888e29d722`，现代前端 `microi-web-dev`／`microi-client-dev` 的版本与 latest 四个标签摘要统一为 `sha256:7572632c8d3e12942d66623e3ca7cf80dde5e8bcefc55c58e773087faa797a20b`，没有生成 Chrome 49 legacy。数据库中文诊断、只读库精确修复与商城私有资产兼容 JS 回归 19/19，C# 定向回归 30/30，Git Bash 发布脚本语法和全仓差异检查通过；公共包、官方应用源、源码测试、客户生产节点部署和 `jisu1` 真实登录仍是独立验收边界，公共发布成功不等于生产环境已经拉取、重启并通过业务验收。

## v7.8.5 - (2026-09-02 00:23)

- **六个 Git 边界完成 v7.8.3 之后的远端、提交、工作树与重复合并审计**：重新抓取全部远端后，根仓、Microi.net、Microi.AI、Microi.VSCode、WebOS 与 Microi.WorkFlow 均为 0 ahead／0 behind；v7.8.3 日志之后没有远端独有提交、英文提交说明或团队自动 merge，六仓当前 HEAD 分别就是上一版的线性封版提交，因此不把已有内容重新包装。本节写入前共有 75 个已跟踪差异文件（2,674 行新增／634 行删除）和 5 个新增文件：根仓 60 个（2,377/339）及 5 个新增文件，其中真正新增源码测试为 464 行、其余 4 个是对应 `dist` 生成物；Microi.net、Microi.AI、Microi.WorkFlow 各 1 个版本文件（均为 3/3），Microi.VSCode 12 个版本／多宿主插件生成文件（288/286），WebOS 为 0。长应用包 JSON、MCP／插件 bundle、source map、`dist/bin/obj/node_modules` 与机械版本文件只核对最终版本、摘要和同源性，不冒充独立功能。
- **子租户数据库连接修复闭环解决动态绑定异常并继续失败关闭**：`TenantProvisioningService` 把 FormEngine 返回的动态行显式物化为 `object/JObject`，再调用 Dos.Common 的 `Val<T>` 扩展，避免运行时把扩展方法误解析成不存在的 `JValue` 实例成员；微服务源码暂存的既有行读取同步跨过相同动态边界。官方修复接口升至 v1.0.2，只接受并精确校验服务端／MCP 自动注入的租户、接口 Key、Server 调用语义和当前用户上下文；历史调试器追加的短标量 `TestParam1` 仅允许最多 200 字符且立即丢弃，对象、超长值、其它未知字段、密码和连接材料仍全部拒绝。SaaS 引擎包升至 v7.7.22，并由安全测试锁定影子账号、旧连接 CAS、确认串及响应脱敏边界。
- **应用商城补齐内容寻址的不可变安装快照与当前版本原位修复 CAS**：`ai_app_publish_store` 升至 v1.9.16；每次 legacy 或 V3 发布在包指针强回读后，按租户、商城记录、精确版本和包 SHA-256 生成确定性 `mic_data_version` 快照，重复发布只允许正文完全一致地幂等复用，冲突或漂移立即失败，安装方可稳定钉住 `StoreVersionId`。新增 `RepairCurrentPackageVersion` 仅允许普通 Publish、现有版本与准备包精确相同的场景，以旧 HDFS 路径、摘要、大小和更新时间组成 CAS 原位修复包字段，绝不改写 committed runtime pointer；应用商城包升至 v7.8.4，并声明 `Marketplace:ImmutableInstallSnapshotV1` 与 `Marketplace:CurrentPackageRepairCasV1` 两项强制能力。
- **MCP 微服务源码同步补齐逐文件流式协议和未知结果恢复策略**：MCP 客户端新增单文件 stage 与完整 manifest finalize 路由，目录模式先获取 `CurrentVersion / AppVersion / SourceManifestHash` 三基线，以 AppKey 与清单哈希生成稳定批次号，逐文件发送原始路径、大小和 SHA-256，最终只在三基线未漂移时原子切换私有源码。幂等 stage 响应中断会先回读后最多精确重放同一单元；finalize 和旧整包 JSON 结果未知时只接受完整路径、大小、摘要与清单哈希全部一致的活动源码回读，绝不盲目重放。无能力标志的旧服务器只允许 256 个文件／8 MiB 的有界兼容通道，非法、非规范或超限 Base64 在写入前拒绝；普通 multipart 首次请求补齐精确 `Content-Length`，代理重置后才进入 gzip 分块兜底。
- **插件、框架版本和定向回归完成封版**：Microi.VSCode、npm CLI、Codex、CodeBuddy、WorkBuddy、DeepSeek Harness 与同步 Skills 升至 v5.4.7；`npm run publish` 的测试、类型检查、构建、VSIX／npm 包校验全部通过，npm、Visual Studio Marketplace 与 Open VSX 均明确返回发布成功，按规则不等待同版本索引刷新。框架当前工作树原为 v7.8.4，按版本规则加一后，客户端、Directory.Build.props、公开后端包及 Microi.net／Microi.AI／Microi.WorkFlow 统一为 v7.8.5，WebOS 保持无差异且不制造空提交。数据库修复与商城资源测试 110/110、MCP 类型检查及测试 160/160、C# 定向测试 10/10 全部通过；完整解决方案 Release 编译 0 错误，只有 10 条既有 xUnit 分析器建议。
- **v7.8.5 全量现代版公共制品发布成功**：`Microi一键编译发布.sh` 按输入 `7.8.5、4、0、2、2、0、0` 完成官方资源 Managed 140 + CreateIfMissing 9 = 149 项强回读，资源投影 SHA-256 为 `6e549b03823e17810cf48aec6833ce207ce6dec2753076ebecec5902803e265c`；随后完成现代前端、后端 Release、独立 18080 liveness HTTP 200、Microi.net／Microi.AI／Microi.WorkFlow DLL 保护、22 个 NuGet 包与 7 个正式／测试 Docker 标签，耗时 9 分 24 秒并以 exit 0 结束，明确没有生成 Chrome 49 legacy。NuGet 22/22 均明确返回推送成功；后端三标签摘要统一为 `sha256:2b7fff37b3f92ec27bb00f34f003d0e377933bda261ea5af2728be9c6446e64d`，现代 Web／Client 四标签摘要统一为 `sha256:0eddb1069641aea724c955de48222a13945d5a80543ccdc0f37c4ea4bc0bc459`。公共包和镜像发布成功仍不等于 `jisu1` 生产节点已经拉取、重启并完成真实登录验收。

## v7.8.3 - (2026-09-01 21:44)

- **六个 Git 边界完成 v7.8.2 之后的远端、历史、工作树与重复合并审计**：重新抓取全部 origin 后，根仓、Microi.net、Microi.AI、Microi.VSCode、WebOS 与 Microi.WorkFlow 均为 0 ahead／0 behind；范围内没有 merge commit，也没有远端独有提交。根仓 `44adf8d19` 以及 Microi.net `45ca8a3ee`、Microi.AI `139142e104`、Microi.VSCode `1895316a53`、Microi.WorkFlow `62476c60c5` 分别译为“发布／维护 v7.8.2”，属于上一版已经完整说明的线性封版提交，不重复包装；WebOS 没有新增提交。真正新增的历史是根仓 `5749f3146`、`f627cbb01`、`263f6bd3a`、`51ac2aaa8` 四次闭源路径门禁加固。写入本节前，六仓共有 146 个已跟踪差异文件（5,317 行新增／1,241 行删除）和 33 个新增源码／测试文件（约 6,180 行）：根仓 109 个（3,866/836）及 30 个新增文件，Microi.net 6 个（29/10），Microi.AI 1 个版本文件（3/3），Microi.VSCode 26 个（1,296/382）及 2 个新增文件，WebOS 为 0，Microi.WorkFlow 4 个（123/10）及 1 个新增文件。长应用包 JSON、多宿主插件生成清单、`dist/bin/obj/node_modules` 与机械版本文件只核对版本、摘要和同源性，不冒充独立功能。
- **公开根仓建立不可绕过的五个闭源仓库隔离门禁**：新增唯一策略文件、根级 ignore、LF 属性和受信 pre-commit／pre-push；检查大小写变体、普通文件、目录、gitlink、replace ref、graft、浅克隆及每个待推送 commit，并在新分支／标签推送时读取远端真实引用计算完整出站范围。后续修订改为校验暂存区中的策略事实，并把两个 Hook 的可信 blob OID 钉入本地配置，同时核对工作树、暂存区和提交树，防止通过修改策略或 Hook 把 `Microi.net`、`Microi.AI`、`Microi.VSCode`、WebOS、Microi.WorkFlow 泄露到 Gitee/GitHub；本地 AI 测试凭据文件也进入忽略清单。
- **后台任务改为按租户与任务类型隔离的公平、可恢复调度**：每个 `(OsClient, ApiEngineKey)` lane 在单节点和跨节点均只允许一个执行者，平台安装、数据库备份、多语言等维护任务在同租户串行、跨租户可并行，并固定为业务任务保留两个 Worker；Redis 唤醒提示按 lane 合并，数据库持久任务、租约、幂等键和 fencing token 仍是正确性事实源。恢复扫描每批最多轮转四个租户、单租户查询限时 2 秒，连续热提示最多 16 次或 1 秒后强制完成持久恢复；取消、耗尽重试、过期租约和基础设施竞争均有有界收口，通知直接携带最新投影而不再把进度推送变成数据库轮询。升级新增 lane 领取索引，运行快照补齐最近／最慢租户扫描与唤醒诊断。
- **数据库失联保护与子租户 DatabaseOnly 修复形成可信闭环**：MySQL 平台连接默认增加 10 秒连接超时，显式禁用 TLS 时才补充公钥认证兼容；连接槽等待被硬限制在 15 秒以内，熔断期间同步／异步请求立即返回可重试超时，不再睡眠占满请求线程。主租户超级管理员新增受控修复接口：只接收租户定位字段，先 `ValidateOnly` 再校验精确确认串，在 C# 内创建一次性影子账号、限定授权目标数据库、验证核心表和当前账号，以旧连接串为 CAS 条件原子切换配置，刷新缓存并双读回验；密码和连接串不进入 V8、响应或日志，失败时保留旧账号并清理未切换的影子账号。数据库备份升级同时移除客户共享计划表上的历史全局同名唯一索引，改为普通查询索引；多语言元数据先取租户门再取全局门且两者均限时 5 秒，坏租户不再拖住健康租户启动。
- **Web／UniApp／MicroService 私有源码交付升级为逐文件流式暂存与清单 CAS 原子切换**：服务端新增 multipart 单文件暂存和 finalize 两个 MCP 路由，文件写入租户私有、批次隔离的 HDFS 前缀并逐个强回读大小与 SHA-256；完整清单在应用分布式锁内同时校验 `ExpectedCurrentVersion / ExpectedAppVersion / ExpectedSourceManifestHash`，一次切换活动元数据并归档旧源码，上传中断不会暴露半份活动源码。Microi.VSCode 5.4.6 在推送前执行 Base／Local／Remote 三方比较，远端独改或冲突一律停止；stage 只对幂等单元有界重试，finalize 传输结果未知时先回读、绝不盲目重放。旧服务器仅允许不超过 8 MiB 的兼容 JSON，HTTP 请求体以 64 KiB 分片遵守 socket backpressure；源码目录拒绝环境文件、私钥、符号链接和越界路径。新建、拉取、打开、保存、构建与推送 Vite 微服务时幂等补齐 `npm run dev`，已有自定义命令保持不变，无法确认构建体系时失败关闭。
- **应用商城发布、安装与 AI 应用资产边界继续收紧**：应用商城包推进至 v7.7.33；普通 AI 应用默认携带源码，但源码 ZIP 永远进入官方私有 HDFS，公开应用的编译 ZIP 才保持公有、私有应用的编译 ZIP 同样私有。商城源只为当前选中 AppKey／版本的私有资产生成不落库的临时地址，拒绝跨应用路径；导入器 v2.5.4 兼容历史公开 `Path`，同时绑定不可变包内应用身份和精确版本，避免跨租户地址解析失败或错包装入。导出器 v1.2.9 使用 FormEngine 事务 fence、HDFS 字节／摘要回读、发布后二阶段快照和最低运行版本回读；包版本选择改为单调递增并支持失败重跑与官网并行更新的三方合并，官方资源发布脚本以远端 SHA-256 做 CAS、发布后强回读九包 149 个接口投影。
- **通知中心与官方商城源统一采用权威安装状态并失败关闭**：官方列表同时返回未安装和可更新平台应用，通知中心不再用浏览器本地缓存猜状态；正式官网路由仅在明确不存在时回退旧地址，浏览器网络／CORS／超时才使用当前租户的受认证同源代理，业务失败不会被伪装成成功。检查请求 single-flight 合并并保留尾随强制刷新，只有平台维护任务首次进入终态才刷新安装记录；切换租户、用户或官方平台身份会让旧在飞结果失效，失败时清空旧角标并显示明确错误。SaaS 包 v7.7.20 重新嵌入 `microi-platform-service v1.9.4 / CurrentVersion 47`；消息通知包 v1.0.14 的 `platform-chat-runtime v1.0.2` 使用自包含本地时间能力，不再依赖租户是否定义 `DateNow`。
- **菜单隐藏列、工作流设计入口和审批结果完成前后端一致性修复**：模块引擎包 v7.6.2 声明 `NotShowFields` 双端自愈能力；FormEngine 通用增删改查、SysMenuLogic、菜单缓存与客户端表格渲染统一过滤 null、空字符串、非法根和无 Id／Name 对象，单条损坏配置不再让列表或子表空白。管理员可从工作流菜单和顶部标签直接进入已绑定的流程设计，入口只信任 `OpenType=WorkFlow + FlowDesignId`。发起、审批、移交、撤回和作废成功后使用居中结果弹窗展示流程结束状态、下一节点与接收人，并抑制重复通用提示；工作流运行时兼容历史角色对象、稳定 Id 和唯一名称，优先按 Id，角色缺失、删除或重名时失败关闭并把错误传回预览和真实发送。
- **平台启动、兼容 API 与 AI 版本展示补齐明确边界**：Microi.net 读取系统配置时优先使用已经规范化的缓存，再执行请求期语言元数据修复；域名解析、系统设置和旧 SSO 能力发现统一设为 15 秒上限，后端失联能进入现有启动失败／导航降级收口。前端 `V8.RunSqlGetModel/RunSqlGetList` 保留显式失败门面并给出迁移到接口引擎或参数化 FormEngine 的指引，不再恢复浏览器直连 SQL；AI 应用的 `legacy-unversioned` 快照显示为“历史归档（未标版本）”并排在真实语义版本之后。VS Code／CLI 在普通未配置 Microi 连接的工作区保持只读，不再自动创建 Microi 目录、Token、Git 或 MCP 配置，后台更新仍可独立升级全局 CLI 与 Codex／DeepSeek Harness 插件。
- **插件、框架版本与定向回归完成封版**：Microi.VSCode、npm CLI、Codex、CodeBuddy、WorkBuddy、DeepSeek Harness 与同步 Skills 统一推进至 v5.4.6；`npm run publish` 的预检、完整测试、类型检查、构建、VSIX／包校验全部通过，npm、Visual Studio Marketplace 与 Open VSX 三个发布命令均明确返回成功，按发布规则不等待同版本索引刷新。框架、客户端和全部后端包更新为 v7.8.3，Microi.AI 仅同步版本，WebOS 没有差异。当前新增功能的后端定向回归 131/131、PC 前端 58/58、官方应用资源 228/228 全部通过，完整解决方案 Release 编译为 0 错误、10 条既有 xUnit 分析器建议。
- **v7.8.3 全量现代版公共制品发布成功并修复脚本退出悬挂**：`Microi一键编译发布.sh` 最终按输入 `7.8.3、4、0、2、2、0、0` 完成官方资源 Managed 140 + CreateIfMissing 9 = 149 项强回读、现代前端、后端 Release、发布目录守卫、独立 liveness HTTP 200、Microi.net／Microi.AI／Microi.WorkFlow DLL 保护、22 个 NuGet 包与 7 个正式／测试 Docker 标签，耗时 11 分 22 秒且 exit 0，明确未生成 Chrome 49 legacy。NuGet 22/22 均返回推送成功；后端三标签摘要统一为 `sha256:41271ace96491d73c4d91745124199848dbab44f12a01194b2d1c76b39f1d7f2`，现代 Web／Client 四标签摘要统一为 `sha256:faabfc3e901ec8913434d14f0e9c52c6bdd1461653fa7c40a8a480e603661b4b`。Docker Desktop 启动改为断开标准流并从 Bash 作业表分离，避免通过 `tee` 记录日志时公共制品已经发布成功、父脚本却收不到 EOF；公共制品成功仍不等于客户生产节点已拉取、重启或完成登录后业务验收。

## v7.8.2 - (2026-09-01 12:43)

- **六个 Git 边界完成 v7.8.1 之后的提交、工作树与重复合并审计**：根仓、Microi.net、Microi.AI、Microi.VSCode、WebOS 与 Microi.WorkFlow 从 v7.8.1 日志时间点到当前 HEAD 均没有新增提交、英文提交说明或团队自动 merge，现有差异仍是 v7.8.1 与本版连续开发的未提交工作树，因此先按最终源码逐项去重，v7.8.1 已完整说明的权限快照、ApiAddress 路由、商城首次表单预载和菜单底部不在本版重复包装。写入本节前，六仓共有 85 个已跟踪差异文件（2,366 行新增／867 行删除），另有 8 个新增源码／测试文件（726 行）和 7 张产品实拍截图；其中根仓为 68 个已跟踪文件（2,110/553）及上述 15 个新增文件，Microi.net 为 2 个文件（34/108），Microi.AI 为 1 个版本文件（3/3），Microi.VSCode 为 13 个文件（216/200），WebOS 为 0，Microi.WorkFlow 为 1 个版本文件（3/3）。`dist/bin/obj/node_modules`、长应用包 JSON、插件生成元数据和机械版本文件只核对版本、摘要与同源性，不冒充独立功能。
- **Android 5+ App 蓝牙打印补齐可靠写入通道协商和 CC4 安全降级**：BLE 同时保留 `write` 与 `writeNoResponse` 候选，Android 优先验证无响应写入，候选特征只有在首包真实成功后才持久化；仅当 ZICOX CC4 的全部 BLE 候选首包均返回 10007 时，才切换到同一台已配对设备的经典蓝牙 SPP/RFCOMM。首包切换后保持原顺序和份数，后续分包失败绝不换通道，避免重复输出半张标签；BLE 与 SPP 都失败时主动断开并返回组合诊断，非 CC4 设备不会猜测 SPP。
- **移动端微应用宿主与确认弹窗体验完成收口**：手机端且非小程序的 MicroApp 页面新增原生安全区导航栏，展示当前模块标题，返回时优先使用历史栈、没有历史时稳定回到 `/mobile/workspace`；确认／警告弹窗把提示色与操作色分离，警告图标继续使用 warning 强调色，主操作按钮则固定跟随平台主题主色，避免把正常确认误呈现为危险操作。
- **DatabaseOnly 内置微服务包增强跨租户可移植性**：嵌入器允许历史 `PackageAssets:null` 并归一化为完整元数据对象，同时继续拒绝标量、数组等非法结构；路由同步会补齐 `PageName`，并删除不可跨环境携带的 `_MicroiV3` 私有状态，使数据库内联运行产物不依赖目标租户对象存储。正式应用包与 `.resource-sync-base` 镜像已逐字节核对一致，SaaS、Store、SysConfig 三组 SHA-256 分别为 `8cfe95f03c079c36550e14ab42518cb5edceb28a1f2283b5848fd66771ab103d`、`04f259a4935168d0548131a75dd08e4bd989816804680a8d5f315f71e54b8296`、`6ec1555060d91daf46feb3001875f251ff22292eeb0972dff3741826b6632833`。
- **官网产品实证与蓝牙打印文档升级**：README 与官网首页改为三列展示 WebOS 接口引擎、AI 平台和 AI 编程助手，并补充移动端 AI 助手、工作台、蓝牙设置与打印规格截图；蓝牙打印文档新增 GP-M322 实机标签、业务测试页、连接状态和规格选择四图画廊。七张新增产品截图、22 图统一灯箱和被替换旧图清理均由自动化契约保护，文档展示的是实际产品界面与打印输出，不以构建占位图替代业务证据。
- **插件和六仓版本元数据同步封版**：Microi.VSCode、npm CLI、Codex、CodeBuddy、WorkBuddy、DeepSeek Harness 与同步 Skills 推进至 v5.4.5；`npm run publish` 完成测试、构建、VSIX、npm、Visual Studio Marketplace 与 Open VSX 发布，各目标命令均明确返回成功。框架 `Directory.Build.props`、客户端及后端项目统一更新为 v7.8.2；Microi.AI、Microi.WorkFlow 本版仅同步版本，WebOS 没有源码差异，不制造空提交。
- **v7.8.2 全量现代版制品完成发布门禁**：蓝牙打印、MicroApp 宿主、确认弹窗、平台资源与官网图片定向回归 53/53 通过；`Microi一键编译发布.sh` 按输入 `7.8.2、4、0、2、2、0、0` 完成官方资源 148 项回读、现代前端、后端 Release、发布目录守卫、独立 liveness HTTP 200、三份 DLL 保护、22 个 NuGet 包与 7 个正式／测试 Docker 标签，耗时 9 分 32 秒并以 exit 0 结束，明确未生成 Chrome 49 legacy。NuGet 22/22 均返回推送成功；后端三标签摘要统一为 `sha256:eb7a04f4831fffdd14eb70ebd572185cbea306e4ba5ada19d50f16b3353638a9`，现代 Web／Client 四标签摘要统一为 `sha256:3777f8a75c4ce2db85a0cc6e0acaab1990545efcea8c3314237da364d393254d`。公共制品推送成功不等于客户生产节点已拉取、重启或完成登录后业务验收。

## v7.8.1 - (2026-09-01 09:40)

- **登录与续签的权限快照改为主库权威、失败不污染且可在当前会话内自动修复**：根因是历史管理员的 `RoleIds` 可能保存为对象数组，旧刷新链把它按普通字符串拆分后得到空角色，再把“0 角色／0 权限”的异常投影写回 DiyToken 缓存；页面因此偶发失去“新增、编辑”等按钮，而重新登录恰好重建了正确快照。后端现在使用主库同一会话读取用户、角色和权限，兼容历史对象数组及未显式软删除的角色；任何权限读取失败都保留最后一份可信缓存，并在同一 Rotate 锁内只替换最新会话的 CurrentUser。Token 已失效或被吊销时拒绝借续签复活，超级管理员身份由权威等级重新计算。
- **前端增加异常授权快照识别与同用户安全自愈闭环**：应用启动、登录和用户 Store 共用当前用户校验器，识别后端 `_RoleLimitsError*`、超级管理员等级与空权限矛盾等技术故障；只对 JWT 中 UserId、OsClient 与本地最后可信用户完全一致的会话执行一次 refreshToken 修复并恢复菜单，不吞掉正常权限回收、换号、换租户或 Token 失效。严格浏览器故障注入连续 3/3 轮均在不重新登录的情况下恢复接口引擎“新增”与首行“更多 → 编辑”，回读 1 个管理员角色、1,953 项权限及 `IsAdmin=true`。
- **接口引擎冷缓存路由坚持完整 ApiAddress 优先并阻断停用地址误回退**：动态路由先按主库权威的完整 `ApiAddress/ApiRoutes` 查找，只有地址从未配置时才把单段 `/apiengine/{Key}` 作为兼容 Key；已停用地址仍视为占用，调用方请求体也不能覆盖宿主解析出的 Key 或地址。命中传统 Key 后只补安全推导别名，解决同名 Key、自定义地址、旧缓存与读副本延迟交织时误执行另一个接口的问题。
- **平台壳与官方应用交付同步收口**：应用商城微服务升级至 v1.9.3 / CurrentVersion 46，宿主首次点击“编辑／制作”会单飞预载异步表单并等待真实 `Init` 就绪，不再提示组件尚未就绪；菜单底部在租户未配置时展示框架版本和动态年份，租户自定义内容仍拥有最高优先级。SaaS 引擎官方包发布 v7.7.18 并声明 `ClientFeature:AuthorizationSnapshotBootstrapRepairV1`，应用商城包保持 v7.7.28；通过 `microi_itdos` 发布并回读 Managed 139、CreateIfMissing 9、总计 148 项，二次同步为零远端变化。
- **源码、资源、构建和公共制品完成分层门禁**：授权／登录前端定向回归 24/24、后端授权与接口缓存定向回归 91/91、官方 Resource 全量契约 426/426、平台商城和菜单底部真实浏览器回归全部通过；Release 构建 0 错误，发布目录独立 liveness HTTP 200。`Microi一键编译发布.sh` 按输入 `7.8.1、4、0、2、2、0、0` 完成现代前端、后端、三份 DLL 保护、22 个 NuGet 与 7 个正式／测试 Docker 标签，耗时 11 分 39 秒并以 exit 0 结束；NuGet.org 最终回读 22/22，后端三标签摘要统一为 `sha256:a8cdd2e0c0ab208834aeadb9e1d73a62b1e6b3c1c5e1eb9ba8026f0eed3003a7`，现代 Web／Client 四标签摘要统一为 `sha256:14e0773ec04de956ff2ffff6059a8a48a44e32a5c40495d1f9bcd8ce9d7f99ae`，Docker 远端 7/7 回读一致。公共制品可取、目标生产节点完成拉取重启与登录后业务验收仍是独立边界。

## v7.8.0 - (2026-09-01 03:57)

- **六个 Git 边界按 v7.7.9 封版点完成历史、工作树与重复合并去重审计**：v7.7.9 日志所列基线之后，根仓 `0b98d1fad2d3`、Microi.net `ef547ffb1530`、Microi.AI `94401306beff`、Microi.VSCode `ee332cb37450`、WebOS `bcdd23e4c3b4` 与 Microi.WorkFlow `8e0bb5db68fc` 仅包含上版已经说明的线性封版提交，范围内没有团队自动 merge、遗漏的语义变更或需要补译的英文说明，不在本版重复包装。写入本节前，根仓为 69 个已跟踪差异文件（2,715 行新增／521 行删除）和 8 个新增源码／测试文件（865 行），Microi.net 为 2 个文件（15/6），Microi.AI 为 1 个版本文件（3/3），Microi.VSCode 为 11 个版本与多宿主插件元数据文件（197/197），WebOS 工作树为 0，Microi.WorkFlow 为 1 个版本文件（3/3）；`dist/bin/obj/node_modules`、VSIX、tgz、长应用包 JSON 与机械生成元数据只核对版本、摘要和同源性，不冒充独立功能。
- **旧租户升级链补齐物理兼容、自诊断和连续租户恢复能力**：平台运行时的每个不变量步骤现在记录精确阶段、根异常类型、脱敏原因和恢复建议，失败不会推进 `ServerVersion`；接口引擎缓存重建先把可变运行时行深拷贝为独立快照，避免并发枚举触发“目标数组长度不足”等异常。单租户在数据库前置阶段失败时跳过无意义的长超时多语言缓存刷新并继续升级后续租户；仅对平台受信元数据表和已知 FormEngine 兼容错误启用参数化物理回退，并严格限制表／字段白名单、租户、Id、数据库方言、升级租约、影响行数和逐字段回读，防止旧结构修复演变为无边界写库。
- **应用商城导入器 v2.5.1 修复极老数据库结构、接口缓存和安装状态闭包**：字段导入前幂等补齐旧库 `diy_field.OsClient`，恢复历史 `sys_menu` 时不再写入其不存在的 OsClient 物理列；Managed 接口在 FormEngine 回读为空时按已验证的物理 Key／Id／地址完成参数化强回读，补正前清理旧 `ApiAddress/ApiRoutes` 别名，再以包声明事实重建缓存。导入失败会返回“物理前置、DDL、表、字段、菜单、工作流、接口、随包数据、最终回读”等准确阶段；商城安装记录按更新时间确定性选取最新有效行、忽略软删除，统一识别失败、空版本、可更新、已安装和本地版本更高等状态。官方 Store 包推进至 v7.7.27、SaaS 包推进至 v7.7.13，两个真实客户租户的后台更新任务均成功且幂等重放保持同一任务。
- **API 异常从笼统“服务器内部错误”升级为安全、可操作的诊断协议**：全局异常中间件与 MVC Filter 统一返回异常类型、根因类型、脱敏根因摘要、TraceId、故障层和恢复建议，覆盖 `AggregateException` 非首分支、请求体过大、超时和连接压力；任何环境都不会把 StackTrace、连接串、Token、密码、文件路径或控制字符返回浏览器，完整堆栈只保留在服务端日志。FormEngine 更新失败也会按 `_TableName → FormEngineKey → TableName → TableId` 解析首个非空表名，不再出现无法定位的空白 `[表：]`。
- **规范接口引擎路由正确兼容 `--OsClient--` 租户后缀**：`/apiengine/{Key}--OsClient--{tenant}--` 会先移除锚定租户后缀，再按规范 Key 读取缓存与主库权威行；控制器优先采用动态路由已经解析的真实 Key，消除旧贪婪正则把整个后缀吞入 `ApiEngineKey`、继而把存在接口误报为 `NoExistData[ApiAddress]` 的问题。自定义 `ApiAddress` 继续保持独立解析边界，本地 iTdos 登录、当前用户、菜单及接口路由浏览器回归均无业务错误。
- **MongoDB SysLog 索引初始化改为有界、脱敏且不阻塞日志写入**：同一端点／库／集合使用 `Lazy<Task>` 单飞初始化，瞬时网络异常最多三次有界重试，连续失败进入指数冷却并按分钟限频输出诊断；连接串只进入不可逆端点指纹，异常摘要会移除凭据、主机、路径和控制字符。索引初始化失败不再阻断日志数据写入，也不会让每批日志无限重试同一故障，从根源上控制客户升级日志中的 `MongoDB 创建SysLog索引失败` 风暴。
- **前端商城记录与表单交互细节完成收口**：新增共享商城安装记录解析器，使启动提醒、通知中心和商城页面采用同一套最新记录与版本判断；首次默认只展示平台应用，用户仍可主动切换全部或其它类型。展示型表单不再给 Select 写死 34px 高度，多选大量选项时由内容自然撑开并保留最小高度；相应测试先剔除注释再校验有效 CSS，避免注释中的旧声明造成假通过。
- **插件、官方资源与分层回归完成公开验收**：Microi.VSCode、npm CLI、Codex、CodeBuddy、WorkBuddy 及同步 Skills 推进至 v5.4.4，`npm run publish` 成功后，npm、Visual Studio Marketplace 与 Open VSX 三端均精确公开回读 5.4.4。后端升级／异常／Mongo／路由定向测试 73/73、官方 Resource 全量契约 422/422（其中导入器与商城安装 82/82）、PC 定向测试 9/9 全部通过；九个官方应用的 148 个接口运行时投影完成发布后回读，应用包、独立源码副本和共同基线保持一致。
- **`Microi一键编译发布.sh` 完成 v7.8.0 全量现代版公共制品发布**：按输入 `7.8.0、4、0、2、2、0、0` 完成现代前端、后端 Release（0 个错误、9 条既有 xUnit 分析器建议）、发布目录守卫、独立 liveness HTTP 200、Microi.net／Microi.AI／Microi.WorkFlow DLL 保护、22 个 NuGet 与 7 个正式／测试 Docker 标签，耗时 9 分 30 秒并以 exit 0 结束，明确未生成 Chrome 49 legacy。后端三标签摘要统一为 `sha256:f9be84668d4d21c482f7ba0e8ca9f274164ab83572800f8ee02a6e83535ff665`，现代 Web／Client 四标签摘要统一为 `sha256:9cdc536cff88874ad0e19e11b699eb458e5d5853ac24d4e18320d2a628cef308`；发布锁已释放。公共制品可取、客户应用包已更新与生产节点已经拉取重启仍是三个独立验收边界，生产编排与登录后业务复测在部署步骤单独完成。

## v7.7.9 - (2026-08-31 18:21)

- **六个 Git 边界完成远端、历史与当前工作树去重审计**：重新抓取全部 origin 后，根仓 `ecba545c1`、Microi.net `b707716`、Microi.AI `34f31ed`、Microi.VSCode `b9438f2`、WebOS `bcdd23e` 与 Microi.WorkFlow `9ab14b1` 均为 0 ahead／0 behind，远端分支与本地 HEAD 一致；六仓最后提交均是 v7.7.6 封版提交，v7.7.8 是尚在工作树中的日志与功能闭包，因此不存在 v7.7.8 之后遗漏的提交、团队自动 merge 或待翻译的英文提交说明。写入本节前，根仓排除 `dist/bin/obj/node_modules` 和四份长应用包 JSON 后为 105 个已跟踪文件（4,148 行新增／1,385 行删除）和 13 个新增源码／测试文件（1,463 行），Microi.net 为 4 个文件（90/13），Microi.AI 为 1 个版本文件（3/3），Microi.VSCode 为 17 个版本、插件元数据与 Skills 文件（244/226），WebOS 工作树为 0，Microi.WorkFlow 为 1 个版本文件（3/3）；六仓净审计共 141 个文件、5,951 行新增／1,630 行删除。v7.7.8 已完整记录的微服务、打印、界面密度、宿主健康检查、SaaS 与官方资源闭包不在本节重复冒充新功能，长应用包 JSON 和机械生成 Bundle 只核对版本与同源性。
- **公开版与创建人版解决方案的依赖拓扑正式分离**：公开 `Microi.net.sln` 固定通过 NuGet 消费 Microi.net、Microi.AI、Microi.WorkFlow，创建人 `Microi.Anderson.sln` 固定引用三份源码；单项目构建继续保留 `Auto`，并对无效引用模式和缺失源码项目增加构建前失败门禁，避免完整源码目录意外改变公开解决方案的依赖闭包。依赖拓扑、私有文件授权与 SaaS 配置缓存定向后端回归 57/57 通过。
- **接口引擎和商城查询补齐主库权威回读闭包**：在 v7.7.8 已完成的租户缓存与路由别名加固之上，冷启动、后台任务和路由缓存初始化改从租户主库权威读取启用接口，再修复共享缓存，避免读副本延迟把真实路由误判为不存在；Key、Route、Id 继续按确定优先级解析并隔离同级歧义。正式商城列表优先调用新接口，仅在明确的路由不存在结果下回退旧接口，不把业务错误伪装成兼容回退。
- **官方应用包升级与历史稳定路由统一到同一份声明式事实**：`Managed` 资源以本次已校验包正文为最终事实，同版本漂移、软删除、稳定 Id 或主／多路由占用会自动覆盖、恢复、重映射或收回路由，并在写入后严格回读；`CreateIfMissing` 对既有记录继续保持不变。旧 `platform-sys-user-session` 与 `platform-workflow` 会从 `_RequestPath/ApiAddress` 大小写无关地归一化 `refreshToken`、`tokenlogin`、`getWFWork`、`getWFFlow` 等历史动作，同时继续拒绝未知动作和不允许的 HTTP 方法。
- **私有文件授权改用主库权威字段元数据并补齐可诊断的失败关闭**：字段查询同时绑定已授权表，精确 Field Id 优先于同名字段，仅在 Id 确实不存在时才兼容字段名；依赖异常只向调用端返回追踪编号，安全日志仅保留执行阶段、异常类型、HResult 与消息 SHA-256，不记录文件路径、业务 Id、URL、Token、密码或对象存储密钥。
- **登录入口、后台任务和弹窗交互完成最后收口**：仅在 `/`、`/login` 与 `/access-login` 登录入口静默消费缺失或过期身份，真实业务页面继续提示准确的 Token 失效信息；后台任务把业务 `OpenUrl` 与文件 `DownloadUrl` 分开，站内路由、Hash 与外链按受控边界打开；全局 Element Plus Dialog 采用视口内 Flex 外壳、固定头尾和独立滚动正文，并以 `100dvh` 兼容低视口。PC 登录／任务／弹窗／打印／微服务／密度定向契约 31/31 通过。
- **Docker 一键安装器补齐终端编码自适应与无副作用自检**：在 sudo、修复流程和任何用户可见中文之前完成 locale 与输出编码探测，现代终端保持 UTF-8，GBK／GB18030／GB2312 终端可通过同步 `iconv` 转码且保留业务退出码；新增 `--encoding-check-only`，本轮回读 `UTF-8 + C.utf8 + 吾码中文显示正常`，脚本源文件继续保持 UTF-8 no-BOM、LF，不修改宿主机全局 locale。
- **AI 建模、微服务和附件迁移补齐失败关闭边界**：表单引擎知识库明确子表父关联键为空时前后端都必须返回空集合、禁止退化为全表查询，新增父表取消时不得预持久化默认子记录，避免孤儿数据；MicroService 每次发布前重新拉取远端源码，按 Base／Local／Remote 三方合并，并以版本和哈希 CAS 阻止覆盖并发更新；外部附件迁移必须按 `FileUpload.Limit` 进入对应私有桶，空文件不得建立占位记录，私有文件还需通过代理 Range 200/206 实读验收。同时移除过宽的通用管理表操作指引。
- **Microi.VSCode 与多宿主插件同步发布 v5.4.3**：`npm run publish` 完成扩展、CLI 与 Codex／CodeBuddy／WorkBuddy 插件 Skills 构建和发布，npm、Visual Studio Marketplace、Open VSX 三端最终回读均为 5.4.3；传播等待期间没有重复升版或重发，工作区与插件 Bundle 的相关 Skills 版本和规则保持同源。
- **`Microi一键编译发布.sh` 完成 v7.7.9 全量现代版公共制品发布**：输入 `7.7.9、4、0、2、2、0、0`，完成官方资源回读（Managed 139、CreateIfMissing 9、总计 148，SHA-256 `3c99401f5fd94df9a7d6a68893a2b5f812106d78b75de17e6a6c251e66cd19d7`）、现代前端、后端 Release、发布目录守卫、独立 liveness 冒烟、三份 DLL 保护、22 个 NuGet 与 7 个 Docker 标签，主流程耗时 10 分 11 秒并以 exit 0 结束，未生成 Chrome 49 legacy。NuGet.org Flat Container 最终回读 22/22；后端三标签摘要统一为 `sha256:0af5a97dceceba4cad70942b07bccb5c2837d5ed0a03fcb103d289a43fb299ab`，现代 Web／Client 四标签摘要统一为 `sha256:7736d002027d925ee05c2c91b50390b819889fb348518ecfe53500f0a4e00719`，远端 7/7 标签独立回读一致。公共制品可取不等于客户生产节点已经拉取、重启或完成真实业务验收。

## v7.7.8 - (2026-08-31 16:44)

- **微服务运行入口、租户文件和宿主加载链完成系统性加固**：MicroApp 运行时会在租户托管文件、父租户共享文件与可信 HDFS 地址之间按明确边界解析入口，发布回读在控制面短暂缺少投影时可从已验证对象恢复，不再把已发布应用误报为 `MICRO_APP_ENTRY_HTTP_ERROR / HTTP 502`。真实租户的 12 个已发布微服务完成本地与线上共 24 个入口 HTTP 200、正文长度与 SHA-256 双端一致验证；草稿应用继续保持不可运行状态，不被伪装成已发布资源。
- **MicroApp 获得平台普通打印桥接并重做蓝牙打印可靠性边界**：子应用可通过受控宿主消息打开平台打印预览，支持在业务页面选择“普通打印”或“蓝牙打印”；Web Bluetooth 现在按可写能力对特征值排序，首包遇到不支持操作会自动尝试下一候选特征，随后按设备安全包长分片写入，同时保留已选设备、断线重连、uni-app 与 SPP/RFCOMM 兼容路径。界面只在浏览器实际完成写入后提示已发送，设备是否出纸仍由现场打印机状态确认。
- **PC 右上角新增全局字体与间距密度设置**：提供 90%、95%、100%、105%、110% 五档，字体、行高、表格、表单、按钮、菜单、标签页和弹层间距联动缩放，切换结果持久化并即时应用；亮色、暗色、桌面和 `1024×600` 低视口均保持可读，移动触控目标继续保留至少 44px，不因紧凑模式破坏可操作性。
- **平台服务健康检查改为宿主原生只读端点**：`/api/Diagnostics/liveness`、`/api/Diagnostics/health` 与兼容地址 `/apiengine/platform-service-health` 由 `Microi.net.Api` Controller 直接提供，统一返回 `no-store`，不再依赖接口引擎是否已写入、租户升级是否完成或 Redis 缓存是否预热；发布目录独立启动仍以原生 liveness HTTP 200 作为冒烟门禁。
- **SaaS 子租户启动与升级链消除共享 Redis、别名冲突和旧结构差异造成的级联失败**：子租户只读继承主租户 Redis 基础设施投影，避免控制面字段不全时连接错误实例；接口路由别名按 Key、Route、Id 的确定优先级解析，同优先级多拥有者会隔离冲突别名并继续加载其余安全路由。Upgrade13 补齐强类型转换与依赖闭包，ServerVersion 只在真实成功后精确推进；HDFS 可信地址、MCP 应用存储父租户回退、发布回读和敏感配置脱敏同步加固。
- **五租户真实升级与分层回归验证通过**：`congshi`、`qiqiang`、`myzsl`、`huayou`、`junchi` 均输出最终迁移成功汇总，junchi 从 7.6.10.0 推进到 7.6.11.0 并完成 148 项运行时闭包。别名与升级定向测试 68/68、后端扩大回归 257/257、官方 Resource 契约 416/416、PC 打印／微服务／密度／健康检查定向测试 56/56 通过；Release 解决方案编译 0 错误，9 条为既有 xUnit 分析器建议。
- **官方应用资源继续保持源码、内置副本和官网投影同源**：发布前通过 `microi_itdos` 回读 Managed 139、CreateIfMissing 9、总计 148 项，投影 SHA-256 为 `3c994401f5fd94df9a7d6a68893a2b5f812106d78b75de17e6a6c251e66cd19d7`；12 个官方资源与商城包内嵌副本逐项核对字节数和 SHA-256 一致，既有 Store v7.7.24 与 SaaS v7.7.12 前向版本未被回退。
- **`Microi一键编译发布.sh` 完成 v7.7.8 全量现代版公共制品发布**：输入 `7.7.8、4、0、2、2、0、0`，完成现代前端、后端 Release、发布目录守卫、独立 liveness 冒烟、三份 DLL 保护、22 个 NuGet 与 7 个 Docker 标签，主流程耗时 18 分 28 秒并以 exit 0 结束；NuGet.org Flat Container 最终回读 22/22。后端三标签摘要统一为 `sha256:69c0ebbbe2ba4a6b3969db9d863bf46e71006b12d86e4cceedd629a287463e56`，现代 Web／Client 四标签摘要统一为 `sha256:0c38fe36e366c7585bf2fcd5a4d5f509ff2dcb3fbb4c77690c775348f6d6d0e4`，远端 7/7 标签独立回读一致；同日 v7.7.7 是在最终 Redis 与路由别名修复前产生的中间制品，已由 v7.7.8 取代。公共制品可取不等于客户生产节点已经拉取、重启或完成登录后的业务验收。

## v7.7.6 - (2026-08-30 21:44)

- **六个 Git 边界按 v7.7.4 封版点完成远端、提交历史与当前工作树去重审计**：重新抓取全部 origin 后，根仓 `256099f10`、Microi.net `0c9191f12`、Microi.AI `13c37ecd4`、Microi.VSCode `c90505071` 与 WebOS `7ad62fd54` 均为 0 ahead／0 behind；日志写入时间之后出现的四笔提交分别只是已经由 v7.7.4 完整说明的线性封版提交，WebOS 没有新增提交，新建的 Microi.WorkFlow 远端只有 `cdd418b7` 初始 README，因此没有团队自动 merge、遗漏英文说明或应重复计算的功能差异。写入本节前，根仓排除 `microi.mcp/dist` 后为 229 个已跟踪文件（9,836 行新增／14,131 行删除）和 56 个新增源码／测试文件，Microi.net 为 10 个已跟踪文件（259/5,011）和 4 个新增运行时文件，Microi.AI 为 1 个版本文件（3/3），Microi.VSCode 为 25 个版本、插件 Bundle、Skills 与源码文件（406/324），WebOS 为 31 个已跟踪文件（2,241/1,187）和 6 个新增组件／工具文件，Microi.WorkFlow 为 17 个新增源码文件；长应用包 JSON、`dist/bin/obj`、VSIX 与 tgz 只核对版本、摘要和同源性，不冒充独立功能。
- **平台 HTTP 入口大规模收敛为“最小可信原子 + Managed 接口引擎 + 租户 Hook”**：诊断、外部登录、强身份验证、旧移动端兼容、商城源码、SSO、用户与访问密钥、租户系统设置、微信和工作流等历史 Controller 的业务编排迁入官方接口引擎，C# 只保留协议、鉴权、密钥、存储和运行时边界。`sys_apiengine.ApiRoutes` 支持一个引擎声明多条稳定兼容路由并随缓存正确失效；统一 HTTP 响应契约支持受信跳转、多 `Set-Cookie`、安全响应头、二进制 Base64 和整段动态路由模板，同时拒绝开放重定向、歧义正文、无效 Base64、Cookie 注入与危险响应头。删除 HomeController 后，根页面由显式 `MapGet` 端点恢复官方站点 iframe 与租户自定义 HTML，并固定 `Cache-Control: no-store`。
- **SSO 从宿主 Controller 抽离为可复用的 `Microi.SSO` 协议组件**：新增 OIDC、CAS、SAML2、外部登录和可信回调运行时，通过 `ISsoProtocolRuntime`、`V8.Method` 最小原子与官方 Managed 引擎提供完整稳定地址；登录成功后仍签发 Microi DiyToken，不建立第二套身份会话。连接配置、签名／断言校验、回调状态、Cookie、跳转白名单与 SaaS 租户隔离统一进入服务端权威边界，接口引擎继续负责日志、写表、通知和业务编排，官网与 Skills 同步补齐入站、出站、配置、安全及验收规范。
- **工作流从 Microi.Core／Microi.net 解耦为独立 `Microi.WorkFlow` 项目和 Git 仓**：原先集中在 Microi.net 的约 5,000 行工作流实现迁入 17 个按取消、发送、发起、撤回、转交、下一处理人、起始节点、上下文和用户辅助职责拆分的源码文件，并新增 `IWorkFlowApiRuntime`、统计缓存、抄送已读状态与宿主扩展。接口引擎通过最小工作流运行时执行完整状态机，平台发布新增 `Microi.WorkFlow` NuGet，并与 Microi.net、Microi.AI 一样在入包和 Docker 前完成 DLL 保护。
- **用户、商城、系统设置、微信与管理工具补齐受信运行时和失败关闭安全边界**：Microi.net 新增旧 Os 兼容、用户登录／刷新／退出、租户管理员凭据、商城源码发现／验证码／登录／查询／断开及租户系统设置读取／保存／敏感值揭示运行时；Microi.WeChat 承接内容安全和 OAuth 原子，访问密钥管理进入专用 V8 原子。License 验证码改为租户作用域、一次性不透明标识并从权威 `sys_config` 读取策略；MCP 管理数据工具只接受安全标识符，默认脱敏，拒绝调用方身份／秘密字段篡改，并使用操作级、行级确认哈希和管理员权限双重门禁。
- **PC 前端与 WebOS 完成登录启动、直接模块窗口和现代工作台闭包**：菜单启动不会再用缺失、过期或已吊销 Token 发起无效请求，当前用户状态给出可恢复错误；官方主租户明确排除应用安装／更新提醒，通知中心继续保证低视口分页可达。WebOS 新增页面级模块宿主、窗口内路由、桌面／Dock 文件夹菜单与加载骨架，拖拽期间保持单一真实窗口，并统一亮暗主题、SVG `currentColor` 玻璃质感、Dock、文件夹、窗口和小组件计时；MicroApp 稳定入口、现代表单工作台、许可证验证码、弹窗状态和关闭清理同步加固，覆盖桌面与移动端布局契约。
- **后端基础设施与高并发路径完成一轮窄范围加固**：属性、JSON 序列化、动态调用、映射、消息等热点缓存改为线程安全且有界；HttpClient 请求头按请求隔离、表单正确编码并明确响应流所有权。Where 同步、Dos.ORM Navigate、数据库诊断、Token 到期边界、SaaS 初始化递归保护、部门子编码、Office 空请求与网络超时均增加参数校验、跨线程隔离和失败关闭回归；新增 ApiRoot、HTTP 响应、缓存并发、DB 诊断、序列化、消息、ORM、Office、验证码、Token 与 MCP 管理数据测试，避免一次性能优化扩大成全局行为回归。
- **官方应用资源形成多路由、精确菜单和 UTF-8 包传输闭包**：应用包升级脚本支持三方同步、完整 `ApiRoutes`、菜单字段精确映射、UTF-8 Base64 正文与同版本不可变历史，商城包独立推进到 v7.7.18，声明通知低视口、官方租户无安装提醒、WebOS 直接模块窗口、菜单鉴权与 MarketplaceSource 迁移等能力。运行时投影最终为 Managed 139、CreateIfMissing 9、总计 148 项，SHA-256 为 `76ffb031916e31b79cd4903dc62abd674ab3c6d34cbae1907e83100576657d42`；升级门对 `microi-store-package-storage@v1.1.0` 改为最低能力版本判断，允许已发布的 v1.2.1 前向兼容，同时继续拒绝 v1.0.9，相关 .NET 门禁 16/16、资源脚本回归 56/56 通过。
- **Microi.VSCode、CLI 与多宿主插件发布 v5.4.2**：接口引擎编辑器新增分号分隔的兼容路由输入，拉取、推送、文件头元数据和 MCP 同步完整保留 `ApiRoutes`；Codex、CodeBuddy、WorkBuddy 的插件清单、Bundle、Skills 与构建元数据同源更新。`npm run publish` 完成 npm `@microi.net/cli@5.4.2`、Visual Studio Marketplace `Microi.v8-engine 5.4.2` 和 Open VSX `Microi.v8-engine 5.4.2` 发布；首次市场索引仍返回旧版，未重复升版，等待传播后再次执行 `publish:verify`，三端均精确回读 5.4.2。
- **`Microi一键编译发布.sh` 按指定序列完成 v7.7.6 全量现代版公共制品发布**：从 `Directory.Build.props` 的工作树版本 7.7.5 按规则加一，输入 `7.7.6、4、0、2、2、0、0`；首次运行在官方商城包已前向推进到存储能力 v1.2.1 时被精确版本门拦截，修正为最低兼容能力并通过回归后保持同一 v7.7.6 从头重跑，最终 exit 0。现代 Vite 前端、后端 Release（0 个错误、9 条既有 xUnit 分析器建议）、发布目录守卫、独立 18080 liveness HTTP 200、22 个 NuGet 与 7 个 Docker 标签全部完成，明确未生成 Chrome 49 legacy；Global CDN 已逐包回读 NuGet 22/22，后端三标签摘要统一为 `sha256:ebe0677675a1406d63e7f30d1ebd19dd76812e9bdfdae0793e730ee772f108d1`，现代 Web／Client 四标签摘要统一为 `sha256:c3b3642c243d09394cb81610c12dcd60c7ee5f918d89a168c53029ed8aa847c3`，远端 7/7 标签独立回读一致。公共包与镜像可取、官方资源可回读和文档发布分别独立验收；这些证据不代表任一客户生产节点已经拉取、重启或完成登录后的业务页面验收。

## v7.7.4 - (2026-08-28 23:36)

- **五个 Git 边界按 v7.7.3 封版点完成远端、提交历史与当前工作树去重审计**：重新抓取全部 origin 后，根仓 `b2aeb0828`、Microi.net `4941783c`、Microi.AI `5c3f3de`、Microi.VSCode `3b380d44` 与 WebOS `7ad62fd54` 均为 0 ahead／0 behind。日志中 v7.7.3 标记的五个旧边界之后，四仓各自只有一笔已经由 v7.7.3 完整说明的线性封版提交，WebOS 没有新增提交；未发现团队自动 merge、遗漏英文说明、远端独有提交或应重复计算的功能差异。写入本节前，根仓排除 `microi.mcp/dist` 后为 53 个已跟踪文件（1,173 行新增／266 行删除）和 8 个新增源码／测试文件，Microi.net 为 1 个版本文件（3/3），Microi.AI 为 3 个音乐兼容与版本文件（72/7），Microi.VSCode 为 14 个版本、插件 Bundle 与 Skills 文件（422/343），WebOS 保持干净且不制造空提交；编译后的 `dist/bin/obj`、VSIX、tgz 与长应用包 JSON 只核对版本、摘要和同源性，不冒充独立功能。
- **通知中心在低高度与小分辨率下保持分页可达**：PC 通知弹层改为纵向 Flex 容器，标题固定、正文独立滚动，并预留稳定滚动条空间；真实业务 E2E 覆盖 `1366×520` 与 `1024×600`，核对后台任务分页可滚动到达、弹层不越出视口以及分页文字对比度。应用商城官方包以独立版本 v7.7.5 声明 `ClientFeature:NotificationCenterScrollableViewportV1`，ChangeLog、ChangeHistory、能力门禁、同步基线与正式资源保持一致；平台框架版本仍为 v7.7.4，避免混淆应用包版本和框架版本。
- **接口引擎角色白名单增加安全可移植的“任意已登录用户”策略**：新增仅由服务端权威 `sys_apiengine.ApiRole` 解析的 `$authenticated` 虚拟角色，真实 DiyToken 用户可运行明确开放的接口，匿名请求不能伪造，调用只修改本次身份克隆且不污染登录缓存；OnlyGet 角色继续保留只查询约束。MCP 新增带 SHA-256 预检确认、角色存在性校验、缓存刷新和数据库回读的 `microi_set_engine_roles`，所有 ApiEngine HTTP 入口在访问密钥检查后统一展开该策略，不放宽菜单、表、行或 Controller 权限。
- **AI 应用源码交付补齐可审计的远端清理闭包**：新增管理员能力 `microi_clear_application_source`，第一阶段只返回目标对象与确认哈希，第二阶段才按精确 AppId／AppKey 删除私有源码、公开 SourceZip 和源码元数据；对象路径必须属于目标租户和应用，否则失败关闭。清理会移除包内源码指针并回读应用上下文，但保留已编译运行资产；源码目录同步同时排除 `.tmp`，防止临时回执、生成缓存和本地发布证据进入私有源码包。
- **MiniMax 原创游戏音频形成服务端优先、窄条件降级的完整工具链**：MCP 新增音乐和中文短对白工具，付费调用必须用稳定 RequestId 二次确认；服务端固定原创、纯音乐、采样率、码率、系统音色、租户 HDFS 落盘与幂等语义。只有 MiniMax 明确返回“新账号停用”的 410 才允许音乐从 `music-2.6` 切到受保护的官方直连 `music-2.6-free`，限流、超时、500 或结果未知均不盲目重试；MCP 还提供 MiniMax Music3 的受限后备，生成循环友好的纯音乐并保存可回放回执，响应明确记录请求模型、实际模型、路由、TraceId 和降级原因。
- **官网、UniApp 与标准前端 SDK 统一兼容四代上传字段和应用稳定地址**：共享解析器同时支持绝对／相对字符串、单文件对象、多文件数组及其 JSON 字符串，并保留 Name、Size、Id 等上传元数据；`/file/` 与 `/micro-app/v3/` 始终走 API Base，普通 HDFS Key 才走当前租户 FileServer。官网应用列表与详情复用同一解析器，修复旧 Web 发布记录缺少租户目录或 `index.html` 时打开对象存储默认页的问题，拒绝把不可变 release/request 产物冒充最新体验入口；Unity 批处理文档同步改为当前实际 AI 应用源码工程路径。
- **阿里云 OSS 的发布后对象核验与删除增强有界重试和诊断**：公有／私有对象存在性检查分别采用 15 秒／30 秒连接超时、SDK 三次错误重试与应用层四次有界尝试，删除等操作同样只对瞬时传输失败重试；403、AccessDenied 与取消立即失败，不掩盖权限问题，也不对不确定写入无限重复。发布回读通过签名 Range GET 精确核对对象大小与首字节，并返回桶范围、Bucket、Object 和配置／代理修复建议，降低一次网络抖动中断不可变应用发布的概率。
- **插件、源码与定向回归完成同源验收**：Microi.VSCode、npm CLI、Codex、CodeBuddy、WorkBuddy 与同步 Skills 发布 v5.4.0；首次市场回读仍处传播窗口，重试 `publish:verify` 后 npm、Visual Studio Marketplace 与 Open VSX 三端均精确回读 5.4.0。当前轮接口角色／应用源码／OSS／微服务契约 .NET 测试 99/99、Microi.AI 音乐测试 10/10、MCP 类型检查及测试 147/147、官网应用地址 28/28、PC 通知中心 6/6、官方资源契约 11/11 和 UniApp 双 SDK 文件地址脚本全部通过；全量发布中的官方 Resource 契约为 383/383。
- **`Microi一键编译发布.sh` 按指定序列完成 v7.7.4 全量现代版公共制品发布**：输入 `7.7.4、4、0、2、2、0、0`，完成现代前端、后端 Release（0 个错误，9 条既有 xUnit 分析器建议）、发布目录守卫、独立 18080 liveness HTTP 200、Microi.net／Microi.AI DLL 保护、20 个 NuGet 与 7 个 Docker 标签，最终 exit 0，明确未生成 Chrome 49 legacy。Global CDN 已逐包回读 NuGet 20/20；后端 `latest/v7.7.4/dev-latest` 摘要统一为 `sha256:a522740951be774a4e29e264d116398cb241ae5a99cff509e130ca62910b928d`，现代 Web／Client 四标签统一为 `sha256:e654444a672455bd51bc6d9a19f1cedc723704d32d3b75e0fb58cade217154a6`，7/7 远端标签独立回读一致。官方资源投影仍为 Managed 105、CreateIfMissing 8、总计 113 项，SHA-256 为 `94b07b744595967dd5976d0686d6661bb9a47d86f1f30f58b41465f8f3259b49da`；这些证据证明公共包与镜像可取，不代表任一客户生产节点已经拉取重启或完成登录后业务页面验收。

## v7.7.3 - (2026-08-27 21:49)

- **五个 Git 边界按 v7.7.2 封版点完成远端、提交历史与当前工作树去重审计**：重新抓取全部 origin 后，根仓 `690342d09`、Microi.net `6107e740b`、Microi.AI `0c127a8ff`、Microi.VSCode `df99e8021` 与 WebOS `7ad62fd54` 均为 0 ahead／0 behind。上版日志边界后的提交分别只是已经写入 v7.7.2 的封版提交，WebOS 没有新增提交，五仓也没有团队自动 merge、遗漏英文说明或可重复计算的功能差异；因此本节只汇总当前最终净增量。写入本节前，根仓排除 `microi.mcp/dist` 后为 72 个已跟踪文件（1,923 行新增／353 行删除）和 8 个新增源码／测试文件，Microi.net 为 2 个文件（21/4），Microi.AI 为 1 个版本文件（3/3），Microi.VSCode 为 14 个版本、插件与 Skills 文件（225/204），WebOS 保持干净且不制造空提交；编译后的 `dist/bin/obj`、VSIX、tgz 与长应用包 JSON 只核对版本、摘要和同源性，不冒充独立功能。
- **启动前平台运行时接口闭包兼容旧租户物理结构，彻底修复 v7.7.2 拉取后反复重启**：Upgrade13 不再把应用包中的逻辑名 `Name` 直接写入旧 `sys_apiengine`，而是规范为真实物理列 `ApiName`，并先读取目标租户实际字段，再把固定白名单与物理列取交集；同时补齐稳定 `UserId/UserName`、启用／匿名／日志／文件／锁／角色等必需默认值，拒绝缺少 Id、Key、地址或源码的损坏资源。官方生成器及发布后修正会显式删除对象展开残留的旧 `Name`，覆盖客户日志中的 `Unknown column 'Name' in 'field list'`、`Field 'UserId' doesn't have a default value` 和未来包字段早于租户表结构三类问题；失败仍保留原始租户、阶段和数据库根因，不把半完成闭包误报为成功。
- **历史数据源统一迁入类型化接口引擎，并保留可控的滚动升级兼容层**：新增 Upgrade34（版本门 `6.9.9.0`）与共享租约，在事务中给 `sys_apiengine` 补齐 `DataSourceType`，把启用的 `sys_datasource` 按稳定 Id／Key、冲突安全回退和单一 `ApiV8Code` 迁移为 V8、SQL、JSON、API 四类接口；只有完整写入与强回读成功后才软删除旧记录，失败回滚且不推进版本。Microi.net 的 ApiEngine 在既有租户、DiyToken、角色、访问密钥和日志边界内运行类型化数据源：SQL 参数化执行、JSON 直接解析，无法安全继承的旧 API 类型失败关闭并提示改写为 `V8.Http`；V8、MCP、旧移动端 `/api/DataSourceEngine/Run|GetData` 与官方 `platform-data-source-run` 均统一委托新事实源，滚动发布期间才有限回退旧表。
- **修复历史按钮字段 V8 代码升级后显示为 0 字并防止再次覆盖丢失**：后端实体、字段查询、描述模型和参数模型补回物理 `diy_field.V8Code`；前端读取时优先采用非空物理 V8Code，旧库才回退 `Config.V8Code`，保存时同步镜像两处，Legacy Base64 解码也覆盖根级 V8Code。回归用历史 `BtnDisplayPwd` 等真实形态验证编辑器内容、字数、优先级和持久化；代码编辑器进一步依据 `DataSourceType` 动态选择 JavaScript、SQL、JSON 或纯文本语言，设计器与版本查看器保持一致。
- **后台任务时间与 Linux 一键安装权限链完成生产兼容加固**：后台任务所有数据库时间统一使用不依赖区域文化的 `yyyy-MM-dd HH:mm:ss.fff`，长度固定不超过 23，避免 MySQL `varchar(25)` 因本地化日期或更长小数秒再次写入失败。安装脚本在任何交互和宿主写入前检查权限：可用时只执行一次保留必要变量的 sudo 重入，无 sudo 则在零改动边界明确失败，root 环境不依赖 sudo；完整安装、更新修复和权限自检共用同一规则，目录创建失败保留准确路径与退出码，部署文档和 Skills 同步说明。
- **官网应用商城交付“推荐应用”完整数据、权限和展示闭包**：官方应用商城包新增 `sys_microistore.IsRecommend` 的 DDL、物理列快照、字段元数据与能力声明；官网在“全部”后展示虚拟“推荐”分类，推荐应用优先排序并带统一徽标，详情页改为按精确 AppKey 读取单条应用和更新日志。只有 Level≥9999 的超级管理员能通过 `official_ai_app_recommend` 设置或取消推荐，普通用户只读且所有写入继续受版本与租户约束；应用详情时间线样式和首页“AI 低代码开发／V8 引擎 AI 编程／微服务定制”能力表达同步优化，10 倍+ 仍限定为高复用典型场景。
- **官方应用不可变发布机制增加一致性自愈，平台版本与应用包版本明确解耦**：资源刷新发现同版本正文变化时，不再只修改 PackageInfo 造成更新日志与历史版本失配，而是原子同步版本、ChangeLog 与 ChangeHistory，并由契约测试拦截重复或倒退版本。平台仍为 v7.7.3；应用商城包在发布后修正旧 `Name` 字段兼容时，按不可变规则从 v7.7.3 自动升为 v7.7.4 并重新发布，包 SHA-256 为 `079e716c093dc33b54ae5a3b0c075d9a00e23bdbb52ac811ba9a08a067baa7e30`。九个官方应用运行时投影最终为 Managed 105、CreateIfMissing 8、总计 113 项，SHA-256 为 `94b07b744595967dd5976d0686d6661bb9a47d86f1f30f58b41465f8f3259b49da`。
- **插件、源码、文档与发布制品完成分层回归**：字段 V8 兼容测试 13/13、官方 Resource 契约 383/383、Upgrade／迁移／后台任务／SaaS 定向测试 68/68、官网推荐／查询／首页 19/19、MCP 类型检查与测试 144/144 全部通过，`install-microi.sh` 通过 `bash -n`。Microi.VSCode、CLI、Codex、CodeBuddy、WorkBuddy 与同步 Skills 发布 v5.3.9，npm、Visual Studio Marketplace、Open VSX 三端均精确回读 5.3.9；Microi.AI 保持原有 `netstandard2.1`，没有为了 Controller 增加 `net10.0` 多目标。
- **`Microi一键编译发布.sh` 按指定序列完成 v7.7.3 全量现代版发布和修正后完整重跑**：输入 `7.7.3、4、0、2、2、0、0`，完成现代前端、后端 Release、发布目录守卫、独立 18080 liveness、Microi.net／Microi.AI DLL 保护、20 个 NuGet 与 7 个 Docker 标签；修正官方包字段后再次从头发布，最终 exit 0，未生成 Chrome 49 legacy。NuGet 中已存在的 7.7.3 包经公共源回读后按幂等成功处理；后端 `latest/v7.7.3/dev-latest` 摘要统一为 `sha256:f2bfcc84b78aa5347253815cd41a763953ab767d6a9ad0dfd4f040eb88b406ba2`，现代 Web／Client 四标签统一为 `sha256:8c0ac2d9927577f2709085a95f6de2c9fe22aa56bfd184808b01a089664988b8d`。

## v7.7.2 - (2026-08-27 06:58)

- **五个 Git 边界按 v7.7.1 封版点完成本地、远端、提交历史与工作树去重审计**：重新抓取全部 origin 后，以根仓 `a1bde8fcf`、Microi.net `fe16a8d5f`、Microi.AI `1e47b81d7`、Microi.VSCode `f20f61d01` 与 WebOS `7ad62fd54` 为边界，五仓均为 0 ahead／0 behind，边界后没有新增提交、推送遗漏、英文说明或团队自动 merge，因此本版只汇总当前工作树的最终净差异，不重复包装历史变更。写入本节前，根仓为 98 个已跟踪差异文件（3,005 行新增／939 行删除）和 13 个新增源码、测试与官网图片文件，Microi.net 为 10 个多数据库与运行时文件（226/113），Microi.AI 为 1 个版本文件（3/3），Microi.VSCode 为 26 个版本、多宿主插件与 Skills 文件（334/276），WebOS 保持干净且不制造空提交；生成的 VSIX、tgz、`dist/bin/obj/node_modules` 和长应用包 JSON 只核对版本、哈希与同源性，不重复冒充独立功能。
- **一键安装把核心平台与 OCR／翻译附加能力彻底解耦**：数据库、Redis、MongoDB、MinIO、API 与 Web 先通过 API liveness、完整 `sys_config.ServerVersion` 升级链和 readiness，随后才部署 PaddleX/PaddleOCR 与 LibreTranslate。附加网络、镜像拉取或架构、容器健康、API Key、Upgrade29/Upgrade31 字段、唯一主租户、配置写入或回读任一失败时，只记录警告、保持对应 SaaS 能力未启用并继续核心安装，不再回滚、删除数据或让用户看到“只有 MySQL 容器存活”的误导终态；只有成功写入至少一项附加配置时才重启 API，并再次以 readiness 判断核心服务是否恢复。
- **安装器的失败现场与恢复信息变得可直接排查**：11 个阶段都会记录准确失败位置，核心错误保持原始非零退出码并输出“安装未完成”、已生成配置、API liveness/readiness、完整升级链、OCR/翻译状态和全部 `microi-install-*` 容器；附加能力失败则在成功汇总中单独列出警告。LibreTranslate 密钥只说明是否已经生成并回读，终端永不输出明文；ARM64 等非 amd64 主机自动跳过当前固定 OCR 镜像而不阻断核心平台，所有恢复提示继续要求保留数据目录、卷、容器与日志，禁止 `down -v`。
- **PostgreSQL 与 SQL Server 空数据库安装链完成兼容修复**：安装器在 PostgreSQL 恢复后事务化地把旧空库中的 BOOLEAN 开关规范为跨数据库 0/1 `SMALLINT` 并强回读，在 SQL Server 导入前补齐必需 ANSI 会话选项，并把 MySQL 可空唯一索引转换为只过滤真实可空列的 filtered unique index。`SqlServer`、`SqlServer9` 与 `mssql` 配置别名统一归入 SQL Server 平台语义，运行时稳定使用支持现代分页的 `SqlServer9` 会话 Provider、ORM/DDL 统一使用同一 SQL Server 方言；因此 License、FormEngine、Upgrade25、数据库备份和应用流 V3 初始化不再因“Unsupported DbType: SqlServer9”、唯一 NULL 约束或方言分叉启动失败。
- **Dos.ORM、SaaS 与 FormEngine 补齐 PostgreSQL／Kingbase 大小写和参数类型契约**：PostgreSQL/Kingbase 不再剥离双引号，官方空库中的 `Id`、`IsDeleted`、`OsClient` 等驼峰字段通过当前 Provider 引用符访问；表单新增、查询、修改、删除、Where、多语言、后台任务、JWT 密钥门禁、`sys_config` 与流量聚合统一处理真实物理表名和字段名。MySQL BIT 在 PostgreSQL 系空库映射为 `SMALLINT` 与 0/1 字面量，Npgsql Boolean 参数仍保持 CLR Boolean，JToken 标量、数值开关、时间与克隆参数按数据库类型绑定，消除 `42703: 字段 id 不存在`、`smallint = boolean` 和时区 Kind 不匹配等空库启动错误。
- **自动升级链在七类数据库上继续失败关闭并可幂等恢复**：Upgrade24 为 MySQL、SQL Server、PostgreSQL/Kingbase、Oracle/达梦分别生成重复备份任务清理 SQL；Upgrade25 增加 PostgreSQL 的字段、默认值、非空、索引、时间和元数据审计方言，并接受转换器生成的等价多列非空过滤索引。V8Limit 迁移会在元数据写入前补齐 `sys_apiengine`／`diy_table` 物理字段，字段元数据读取失败保留根因，HostedService 与 ServerVersion 推进统一走 ORM 标识符、租约校验、乐观并发回读和缓存清理，任何中间失败都不会误推进版本。
- **表单图片、文件与富文本上传恢复字段级公有／私有桶语义，同时保持服务端防篡改**：PC 与 UniApp 统一提交 `FormEngineKey + FieldId + SysMenuId`，编辑记录再带 `FormDataId`，TableChild 再序列化父子授权上下文；API 先校验当前菜单、表、记录及新增／编辑动作权限，再回读当前租户 `diy_field.Component/Config`，用权威 `Limit` 和控件固定目录覆盖客户端提示。普通用户有业务权限时可按公有字段写公有桶，私有字段、伪造字段／菜单／路径和无上下文裸 `Limit=false` 均无法放宽；旧图片／文件字段缺省兼容公有、旧富文本缺省保持私有，微信待审图片和裁剪／压缩原图始终私有。
- **可靠后台任务、运行观测与启动密钥门禁补齐 PostgreSQL 运行能力**：后台任务的查询、分页、领取、续租、取消、进度、终态与 Schema 探测只引用白名单标识符，不会误改参数名或字符串字面量；空库环境延长数据库／Redis 租约并保留基础设施争用的独立有界重试。流量聚合改为类型化无时区 UTC 参数和引用字段，JWT 启动门禁先修复旧字段容量再读取活动租户，数据库慢启动、租约丢失或配置事务失败继续保持可观察且不发布半完成配置。
- **微信支付 API v3 解密原子按协议修正并纳入 V8 回归**：`V8.WeChat.AesGcmDecrypt` 现在严格把 APIv3 Key 与 nonce 当作 UTF-8 原文、只对 ciphertext 做 Base64 解码，并把尾部 16 字节 GCM Tag 一起交给 BouncyCastle 完整性认证；密钥必须 32 字节、nonce 必须 12 字节，篡改密文或标签必然失败。文档同步明确该原子不替代微信平台 HTTP 回调签名验证，订单、金额、幂等、事务、日志和通知仍由接口引擎编排。
- **官网信息架构、真实截图和系统能力全景完成一轮重构**：README、官网首页与源码架构页把架构图收敛为以 V8 为在线运行核心的 20+ 成熟系统引擎，机器可读能力索引去重为 226 个唯一标签，SVG、1080P、4K PNG 与 manifest 已按 v7.7.2 同源重生成。首页提前展示真实产品预览，接口引擎、表单字段、系统设置与 MicroService 文档新增亮暗主题和真实案例原图及统一 Fancybox 预览；分布式存储、Dos.ORM、Office、SSO、强身份验证文档只调整到“系统引擎”侧栏而保持历史 URL 不变。
- **部署文档把三条最高频命令提升到宿主机资源保护之前**：CentOS 7/8/9、Ubuntu 20/22/24、Debian 10/11/12 的一键安装，一键更新／修复 API 与 Web，以及强制删除全部 `microi-install-*` 容器三条命令现在紧邻安装说明顶部；删除命令明确只删除容器、不会主动删除 `/microi` 数据目录或 Docker volume，但执行前仍必须备份并核对目标。OCR、LibreTranslate 的降级边界、多数据库空库修复、表单上传和微信支付协议说明同步进入官网与工作区／插件 Skills。
- **插件、源码和发布制品完成分层验收**：Microi.VSCode、CLI、Codex、CodeBuddy、WorkBuddy 与同步 Skills 发布 v5.3.8，npm、Visual Studio Marketplace、Open VSX 三端均已精确回读 5.3.8。后端多数据库／安装器／升级／上传／任务／微信定向回归 144/144、PC 上传 3/3、UniApp 上传 4/4、官网 52 个测试及链接、侧栏、仓库、可读性静态门禁全部通过，安装脚本 `bash -n` 通过；`Microi一键编译发布.sh` 按输入 `7.7.2、4、0、2、2、0、0` 完成现代前端、后端 Release、API liveness、20 个 NuGet 与 7 个 Docker 标签发布，未生成 Chrome 49 legacy。后端 `latest/v7.7.2/dev-latest` 摘要统一为 `sha256:8858b6258c400154d6ac0afb538f93d9bf3b4112c15abfda47083ed99b596f38`，Web／Client 现代版四标签统一为 `sha256:dca1ae0d017f379cbf2cf045137d0a87d1419afca96b964d1a41ac86f45eee66`，7/7 远端标签独立回读一致。

## v7.7.1 - (2026-08-26 19:16)

- **五个 Git 边界按 v7.7.0 封版点完成本地、远端与当前工作树去重审计**：本轮以根仓 `b0c9b5f62`、Microi.net `3c3b700`、Microi.AI `4988eab`、Microi.VSCode `0a96cb5` 与 WebOS `7ad62fd` 为边界重新抓取全部 origin。根仓边界后共有 11 个远端提交，其中 5 个是 UniApp 实际功能提交、6 个是团队自动 merge 包装；按最终树去重后净增量为 23 个文件（620 行新增／93 行删除），其余四仓没有遗漏提交或英文说明。写入本节前，根仓为 137 个已跟踪差异文件（1,900/4,781）和 15 个新增源码／测试文件（3,210 行），Microi.net 为 1 个版本文件（3/3），Microi.AI 为 2 个版本／内置 Skill 文件（20/3），Microi.VSCode 为 19 个版本、多宿主插件、Skills 与测试文件（256/217），WebOS 保持干净且不制造空提交；长应用包 JSON、插件生成元数据、VSIX、tgz、`dist/bin/obj/node_modules` 只核对版本、哈希和同源性，不重复冒充独立功能。
- **彻底纠正 Controller 下沉方向，所有 ASP.NET Controller 继续只属于 `Microi.net.Api`**：物理删除已迁入 V8 或已合并的 `DataSourceEngineController`、`ModuleEngineController`、`OcrController`、`OfficeController`、`SecurityGuardController`、`TranslateController`、`UserBehaviorController`、`HomeController`、`OsController`、`UEditorController` 及旧 `SsoController` 全部分片；没有创建 `Microi.AspNetCore`、`Microi.SSO` 或任何新的 Controller 类库。`AiController` 仍留在 API 宿主，仅保留 SSE、支付验签、供应商密钥隔离、OpenAI/MiniMax 原始协议等 V8 无法承担的边界；`Microi.AI`、`Microi.net`、Captcha、WeChat 等类库保持原有 `netstandard2.1`，绝不为容纳 Controller 增加 `net10.0` 多目标。
- **七类平台业务 HTTP 入口正式迁入官方 Managed 接口引擎，运行时闭包由 106 项扩展为 113 项**：新增 `mci-system-observability-action`、`platform-data-source-run`、`platform-module-data`、`platform-ocr-recognize`、`platform-office-export-word-by-template`、`platform-translate-runtime`、`platform-user-behavior-signal`，覆盖 IP 安全治理、数据源、模块查询、OCR、Word 模板导出、翻译和用户行为。每个接口顶部均保留极醒目的官方应用还原提示，并在前后阶段调用租户维护的 `platform-runtime-custom-hook`；该 Hook 使用 `Tenant/CreateIfMissing`，首次创建后官方升级不覆盖，官方默认源码精确为 `return { Code : 1 };`。文件 Base64、识别／翻译原文、供应商地址和密钥不进入 Hook。
- **接口引擎只编排业务，可复用 C# 仅提供最小可信原子**：Microi.Core 新增数据源运行、模块查询、Word 模板导出和行为信号四个 V8 原子，重新绑定 DiyToken、当前租户、Client 调用标记、菜单／表／行读取权限及访问密钥白名单；OCR 和翻译继续复用既有插件原子。访问密钥策略同步切换到新的 Managed Key：数据源要求 `data-source:run`，模块查询要求 `form:read` 与全授权数据范围，用户行为信号明确拒绝长期 AccessKey；已删除 Controller 的旧 `/api/DataSourceEngine`、`/api/ModuleEngine`、`/api/UserBehavior` 等路径不再成为隐式旁路。
- **旧客户端兼容入口集中收口，原生协议网关保持薄且可审计**：普通 AI JSON 动作、租户设置 CRUD、用户资料、系统设置、当前用户、私有文件、租户发现、消息等旧 `/api/*` 地址统一并入 `LegacyMobileCompatibilityController`，文件上传及 UEditor 历史 multipart 地址归入保留的 `HDFSController`。SSO 原生 OIDC/SAML2/CAS 协议保留为 API 内的 `SsoProtocolGatewayController`，固定继续使用 `/api/Sso/*`；可迁移的 SSO 业务动作仍由官方应用接口引擎拥有。所有 21 个现存 Controller 类及迁移去向写入所有权目录，测试同时禁止在其它类库重新出现 Controller、恢复已删除文件或通过 `AddApplicationPart` 绕过边界。
- **PC、UniApp、MCP 与 AI 应用脚手架同步切换新接口地址**：PC 的数据源、模块查询和行为信号，UniApp 的模块列表／树、表单选项数据源与任务列表，MCP 的 OCR、翻译和数据源调用均改为 `/apiengine/platform-*`；翻译通过白名单 `Action` 统一承载六种动作，Office 文件响应继续保持 `ResponseFile=1 / ResponseType=File`。应用商城包升至 v7.7.0，`ai_app_create` 升至 v1.2.3，新建 Web／MicroService 自带的维护版前端 SDK 不再生成已迁移 Controller 的旧地址。
- **自动升级与官方应用源把新增闭包作为启动硬门禁，而不是只修改当前主库**：SaaS 引擎官方包升至 v7.7.1，最小可接受包版本、能力目录、稳定 Id、Managed 所有权、Hook 调用、文件响应及升级强回读同步加固；接口源码使用单引号或双引号调用 Hook 均按等价语义识别，避免完整包被误判后反复重装。完整发布阶段从 `microi_itdos` 发布并回读 Managed 105、CreateIfMissing 8、总计 113 项，投影 SHA-256 为 `79a9bb282dcf32b093a91dd629c9f1a5eaeddd04af324ec47658afe5ef6ea8642`；官方数据库再次回读 SaaS 引擎 v7.7.1、应用商城 v7.7.0 及七个新增接口均已存在。
- **`Program.cs` 恢复清晰插件注释，同时继续只做宿主组合**：每个 Microi、ORM、Cache、HTTP、MongoDB、Upgrade、WeChat、Office、Spider、OCR、MQ、Search、AI、MQTT、HDFS、Captcha、Job 与 API 传输层注册均补回用途注释；自动升级、启动诊断和生命周期逻辑没有塞回 Program，启动诊断只在同一 API 项目的 `Hosting` 目录归类。Handler、Middleware 与保留协议 Controller 对非直观边界补充中文注释；三套工作区 Skills 同步加入“复杂代码必须解释为什么、简单代码不写噪声注释”的规范，以及“Controller 只在 API、类库保持原 TFM”的强制回归门禁。
- **团队侧 UniApp 增量按真实提交去重纳入源码交付**：设备地图补齐角色数据权限、客户名称和商城接口兼容；拜访打卡与人员定位按商家所有者／普通客户区分拜访对象及菜单可见性；售后流程后三类流程能力、设备可选引用、任务列表／详情权限与对应测试一并完善。上述内容来自 5 个实际提交，外围 6 个自动 merge 不重复计为功能，也不把同一文件在多次分支合并中的机械冲突结果重复描述。
- **官网主定位统一为“开源 AI 开发框架”并建立回归约束**：根 README、官网首屏、登录页、AI Studio、SEO、结构化数据、培训大纲和文章模板统一表达“20+ 成熟引擎 + AI 低代码 + 微服务 + V8 引擎”，并在明确的典型业务复用边界内展示“Token 更省 10 倍+、速度提升 10 倍+”；旧“开源 AI 应用开发平台”只保留为 SEO 兼容关键词。新增官网定位测试，防止后续不同入口重新出现互相冲突的产品名。
- **源码、真实租户、官方源和公开插件完成分层验收**：Controller 所有权测试 11/11、定向 C# 架构／安全测试 45/45、官方升级资源契约 378/378、后端非在线回归 2,710/2,710 通过；需要真实环境的 TenantSmoke 与 ApiEngine FullStack 另行执行 2/2 通过。本地共享后端启动后所有已发现租户均完成 113 项闭包，`lsg`、`a123456777` 的系统设置、服务健康、商城及旧系统设置回退均不再出现 `sys_apiengine NoExistData`；菜单、模块统计等匿名请求只返回正常权限拒绝。Microi.VSCode、CLI、Codex、CodeBuddy 与 WorkBuddy 最终发布 v5.3.7，修正租约 Broker 测试以验证“完整临时 JSON + 硬链接原子抢占”，npm、Visual Studio Marketplace 与 Open VSX 三端均精确回读 5.3.7。
- **`Microi一键编译发布.sh` 修复跨仓 Release 包陈旧风险并完成 v7.7.1 全量现代版发布**：脚本现在在主解决方案前显式构建 Microi.net Release，打包时显式生成并只允许选择当前版本 `Microi.net.7.7.1.nupkg`，禁止目录“最新包”误改旧版本；首次冒烟准确拦截了 API 引用 7.7.1、发布目录却携带 7.7.0 DLL 的错误，修复后按输入 `7.7.1、4、0、2、2、0、0` 完整重跑成功，耗时 9 分 26 秒。发布目录独立启动 HTTP 200，受保护 Microi.net／Microi.AI 均为 7.7.1.0，NuGet.org 20/20 个包公开回读 HTTP 200；后端 `latest/v7.7.1/dev-latest` 摘要统一为 `sha256:e378c536a9e5d2ef273f578c2d5a2eb59d62a36fce9f323bfbdf1f09fe0ca54d`，现代前端 Web／Client 四标签统一为 `sha256:0b1883bf773dd6d6fc57d2428c7decd0cd9336e2aed3202f4c20d930e555872a`，本轮明确没有生成 Chrome 49 legacy。

## v7.7.0 - (2026-08-26 14:04)

- **五个 Git 边界按 v7.6.9 封版点完成提交、推送与当前工作树去重审计**：重新抓取全部 origin 后，以根仓 `6b47dbac0`、Microi.net `ddcac66`、Microi.AI `d7cb45d`、Microi.VSCode `cefa5b7` 与 WebOS `7ad62fd` 为边界，五仓均为 0 ahead／0 behind，边界后没有遗漏提交、英文说明、团队自动 merge 或重复包装。写入本节前，根仓为 99 个已跟踪差异文件（1,061 行新增／5,582 行删除）和 21 个新增源码／测试文件（2,774 行），Microi.net 为 3 个已跟踪文件（105/5）和 10 个新增运行时文件（2,298 行），Microi.AI 仅 1 个版本文件（3/3），Microi.VSCode 为 14 个已跟踪文件（357/331）和 1 个新增测试（19 行），WebOS 保持干净且不制造空提交；应用包长 JSON、多宿主 bundle、VSIX、tgz、`dist/bin/obj/node_modules` 与机械哈希元数据只核对最终版本和同源性，不重复冒充独立功能。
- **彻底修复 Upgrade25 在不同服务器区域文化下写入非法日期的问题**：Dos.ORM 的 `object` 参数不再先按当前 Culture 转成 `08/26/2026 07:37:10` 一类字符串，而是按 CLR 运行时类型推断并保留 `DateTime`、`DateTimeOffset`、Guid、二进制和数值数据库类型；应用发布 V3 升级同时对 `CreateTime`、`UpdateTime` 显式绑定 `DbType.DateTime`。新增 `en-US` 参数推断回归测试，确保 MySQL、SQL Server、Oracle 等驱动收到类型化日期，历史 `mci_ai_app_file.VersionId` 的确定性归档和唯一索引创建也可继续幂等完成。
- **自动升级租约、租户启动顺序与失败推进规则全面加固**：升级租约调整为 120 秒 TTL、30 秒续租和 5 秒竞争重试，续租器改用独立 LongRunning 线程，避免 Redis／数据库慢操作占满线程池时误判租约丢失；推进 `ServerVersion` 前再次确认锁所有权，失败绝不推进版本。启动前只门禁主租户的物理字段和 106 项运行时接口闭包，其它租户由持久 HostedService 在服务可用后后台恢复，解决官方主库 74 个租户串行门禁导致约 32 分钟才监听的问题；应用发布、SaaS 结构、后台任务等每个升级步骤继续输出明确的开始、成功或失败终态。
- **Upgrade13 与运行时接口闭包不再依赖低代码元数据或失效缓存**：官方包补缺先识别已安装版本，缺失资源可直接、参数化地写入物理 `sys_apiengine`，即使历史租户缺少对应 `diy_table` 元数据也不会再因 `copy_table` 创建失败；关闭缓存的租户不再执行通配 Redis SCAN，从根源消除日志中的连续 Redis timeout、租约丢失和后续升级级联失败。主租户本地启动已强回读 Managed 98、CreateIfMissing 8、总计 106 个接口，系统设置、菜单、当前用户、服务健康、私有文件、组织机构、模块统计和应用商城等关键路由均不再出现 `sys_apiengine NoExistData`。
- **后端控制台输出统一为唯一可检索格式**：`ConsoleLogInterceptor` 对应用、升级、插件、后台任务及异常队列的 stdout/stderr 统一规范为 `Microi：【✅成功/❌失败】【yyyy-MM-dd HH:mm:ss】输出内容`，旧的 `【信息】`、`【Error异常】`、缺少时间、括号残缺和重复 `Microi：` 会在输出边界归一化；启动失败覆盖 Host 构建、主租户门禁和 `RunAsync` 全阶段，并总能给出最终失败汇总。定向契约验证了成功、失败、历史前缀、换行与异常消息，真实 HTTPS 61501 启动日志已逐行验证格式。
- **`Microi.net.Api` 完成第二轮宿主瘦身，Program 从 909 行收敛为 33 行**：`Program.cs` 现在只有进程初始化、插件清单、传输层注册和运行入口（27 行有效代码）；数据库连接观察、主租户门禁、微信初始化、MQTT、许可证、接口路由缓存、宿主生命周期、中间件管线与失败汇总分别下沉到 Microi.Core、Microi.Upgrade、Microi.WeChat、Microi.MQTT、Microi.net 和固定 ASP.NET 传输层。API 的 7 个 Services 全部移入 Microi.net／HDFS／WeChat，流量观测与代理信任中间件移入 Microi.net，Redis 连接器移入 Microi.Cache，Managed 兼容原子移入 Core，完整 UEditor 实现移入 HDFS；废弃 ErrorModel、旧 Razor 页面及 `DiyChatController`、`UploadController`、`iTdosHeartController`，旧地址统一进入 `LegacyMobileCompatibilityController` 或诊断入口。剩余 Controller 均由所有权目录约束为内核、启动身份、协议网关或运维边界，不允许新增租户可变业务编排。
- **“一键制作主库空数据库”继续清除新增运行态残留并发布七数据库制品**：官方清理接口升至 v1.3.5，SaaS 引擎应用包升至 v7.6.22，把 `mci_network_traffic_rollup`、`mci_app_stream_gate_transition`、`mci_nuget_stats_daily` 同时纳入保护表、业务清零表和结果审计；任务 `160546a2af3f4db4b9338f90e46d73d2` 成功生成 MySQL 5.7／8.0、SQL Server、Oracle、DM8、PostgreSQL、Kingbase 七个 HDFS 包。公开 MySQL 5.7 ZIP 独立下载后 SHA-256 为 `24f72fde7894a59f560437a118321f97e0eca897a0e6ed21614594cffedaa1c0`，解压 SQL 中三个运行态表的 `INSERT` 均为 0，`sys_menu` 数据块中的 AI 应用菜单匹配数为 0；保留 AI 表结构与翻译元数据，不把官方应用实例、商城资源或运行统计误带入新租户。
- **官方应用源与 V8 版本头保护完成同源发布**：官方空库清理接口源码 SHA-256 为 `0630af69830abb822cdcf9657b6662b915c89119dcd3b169ab95fdf05a27d3ec`，SaaS 包 SHA-256 为 `3e0513ffe23ad762c795606656a8d0ecce1258269c76bf13ef6bfaec1bec3404`，官网 live 投影回读为 106 项、SHA-256 `a25b0bd2ab6a74722418bfda3253c4993ab6ec37e8ce9bf51e72582216c622b3`。MCP 与 VS Code 的 V8 版本头生成器会把“此接口包含在官方应用源、重装会还原，请把个性化逻辑写入独立 Hook”的醒目所有权提示永久保持在第一行，保存升版不再把版本注释插到其上方；新增两端回归测试防止后续再次破坏官方提示。
- **源码、真实租户、官方资源和公开插件完成分层验收**：后端定向架构／升级／日志测试 62/62，空库服务测试 29/29，最终非在线回归 2,710/2,710；HTTPS 61501 上登录租户、ApiEngine GET／JSON POST、真实 FormEngine CRUD／批量／按条件更新与清理 3/3 通过，八组关键 Managed 路由均无 `NoExistData`。官方应用通知与资源契约 64/64、空库 V8 语法与清理契约 9/9 通过。Microi.VSCode、CLI、Codex、CodeBuddy、WorkBuddy 与 65 个 Skills 同步发布 v5.3.5，npm `@microi.net/cli`、Visual Studio Marketplace、Open VSX 三个公共目标最终均精确回读 v5.3.5。
- **`Microi一键编译发布.sh` 按指定序列完成 v7.7.0 全量现代版发布与公共制品回读**：吾码版本规则将 7.6.9 自动进位为 7.7.0，输入 `7.7.0、4、0、2、2、0、0`，完成现代前端、后端 Release、独立 18080 liveness、发布目录守卫、Microi.net／Microi.AI DLL 保护、20 个 NuGet 与 7 个正式／测试 Docker 标签，脚本耗时 11 分 17 秒并以 exit 0 结束；NuGet.org Flat Container 最终逐包回读 20/20，Docker 远端回读 7/7。后端 `latest/v7.7.0/dev-latest` 摘要统一为 `sha256:101e5515058fac2e5ad63501c44851640955a1e5a58dce741b68b5a709174c12`，现代前端 Web／Client 四标签统一为 `sha256:516c52cfde5867e4dfefabbccc46570e6e3456ed4a69a9e183d2ed7f86987d7a`；本轮明确没有生成 Chrome 49 legacy。

## v7.6.9 - (2026-08-26 11:12)

- **五个 Git 边界按 v7.6.8 封版点完成提交历史、推送历史与当前工作树去重审计**：重新抓取全部 origin 后，以根仓 `b36a484ba`、Microi.net `62c355d`、Microi.AI `9d3a73b`、Microi.VSCode `55588f0` 与 WebOS `608efdc` 为边界，五仓均为 0 ahead／0 behind。边界后只有根仓提交 `f117933d5`（完善官方运行时接口闭包与微信通知适配）和 WebOS 提交 `7ad62fd`（迁移 WebOS 桌面到官方 Managed 接口），没有遗漏英文说明、团队自动 merge 或重复包装；写入本节前，根仓最终净差异为 59 个文件、1,665 行新增／226 行删除，Microi.net 与 Microi.AI 各 1 个版本文件（3/3），Microi.VSCode 为 13 个版本、多宿主插件元数据与 Skills 文件（209/200），WebOS 为 5 个源码文件（105/22）。`dist/bin/obj/node_modules`、VSIX、tgz、长应用包 JSON 和机械生成元数据只核对最终版本、哈希与同源性，不重复冒充独立功能。
- **九个官方基础应用的运行时接口闭包由 102 项补齐到 106 项，彻底收口历史租户大面积 `sys_apiengine NoExistData`**：最终闭包为 98 个 `Platform/Managed` 核心接口与 8 个 `Tenant/CreateIfMissing` 个性化 Hook，live 投影 SHA-256 为 `0bdc33b5a84d7b0edf0e00d6938773a0e3111547db48b3749c9bc2da09fa5930`。除系统设置、菜单、当前用户、私有文件、组织机构、服务健康和商城等既有页面入口外，继续补齐 `platform-online-terminal`、`platform-cache-manager`、`mci-system-observability-query`、`wechat_send_tpl_msg`，并保留 `/apiengine/get-microi-store` 对正式商城批量计划接口的 Managed 兼容门面；官方业务接口调用独立 CreateIfMissing Hook，租户首次安装后可安全增加个性化逻辑且后续官方升级不会覆盖。
- **自动升级从“版本号已推进”改为可观察、可复检、失败不放行的运行时门禁**：API 接收页面流量前逐租户完成物理字段与 106 项官方闭包检查，缺失时从固定程序集内置官方包补齐并强回读，后台再次复检；启动日志逐项输出租户、步骤、开始、待修复、成功或失败及最终租户汇总。Upgrade13 对每张表、每个物理字段分别记录检查结果，nullable DDL 失败不再被吞掉而会阻止版本推进；Upgrade25 对历史 `mci_ai_app_file.VersionId` 的 NULL／空值执行确定性无损归档后再建立唯一索引，解决旧库 50 条未绑定记录导致升级链中断的问题。
- **旧 C# Controller 只保留协议、可信鉴权、文件传输和旧移动端兼容边界，业务编排继续迁移到 V8**：17 个旧 PC／UniApp 地址集中在 `LegacyMobileCompatibilityController` 并明确标注仅为旧吾码移动端兼容、满足遥测门槛后可整体移除；ApiEngine、FormEngine、WorkFlow、HDFS、Upload 等内核／特殊协议入口按兼容边界保留。PC、UniApp 与 WebOS 改调官方 Managed 接口；系统设置仅在 HTTP 404／405／501 或精确缺少 `platform-sys-config` 时回退旧 `/api/FormEngine/GetSysConfig`，401、租户错误和其它业务失败不会被二次请求掩盖，文件管理模块同时补齐 HTML 404 与远端租户回退处理。
- **官方应用源、应用商城批量更新与生产子租户完成独立发布和真实回读**：12 个官方资源内容精确一致，控制面包 v1.3.2 源码 SHA-256 为 `11db951f1cccf31fe3743ed2b58cb69163e2aa6b71b8c59292c9817cd6d6cd9d`。xjy 的 13 应用全平台任务成功；Jhy 父任务 `8a9e30fa39d04a88a5c6f6759e40a` 与子任务 `417a7ee1c75a492cb0833df1ce2d1a9b` 最终均成功，13/13 应用完成（安装 5、更新 8、失败 0），一次 Redis 瞬时超时通过持久检查点与有限重试自愈；xjy、Jhy、lsg、a123 的 16 项新旧路由矩阵全部 HTTP 200、缺失接口数为 0，包含 `/apiengine/get-microi-store` 与旧系统设置地址。
- **“一键制作主库空数据库”彻底清除不应随母版发布的 AI 应用菜单并重新发布 7 份 HDFS 制品**：清理器按菜单树递归删除顶级“AI应用”及其全部后代，同时保护平台必须的“AI助手”，任一必清表失败都会中止发布。任务 `94adfc6d2919423ca894c2561f07bc39` 成功生成 7 个数据库包；最新 MySQL 5.7 ZIP 已独立下载并按 SHA-256 `e00d23aecb9441b639ddfe4767f95e70cac494fac0680e700d18b6f34e79a7a9` 精确回读，解析后的 `sys_menu` 共 146 行，顶级“AI应用”及其后代均为 0，仅保留 1 个“AI助手”。
- **源码契约、数据库制品、后端与三套前端完成分层回归**：官方资源契约 378/378、空库清理 9/9、PC 接口迁移 11/11、UniApp 定向 4/4 全部通过，UniApp 完整 `check:ui` 通过；后端 Release 测试 2,697/2,697，PC 现代版和 UniApp H5 生产构建均成功。共享本地 API 启动时 9 个租户逐一输出 106 项闭包成功且失败数为 0，本地 iTdos 16/16 路由通过；本地、Jhy、lsg、a123 匿名真实浏览器均进入登录页且没有页面启动接口错误。登录后浏览器验收仍严格区分于匿名页面、MCP 任务和 HTTP 路由证据。
- **Microi.VSCode、CLI、Codex、CodeBuddy 与 WorkBuddy 同步发布 v5.3.4**：`npm run publish` 自动完成升版、生产构建、类型检查、多宿主路由与 65 个 Skills／178 个文件同源测试、VSIX 与 CLI tgz 打包；npm `@microi.net/cli`、Visual Studio Marketplace 和 Open VSX 三个公共目标最终均精确回读 v5.3.4。插件同步九包 106 项闭包、两阶段启动恢复、应用商城所有权与可信微信原子能力文档，不把传播窗口中的旧索引误判为发布失败，也没有重复发布并误升到下一个版本。
- **`Microi一键编译发布.sh` 按指定序列完成 v7.6.9 全量现代版发布与公共制品回读**：输入 `7.6.9、4、0、2、2、0、0`，完成现代前端生产构建、后端 Release、独立 18080 liveness、发布目录守卫、Microi.net／Microi.AI DLL 保护、20 个 NuGet 与 7 个正式／测试 Docker 标签，脚本耗时 11 分 29 秒并以 exit 0 结束；NuGet.org Flat Container 逐包回读 20/20，Docker 远端回读 7/7。后端 `latest/v7.6.9/dev-latest` 摘要统一为 `sha256:11767e560ca8caa03df5260fce4a08d819758254e6109a17e6ec9aa855d1616ee`，现代前端 Web／Client 四个标签统一为 `sha256:fcb486fb3a24c16ba5d57aefbb3c34e608423adce1de3dd380a7074f11af34184`；本轮明确没有生成 Chrome 49 legacy。

## v7.6.8 - (2026-08-26 07:01)

- **五仓历史与当前工作树按 v7.6.7 封版边界完成逐文件去重审计**：重新抓取全部 origin 后，以根仓 `23676b9022fcf34ef6c24a9f03bfee59779b415e`、Microi.net `35c196dbb5550db319ee9c399f3c80b304cce464`、Microi.AI `485bb10492a32fd175919aab5d562f2f039ce7e8`、Microi.VSCode `ab3d52802ff057de79aa35285b80b4a6d3fe31d0` 与 WebOS `608efdc80e7283918160ba8b338fb0823a38679f` 为边界，五仓均为 0 ahead / 0 behind，边界后没有遗漏提交、英文说明、团队自动 merge 或重复包装。本节写入前，根仓有 70 个已跟踪差异文件（1,542 行新增／381 行删除）和 1 个新增 V8 源码文件，Microi.net 与 Microi.AI 各 1 个版本文件（均为 3/3），Microi.VSCode 为 13 个版本、多宿主插件元数据及 Skills 文件（211/204），WebOS 保持干净且不制造空提交；`dist/bin/obj/node_modules`、VSIX、tgz、长 JSON、哈希清单和机械生成 bundle 只核对最终版本与同源性，不重复冒充独立功能。
- **API 启动门禁由“能登录的七接口”升级为九个官方基础应用包声明驱动的完整运行时接口闭包**：后端接收流量前，从表单、模块、SaaS、SSO、应用商城、系统账号、系统配置、消息通知和 AI 引擎九个程序集内置官方包读取全部 `SysApiEngines`，校验 Key 与稳定 Id 全局唯一、地址有效、`Platform/Managed` 或 `Tenant/CreateIfMissing` 所有权正确，再补缺并逐项物理强回读。当前用户 Hook、服务健康、组织机构、菜单角标、私有文件、消息、SSO、AI、商城工作器及内部依赖因此与登录页接口同时就绪；七项 `StartupDependencyBootstrapOnly` 仍只是跨历史租户事故恢复的最小可登录范围，不再被误称为完整运行时就绪。
- **内置官方包重放获得不可伪造、一次性消费的宿主可信上下文，同时继续保护租户代码**：Upgrade13 仅可从固定九包白名单读取并完整校验资源，调用固定 `import-microi-store-package v2.4.9` 时由宿主建立绑定当前租户与导入器 Key 的 Managed 协议上下文；V8 参数单独伪造可信标志会失败关闭。可信重放只恢复 `Platform/Managed` 官方核心，`Tenant/CreateIfMissing` 即使已禁用、软删除、大小写不同或正文为空也继续归租户维护，不会被启动过程恢复或覆盖；普通、社区、离线及非权威来源继续执行 Base／Local／Incoming 三方冲突保护。
- **应用商城补回旧批量安装地址，并收口官方包所有权与结构化版本日志**：新增独立 Managed 接口 `get-microi-store-legacy-route v1.0.0`，保留匿名地址 `/apiengine/get-microi-store` 并转发正式 Key `get-microi-store` 的 `/apiengine/get-microi-store-list`，避免旧工作器在新路由上线后报接口不存在。应用商城升至 v7.6.15、SaaS 引擎升至 v7.6.20、表单引擎升至 v7.6.7、SSO 升至 v7.5.8、消息通知升至 v1.0.10；官方包的 Managed 资源统一固化为 Platform 所有权，生成器对当前结构化日志增加幂等短路，重复刷新不再制造无意义差异。
- **Upgrade25 对历史未绑定版本的 AI 应用文件执行确定性、无损且可重放的 V3 归档**：升级先按 `OsClient + AppId + legacy-unversioned-v3` 生成稳定历史版本 Id，对同一路径冲突使用确定性分片版本保留全部文件，再补 `VersionId`、计算路径 Hash 并创建唯一索引；部分迁移重跑会保留已有效分片，只把冲突或未归档行分配到最低空闲分片。MySQL、SQL Server、Oracle 均使用参数化方言 SQL 并提供有界诊断样本；文件缺少 `AppId`、稳定 Id 被占用或版本身份不一致时失败关闭，不猜测、不合并、不删除历史文件。
- **长表名升级兼容与多数据库扩容同步修复**：`diy_field.TableName` 的运行时升级、新装 DDL、物理字段元数据和低代码字段元数据统一由 `varchar(50)` 扩容为 `varchar(255)`，并在从 `diy_table.Name` 回填之前先扩容，避免历史长表名触发 `Data too long` 中断整租户升级；字段容量检查与变更同时支持 MySQL、SQL Server 和 Oracle。应用商城、SaaS、V8 安全边界与事故复盘已同步写入中文官网文档及插件 Skills 单一事实源。
- **源码、官方资源、插件与 v7.6.8 现代版公共制品完成分层验收**：升级与运行时门禁 C# 定向测试 83/83、官方资源与应用包完整契约 373/373；后端 Release 编译 0 错误、9 条既有 xUnit analyzer 建议，独立 18080 liveness 返回 HTTP 200。官方资源 live 投影回读为 Managed 94、CreateIfMissing 8、总计 102，SHA-256 为 `a3abf5545ae11f2316543b4ee4d43f91d6137b1530165593d4ce894237fc901c7`。Microi.VSCode／CLI／Codex／CodeBuddy／WorkBuddy 统一升至 v5.3.3，npm、Visual Studio Marketplace 与 Open VSX 三个公开目标均已精确回读 v5.3.3。
- **`Microi一键编译发布.sh` 按指定输入完成 v7.6.8 全量现代版发布与公开回读**：输入 `7.6.8、4、0、2、2、0、0`，完成现代前端生产构建、串行压缩、后端发布目录守卫、Microi.net／Microi.AI DLL 混淆、20 个 NuGet 和 7 个正式／测试 Docker 标签，脚本耗时 10 分 8 秒并以 exit 0 结束；NuGet.org Flat Container 逐包回读 20/20，Docker 远端回读 7/7。后端 `latest/v7.6.8/dev-latest` 摘要统一为 `sha256:d9728189ff44941f9555b88b6bd3758444ed53a110c41c3533c1afcbe54c04cd`，现代前端 Web／Client 四个标签统一为 `sha256:fe9457e00592b9f2e47c0bcb8684f2c563d8a9376ba7a3d03512871b93261523`；本轮明确没有生成 Chrome 49 legacy。上述证据分别证明源码测试、官方资源、插件和公共制品可用，不等同于所有客户生产节点已经拉取、重启或完成登录后真实业务页面验收。

## v7.6.7 - (2026-08-26 04:00)

- **五仓历史与当前工作树按 v7.6.6 封版边界完成逐文件去重审计**：重新抓取全部 origin 后，根仓 `15d4e0797e064488ce033ef21c3a9f362fa241bf`、Microi.net `e385db7299c50991e869b0625284a64acd3fd31a`、Microi.AI `e01282242a32fa37dc762ad512b095272f9319de`、Microi.VSCode `e408cffb78d98c6910520951cd8a144e6834b73f` 与 WebOS `608efdc80e7283918160ba8b338fb0823a38679f` 均为 0 ahead / 0 behind；这五个提交分别是 v7.6.6 的根仓、核心、AI、插件封版及仍适用的 WebOS 边界，边界之后没有遗漏提交、英文说明、团队自动 merge 或重复包装。本节写入前的最终净工作树为：根仓 52 个已跟踪差异文件、3 个新增源码文件（合计 1,837 行新增／654 行删除），Microi.net 与 Microi.AI 各 1 个版本文件（均为 3/3），Microi.VSCode 11 个版本及多宿主插件元数据文件（197/197），WebOS 保持干净且不制造空提交；`dist/bin/obj/node_modules`、VSIX、tgz、长 JSON、哈希清单和机械生成 bundle 只核对版本与同源性，不重复冒充独立功能。
- **API 在接收流量前按租户建立七项页面启动接口闭包**：固定强回读 `platform-sys-menu`、`platform-os-client-by-domain`、`platform-sys-config`、`platform-lang-bundle`、`platform-current-user`、`platform-private-file-url`、`platform-sys-user-public-info` 的稳定 Id、官方地址、启用／匿名／HTTP 标志与 Managed 源码标记；缺失资源只从程序集内置的官方应用包补齐，不依赖可能已错误推进的 `ServerVersion`。配置主租户任一门禁失败会拒绝启动；其它权威运行租户逐一解析、修复并列出失败明细，单个历史子租户异常不再让全部 SaaS 租户离线。
- **启动自愈继续遵守共享升级租约、fencing 与客户源码保护**：每个节点先检查物理字段和七接口不变量，未就绪时最多等待同租户持锁节点 30 秒；取得租约后在写入前后检查租约有效性，逐项物理强回读并刷新 Key／Id／地址缓存。已有稳定身份、地址占用、非官方源码或 Managed 源码漂移均失败关闭，不用紧急启动门禁覆盖客户 V8；官方发布源仍禁止由程序集内置包反向写入，完整源码升级继续通过官方应用商城的 Base／Local 冲突规则完成。
- **历史版本迁移与运行时不变量获得可追踪、不可越权的统一执行器**：后台 HostedService 会在完整迁移前后复检启动闭包，并把应用发布租户门禁、V3 流结构、后台任务、数据库备份及表单 V8 限额等 expand-only 不变量统一放入 `RunRuntimeInvariantAsync`；每项动作前后都重新校验租约，失败立即中止。启动与迁移链按 `【自动升级状态】` 输出当前租户、步骤、版本门状态和最终汇总，日志拦截器固定保留这些信息到 stdout，运维可通过容器日志区分“版本已覆盖”“实际执行”“跳过”和失败原因。
- **17 个旧 PC／UniApp／定制端地址集中到可整体删除的兼容 Controller**：域名、系统设置、语言包、登录壁纸、当前用户、用户管理、租户创建、个人资料、默认首页及私有文件旧路由从 `FormEngineController`、`HDFSController`、`OsController`、`SysUserController` 移入 `LegacyMobileCompatibilityController`，所有业务仍委托固定 Managed 接口引擎；原四个宿主 Controller 删除 380 行兼容分支，所有权目录把新 Controller 标记为 `MigrationCandidate`，并为每项历史地址记录“已知客户端全部切换且连续两个正式版本遥测为零”的删除门槛。官网个人资料保存同步改为直调 `platform-user-update-profile`，不再新增旧地址调用者。
- **系统设置启动回退收窄为一个独立、匿名且安全的原子能力**：Microi.Core 新增 `PlatformBootstrapCompatibilityService.GetPublicSysConfigAsync()`，Managed V8 与旧 `/api/FormEngine/GetSysConfig` 共用同一浏览器安全投影，只额外返回部署端 RSA 公钥，私钥、连接串和 SaaS 敏感配置不会进入结果。Microi.Client 与 UniApp 始终先请求 `platform-sys-config`，仅在 HTTP 404／405／501 或返回值精确表明 `sys_apiengine` 缺少该 Key 时回退旧路由；401、租户错误、校验错误及其它业务失败不会被兼容重试掩盖，启动匿名请求也不会误带历史 Authorization。
- **源码契约、插件与 v7.6.7 现代版编译发布完成分层验证**：PC 平台接口迁移测试 10/10、UniApp 测试 4/4、后端启动门禁／Controller 所有权／安全与迁移契约测试 83/83 全部通过；Release 编译 0 错误、9 条既有 xUnit analyzer 建议，独立 18080 liveness 返回 HTTP 200，Microi.net 与 Microi.AI DLL 已完成混淆并写回 NuGet。Microi.VSCode／CLI／Codex／CodeBuddy／WorkBuddy 统一升至 v5.3.2，npm、Visual Studio Marketplace、Open VSX 三个公开目标均已回读 v5.3.2。
- **公共制品、官方应用源与客户生产验收继续作为独立事实边界**：`Microi一键编译发布.sh` 按输入 `7.6.7、4、0、2、2、0、0` 完成现代前端、后端、20 个 NuGet 与 7 个正式／测试 Docker 标签，耗时 10 分 20 秒并以 exit 0 结束；公共端逐项回读 NuGet 20/20、Docker 7/7，后端摘要为 `sha256:9dc2f7b8139222db44c11e3883efd39d523aefb1e33224da9e71aa255c4396f5`，前端 Web／Client 摘要为 `sha256:d52bb90b609f782546a716dc30ced6aecebdf16c4c81cdaa06199c883d9f6e3d`，只生成现代版、未生成 Chrome 49 legacy。官方应用包字节保持不变，第二次 live 投影仍为 Managed 93、CreateIfMissing 8、总计 101，SHA-256 为 `528316684de87c7dd9b2ecfa7a75a4a070a4ca3e8e843a0064a6f62dea9ffb763`；这些证据不等同于所有客户生产节点已经拉取、重启或完成真实业务页面验收。

## v7.6.6 - (2026-08-26 00:28)

- **五仓提交、推送历史与最终工作树按 v7.6.5 封版边界完成逐文件去重审计**：发布前重新抓取全部远端，根仓 `981ca2d0abc3906ad1b174a9f498c7ae055275a4`、Microi.net `02f07d769597f97fc14f7d2aa2830dafc5b4b06b`、Microi.AI `b9048f953120d3d486b48f7f3929208f9c274e05`、Microi.VSCode `e722e003d64dc3aaa984d835e104694207c3877f` 与 WebOS `608efdc80e7283918160ba8b338fb0823a38679f` 均为 0 ahead / 0 behind。根仓在 `180576c5c` 的 v7.6.5 封版提交后只有 `4155e0b88`、`547bd1faf`、`981ca2d0a` 三次同一事故链补充，最终净差异已由本节按启动闭包、任务防重复投递和精确定向自愈去重归纳；其余仓库自各自 v7.6.5 封版提交后没有新提交，范围内也没有遗漏的英文业务说明、团队自动 merge 或重复变更。本版写入本节后的净差异为：根仓 69 个文件（含 5 个新文件及本节日志），Microi.net 与 Microi.AI 各 1 个版本文件（均为 3/3），Microi.VSCode 15 个版本、插件元数据与 Skill 文件（212/207），WebOS 保持干净且不制造空提交；`dist/bin/obj/node_modules`、长 JSON、Base64、哈希清单和机械生成 bundle 只核对版本、能力与同源性，不重复冒充独立功能。
- **客户端改用固定平台健康契约判断全局 API 可用性**：新增 `/apiengine/platform-service-health` 主探针，只有该固定接口连续失败才显示全局服务不可用；普通菜单、表单或其它业务接口失败保持局部错误，业务请求成功也不能越权清除全局故障。滚动升级中的旧节点仅在固定接口不存在时回退 `/api/Diagnostics/health`，502／503／504、权限阻断和外部官方 API 异常均保留各自精确语义，避免单个接口故障遮住整个租户系统。
- **前后端真实版本进入框架常驻诊断与不可用页面**：客户端启动后主动预热健康状态，从 `package.json` 读取 Web 版本、从固定健康接口读取 Api 版本，并在 TagsView 右侧常驻显示 `Api vX.Y.Z - Web vX.Y.Z`；后端版本暂不可用时使用稳定占位而不误报离线。服务不可用弹层同步展示前端版本、后端版本、固定健康地址、探针模式和触发业务请求，改善亮色／深色对比度，便于区分前端未更新、后端未重启与真实服务故障。
- **Microi.Core 新增只读 `V8.Method.GetBackendVersion()` 与匿名 Managed 健康门面**：原子只从当前 `Microi.Core` 程序集 `FileVersion` 返回规范化 `vX.Y.Z`，不暴露程序路径、主机名、密钥或其它环境信息；SaaS 引擎新增 `platform-service-health v1.0.1`，固定返回 `Status=Healthy`、健康契约、检查时间和可用的后端版本，不查询业务表、不调用租户 Hook。应用包先于后端二进制发布时，版本原子缺失仅让 `BackendVersion` 暂时为空，不反向破坏健康结论。
- **历史子租户启动自愈补齐物理表、稳定 Id、低代码元数据与运行缓存四层前置条件**：受信控制面在复制固定官方工作器前权威回读 `sys_apiengine`；极老 MySQL／SQL Server 空库可幂等创建最小物理表、补齐并回填稳定 Id，再从内嵌官方 `PhysicalColumns` expand-only 补全 `diy_table`、`diy_field`、`sys_apiengine` 生成实体列。目标库缺少 `sys_apiengine` 的低代码自描述时，只复制缺失的固定表／字段元数据并刷新缓存；已有租户定义、不同源码、较新版本、地址或稳定 Id 冲突继续失败关闭，不覆盖客户代码。
- **启动依赖事故恢复支持七项接口只自举并保持普通商城安装语义不变**：精确闭包固定为应用商城与 SaaS 引擎两个唯一资源所有者，先从不可变官方包补齐 `platform-sys-menu` 与六项 SaaS 运行门面，再逐项强回读 Key、地址、版本、启用／匿名／HTTP 标志和官方源码标记。受信 `StartupDependencyBootstrapOnly=true` 只在持久任务、有效 fencing token、权威当前分区租户目录和两应用闭包同时成立时生效，七项一致后立即成功，不写应用安装版本、不把大范围事故恢复拖入完整包重装；定向完整维护单个历史租户和普通“一键维护全部平台应用”继续使用原完整流程。
- **Product/Internet 子租户启动故障完成生产恢复与逐租户回读**：对权威目录中的 78 个唯一启用子租户逐一请求系统设置与菜单两个入口，共 156/156 个路由不再出现 `NoExistData`；匿名菜单请求按预期返回鉴权错误而不是“接口不存在”。`a123456777.microi.net` 已使用真实浏览器回读完整登录页、Logo、账号密码框、登录按钮与版本号，`jhyxdkj`、`lsg` 等同批租户通过相同物理路由门禁；这证明当前生产入口已经恢复，但仍不把后台完整应用包维护进度等同于每个租户的全部业务页面回归。
- **官方应用商城与 SaaS 引擎包完成结构化升版、官网发布与 live 投影回读**：应用商城由 v7.6.9 升至 v7.6.12，最终 SHA-256 为 `03311e158139efde9f38b1f3ffcf627a632cc21b719e5fa814a0853159261190`；SaaS 引擎由 v7.6.14 升至 v7.6.19，最终 SHA-256 为 `97bb6c8cd1431b7dd3295e5cf7cecef1fe1bc18ca38532b8f6f688805016b8c1`。本地升级资源、官网资源、官方同步基线、内嵌／独立工作器和结构化 ChangeLog 完成同源校验，live 投影回读为 Managed 93、CreateIfMissing 8、总计 101；该证据证明官方源可安装，不等同于全部生产租户已经完成更新和真实页面验收。
- **“一键制作主库空数据库”递归清除 AI 应用菜单树并重新发布七种数据库成品**：清理器识别顶级“AI应用”后不依赖记录顺序递归删除全部后代，同时保护非目标“AI助手”语义；新增真实引擎回归覆盖后代先于父节点返回的情况。后台发行任务 `71e539deb41140a8b3707210aed474ea` 终态成功，MySQL 5.7／8.0、SQL Server 2022、Oracle 19c、达梦 DM8、PostgreSQL 17、人大金仓 V9 共 7 个 HDFS ZIP 均发布；零残留门禁确认应用菜单、表／字段、接口、任务、微服务、AI 商城数据和敏感运行数据均为 0。公开 MySQL 5.7 ZIP 下载后 SHA-256 回读为 `0bda56122dc88e4579d9675efb04dbbca9422ccdbbf14addcd0a3f418617d694`，解压后的 `sys_menu` 数据块中“AI应用”出现次数为 0，验证的是线上成品而非仅本地 SQL 源码。
- **官网文档、Skills、Monaco 类型与 Microi.VSCode 多宿主插件同步新契约**：V8 后端、SaaS、应用商城及平台路由索引补齐固定健康接口、`GetBackendVersion()`、多运行分区验收和只自举安全边界；客户端 Monaco 同步后端版本原子声明。`npm run publish` 完成构建、类型检查、生命周期测试、VSIX 打包与公开发布，Microi.VSCode／CLI／Codex／CodeBuddy／WorkBuddy 的版本和 Skills 统一升至 v5.3.1，npm、Visual Studio Marketplace、Open VSX 三个公开目标最终均回读为 v5.3.1。
- **源码、契约测试与 v7.6.6 现代版公共制品完成分层验收**：客户端健康与版本定向测试、官方资源与应用包总契约 372/372、空数据库递归清理 9/9、子租户物理自愈相关 C# 测试 35/35 全部通过；Release 编译 0 错误、9 条既有 xUnit analyzer 建议。`Microi一键编译发布.sh` 按输入 `7.6.6、4、0、2、2、0、0` 完成现代前端、后端、独立 18080 liveness、DLL 加密、20 个 NuGet 与 7 个正式／测试 Docker 标签，耗时 9 分 55 秒并以 exit 0 结束；公共端回读 NuGet 20/20、Docker 7/7，后端推送摘要为 `sha256:d25f0245735907c536dc90663ac364bb46b1df199acf59d398f11f54291be89a`，前端 Web／Client 推送摘要为 `sha256:58dc31605431f7a4d33c0b514c0b8f4b94d3346fd76785f6a272ae6e5e528865`，只生成现代版、未生成 Chrome 49 legacy。公共制品、官方应用源、源码测试和所有客户生产节点的拉取／重启／真实业务验收继续作为四个独立事实边界。

## v7.6.5 - (2026-08-25 15:40)

- **五仓提交、推送历史与最终工作树按 v7.6.4 封版边界完成逐文件去重审计**：发布前重新抓取全部远端，根仓 `076f0000cafafb992dc59e080f6a0476cbfdbf09`、Microi.net `d75e1b577f2c68f63e8619eb9ae7b0730c0fe14c`、Microi.AI `6479746a54388932ec045f07569bcbcaed0886e7`、Microi.VSCode `7457fcb168857cceb6f35bd89e63934f05889bc0` 与 WebOS `608efdc80e7283918160ba8b338fb0823a38679f` 均为 0 ahead / 0 behind；这些正是 v7.6.4 已记录的五个封版提交，边界后没有遗漏提交、英文业务说明、团队自动 merge 或重复包装。本版最终净差异为：根仓 48 个文件（1,449 行新增／183 行删除，包含本节更新日志与封版期间收口的在途任务迁移、精确定向修复），Microi.net 与 Microi.AI 各 1 个版本文件（均为 3/3），Microi.VSCode 12 个版本、插件元数据与 Skill 文件（201/199），WebOS 保持干净且不制造空提交；`dist/bin/obj/node_modules`、长 JSON、Base64、哈希清单和机械生成 bundle 只核对版本、能力与同源性，不重复冒充独立功能。
- **子租户平台应用维护新增受信的启动依赖事故恢复范围**：主租户后台任务只有在权威调用链显式传入 `MaintenanceScope=StartupDependencies` 时，才把子任务严格收窄为 `RequiredAppIds=['app.microi.saas-engine']`；入队后再次写入并回读任务标题、停止标记、维护范围与精确应用集合，不一致即取消不安全任务。日常“一键维护全部平台应用”继续执行原完整计划，不会被事故恢复逻辑永久缩窄。
- **运行中的子租户恢复任务切换到最新工作器后不再重复投递，并支持权威定向修复**：后台任务每个分片都会按 ApiEngineKey 获取最新接口源码，因此编排器 v1.2.8 将修订固定为 `startup-api-live-worker-v6-no-requeue`；v1.2.1–v1.2.6 遗留的 `RefreshBootstrap` 检查点只迁移回 `Monitor`，继续汇总原 `ChildTasks`，迁移区间明确禁止调用 `QueueChildTenantPlatformAppMaintenance`。受信 `StartupDependencies` 任务可通过 `TargetOsClients` 只修复指定历史租户，但每个目标必须通过当前运行环境的权威启用子租户目录校验，非事故范围、格式错误、超过 100 个、未知或越界目标均失败关闭；这样单租户恢复无需重跑全量租户，也不会因历史幂等键格式变化产生第二批安装任务。先前 v7.6.7–v7.6.12 的重新投递刷新方案已由 v7.6.13/v7.6.14 收敛替代，不再作为最终执行路径。
- **应用商城导入器 v2.4.7 为可信 SaaS 启动恢复增加六个 Managed 接口快速自举**：`platform-os-client-by-domain`、`platform-sys-config`、`platform-lang-bundle`、`platform-current-user`、`platform-private-file-url`、`platform-sys-user-public-info` 会在大包字段与 DDL 分片前完成创建／对账；若租户已修改源码、稳定 Id 或 ApiAddress 发生冲突则失败关闭，不静默覆盖。安装计划固定优先“应用商城 → SaaS 引擎 → 其余应用”，商城列表调用也改为真实自定义地址 `/apiengine/get-microi-store-list?OsClient=...`。
- **旧 SaaS 数据库补齐稳定接口 Id 与 MySQL BIT(1) 兼容**：`UpgradeAppStore` 先确保 `sys_apiengine.Id varchar(36)` 存在并为历史空值回填 UUID，准备度检查要求 Id 非空；导入器对 `IsEnable`、`StopHttp`、`AllowAnonymous` 只拼接内部归一化的 0/1 常量，接口 Id 继续参数化，避免 MySQL 驱动把 Jint 数字按字符串绑定后报 `Data too long`。写入后会物理回读三个运行标志并刷新相关缓存，普通租户源码和个性化 Hook 仍受 Managed／CreateIfMissing 策略保护。
- **空数据库发行把应用菜单所有权纳入递归清理与零残留门禁**：除既有应用表、接口、任务、微服务和敏感数据外，后端与 `admin_get_empty_database_sanitization_sql` v1.3.2 会识别应用 StoreId/AppKey、应用表绑定菜单以及顶级“AI应用”整棵树，递归清除全部后代并复核名称、引用和物理数据零残留；平台核心菜单与“AI助手”继续受保护，避免把业务演示入口带入可公开的空数据库。
- **官方应用商城与 SaaS 引擎包完成同源升级及 live 投影回读**：应用商城升至 v7.6.9，最终 SHA-256 为 `ccde7b8c6967bc8a1f0872a8e63ced9fcc5a234e7078bbb055121b24729408d62`；SaaS 引擎最终升至 v7.6.14，SHA-256 为 `d679d4ca689c418b3c868f672c33c1d47edcdfb741033ac9113effd4e1ede01a`。两包的本地升级资源、官网资源、官方同步基线、结构化 ChangeLog 和内嵌工作器字节一致，独立第二次 live 投影回读为 Managed 92、CreateIfMissing 8、总计 100；该证据证明官方源具备后续安装能力，不等同于所有子租户已完成本轮恢复任务。
- **Microi.VSCode、CLI 与多宿主插件统一发布 v5.3.0**：`npm run publish` 完成构建、类型与生命周期测试、VSIX 打包以及 npm／Visual Studio Marketplace／Open VSX 发布；公开回读最终均为 v5.3.0。Codex、CodeBuddy、WorkBuddy 插件元数据、Skills 版本与 MongoDB Skill 同步更新，机械生成的元数据和 bundle 不重复计为业务功能。
- **源码、契约测试与 v7.6.5 现代版公共制品完成分层验收**：本轮针对性 C# 测试 64/64、v7.6.13/v7.6.14 迁移与定向修复 C# 测试 27/27、应用商城与导入器资源总契约 86/86、最终变更文件定向契约 22/22 全部通过，Release 编译 0 错误、9 条既有 xUnit analyzer 建议。`Microi一键编译发布.sh` 按输入 `7.6.5、4、0、2、2、0、0` 完成现代前端、后端、独立 18080 liveness、DLL 加密、20 个 NuGet 与 7 个正式／测试 Docker 标签，耗时 10 分 37 秒并以 exit 0 结束；公共端回读 NuGet 20/20、Docker 7/7，后端摘要为 `sha256:cff80ebbafc9703122f9bdcfd449a6e634e83bde08e048fb7cac59b71e365905`，前端 Web／Client 摘要为 `sha256:514835ad7d2875070cb4aa4ea71b4f4ff7b8c4579c342a7b3118552c66abcde1`，只生成现代版、未生成 Chrome 49 legacy。v7.6.14 应用包修订通过官方应用源独立交付，不声称已进入先前不可变的 v7.6.5 NuGet／Docker 二进制；公共制品、官方应用源和源码测试分别成立，不把它扩写为所有客户生产节点已经拉取、重启或完成真实业务验收。

## v7.6.4 - (2026-08-25 12:15)

- **五仓提交、推送历史与当前工作树按原文档最后既有正式版 v7.5.10 的封版边界完成逐文件去重审计**：根仓 `689442fc..30b0d768` 只有 `30b0d768 发布 v7.5.10：收口接口引擎架构并完善系统观测与商城存储`，Microi.net `6334cd65..5697f819` 只有 `5697f819 发布 v7.5.10：开源迁移 V8 与翻译运行时`，Microi.AI `40d154d6..01c0929d` 只有 `01c0929d 发布 v7.5.10：接入 AI 工作流运行时接口`，Microi.VSCode `a52aaebb..514f5a39` 只有 `514f5a39 发布 v5.2.7：支持接口引擎流式传输并同步平台规范`，WebOS 自 `cf8b4703` 后无提交；四个提交都是 v7.5.10 已记录内容的封版包装，本版不重复计入。范围内没有 post-anchor merge、自动 merge 或遗漏的英文业务说明，因此也没有需要另行翻译的英文提交。写入本节前，根仓有 250 个已跟踪差异文件、88 个新增项（12,022 行新增／5,705 行删除），Microi.net 为 5 个文件（79/7），Microi.AI 为 5 个已跟踪文件、1 个新增文件（312/259），Microi.VSCode 为 34 个文件（476/302），WebOS 为 1 个文件（1/1）；`dist/bin/obj/node_modules`、长 JSON、Base64、哈希清单和机械生成 bundle 只核对版本、能力及同源性，不逐份冒充独立功能。
- **Microi.net.Api 完成第二轮 V8 优先瘦身，并用所有权目录阻止业务重新长回 Controller**：最终目录审计为 `Controllers=33`、`MigratedControllers=15`、`ActionOverrides=30`、`KeepApi=37`；其中 15 组业务 Controller 是 v7.5.10 已建立的迁移基线，本轮没有重复声称再次删除，本轮把 Action 兼容／协议例外从 8 个扩展并细化到 30 个，净增 22 个，同时迁移或删除 7 个支撑类型。`ApiEngineController`、`FormEngineController`、`HDFSController`、`UploadController`、`WorkFlowController` 等只保留动态路由、旧客户端兼容、可信鉴权、协议、存储传输或工作流内核；`api-ownership-catalog.json` 与源码契约测试禁止重新引入可由低代码或 V8 编排的业务分支。
- **PC、UniApp、WebOS 与新建 AI 应用的六个历史定制入口统一迁入官方 Managed 接口引擎**：`/api/Os/GetOsClientByDomain`、`/api/FormEngine/GetSysConfig`、`/api/FormEngine/GetLangBundle`、`/api/SysUser/GetCurrentUser`、`/api/HDFS/GetPrivateFileUrl`、`/api/SysUser/GetSysUserPublicInfo` 分别由 `platform-os-client-by-domain`、`platform-sys-config`、`platform-lang-bundle`、`platform-current-user`、`platform-private-file-url`、`platform-sys-user-public-info` 接管，标准前端、UniApp、WebOS、Skills SDK 与 AI 应用模板不再调用六个旧地址。旧 Controller 路由继续为少量历史 UniApp／定制客户端提供受控代理，但只复用同一接口引擎，不再维护第二份业务实现；应用商城 v7.5.57 内的 `ai_app_create` v1.2.2 也从首个请求开始使用平台接口。
- **可信原子与跨产品通用能力下沉到开源 Microi.Core**：新增平台运行时、系统账号、租户设置、官方资源发布、私有文件授权、可信执行上下文、用户行为审计及 MongoDB 参数等通用契约；域名解析、公开系统设置、语言包、登录壁纸、私有文件授权、用户状态刷新、租户创建授权等不能安全交给任意租户脚本的最小原子由 `V8.Method` 暴露，并限定只能从对应官方 Managed 引擎调用。Microi.net、Microi.AI 只保留许可证、运行时、Provider、支付验签等不可开放边界，业务动作选择、状态流转与个性化编排留在可升级的 V8 应用层。
- **九个官方基础应用统一补齐醒目的源码恢复警告与不可覆盖个性化 Hook**：当前 100 个接口引擎的 `ApiEngineKey`、稳定 `Id` 均全局唯一，92 个 Managed 引擎都有“安装、更新或重装会恢复官方源码”的醒目头注释；8 个 `CreateIfMissing` 扩展点为 `platform_auth_login_hook`、`platform-ai-custom-hook`、`platform-marketplace-source-hook`、`platform-message-notification-custom-hook`、`platform-runtime-custom-hook`、`platform-system-settings-custom-hook`、`platform-user-custom-hook`、`sso_event_hook`，默认均直接返回 `return { Code : 1 };`，首次创建后不被官方包覆盖。Managed 代码仍按 Base／Local 哈希冲突失败关闭，避免静默覆盖租户修改。
- **系统账号、个人资料、偏好与租户设置从 Controller 分支收敛为可扩展平台接口**：`platform-sys-user-admin` 统一用户增删改、角色层级、密码哈希及登录态刷新，`platform-user-update-profile`、`platform-user-update-preferences` 只接受固定白名单字段，`platform-tenant-system-settings` 负责租户隔离的系统设置读写；可信原子在 Hook 前后重新校验权威当前用户、租户、角色、组织和认证边界。微信小程序头像／昵称等资料继续由最小协议网关审核，审核票据与登录码不进入 V8 或持久数据；已提交成功后的 Hook 异常仅作为告警返回，不伪装成业务回滚。
- **SaaS 初始化、外部登录与用户绑定统一由官方接口编排**：新增或接管租户创建、外部账号绑定、微信绑定、当前用户、公开用户信息、域名解析、系统设置、语言包、登录壁纸和 `microi-init`；域名只接受精确启用映射，冲突失败关闭，登录投影递归剔除密码、AI 密钥和一次性票据。PC 与 UniApp 的启动、登录、个人中心及用户管理同步切换新入口；前端仅在确实缺少 `platform-sys-menu` 时执行小于 60 秒的有界升级等待，超时返回可操作错误。
- **普通聊天、联系人、未读、历史与删除迁入消息通知应用，并在 v1.0.9 修复 SignalR 的可信内部调用**：`platform-chat-runtime` v1.0.1 以 MongoDB 为事实源，SignalR 仅承担认证连接和提交后的实时投递；`platform-chat-system-message` 负责系统消息，个性化逻辑进入消息通知 Hook。v7.6.2 真实页面回归发现 Hub 通过普通兼容调用把 `_InvokeType` 设为 `Client`，导致 `StopHttp=1` 的聊天运行时在 `SendLastContacts`／`SendUnreadCountToUser` 内部调用时报“禁止 http 调用”；现改为由宿主建立绑定固定 Key、权威 `OsClient` 和当前用户的一次性可信协议上下文，V8 首先用 `RequireManagedProtocolContext` 原子消费。`StopHttp=1`、`AllowAnonymous=0` 保持不变，直接 HTTP 仍被拒绝，没有以放宽安全策略掩盖回归。
- **消息通知与 AI 运行时稳定 Id 冲突彻底消除**：消息通知应用 v1.0.9 将 `platform-chat-runtime` 固定为 `019d2a01-9d63-7f91-8c02-00000000000b`，AI 应用 `platform-ai-runtime` 使用 `019d2a01-9d63-7f91-8c02-00000000000a`；官方九包新增全部 `sys_apiengine.Id` 与 `ApiEngineKey` 全局唯一契约，避免在线或离线整包安装遇到“同 Id、不同 Key”时冲突回滚。
- **AI 账号、额度、订阅支付与 Provider 调用形成“V8 编排 + 最小原生适配器”边界**：继 v7.5.10 已迁出的 `platform-ai-workflow` 后，本轮新增 `platform-ai-account`、`platform-ai-runtime`，统一套餐、订阅、订单、支付完成、额度快照和非流式 AI 动作，并在关键阶段调用不可覆盖的 AI Hook。支付宝 C# 网关只按精确租户读取配置、绑定回调租户、验签并输出规范事件；订单认领、金额核验、订阅更新和 API Key 分配在 V8 事务中完成。额度扣减使用租户级 Redis 分布式锁并在共享缓存不可用时失败关闭；私有 Provider 密钥不进入 Hook，运行时也不再自行创建或修改业务表结构。
- **应用商城 v7.5.57 收口 HDFS 单次写入、新建应用运行时与官方资源投影**：v7.5.54 引入的 `microi-store-package-storage` v1.1.0 与 v1.2.8 资源运行时固定复用线上已验证的 Base64 单次上传协议，不再运行时探测 `UploadText`，也不在首个 HDFS 写入返回空值或抛异常后盲目降级重试，避免“第一次其实已落盘”时制造孤儿对象；成功后仍按 UTF-8 精确字节数与 SHA-256 回读，再保存包指针。v7.5.55 把新建 AI 应用 SDK、鉴权桥和系统配置／当前用户／私有文件入口切到平台接口引擎；v7.5.57 与官方资源控制面 v1.3.1 则自动投影平台接口并兼容物理表结构，父发布器仍严格区分“存储接口返回空”“返回非 1 的 Code／Msg”“Code=1 但缺少 Data”三类错误。
- **升级程序以同一依赖图安装并校验九个官方基础包**：`UpgradeAppStore` 6.4.11.0 按“应用商城 → 表单／模块 → SaaS／系统账号／系统设置 → 消息通知／AI／SSO”的顺序安装与验证；v7.6.4 内置应用商城 v7.5.57、表单引擎 v7.6.6、模块引擎 v7.6.1、SaaS 引擎 v7.6.1、系统账号 v7.6.0、系统设置 v6.3.8、消息通知 v1.0.9、AI 助手 v7.6.0、SSO v7.5.7，并检查六个启动入口、聊天、AI、账号、设置、商城源和全部 Hook 的存在与版本。共享租约、持久版本门、幂等安装、整包事务和失败不推进语义保证其它用户只升级前后端后仍能获得必需接口资源；表、菜单、接口引擎和种子数据没有重新塞入一次性 .NET 业务迁移。
- **官方应用 ChangeLog 成为制包、同步和上线前的强制门禁**：每个官方包必须提供对象形式的 `PackageInfo.ChangeLog`，其 `Version` 必须与 `PackageInfo.Version` 精确相等，`Title`、`ChangeType`、`Content`、`ReleaseTime` 均不能为空，时间必须使用 `yyyy-MM-dd HH:mm:ss`；当前版本的 `ChangeHistory` 日期与正文也必须一致。应用商城制包按钮同时要求 `sys_microistore_changelog` 已存在同租户、同 StoreId、同精确版本且未删除的完整记录；本地、基线或官网任一候选缺失即失败关闭，不能靠升版本绕过更新日志。
- **四个本轮变更的官方应用已通过 `microi_itdos` 在线投影并完成商城指针、升级资源、HDFS 与本地四方回读**：表单引擎 v7.6.6（SHA-256 `d21cf79461fecee1e99c1ff0733141dbf29198416009528a2557cf34b48047c9`）、模块引擎 v7.6.1（`0f7bd5d1ba9b3481de105b788162e72b5a63df88379df549eb1c8637933c4142`）、SaaS 引擎 v7.6.1（`a94c4ca4e4e2bb499593b316411daa3003a8f8d4018163d2a03cab0b6a94bd94`）与 SSO v7.5.7（`e4b7f6cdf6b88699dc94dfa2848a9d236e53280cb5bfab8d6aed7d02f033b864`）版本和字节均一致。九个官方包的 live 接口投影仍为 92 个 Managed + 8 个 CreateIfMissing = 100 个，投影 SHA-256 为 `1bba4b37b8349e14126a8d8cd88385b151de21c437bc63a61873d8c9f18ee0ea`；该证据证明官方源可升级，不代表所有目标租户已主动安装。
- **私有文件、富文本、CAD、头像与跨平台文件同步统一走权威资源引用授权**：`platform-private-file-url` 在签发短链前按当前租户、用户、资源类型、表／记录／字段／菜单和原始对象键重新授权；文件柜、远端文件同步、表单附件、富文本、OnlyOffice、工作流头像、CAD 衍生预览与 UniApp 同步传入权威引用。AccessKey 会话和只有裸路径的请求失败关闭，签名 URL 只用于短期展示而不回写业务字段；`HDFSController` 仅保留历史下载／上传兼容和传输协议。
- **UEditor 旧实现完成租户与网络边界清理**：配置改为请求级租户读取，不再使用跨租户静态缓存；上传流直接写入当前租户私有 HDFS；删除递归枚举本机文件的旧列表处理器，防止通过文件名泄露其它租户资源；移除未使用且存在 SSRF 风险的 `catchimage` 抓取路径及关联 `CrawlerHandler`、路径／字符串工具类。
- **MongoDB V8 能力补齐批量条件更新／删除、排序、分页和投影，并修复旧数据 `_t` 动态投影兼容**：`UptFormDataByWhere`、`DelFormDataByWhere` 要求非空、参数化 `_Where`，禁止修改 `_id`，租户来自可信运行上下文且拒绝伪造。旧强类型 Mongo 写入可能留下内部 CLR 判别字段 `_t`，V8 以 `dynamic/System.Object` 读取时会因原 CLR 类型已迁移或移除而抛出 `Unknown discriminator`；现在六条通用动态读取路径统一在服务端投影阶段剔除 `_t`，调用方即使显式请求也不能读取，强类型读取仍保留既有多态行为。Microi.Client Monaco、Microi.VSCode 类型提示、后端文档与 `v8-mongodb` Skill 同步明确 `_t` 不是业务字段，聊天等 Managed 引擎无需把数据业务退回 Controller。
- **身份、访问密钥、实时协议和请求压力保护继续收紧**：DiyToken 仍是唯一会话入口；SSO 的身份解析、JIT／绑定、角色映射、Claim 投影和审计留在 Managed V8，脱敏事件交给 `sso_event_hook`。AccessKey 只允许精确能力域，不能用相似前缀命中六个兼容路由，也不能订阅尚未定义能力的实时 Hub；游戏／聊天实时连接重新核对 JWT 到期时间和活动 Token 缓存。请求压力门将静态 Gate 注册表限制为 4096 项，规范化高基数匿名路径、租户和接口 Key，并把键长限制为 256，避免攻击者用随机路由持续扩张进程内存。
- **API 支撑代码迁入可复用 Core，旧目录只保留协议层**：用户行为会话追踪迁入 `Microi.Core/Runtime`，富文本私有资源引用迁入 `Microi.Core/Security`；Microi.net.Api 删除对应重复服务与 5 个废弃 UEditor 类型。新的兼容调用器、协议上下文、私有资源授权和测试均放在可复用层，避免 Middleware、Handler、Services、Models 继续成为隐形业务堆积区。
- **官网文档、Skills、MCP、类型提示与 Microi.VSCode 同步本轮边界**：V8 后端、接口引擎、前端 SDK、UniApp、SSO、安全、消息通知、应用商城、部署和工作区规则补齐 Controller 四级归类、Managed／CreateIfMissing、官方恢复提示、可信内部协议、私有文件引用、AI 支付与官方包 Id／Key 唯一约束。Microi.VSCode／CLI／Codex／CodeBuddy／WorkBuddy 的 Skills 与类型声明机械重建，`npm run publish` 完成构建、校验及公开发布并统一升至 v5.2.9；npm、Visual Studio Marketplace、Open VSX 三个公共市场均已回读 v5.2.9。生成 bundle、长 JSON、Base64 和哈希清单只作为源码同源产物，不重复冒充独立功能。
- **源码、测试与 v7.6.4 现代版公共制品完成分层验收**：官方资源契约 362/362、后端 Quick 2,679/2,679 全部通过，Release 编译 0 错误、9 条既有 xUnit analyzer 建议。`Microi一键编译发布.sh` 最终按输入 `7.6.4、4、0、2、2、0、0` 完成后端、现代前端、独立 liveness、DLL 加密、20 个 NuGet 与 7 个正式／测试 Docker 标签，成功重跑耗时 9 分 18 秒并以 exit 0 结束；公共端逐包下载／解析回读 NuGet 20/20、Docker 7/7。后端 OCI 摘要为 `sha256:1601bc76a9a362dd1c29df6f92cc9e9faabe41e8f2ee2599fa060f0a55af65ca`，前端 Web／Client 摘要为 `sha256:e291d9ec842a0675b0b83d8be05e4eb8873f33a359016927d6353378a6989581`，只生成现代版、未生成 Chrome 49 legacy。
- **共享本地 v7.6.4 浏览器与 MCP 终验覆盖登录首页及 SignalR 真实读取链路**：使用 admin 登录 `http://localhost:61500/?OsClient=iTdos` 后，首页真实显示 Admin、v7.6.4 与“我的工作”，后端 liveness／health 均为 HTTP 200；SignalR WebSocket 已连接、PC 聊天监听已注册、联系人加载成功，浏览器控制台错误、匹配到的运行时错误、HTTP ≥400 与网络加载失败均为 0。随后通过本地 `microi_itdos_localhost_61501` MCP 调用 `platform-chat-runtime` 的 `ListContacts`，返回 `Code=1`、`DataCount=7`，该只读动作没有伪造存储提交或消息投递。依赖 `mci_release_gate`／`release_gate_echo` 且需要隔离可写 fixture 与运行端环境变量的 3 个 FullStack 用例本轮未执行，也不计入通过数；本地真实页面、官方源和公共制品可用仍不等同于所有客户生产节点已经拉取、重启并完成业务验收。

## v7.6.3 - (2026-08-25)

- **公开候选版已由 v7.6.4 取代，请勿作为本轮最终升级基线**：v7.6.3 曾完成现代前端、后端 Release、20 个 NuGet、7 个 Docker 制品与当时的应用资源发布；其后最终收口又发现旧 Mongo 强类型文档中的 `_t` 判别字段会破坏 V8 动态读取，并补上官方包 ChangeLog 强门禁、表单／模块／SaaS／SSO 新版本在线投影及最终 SignalR 浏览器验收。公共 NuGet／镜像不可变，因此没有覆盖同版本，而是以 v7.6.4 重跑全部发布和回读；v7.6.3 只保留为候选版历史事实。

## v7.6.2 - (2026-08-25)

- **公开候选版已由 v7.6.4 最终取代，请勿作为本轮最终升级基线**：v7.6.2 曾完成现代前端、后端 Release、20 个 NuGet 与 7 个 Docker 制品的公共发布，且应用商城 v7.5.54 的 HDFS 单次写入修复已通过其发布前资源门；随后真实页面回归发现 SignalR Hub 内部调用 `StopHttp=1` 的 `platform-chat-runtime` 被误判为外部 Client HTTP。已发布的 NuGet／镜像不可变，未以同一版本覆盖不同内容；后续版本已把消息通知升级至 v1.0.9、聊天运行时升级至 v1.0.1，并最终在 v7.6.4 完成页面验收。v7.6.2 只保留为候选版历史事实。

## v7.6.1 - (2026-08-25)

- **公开候选版已由后续版本取代，请勿作为本轮离线升级基线**：v7.6.1 曾完成现代前端、后端 Release、20 个 NuGet 与 7 个 Docker 制品的公共发布，但整包验收发现消息通知 `platform-chat-runtime` 与 AI `platform-ai-runtime` 误用同一个稳定 `sys_apiengine.Id`；导入器会对“同 Id、不同 ApiEngineKey”失败关闭，而公共 NuGet 制品不可覆盖。后续版本已拆分稳定 Id 并增加九包全局唯一契约；v7.6.1 只保留为候选版历史，不代表通过最终离线升级验收。

## v7.5.10 - (2026-08-24 18:51)

- **五仓提交、推送历史与当前工作树按 v7.5.9 封版边界完成逐文件去重审计**：重新抓取 Gitee 后，根仓 `689442fc9a936cd5b8c28f61d6cd019072b68d76`、Microi.net `6334cd65f18f8abc043d1545ac68f712624f0fae`、Microi.AI `40d154d6dfd183bb0e93cf40b29caefee396a4b7`、Microi.VSCode `a52aaebb17971789d6b6657fde7bef5ac686c20d` 与 WebOS `cf8b4703b51f2ddade779db4081e6d6ee18b6e9e` 均为 0 ahead / 0 behind，边界后没有遗漏提交、英文说明、团队自动 merge 或重复包装。本版来自最终工作树净差异：根仓 231 个已跟踪文件（9,599 行新增／5,506 行删除）和 82 个新增源码、测试、文档与图片文件；Microi.net 39 个迁出／版本文件和 1 个最小闭源工厂文件（24/13,119）；Microi.AI 3 个运行时接口／版本文件（47/4）；Microi.VSCode 50 个版本、插件、类型和 Skills 文件（616/496）；WebOS 无变更且不制造空提交。V8/翻译源码从闭源仓迁至根仓属于同一份代码的移动，`dist/bin/obj`、长 JSON、Base64、哈希清单和机械生成文件只核对最终同源性，不重复冒充新功能。
- **平台业务 HTTP 层按“低代码 CRUD → 接口引擎 → 最小 V8 原子 → 必要协议 Controller”完成系统性收口**：AI 工作流、后台任务、缓存、腾讯 IM、定时任务、RabbitMQ、MQTT、在线终端、搜索、采集、基础数据、部门、日志、菜单和角色等 15 组业务 Controller 已删除，改由官方 Platform/Managed 接口引擎通过应用商城交付；前端、UniApp、MCP 与表单事件同步调用 `/apiengine/{ApiEngineKey}` 自定义地址。剩余 Controller 由 `api-ownership-catalog.json` 明确归类为宿主内核、可信鉴权、协议网关、存储传输或兼容边界，并由测试阻止业务接口重新膨胀；定时任务表单不再携带 Token 回调自身 Controller，而是在原事务中调用 `platform-schedule-job`。通用 `/api/ApiEngine/Run` 仅保留旧客户端兼容，新动态路由即使应用尚未安装也返回结构化错误，并按真实引擎 Key 记录耗时与流量。
- **接口引擎原生支持 SSE／NDJSON 流式传输并保持事务语义**：`sys_apiengine.ResponseType=Stream` 时提供 `V8.Stream.IsAvailable/Write/WriteAsync/WrittenBytes/WrittenChunks`，宿主实施单片与总字节上限、客户端背压、断线取消、心跳和 TraceId；业务分片为 provisional，只有最终结果 `Code=1` 且事务提交后才发送 `done`，异常发送脱敏 `error`，不把已输出分片伪装成已提交结果。动态自定义路由、访问密钥能力域、MCP 创建／保存接口引擎的写后回读、VS Code 类型提示和服务端文档同步覆盖 `JSON/String/File/HTML/Stream`，兼容旧 `ResponseFile`、`SSE` 与数值配置。
- **可开源 V8 运行时从闭源 `Microi.net` 迁入 `Microi.Core`，商业边界缩至最小闭源工厂**：Jint 宿主、执行域、租户上下文、缓存／HDFS／OCR／MQ／Office／Spider、调试会话、通知、平台认证、SSO、系统观测、定时任务和工作流统计等约 1.3 万行通用运行时代码进入开源核心；闭源仓只保留 `V8ClosedRuntimeFactories` 等确实涉及 License／商业插件装配的最小桥接，避免用户通过修改开源实现绕过授权。新增接口隔离 AI、缓存、搜索和平台适配器，使开源核心不反向依赖闭源实现；`Microi.V8Engine` 只负责扩展注入与兼容入口。
- **翻译引擎拆为独立开源 `Microi.Translate` NuGet**：原 `Microi.net/TranslateEngine` 的引擎、LibreTranslate 实现和部署文件迁入新类库，依赖注入、V8 `TranslateEngine` 与安装器保持兼容；官网翻译引擎文档和 `translate-engine` Skill 同步说明开源边界、配置、缓存与部署。本次全量发布已首次公开推送 `Microi.Translate.7.5.10.nupkg`，不再要求为通用翻译能力引用闭源程序集。
- **系统日志/监控补齐统一历史驾驶舱、流量明细与真实接口归因**：默认时间范围改为“今天”，并统一支持实时 5 分钟、今天、昨天、近 3／7／15／30 天、近 3／6 个月和近 1 年；5 分钟、小时、天三级 MySQL 固定桶按确定性 Key 幂等汇总，MongoDB 只保存有诊断价值的大流量、上传下载和可疑明细，Redis 使用租户隔离的短缓存，历史查询不扫描海量原始日志。新增 `HistoricalDashboard` 与 `TrafficDetails`，可按接口、ApiEngine Key、用户、匿名、IP、租户、方向、内容类型、文件元数据和时间筛选接收／发送字节；热点接口最多保留 50 个端点，上传文件在进入 V8 前即标注净化后的文件名、扩展名、数量和字节数，不记录正文、凭据或签名地址。多核原始 CPU 超过 100% 只换算为占用逻辑核数，告警以归一化 CPU 为准；“耗时占比”明确为窗口墙钟耗时贡献而非 CPU 精确归因。
- **热点接口与高流量返回继续按真实调用链降载**：模块展示统计的表格、表单、Banner 与侧栏统一进入 40ms 合并窗口和批量接口，用户级只读缓存 10 秒且保留 `ForceRefresh`；待办角标使用版本门主动失效并限制查询投影，FormEngine 批量计数并发上限从 8 收紧到 4。菜单响应不再为上千行重复输出无语义的 null／空字符串，缓存协议升至 v2；主题偏好 250ms 防抖并跳过无变化写入。后台任务默认 15、最大 100，列表只读取摘要列，日志、结果与完整参数必须按所有者单独取详情；导入进度改为直接查询指定任务，彻底避免 `/api/BackgroundTask/List` 现场 7.36MB 级返回再次出现。
- **应用商城安装包改为“数据库小指针 + HDFS 不可变 JSON”，解决 400MB／5.7GB 大字段膨胀及二进制包错写问题**：公开应用使用 `HdfsPublic`，私有应用使用 `HdfsPrivate`；发布器按 UTF-8 原文上传，HDFS 回读后同时核对 `PackageSize` 与 `PackageSha256`，再写入内容寻址索引、不可变版本快照和商城指针，并以原值 CAS 清空 `AppPakcet`。安装器从公有 FileServer/CDN 或经来源后端授权的私有对象下载后独立复验，不把 JPG、字体等二进制文件当文本重写，应用 ZIP 内仍以逐文件 Base64 保留原始字节。`microi-store-package-storage` 升至 v1.0.8，历史压缩器 v1.1.2 已在官方主库完成 20,983/20,983 个候选处理且当前正文剩余 0；任务不执行可能锁表的 `OPTIMIZE TABLE`，如需立即缩小 MySQL 物理文件仍应在备份后的维护窗口单独评估。
- **应用商城自身与平台微服务的 404／无效 HTML 根因完成官方源、目标租户和真实页面闭环**：应用商城官方包升至 v7.5.47，包正文位于公有 HDFS `/itdos/microi-store/packages/dc95ac82-2858-47f3-99cd-195e0d80a860/202608/app_microi_store-v7_5_47-70335baebfc6c91a.json`，SHA-256 为 `70335baebfc6c91a9db86efdb1932c014f0a2bf7ffd4b4f60280d4a35571e9b4`；统一导入器会完整保存数据库内置微服务 HTML/Base64 字节、运行清单和租户路由，不再把入口裁成“不完整 HTML”。利轩维保租户已通过固定版本后台任务更新到 v7.5.47，并在真实登录浏览器中确认 `/microi-store` 不再 404、系统观测页可以挂载；公共包可安装仍不等同于其它客户节点已经主动拉取并重启。
- **官方“系统日志/监控”与平台内置微服务继续作为完整源码应用交付**：`app.microi.sys-log` 升至 v7.1.5，统一携带观测表、复合索引、Managed 查询引擎和平台内置微服务 v1.9.0 的 37 个私有源码文件、4 个构建文件及 10 个运行页面；系统日志固定第一 Tab，统计卡、日志详情、15 条分页、两行省略、异常标红与解决建议、历史范围、流量排行榜和平台级弹层均包含在包内。官方源、不可变版本、HDFS 指针、源码／运行清单和目标租户重装均已回读，平台框架升级与商城应用更新仍保持两条明确交付链。
- **文件柜完成远程连接凭据安全、空白页面和真实同步闭环**：新增仅允许接口引擎调用的 `ProtectApiEngineSecret/UnprotectApiEngineSecret`，密文绑定当前 OsClient 与 ApiEngineKey，调用者不能自定义用途；旧 v1.0.2 的 AuthSecret 派生密文只允许固定 Managed 引擎一次性兼容读取，新写入全部使用隔离密文，列表不返回密码或 Token。同步接口统一走自定义地址并持久清理退出 Token，修复 Vue 根节点内无效嵌套 `<template>` 导致文件柜空白，以及 `UploadText/GetPrivateFileText` 在 Jint 动态对象边界把 `JValue` 当作带 `Val` 方法对象的问题。官方 `mci_file_remote_connection` 升至 v1.0.4，文件柜应用 v6.2.4 的公有包 SHA-256 为 `bc22ed19f6fba55b9f8566a8443353066c5511c5a441e7285c88a3612b16ee58`，官方源与利轩维保目标租户安装、保存／回读／连接／退出及真实浏览器 E2E 均通过。
- **PC、UniApp、MCP 和官方应用全面改用可识别的接口引擎自定义地址**：AI 工作流、后台任务、缓存管理、角色／菜单授权、腾讯 IM、SSO、表单选项数据源、JSON 表格、文件同步及 UniApp 业务运行时不再把请求聚合到 `/api/ApiEngine/Run`；IM Secret 留在可信后端，访问密钥先校验 `api-engine:run` 能力域，再以服务端解析出的真实 Key 做二次白名单校验。Redis 管理改由 `platform-cache-manager` 统一动作编排，AI 插件通过 `IAiWorkflowRuntime` 最小接口接入，搜索、MQ/MQTT、采集和在线终端同样由接口引擎拥有。安装器的 Docker 资源保护同时纠偏为“API + 本机主数据库”共享宿主 95% CPU／内存上限，Redis、MongoDB、Web、MinIO、OCR 等不被错误塞入同一硬限额；外部 MySQL 明确由其所在环境独立治理。
- **官网、应用更新日志、Skills、MCP 与 VS Code 插件形成同一 AI 可发现契约**：官网新增缓存引擎文档，扩展系统观测、应用商城、翻译、SSO 与 V8 服务端文档；应用详情新增按精确版本读取的更新日志时间线、类型标识、加载／失败态、亮暗主题、移动端适配和低动态模式。框架更新日志增加“平台框架 vs 商城应用”提示，并以 v4.1.0 分水岭为旧前后端独立版本线补齐标识和倒序测试。Skills 强制“能用接口引擎就不写 Controller、缺少底层原子才扩展 V8”，并固化观测存储性能、HDFS 包指针、公私有下载、平台级弹层和凭据隔离；MCP 增加流式 `ResponseType`、`HistoricalDashboard/TrafficDetails` 与自定义后台任务／Redis 路由。`npm run publish` 已把 npm `@microi.net/cli`、Visual Studio Marketplace、Open VSX 和 VSIX 同步发布为 Microi.VSCode v5.2.7。
- **源码、官方数据、目标租户、真实 UI 与公共制品完成分层验收**：官方资源契约 249/249、后端全套 2,597/2,597、MCP 139/139、平台微服务 14/14 及生产构建通过；系统观测与文件同步 Playwright 用例通过，日志接口真实返回 15 条数据，利轩维保真实浏览器确认商城和观测页。`Microi一键编译发布.sh` 按输入 `7.5.10、4、0、2、2、0、0` 完成现代前端、后端 Release（0 个错误、9 条既有 xUnit 建议）、受保护程序集、独立 18080 liveness HTTP 200、20 个 NuGet 及前后端正式／测试 Docker，耗时 8 分 2 秒且 exit 0；后端 v7.5.10 摘要为 `sha256:a34d5b10f251e5db247d3677244407318806650ba6492c13f62074d5001ca75f`，前端 Web／Client v7.5.10 摘要为 `sha256:412f280bde2e548762ddd256e3a3b46a10658fef185ac6fdc4ebdfab0c105dd4`，只生成现代版、未生成 Chrome 49 legacy。以上分别证明源码、包、官方源、目标租户和公共制品，不把它扩写为所有生产节点均已升级或真实攻击流量下已完成容量验收。

## v7.5.9 - (2026-08-24 04:29)

- **五仓提交、推送历史与当前工作树按 v7.5.8 封版边界完成逐文件去重审计**：本轮抓取根仓 `623c322e601b872b8f2ae3d12619c623182b0323`、Microi.net `a7eabdbefffa25519f06f2f270bcbd64c133454e`、Microi.AI `1ffcfb53110b313df66b16b7d3065497f8f13705`、Microi.VSCode `18d52046fe10e455101f2f7eddddc95ba5268ed5` 和 WebOS `cf8b4703b51f2ddade779db4081e6d6ee18b6e9e` 之后的全部本地／远端历史，五仓均为 0 ahead / 0 behind。根仓 `48e6d938`、Microi.net `cc1b5f49`、Microi.AI `83def125`、Microi.VSCode `b9e520f3` 是 v7.5.8 已记录内容的发布封装，本版不重复冒充新功能；真正的封版后增量为根仓 `c695ba46`，以及 Microi.net `0d77b0d8`、`91c05784`，没有 merge commit 或团队自动 merge 包装。写本节前，根仓有 53 个已跟踪差异文件（1,095 行新增／113 行删除）、7 个新增文本源码／测试／文档文件（1,791 行）和 2 张截图；Microi.net 为 4 个文件（75/4），Microi.AI 为 1 个版本文件（3/3），Microi.VSCode 为 19 个版本、插件及 Skills 文件（310/287）和 1 个 92 行新 Skill，WebOS 保持干净且不制造空提交。`dist/bin/obj`、长 JSON、Base64、哈希清单与机械生成文件只核对最终版本和语义，不重复计算为业务功能。
- **“系统日志/监控”恢复旧版完整日志能力并重构为统一七页驾驶舱**：系统日志固定为首个 Tab，重新提供日志总数、错误、警告、慢 SQL、慢执行、异常和日志类型统计，保留类型／级别／时间／关键字组合筛选、搜索、刷新、15 条默认分页与可选页大小；点击任一日志可查看标题、正文、参数、可执行 SQL、用户、IP、Api、AppId、TraceId、节点、客户端及同 Trace 时间线，弹层使用平台级挂载遮罩整个框架。标题与副标题最多两行，超出省略；错误、超阈值、可疑流量等异常状态标红，并通过提示给出可能原因和处置建议。运行诊断、请求接口、系统监控、安全数据、应用日志等旧功能全部保留在同一微服务中，颜色跟随平台主题，持续动画只使用低成本属性，并尊重页面隐藏、低性能设备和 `prefers-reduced-motion`。
- **新增可回答“谁、从哪个 IP、通过什么接口传了多少”的网络流量归因链路**：中间件以只计数、不缓存、不拥有底层流的包装器统计请求体和响应体，按接口／ApiEngine／FormEngine 表 Key／微服务 AppKey、IP、帐号或匿名、租户、内容类型聚合接收、发送、请求、错误、慢请求、上传和下载；HDFS 上传、私有文件下载及用户身份单独补充受限标记，文件只保留经过裁剪的名称与扩展名，不记录文件内容、请求体、QueryString、Authorization 或 Cookie。驾驶舱同时展示网卡累计／窗口收发、HTTP 可归因流量、未归因残差、归因率、实时速率、五类 TOP、大文件与可疑请求；明确提示 TLS、HTTP 头、TCP 重传、数据库、Redis、MongoDB、对象存储和外部 HTTP 等边界，避免把容器总流量误判成攻击或逐接口精确流量。
- **流量观测按高并发场景实行“内存热窗口 + MongoDB 有界明细 + MySQL 分层固定桶汇总”**：热路径使用并发分钟桶、固定保留时间和接口／IP／用户／租户／内容类型基数上限，普通请求不逐条写 MySQL；仅大流量、上传下载或可疑请求进入现有异步日志队列并批量落 MongoDB。后台服务以确定性 Key 幂等写入 5 分钟桶，再重建保留 45 天的小时桶和保留 400 天的天桶；控制台支持实时 5 分钟、今天、昨天、近 3／7／15／30 天、近 3／6 个月和近 1 年，历史榜单只扫描带复合索引的 MySQL 汇总并使用租户隔离的短 TTL Redis 缓存。单请求 1 MB、20 MB、100 MB 分级识别，匿名大文件上传、失败大响应等场景给出风险级别、原因与解决方案；持久化失败只写受限诊断且不阻断业务请求。
- **v7.5.8 发布后的菜单、微服务、后台任务与日志安全增量正式进入 v7.5.9 不可变制品**：菜单启动优先窄字段投影，仅在旧／冷库元数据把结果裁成 Id 时回退物理行，并新增 30 秒租户缓存；同一接口引擎同时服务模块指标和菜单角标时合并为一次 Jint 执行，前端对角标批量请求、稳定缓存和局部失败完成回归约束。微服务 Resolve 只返回运行所需的轻量元数据并细分 AppKey 观测标签；后台任务通知优先复用刚写入 Redis 的有界投影。V8 新增当前用户工作流统计原子方法，系统日志详情扩展参数、环境、浏览器和请求方法等受限字段，并递归脱敏 JSON 键及普通文本中的 Token、密码、Cookie、密钥和连接串；此前已提交但晚于 v7.5.8 NuGet／Docker 的源码不再停留在“下版待发布”状态。
- **热点前六名按真实调用链完成后续源码降载，后台任务列表响应从兆字节级收口为摘要分页**：模块展示统计在表格、表单、侧栏和 Banner 之间统一走批量去重；商城列表把 5000 条应用和 5000 条安装记录的 Jint 物化改为数据库条件、窄字段、真实分页、当前页安装状态和 5 分钟分面缓存；待办角标只投影流程 Id 与抄送人，消息中心首屏从 100 条降为 15 条；个人偏好无变化时不写库、不刷新登录态并在前端短防抖。`/api/BackgroundTask/List` 默认 15、最大 100，只返回摘要并提供所有者范围的详情／状态端点，实测 15 条响应为 11,714 bytes，相对现场 7.36 MB 缩小约 99.8%。进程 CPU 告警以逻辑核归一化值为主，原始 `247.7%` 解释为约 2.477 个逻辑核；热点“耗时占比”继续明确为请求墙钟耗时贡献而非 CPU 精确归因。
- **官方文档、Skills 与 MCP 建成同一套 AI 可发现、可查询、可治理的观测契约**：在【系统引擎】下新增独立《系统日志/监控》页面和真实页面截图，说明七个功能页、统计口径、CPU／热点接口、网络流量、存储性能、权限隐私、安装升级和验收边界；新增 `system-observability` 专用 Skill，并同步微服务、调试、性能、SSO、UI 及能力覆盖索引。MCP 新增 `microi_query_system_observability` 和 `microi_manage_system_observability`：前者通过 `Capabilities` 目录统一查询 `Snapshot/Logs/LogTypes/LogStats/Signal/Trace/ApiRank/AppLogs/PlatformStats/SecurityData/TrafficHistory`，后者仅允许平台管理员执行 `BlockIp/UnblockIp`，继续受租户隔离、参数上限、脱敏和审计保护；MCP 全套 139/139、平台微服务 14/14 测试及生产构建通过。
- **官方商城“系统日志/监控”独立升级为可完整迁移的 v7.1.0 应用**：官方源 `app.microi.sys-log` 已发布不可变版本 `01M0RMA1Y6MY0RXQGP39KJK7ZK`，内含 10 张表定义、204 个字段、15 条 DDL（含 5 条流量历史复合索引）、431 个物理列、2 个 `Platform/Managed` 接口引擎，以及平台内置微服务 v1.8.8 的 37 个私有源码文件、4 个公有构建文件和 9 条路由；查询引擎升至 v1.0.7 并交付统一历史驾驶舱。应用包不携带历史 `upload/` 路径，源码包与运行包清单、SHA-256、版本日志和可安装状态均从官方 `microi_itdos` 回读确认。该应用仍须在【应用商城】单独安装／更新，升级平台框架不会替用户自动改写已安装应用。
- **Microi.VSCode、CLI、Codex、CodeBuddy 与 WorkBuddy 插件发布 v5.2.6**：首次 `npm run publish` 在版本从 5.2.5 递增后，被新 Skill 缺少“Codex 非阻塞自动更新”标准块的发布门禁正确拦截，补齐规范并通过构建、类型检查和测试后使用 `--no-bump` 继续同一 5.2.6，未错误递增版本；npm `@microi.net/cli`、Visual Studio Marketplace 和 Open VSX 均发布成功并精确回读 5.2.6。插件同步新的系统观测 Skill、MCP 方法、能力目录和多宿主清单，生成后的 MCP Bundle 与元数据不重复列为独立业务功能。
- **v7.5.9 现代版公共制品完成前后端全量发布**：`Microi一键编译发布.sh` 按输入 `7.5.9、4、0、2、2、0、0` 执行，现代前端生产构建与串行压缩、后端 Release（0 个错误、9 条既有 xUnit 分析器建议）、发布目录守卫、Microi.net／Microi.AI 受保护程序集替换、独立 18080 liveness HTTP 200、19 个 NuGet 推送以及前后端正式／测试 Docker 全部成功，耗时 8 分 22 秒；后端正式／测试标签统一为 `sha256:feeb2f2306ac9281f93f77c63be75a49f4b77a5b2853cc709d56a37100a39bd1b`，前端 Web／Client 正式／测试标签统一为 `sha256:7cb7b292a13495d2b3726c4044ee7b84b27c2505f00c2ff2cc4a697b067073afd`，且只生成现代版、不生成 Chrome 49 legacy。公共制品可取不等于任一客户生产容器已经拉取重启，也不代替客户数据规模、代理链路或真实攻击流量下的容量验收。

## v7.5.8 - (2026-08-23 23:19)

- **五仓提交历史与当前工作树按 v7.5.7 最终封版点完成逐文件去重审计**：本轮重新抓取根仓 `623c322e601b872b8f2ae3d12619c623182b0323`、Microi.net `a7eabdbefffa25519f06f2f270bcbd64c133454e`、Microi.AI `1ffcfb53110b313df66b16b7d3065497f8f13705`、Microi.VSCode `18d52046fe10e455101f2f7eddddc95ba5268ed5` 和 WebOS `cf8b4703b51f2ddade779db4081e6d6ee18b6e9e` 之后的本地、远端提交与推送历史；五仓边界后均无遗漏提交，ahead/behind 均为 0/0，也没有需要重复归纳的团队自动 merge。写本节前，根仓排除 `dist/bin/obj` 后有 95 个已跟踪差异文件（7,180 行新增／3,341 行删除）和 14 个新增源码／测试文件（1,792 行）；Microi.net 为 3 个已跟踪文件（15/3）及 1 个新增源码文件（342 行），Microi.AI 为 1 个版本文件（3/3），Microi.VSCode 为 15 个版本、插件与 Skills 文件（249/214），WebOS 保持干净且不制造空提交。应用长 JSON、Base64、哈希清单和编译产物只核对最终版本与语义，不把机械生成物重复包装成业务功能。
- **系统监控与系统日志收口为统一、可由低代码编排的“系统可观测性”能力**：新增当前节点低开销请求指标、吞吐与错误率、慢请求和延迟分布、活动／最近请求、接口与 IP 排名、进程运行状态、诊断建议、信号、调用链及应用日志查询；请求上下文可标记 TraceId、ApiEngineKey 和 OsClient，但不记录请求体、查询参数、Authorization 或 Cookie。平台管理员可通过受权 V8 原子方法查询与封禁／解封 IP，所有输入均有数量边界、敏感字段脱敏和审计；旧系统监控控制器、独立监控页及日志查询／队列／Docker 日志入口移除，`SysLogController` 仅保留旧客户端新增日志兼容，AI 平台统计改走 `mci-system-observability-query`。该实现只代表当前节点视角，多节点精确聚合仍需逐节点或外部采集，也不把运行时指标误称为逐请求精确 CPU 计量。
- **菜单启动投影兼容旧库与冷库字段元数据，避免登录后菜单被裁成仅剩 Id**：后端先在受信上下文读取物理菜单、完成身份与角色过滤，再在内存中应用浏览器传入的 `_SelectFields`，并强制保留 `Id/ParentId/Sort` 等树形关键字段；当旧数据库的 `diy_field` 元数据不完整时不再让字段投影压垮菜单结构，读取异常也返回真实失败而不是伪装为空列表。模块引擎文档、Skills 与回归契约同步补齐。
- **Excel／CSV 智能导入升级到 v2.2，并由服务端统一裁决 Upsert 与错误策略**：导入工作台支持 `.xls/.xlsx/.csv`、工作表和多级表头自动识别、数据范围校正、全部源列／源行预览、未匹配列提示、字段映射及原始工作簿页签；仅用于预览的副本会规范化 OpenPyXL 锚定图片 DrawingML 命名空间，上传原文件不被改写。CSV 在浏览器与后端识别 BOM、UTF-8、UTF-16、GBK 及逗号／制表符／分号／竖线分隔符。新增默认整批事务回滚 `RollbackAll` 与逐行跳过 `ContinueOnError`：每个 `Alone` 字段独立构成唯一规则，全部 `All` 字段共同构成一个复合规则；前端快照只用于展示，后端按当前 `diy_field` 权威重算，唯一命中则更新、未命中则新增、命中不同记录或重复数据则报行错误，并返回新增／更新／失败统计。Microi.net 同步公开文件类型、文件名、编码和分隔符参数，表单引擎官方包由 v7.6.1 升至 v7.6.3。
- **AI 助手显隐在 PC 与 UniApp 统一为单一负向开关**：只有 `DisableAiAssistant=1/true` 会隐藏助手；字段缺失、`null`、`0/false` 或异常旧值均按兼容默认显示，废弃的 `IsShowAiAssistant` 不再参与判断。UniApp 的页面、启动器、TabBar 和消息入口初始状态同步改为可见，配置请求失败也不误隐藏；运行端解析不再被 profile 的 `features.ai` 二次截断，并新增通用特性开关函数及 PC／UniApp／E2E 回归契约。
- **子租户平台应用任务、商城安装器和官方包继续加强失败隔离与旧环境自愈**：子租户目录发现保持只读，目标队列通过 `ReloadOsClient` 刷新运行态，单个失效目标不会终止整批，真实共享数据库仍使用固定工作器串行，Monitor 汇总队列失败与持久化终态。`bulk-update-child-tenant-platform-apps` 升至 v1.1.7；SaaS 引擎升至 v7.5.31，补齐 `diy_LeftJoinRightView` 结构；应用商城升至 v7.5.33。导入商城包引擎升至 v2.3.6，补齐旧微服务菜单 Key 推断、对象存储 MoveObject 不可用时的已验证路径复用、菜单 URL 冲突的有界稳定回退，以及旧元数据环境下 Managed 接口布尔标志的物理核对、缓存刷新和回读；AI 应用发布器升至 v1.8.7，从已提交 Bundle 补齐微服务菜单绑定并拒绝跨应用或非法路由。
- **一键安装器改为“系统保底 + Microi 服务共享”的单一资源池**：不再给 OCR、数据库等容器静态切割固定 CPU／内存，而是通过父级 `microi.slice` 为系统、SSH 和宝塔预留至少 5% CPU 及 `max(5% 内存, 1536MB)`，其余容量由 Microi 全部服务按需共享；兼容 systemd、cgroup v1/v2 及 Docker 的 systemd/cgroupfs parent，设置硬 CPU／内存上限和可用时的交换区限制。rootless、驱动或内核能力不满足时失败关闭，不覆盖非 Microi systemd 单元；启动前校验 Compose，启动后逐容器回读父级与限额，并提供不接触 Docker、网络或系统文件的测试计算／渲染模式。低于 4 核 16GB 的 OCR 环境继续给出资源风险提示。
- **字段唯一规则、右侧设计面板、文档与 Skills 完成同源收口**：唯一规则新增只读解析和构建方法，不修改字段 `Config`，保证多个 `Alone` 各自独立、多个 `All` 只组成一组复合规则；设计面板将“数据日志／数据评论／数据版本”压缩为“日志／评论／版本”，同步收紧 Tab、间距和角标。Docker、模块引擎、V8 前后端与 Office 导入文档，以及 app-store、module-engine、v8-export-import 等 Skills 均原位更新；只维护中文文档单一事实源，语言包同步更新但不手工复制一套英文文档。
- **Microi.VSCode、CLI、Codex、CodeBuddy 与 WorkBuddy 插件发布 v5.2.5**：`npm run publish` 完成版本递增、生产构建、类型检查、自动测试、VSIX 和多宿主清单／Skills 同源组装，并成功发布 npm `@microi.net/cli`、Visual Studio Marketplace 与 Open VSX；Open VSX 首次即时回读处于正常传播窗口，传播完成后重新执行 `npm run publish:verify`，三个公共端均精确回读 5.2.5。插件同步本版智能导入、系统可观测性、商城兼容和平台交付规范，生成后的 Bundle、哈希与构建元数据不重复列为独立功能。
- **v7.5.8 现代版公共制品完成全量发布与独立回读**：`Microi一键编译发布.sh` 按输入 `7.5.8、4、0、2、2、0、0` 执行，现代前端生产构建、串行压缩、后端 Release（0 个错误、9 条既有 xUnit 分析器建议）、发布目录守卫、受保护程序集、独立 18080 liveness HTTP 200、19 个 NuGet 包和前后端正式／测试 Docker 全部成功，最终轮耗时 9 分 14 秒；NuGet.org Flat Container 已逐包回读 19/19。后端 `v7.5.8/latest/dev-latest` 三个标签统一为 `sha256:c481be220dddf7ee3eae9dd85c83ebd73ec51d2998467c008e01a2eee29a1e88`，前端 Web／Client 四个正式／测试标签统一为 `sha256:05055ae20e120bc41a1386326d30f76f799920c907cc119e3a3ee713090edbff`；本轮明确只生成现代版，没有 Chrome 49 legacy。源码、构建与公共制品发布均已闭环，但不把它扩写为任一客户生产节点已拉取重启、登录后业务页面已完成视觉验收或原生设备已验收。
- **封版后的并发源码增量完成独立审计，但未改写 v7.5.8 不可变制品**：首次五仓提交推送完成后，共享工作区又写入菜单、微服务、后台任务、模块展示和观测日志的补充优化。菜单启动改为优先使用窄字段投影，只有旧／冷库元数据把结果裁成 Id 时才回退物理行，并以 30 秒租户缓存减少旧微服务别名重复扫描；微服务宿主可在一次 Resolve 中尽力携带页面源码元数据，旧页面结构失败不阻断已解析运行入口；同一个接口引擎同时服务模块指标和按钮角标时合并为一次 Jint 执行；后台任务通知优先复用刚写入 Redis 的有界投影，显式列表查询仍负责权威核对。V8 新增当前用户工作流统计原子方法，并为观测日志详情补齐参数、环境、浏览器、请求方法等受限字段，对 JSON 键和普通文本中的凭据统一递归脱敏。相关后端聚焦回归 21/21、微服务宿主 31/31、模块展示 14/14 通过。这些文件的写入时间晚于本版 NuGet／Docker 发布，故只作为已提交的后续源码增量保留，不覆盖同版本公共包；下一个框架版本发布后才会进入新的不可变制品。

## v7.5.7 - (2026-08-23 13:07)

- **五仓提交历史与当前工作树按 v7.5.6 封版点完成逐文件去重审计**：本轮以根仓 `ba46590d9e44bb395b63d5bd0a517488df133efa`、Microi.net `db22bb2eb8605f7b61deb9c6da4627a4c48d8dd6`、Microi.AI `16cd5db5486470ad1484ca664f20d0a8db51ece0`、Microi.VSCode `350d03754f172ef5be2e741dad2c0fdf3238eb3f` 和 WebOS `cf8b4703b51f2ddade779db4081e6d6ee18b6e9e` 为边界执行远端抓取。边界后只有根仓线性提交 `52fd1f9f80db114723b5bec336b80388937ac0ff`，英文说明 `feat(system-settings): unify security dialogs` 统一译为“功能（系统设置）：统一安全设置弹层”；该提交及仍在工作树中的系统设置公私边界、开发配置分组和代码弹层已由 v7.5.6 首条日志完整记录，本版只核对最终源码，不再冒充新功能重复归纳。没有 merge commit、团队自动 merge 包装或其它本地／远端漏记提交。写本节前，根仓排除 `dist/bin/obj` 后有 92 个已跟踪差异文件（4,721 行新增／1,485 行删除）和 11 个新增源码／测试文件（2,044 行）；Microi.net 为 3 个 Office／版本文件（18/3），Microi.AI 为 1 个版本文件（3/3），Microi.VSCode 为 22 个版本、插件与 Skills 文件（322/289），WebOS 保持干净且不制造空提交。应用长 JSON、Base64、哈希清单和插件生成脚本只核对最终版本与语义，不逐份重复计算为业务更新。
- **Excel 导入从“固定首表首行”升级为可确认的智能解析工作台**：所有通用导入和接口引擎自定义导入先解析原始工作簿，自动跳过说明页，识别最佳工作表、单行／合并多级表头、首末数据行和列置信度；80% 宽弹层同时提供工作表、表头范围、数据范围、逐列字段映射、异常提示及严格每页 15 条预览，低可信文件也可人工校正后继续。确认时前端上传原文件及零基列坐标／一基行号元数据，后端 NPOI 按同一范围重新解析公式值、合并表头和映射列，拒绝同一目标字段被重复映射；未传新参数时继续保持“第一个工作表、首行表头、第二行开始”的旧契约。Microi.net 的 `V8EngineOffice` 同步公开完整参数，表单引擎官方包升至 v7.6.1。
- **应用商城新增与应用版本一一对应的正式更新日志能力**：新增租户隔离的 `sys_microistore_changelog` 一对多子表，以 `(OsClient, StoreId, Version)` 唯一约束防止同版本重复，并为版本、类型、发布时间和排序建立真实字段与索引；商城表单通过隐藏子模块维护版本、标题、变更类型、内容、发布时间和顺序。制作或发布应用前必须存在与当前 `AppVersion` 精确匹配的完整日志，日志随不可变包元数据一并发布；商城详情按时间线展示，旧来源不支持时与确实无日志时使用不同诚实空态。查询引擎继续从指定版本快照读取应用，新增 `DataAppend.ChangeLogs/ChangeLogCount/ChangeLogAvailable`，表缺失时只降级能力而不把读取失败伪装为空。应用商城官方包最终升至 v7.5.31，发布器 `ai_app_publish_store` 升至 v1.8.6，平台内置微服务升至 v1.7.7。
- **AI 对话与平台通知统一投影到唯一“AI助手”会话**：前端不再创建只读的“平台／管理员”伪联系人，AI 模型回复、WebSocket 系统消息和持久化平台通知统一使用固定 `AI` 身份与机器人头像，按真实时间合并并用通知稳定标识去重；通知未读数仍以服务端权威快照为准，链接继续拒绝脚本协议。联系人显示名固定按“姓名 → 账号”回退，绝不把内部用户 Id 暴露为昵称；后端将 AI 身份收口为共享 `ChatAssistantIdentity`，历史查询、SignalR 推送和异常消息不再依赖数据库中必须存在 admin 用户，同时保留用户继续向 AI 助手发送消息的正常入口。
- **租户批量平台应用任务修复同租户自助更新被旧工作器抢先接管的问题**：固定工作器 Key 不再被单独当作跨租户证据；只有任务所有者与执行租户不同、服务端持久化目标标记非空且与执行租户完全一致时，才在执行前从主租户自愈目标工作器。普通租户为自己执行“全部安装／更新”时明确跳过跨租户引导，让批量计划先更新应用商城，再运行随包交付的新工作器；真正的主租户到子租户任务仍保留重启恢复、检查点接管和受信工作器补齐，错误提示也区分资源缺失与作用域不匹配。
- **低代码系统生成器获得受限的租户索引字段可信通道**：`OsClient` 仍是普通请求和可编辑 V8 禁止创建的系统保留名，仅允许可信服务端调用以 `varchar(50) + Text + PC/移动端隐藏 + 只读 + 必填` 的完整固定形态创建，用于生成租户隔离子表及其首列索引；来源、名称、类型、控件或任一显隐／只读／必填条件不符都会继续拒绝，避免业务字段伪装租户上下文。新增正反向测试锁定完整可信形态，并保留既有 `CreateTable` 强类型来源传递契约。
- **系统设置、代码编辑器与官方离线资源完成 v7.5.6 已记录能力的源码封版**：Passkey、Authenticator、外部登录、严格人脸及改密二次验证等公开行为继续以 `sys_config` 九个实体字段为主，旧私有 Key 只在字段缺失时兼容回退，并从私有设置列表、保存和删除入口过滤；CodeEditor 的字段级 `DisplayMode=Dialog` 只在点击时加载 Monaco，按钮始终显示 Unicode 字符数，开发配置按四个默认展开分组交付。SaaS 引擎最终为 v7.5.30，重新内嵌同一 `microi-platform-service` v1.7.7 DatabaseOnly 运行时；系统设置、身份验证、V8 前后端、模块引擎和消息通知文档及 Skills 同步更新。更新日志页新增框架升级与商城应用升级的醒目范围说明，明确两条升级链互不替代，并适配主题色、深色和移动端。
- **Microi.VSCode、CLI、Codex 与多宿主插件发布 v5.2.4**：`npm run publish` 自动完成版本递增、生产构建、CLI 类型检查和测试、更新器与并发 Broker 回归、MCP／Skills 同源组装、VSIX 内容验证，并成功发布 npm `@microi.net/cli`、Visual Studio Marketplace 和 Open VSX；传播完成后再次执行 `npm run publish:verify`，三个公共端均精确回读 5.2.4。插件副本同步应用商城版本日志、智能 Excel 导入、消息会话、SaaS 公私边界和安全规则；bundle、元数据及生成后的 MCP 脚本不重复列为独立功能。
- **源码与官方资源完成分层回归，浏览器凭据边界保持诚实**：智能导入、聊天、通知、CodeEditor 和框架来源静态／逻辑测试 28/28，商城日志、系统设置、展示资源和历史兼容契约 37/37，Excel 服务端复核、可信 `OsClient`、租户任务、聊天身份和安全设置 .NET 聚焦测试 85/85 全部通过；平台内置微服务 13/13 且生产构建成功。新增真实登录 Playwright 用例用于检查微服务来源说明已改为租户中性的“平台”文案，但本轮未提供 `PW_LOCAL_PASSWORD`，因此不把该真实页面视觉验收写成已通过；主发布完成的现代前端生产构建与后端编译属于制品门禁，不替代登录后业务页面验收。
- **v7.5.7 现代版公共制品完成全量发布与独立回读**：`Microi一键编译发布.sh` 按输入 `7.5.7、4、0、2、2、0、0` 执行；首次预检发现唯一正式平台微服务源码根存在未提交改动，先完成 13/13 测试、生产构建和该目录独立提交，再复用同一个未提交版本重跑，没有错误递增为新版本。最终现代前端、后端 Release（0 个错误、9 条既有分析器建议）、发布目录守卫、受保护程序集、独立 18080 liveness HTTP 200、19 个 NuGet 和前后端正式／测试 Docker 全部成功，最终轮耗时 8 分 45 秒；NuGet.org Flat Container 已逐包回读 19/19。后端 `v7.5.7/latest/dev-latest` 三个标签统一为 `sha256:c0b84fb593e4dc4ff1c16ad013c5f241d102ca97ab6d9894199ac1873ccbe334`，前端 Web／Client 的四个正式／测试标签统一为 `sha256:5e7e7d8a3c3bff1b5d8117539000042ad3bf599d7f2fac6b745cb1952d576dc2`；本轮明确只生成现代版，没有 Chrome 49 legacy。上述证据证明公共包与镜像可取，不代表客户或生产节点已经拉取 v7.5.7、重启实例或完成真实业务页面验收。

## v7.5.6 - (2026-08-23 06:26)

- **系统设置公开开关与后端私密接入重新划清单一事实源**：Passkey、Authenticator、外部登录、严格人脸和改密二次验证等“是否启用”能力迁入 `sys_config` 实体字段，“安全与服务接入”只保留 ClientSecret、ApiKey、RP/Origin/Issuer、供应商地址等后端参数；旧 `mci_system_setting` 开关仅在新字段缺失时兼容回退，并从私密设置列表、保存和删除入口移除。开发配置按访问运行、V8 治理、全局脚本、模板代码四个默认展开的折叠组归类，九个 CodeEditor 默认显示“编辑代码（N字）”按钮并使用统一大圆角弹层编辑；官方 SaaS、独立系统设置和平台服务应用同步升级后，存量租户可直接安装新版字段与布局，不会覆盖既有明确选择。
- **五仓提交历史与当前工作树按 v7.5.4 最终封版点完成去重审计**：本轮重新抓取根仓 `3eb06dbc98ca25e73e59923060345ad0ec1b4e16`、Microi.net `ed366a8acc006a80e44a4c6def1fda201f77fc6b`、Microi.AI `ec2d4397b0567ea9ba39c65460525158886c0c15`、Microi.VSCode `7db0b30b42e5fc08ea1bb330334043581829627e` 和 WebOS `cf8b4703b51f2ddade779db4081e6d6ee18b6e9e` 之后的本地、远端提交及推送历史，五仓边界后均无漏记提交，ahead/behind 均为 0/0，也没有需要重复归纳的团队自动 merge。写本节前，根仓排除 `microi.mcp/dist` 后有 137 个已跟踪差异文件（7,263 行新增／2,057 行删除）和 31 个新增源码／测试文件（3,057 行）；Microi.net、Microi.AI 各为 1 个版本文件（3/3），Microi.VSCode 为 24 个版本、插件、Skills 与并发修复文件（464/218），WebOS 保持干净。应用长 JSON、Base64、哈希元数据和 `dist/bin/obj` 只核对最终版本、内容哈希与语义，不把机械生成物逐份冒充独立功能，也不为无差异仓库制造空提交。
- **图片上传形成“默认压缩、原图私存、显示图轻量化”的统一后端契约**：PC、UniApp/H5/小程序即使未显式传压缩参数，也会按约 500KB、最长边 1920px 的安全默认值处理图片；调用方仍可提交受限参数调整质量、体积和尺寸。后端使用跨平台 ImageSharp 识别并压缩可处理格式，异常、超限或原图持久化失败时失败关闭；无论展示图进入公有桶还是私有桶，上传请求中的未裁剪／未压缩原图都会以 `MicroiOriginalFile` 证据先写入 HDFS 私有桶，避免为速度牺牲可追溯原件。
- **表单图片裁剪升级为完整且可选择的专业交互**：图片控件支持自由比例、常用固定比例、管理员预设比例与用户自定义比例，并提供缩放、旋转、水平／垂直翻转、重置、批量多图逐张处理和“应用裁剪并上传”；左侧新增“不裁剪直接上传”，表单使用者还能在控件内临时开关本次裁剪，默认值才由设计器的“是否默认开启裁剪”决定。修复多图裁剪后响应数组没有正确回填预览的问题；裁剪弹层、上传弹层及其它统一弹层遵循系统遮罩毛玻璃开关，旧租户默认不开启。
- **图片与文件上传合并为紧凑、可解释的拖拽面板**：原“上传前裁剪”提示和拖拽框合并为单一紧凑组件，在不放大表单高度的前提下展示公有／私有桶、单图／多图、最大数量、是否压缩及最大体积；图片与普通文件上传采用同一信息层级、主题色、图标和响应式排版。设计器补齐裁剪模式、比例、压缩、体积与桶策略配置，官方文档和 Skills 同步给出 AI 可直接生成的字段参数，旧字段未配置时继续使用安全兼容默认值。
- **富文本编辑器补齐图片、视频和文件的完整存储策略**：字段配置可分别指定附件公有桶／私有桶、类型、数量、体积与图片压缩参数，适配外部公开公告和内部私密文章。私有附件不再把短期临时 Token 固化进 HTML，而是持久化稳定的私有资源标记；再次打开文章时，后端依据当前记录、菜单、表、字段和行权限签发短时可访问地址并在展示层替换，Token 过期后仍可重新预览，复制原始 HTML 也不会泄漏长期凭据。
- **地图与区域控件统一支持高德、百度、腾讯三种租户级地图供应商**：Map、MapArea、搜索建议、地理编码、逆地理编码、点选和区域编辑使用同一运行时适配层；当前租户只启用一个供应商，Key、Secret、服务 Host 等凭据由 SaaS 私有系统设置保存，前端通过 `no-store` 的受控投影端点取得当前供应商所需的最小配置，不暴露其它供应商秘密。缺配置、SDK 加载或增强接口失败时提供明确原因、重试和旧配置兼容回退，不阻断已有坐标数据查看。
- **标准 UniApp 增加受控运行环境切换并隔离每个端点的身份状态**：仅标准 `APP-PLUS` 包允许用户切换 HTTPS/HTTP、服务器 Host 与 OsClient，切换前执行匿名探测；成功后清理旧端点 Token、用户缓存、RSA、SignalR 和业务缓存，再按新坐标登录。凭据与安全材料按“协议 + Host + OsClient”隔离，异步响应带端点代际防护，旧请求不能覆盖新环境；标准 Android/iOS 清单按需开放 ATS/cleartext，XJY 等定制 profile 不被意外开放该入口。
- **框架个人状态、主题与设计入口继续收敛为单一真实状态**：主题色和亮暗模式继续按“当前用户显式值 → 租户系统设置 → 安全默认”解析，右上角选择后立即应用并可靠自动保存，失败会恢复真实服务端状态而不是只留下本机假象。Tab 右键设计上下文同时解析路由、菜单和 `DiyTableId`，绑定表单的系统模块现可直接出现“表单设计”，并继续保留“模块设计”；设计器删除布局字段后同步清理运行态分组和选中状态，避免残留空区域。统一弹层遮罩、表单模糊策略、国际化文本和表格列设置回归也一并补齐。
- **应用商城安装改为不可变版本快照驱动，后台批量维护具备真实恢复语义**：安装／升级任务在创建时固定 `StoreVersionId`、版本、资源清单与内容摘要，执行时再次比对快照，缺失、被替换或不完整即失败关闭，禁止“排队时看见 A、执行时安装 B”。父任务进入 Monitor 后只汇总持久化子任务，不再重复扫描租户或重复自举 Worker；平台基础包优先，分片大小有界，数据库争用与临时 Redis 发布失败采用有限重试、冷却与检查点恢复，进度只单调前进，取消、续跑及无操作重跑保持可解释和幂等。
- **官方应用资源与离线升级包完成同源发布和执行上限规范化**：表单引擎最终为 v7.6.0、模块引擎 v7.5.5、SaaS 引擎 v7.5.26、SSO v7.5.5、应用商城 v7.5.24，SaaS／商城继续内嵌同一个 `microi-platform-service` v1.7.3。资源同步器在发布候选中把历史接口引擎 `LimitRecursion` 大于运行时硬上限 5000 的值规范化为 5000，保留 0 和更小的显式配置，并通过三方差异真实写回官网而不是同时改写基线掩盖漂移；本地、官网及商城内嵌副本均已按 SHA-256 回读一致。
- **MCP、中文官网文档与 Skills 覆盖本轮全部新增配置**：`microi.mcp` 补齐固定商城版本快照、后台任务状态和安全确认契约；表单组件、HDFS、系统设置、多端客户端及 MCP 文档原位补充图片裁剪、默认压缩、原图私存、富文本私有附件、地图供应商和运行端点切换。`microi-form-engine`、`v8-file-upload`、`microi-uniapp-frontend`、`microi-frontend-sdk`、`microi-mobile-app-quality`、`app-store` 与工作区规范同步更新，让 AI 能直接生成正确配置且不会把短时 Token、私密 Key 或定制 profile 当作公共默认值；`microi.mcp/dist` 仅作为 TypeScript 编译产物提交，不重复计入功能。
- **Microi.VSCode、CLI、Codex 与多宿主插件发布 v5.2.3**：`npm run publish` 完成生产构建、类型检查、CLI／插件／Skills 同源测试和 VSIX 打包，并成功发布 npm `@microi.net/cli`、Visual Studio Marketplace 与 Open VSX；随后 `npm run publish:verify` 从三个公共端统一回读 5.2.3。并发 Broker 的租约文件改为先完整落盘、再原子链接发布，避免竞争进程读到空 JSON；生命周期测试给出可定位路由与载荷的错误信息，并以 20 轮压力循环证明创建、续租、接管和清理稳定。
- **源码、资源、移动端和发布链路完成分层门禁**：前端定向回归 55/55、后端上传／富文本／SaaS／后台任务聚焦测试 167/167、官方资源契约 216/216、MCP 全套 138/138 全部通过；UniApp 两个 profile、默认图片压缩和运行端点测试分别通过 2/2、1/1、8/8。共享进程管理器增加“临时测试后端必须同时满足可执行文件身份和当前工作区精确 CWD”的安全识别，既能释放本工作区 Release 锁，又继续拒绝结束外部 dotnet/node/浏览器进程。
- **v7.5.6 现代版全量发布完成不可变制品纠偏**：首次 v7.5.5 完整发布已成功，但封版门禁发现其离线官方应用包仍保留历史 `LimitRecursion=10000` 字节；运行时安装器本就会钳制为 5000，因此不存在已安装环境越界，不过不可变包本身不满足最终静态契约。没有覆盖同版本，而是发布修正后的官方包并按规则升至 v7.5.6，再以输入 `7.5.6、4、0、2、2、0、0` 重跑完整流程。最终现代前端、后端 Release（0 个错误、9 条既有 xUnit 建议）、发布目录守卫、独立 liveness、DLL 加密、19 个 NuGet 和正式／测试 Docker 均成功，耗时 10 分 12 秒；后端 3 个公开标签摘要统一为 `sha256:fce844022be8699bba30c2647e7b22063c4450fd0d7923408a21d049a3aa51e2`，前端 4 个公开标签统一为 `sha256:39ed8c9a8a4e2d926b33875054664fc1d7f905e8ad96b260df38f4311365a2b1`。本轮只生成现代版，没有 Chrome 49 legacy；v7.5.5 是已被 v7.5.6 纠正的中间不可变制品。

## v7.5.4 - (2026-08-22 18:14)

- **五仓提交历史与当前工作树按 v7.5.2 最终封版点完成去重审计**：本轮以根仓 `58441044c0f6586935defe71dcf51edadd0082cd`、Microi.net `11a62a411006482f31f04581ba1abed53ac34e23`、Microi.AI `1bb5734b30814a6bc0fa01857612738080505fab`、Microi.VSCode `f572c4097722572b2335b289881ae29c27bb92b7` 和 WebOS `cf8b4703b51f2ddade779db4081e6d6ee18b6e9e` 为边界重新抓取远端；边界后没有漏记的新提交或英文说明。v7.5.2 已记录的根仓功能提交、自动 merge 包装和四个封版提交不再重复归入本版。写本节前，根仓有 76 个已跟踪差异文件（3,421 行新增／624 行删除）和 16 个新增源码／测试文件（2,684 行），Microi.net、Microi.AI 各为 1 个版本文件（3/3），Microi.VSCode 为 16 个有效版本、插件与 Skills 差异文件（248/210），WebOS 保持干净；官方应用长 JSON、内嵌微服务 Base64、插件元数据与 `dist/bin/obj` 只核对版本、哈希和最终语义，不把机械产物逐份冒充独立功能，也不为无变更仓库制造空提交。
- **微服务与定制组件来源标识改为框架常驻能力**：删除租户级 `RenderSourceBadgeMode`／“渲染来源标识”字段、自动收起和全局关闭逻辑。`V8.OpenAppDialog`、Drawer 等弹层在标题后始终显示来源标签且不提供关闭按钮；菜单整页微服务由真实 `micro-app/host.vue` 在内容区右上角渲染，默认常显，只允许用户关闭当前页面实例，切换实例后重新出现。表单、表格与 `DevComponent` 同步识别本地定制组件和 `LegacyComponentPaths` 微服务兜底的真实来源，避免把远端兜底误标为本地组件。
- **来源标签成为可解释、可直接指导 AI 修改的交互入口**：标签使用按钮语义、键盘焦点和 `cursor:pointer`；点击后打开可拖动、亮暗主题适配的吾码大圆角弹层，按卡片展示 AppKey、PageKey、应用内路由、框架路由、运行入口、版本、发布/资产/挂载状态、租户坐标与可用源码路径，并可一键复制带 MCP 坐标的 AI 修改指令。后端只从页面 `RouteMetaJson` 安全投影规范化 `SourceFile`，拒绝 `..`、超长和异常路径；详情不返回原始路由元数据、DiyToken、访问密钥或租户私密配置，旧数据库缺少该字段时仅降级不显示源码路径，不影响微服务挂载。
- **全屏框架水印按最终默认值和空值语义完成**：水印由根框架固定覆盖 `100vw × 100vh`，`pointer-events:none`，不遮挡主界面、弹窗、抽屉或微服务交互，并适配浅色、深色和主题色。开启后内容空值默认 `$SysTitle$ - $UserName$`，`UserName` 为空时回退 `Account`，任何变量缺失都输出空串而不是 `undefined/null`；方向默认斜向上、密度默认舒适、字号空值/0 默认 14，透明度空值/0 最终默认 30，显式有效值继续保留，旧租户仍默认关闭水印。
- **系统设置与系统账号完成“一级 Tab + 二级折叠分组”的正式应用交付**：官方 `microi_itdos` 主租户先补齐物理字段、元数据和布局，再发布“系统设置” `app.microi.sys-config` v6.3.5 与“系统账号” `app.microi.sys_user` v6.2.8。系统设置“界面风格”使用 8 个带图标、说明和字段计数的 `CollapseGroup` 归类品牌、主题、菜单、框架水印、登录页、表单、表格与页面标签；系统账号使用 8 个折叠分组归类基本资料、账号安全、组织权限、外部身份、AI 额度、语言首页、主题菜单和桌面外观。Junchi 的系统设置更新任务 `56e47059f6e24234ac66de770d954bea` 已到 `Succeeded`，真实表单回读确认旧“渲染来源标识”不再出现。
- **主题色、浅色/深色与菜单展开方式升级为跨设备个人偏好**：`sys_user` 新增 `ThemeColor`、`ThemeMode`、`MenuChildExpandMode`，优先级固定为“当前用户显式值 → 租户系统设置 → 安全默认”，个人菜单值 `System` 表示继续继承租户。右上角主题面板乐观应用并短防抖调用官方 Managed 接口引擎 `platform-user-update-preferences`；引擎只能根据 `V8.CurrentUser.Id` 与 `V8.OsClient` 更新当前账号的固定白名单，保存后调用 `RefreshLoginUser`，参数不能改写 Account、Phone、Level、角色、组织、租户、密码或认证字段。旧租户未安装新字段时继续使用本机兼容值，不让上一位用户的本地主题覆盖下一位已登录用户。
- **个人中心与账号自助资料使用同一白名单和完整可修改字段集**：平台内置微服务 `microi-platform-service` 升至 v1.7.2、应用版本 29；个人设置页面可维护姓名、邮箱、性别、语言、私有/公开头像、登录后首页、主题、显示模式、菜单展开方式、桌面模式/背景、随机壁纸、菜单默认展开和任务栏菜单，并对邮箱、枚举、站内路由、租户文件路径及菜单数量做服务端校验。Level、角色、部门、组织、账号、手机号、状态、密码、认证因子和登录审计等管理员字段不开放自助修改；C# 现有资料端点只扩展缺少的邮箱/性别/语言可信原子，没有建立第二套用户或权限体系。
- **平台内置微服务兜底与系统设置入口修复版本不匹配**：“安全与服务接入”按钮不再固化历史 `Version`，始终解析当前 DatabaseOnly 数据库运行包；SaaS/商城离线包内嵌的运行产物与唯一正式源码根按文件字节、清单和哈希校验，HDFS/CDN 或其它租户文件服务不可用时仍可打开平台基础功能。嵌入脚本在运行时变化时强制同时递增应用版本、SaaS 包和商城包，禁止只换 Base64 资产不升包；最终 SaaS 引擎 v7.5.21、商城 v7.5.16 内嵌同一 v1.7.2 运行时，SHA-256 为 `8b7f11ab6347e65d59ef18d480320cce8363c2b5aa48d9141c3f208f763d371f`。
- **表单设计器不存在的 `BadgeApiEngineKey` 元数据错误完成根因修复**：`diy_table.Tabs` 中分组统计接口引擎的设计器代理改为真实 `sys_menu.MenuBadgeApiEngineKey` 字段，不再把前端展示属性当作 `diy_field` 名称查询；表单引擎官方包升至 v7.5.6，并通过回归测试确认设计器属性、数据源绑定和旧包升级兼容。CollapseGroup 同时收敛为主题表面、短色条、轻量“x 项”和单一外框，字段计数改用原始可见性，折叠后不再误显示 0 项；系统设置末尾分组使用显式 `FieldCount` 边界，避免未来扩展字段被错误吞入。
- **NuGet 下载统计增加浏览器、Redis 与数据库三层单调兜底**：官网先读当前官方接口汇总，成功结果写入 Redis 后再按日幂等写入 `mci_nuget_stats_daily`；缓存丢失或区域索引失败时从最近数据库快照恢复并回填 Redis，任何上游较小值都不会覆盖更高历史总数。浏览器保存最近一次成功快照，API、Redis 与数据库同时暂不可用时仍展示已验证值，并明确标注“浏览器最近成功快照／数据库每日持久快照”等来源；损坏快照自动丢弃，失败请求可被后续组件重新发起。
- **官网文档与 Skills 固化本轮平台规范**：系统设置、模块引擎文档补充水印、来源标签、个人菜单偏好和微服务入口语义；`microi-client-frontend` 明确框架统一标签、水印默认值和跨设备偏好优先级，`microi-form-layout` 增加配置类表单二级分组、字段计数和末尾作用域规则，`app-store` 明确标准功能必须先更新官方主租户，再同时维护基础 SaaS 包与独立存量租户应用。Microi.VSCode/CLI/Codex 插件同步这些 Skills 并发布 v5.2.2；npm `@microi.net/cli`、Visual Studio Marketplace 与 Open VSX 已统一公开回读 5.2.2。
- **源码、资源、真实 UI 与正式制品完成分层验收**：前端定向测试 43/43、后端定向测试 61/61、官方资源与发布契约测试 20/20 全部通过；Junchi 真实浏览器用例 1/1 通过并截图确认水印、弹窗标签、菜单标签、详情弹层、当前实例关闭和系统设置折叠分组。官方 SaaS v7.5.21 内容 SHA-256 为 `de1c3ac062c6710c6e0ea13dd6a279ba0d4772febde50287d668ef38a48b9de7`，公开 `Microi.Upgrade.7.5.4` 反射回读同样包含 v7.5.21、透明度默认 30 且不存在 `RenderSourceBadgeMode`；以上证明源码、官方母版、应用包、目标租户和公开制品各自成立，不把任一层的成功替代其它层。
- **v7.5.4 现代版全量发布完成不可变制品纠偏**：首次 v7.5.3 全量流程成功后，封版审计发现其不可变 `Microi.Upgrade` 包因错误同步三方基线而仍嵌入旧 SaaS v7.5.20；没有覆盖或伪造同版本，而是先把官方 SaaS v7.5.21 发布、哈希回读并恢复三方一致，再以补丁版 v7.5.4 重跑完整发布，因此 v7.5.3 仅为被纠正的中间制品，最终正式版本为 v7.5.4。脚本按 `7.5.4、4、0、2、2、0、0` 完成现代前端、后端 Release（0 个错误、9 条既有 xUnit 建议）、独立 liveness、DLL 加密、19 个 NuGet 和正式/测试 Docker，耗时 8 分 9 秒并以 exit 0 结束；NuGet.org 已公开回读 19/19。后端 `microi-api:v7.5.4`、`latest`、`microi-api-dev:latest` 摘要统一为 `sha256:d10fb8efd525715092cfdab4815407d0f1c91fd0176c08a18e3f3d6c05c02c2a`，前端 `microi-web-dev:v7.5.4`、`latest`、`microi-client-dev:v7.5.4` 统一为 `sha256:7cfa03736e5684c8d3a79a9688cd82d7381db92663cd0d4d8ffd2837cfd25c1c`；本轮只生成现代版，没有 Chrome 49 legacy。发布锁已释放，共享 61500 HTTP 与 61501 HTTPS 已恢复；公共制品可取不等同于任一生产/客户节点已经拉取并重启。

## v7.5.2 - (2026-08-22 07:43)

- **五仓提交历史、远端跟踪引用与当前工作树按 v7.5.1 最终封版点完成去重审计**：以根仓 `2ac2b5dca7fc0064770ca6cecd6726df374ce72e`、Microi.net `a20a5e254031f808ff6d8b8e0640fd4ec27e3776`、Microi.AI `ff616d53eb0db89e94014d1d8cd7823cd411ed54`、Microi.VSCode `9f6b568bea13989eb008e88ead35fc99c029c829` 和 WebOS `cf8b4703b51f2ddade779db4081e6d6ee18b6e9e` 为边界重新抓取 Gitee。Microi.net、Microi.AI、Microi.VSCode 与 WebOS 均为 ahead 0、behind 0；根仓远端新增 `aff80e78`、`98cda7b` 两个真实功能提交，以及 `0df4cff8`、`55664473` 两个分支同步／合并包装，按最终树去重为 16 个源码与测试文件、452 行新增和 90 行删除，合并包装和其中已在 v7.5.1 记录的内容不重复计数。写本节前根仓排除 `microi.mcp/dist` 后有 108 个已跟踪差异文件（4,223 行新增／695 行删除）和 16 个新增源码／测试文件（3,795 行），Microi.net 为 3 个文件（103/8），Microi.AI 为 1 个版本文件（3/3），Microi.VSCode 为 20 个版本、插件与 Skills 同步文件（383/308），WebOS 保持干净；应用包长 JSON、插件 bundle、哈希元数据与 `dist/bin/obj` 只核对最终语义和同源性，不逐份冒充独立功能，也不为 WebOS 制造空提交。
- **合并进入主线的 UniApp 与接口修复按最终净差异一次性归纳**：接口引擎缓存兼容旧数据格式并补充请求路由测试，MCP PageTabs 参数在提交前统一规范化，避免数组／字符串格式漂移导致页面数据错乱；小程序 Token 缓存失效后恢复单次授权登录，上传媒体保留可回读的权限上下文。售后任务完成服务字段改为可选，已完成设备详情进入真正只读态并正确展示私有图片，草稿、定位和重复提交入口同步关闭；售后工程师查看设备／任务地图时先走任务与子表权限，再以受控接口为历史无坐标数据补齐客户坐标，增强接口失败只降级缺失定位而不阻断已授权业务数据。以上对应 `aff80e78`、`98cda7b`，两个自动 merge 包装不再重复列项。
- **标准表单 Banner 成为所有表单默认可用、设计器物理字段驱动的通用能力**：`diy_table` 新增 `FormBannerEnabled`、标题、副标题、左侧图片／图标、背景、右侧标签与动态统计 8 个物理字段，并在表单属性增加独立“表单 Banner”页签；旧数据库无配置时默认显示，按名称／标题／编号等可读字段推断主副标题，按图片、下拉、单选和数字字段推断图片、标签及有效指标，显式空数组仍可关闭标签或统计。单图、多图、公私有文件路径遵循吾码文件规范，背景可来自图片／颜色／渐变字段；统计支持当前记录字段、同一接口引擎分组回读和记录级参数映射，刷新最短 15 秒。桌面亮色使用主题色中深渐变和柔和阴影，暗色降低主题色强度并叠加中性表面，统计卡去掉成排边线；移动端直接隐藏 Banner，避免挤占首屏。
- **查看、编辑、弹窗、抽屉与表单设计器继续收敛到同一套现代表单体验**：只读模式直接显示经过数据源解析的业务值，不再渲染禁用控件或暴露 Id／JSON；窗口标题统一为“查看／编辑 - 第一个有意义的查询列值”，跳过空值、布局字段和不可读对象，选项 JSON 只显示 Label，超长标题保持单行省略。分组标题补齐图标、HTML 副标题和必填计数；CollapseGroup 使用同一个外边界、圆角与阴影完整包裹头部和内容，修复左右边线错位及行 gutter 断层。全局确认提示统一为带图标、大圆角、大按钮的吾码弹窗，登录成功后的按钮保持“正在进入工作台”直到框架接管；低分辨率复合列提供横向滚动，表单弹层只保留最外层纵向滚动，长标题和记录工具栏不再挤压换行，设计器切换左侧分组也不会触发首列宽度反馈循环。
- **Banner 的零配置统计改为与当前记录真实相关且受权限约束的后端聚合**：Microi.net 的 FormEngineAdd 复用权威表元数据并返回明确字段错误；GetTableData 仅在当前表单没有可用本地指标时，从已授权的 `TableChild` 关系中最多选择 3 个有意义的数值字段，对完整关联数据执行求和并附带行数，技术字段、主键、排序号、时间戳和不可靠数值不会被当作业务统计，无法形成可信指标时直接隐藏而不虚构数字。该能力只扩展现有 FormEngine 授权查询，不建立绕过菜单、表、字段或行权限的新通道；Microi.net 与 Microi.AI 同步发布 7.5.2。
- **菜单数字角标补齐可解释语义，系统模块配置与表单属性保持单一事实源**：`sys_menu.MenuBadgeTooltip` 从物理字段、后端投影、MCP Manifest 到侧栏渲染完整打通，角标可显示接口引擎统计结果并通过真正左向三角 Tooltip 说明业务含义；接口引擎总数、我的工作、系统用户等官方菜单同步配置语义说明。表单分组方向、标题、副标题、Banner 与折叠样式继续由表单设计器维护，模块引擎只保留列表、菜单、记录直达和模块级展示职责；官方模块引擎包升至 v7.5.4。
- **主租户批量维护子租户平台应用升级为耐久、可恢复的后台任务**：新增可信 `ChildTenantPlatformAppControlService`，仅主租户 Level 9999 管理入口可发起，固定使用官方平台应用清单和受控目标租户标记；任务进入持久化后台中心，支持查询、取消、续跑、通知和精确结果，子租户维护页面可观察真实进度。最多 8 次有限重试、Redis 租约与 fencing token 防止多节点重复执行，同一租户的跨库任务串行化，最老待处理分片优先；公共提交中的保留目标参数会被剥离，受信任续跑时再恢复，避免调用方伪造目标租户。SaaS 引擎官方包升至 v7.5.15，内嵌唯一平台服务升至 v1.7.0。
- **多租户 Redis、后台任务与私密配置边界进一步失败关闭**：Redis 连接严格使用当前租户的主机、端口、密码和数据库号，不再从相邻配置推断数据库；后台 Worker 监督、抢占、取消和恢复补齐并发测试，异常节点不能把未完成任务误记为成功。`IsMainTenant` 只作为非敏感能力投影，数据库连接、Secret、身份配置和其它租户私密值仍只允许可信后端读取；生产 API 没有新增环境变量或第二套配置事实源。
- **空数据库发行与 V8 运行限制只保留真正需要程序升级兜底的物理兼容**：一键制作主库空数据库新增逐表失败关闭的业务／敏感数据清理，覆盖租户私密设置、身份绑定、访问密钥、AI 与消息运行残留、数据库／Redis／微信等敏感配置，并重置保留用户的秘密和缓存，任何应清理表失败都不会生成“已完成”空库。版本门控的 `Upgrade33` 在旧库首次升级到 6.9.8.9 时一次性把全部 `diy_table.V8Limit` 置 0、旧 `V8Unlimited` 置 1，之后用户重新开启限制不会被启动过程反复覆盖；运行时只初始化空值。其余表、字段、Tab、菜单、接口和可幂等种子继续由应用商城交付，`Microi.Upgrade` 不扩张为第二套业务资源安装器。
- **应用商城覆盖升级、物理结构自检和三方资源同步修复旧库安装边界**：统一导入器升至 v2.2.8，批量安装器升至 v1.2.4，AI 应用发布器升至 v1.8.4；安装前按 Manifest 幂等补齐实体物理列，官方 Application-owned Managed 资源允许覆盖升级，社区／租户扩展继续遵循 Base/Local/Incoming 保护，官方数据库禁止安装应用的边界不放宽。修复表单资源生成器误写三方共同基线的问题：默认只修改工作副本，只有显式 `--sync-base` 才能更新基线，避免官网旧包被误判为正确变更；表单引擎 v7.5.5 已通过绑定 `api.itdos.com + iTdos` 的 `microi_itdos` 发布并按 SHA-256 `7bd9ed4c40c446862ecbe5402c2214e31c8835aedfcf50ba2fb7c30e64aaa62e` 回读，模块、SaaS、SSO、商城包最终分别为 v7.5.4、v7.5.15、v7.5.3、v7.5.13。
- **MCP、官网中文文档与 Skills 同步覆盖 Banner、菜单角标和子租户后台维护**：MCP 新增表单 Banner 默认推断／配置校验、后台平台应用任务的运行／列表／取消工具和稳定 API 路由，取消动作要求显式确认，Manifest 继续输出物理字段而不是私有 JSON；表单引擎、布局、模块引擎、应用商城、多租户交付与 Redis 规范均已原位补充。合并团队 PageTabs 与后台任务契约后，`microi.mcp` 类型检查及全套 137/137、前端 Banner／只读／分组／设计器／确认／登录／侧栏定向回归 67/67、升级与商城资源契约 107/107 全部通过，生成后的 `microi.mcp/dist` 只作为源码编译产物提交，不另算功能。
- **Microi.VSCode、CLI 与多宿主插件发布 v5.2.1**：`npm run publish` 自动升版后完成生产构建、类型检查、CLI／Codex 插件／路由／Skills 同源测试和 VSIX 打包，npm `@microi.net/cli`、Visual Studio Marketplace、Open VSX 均上传成功；Open VSX 首次只读验证仍处于传播窗口时没有重复发布，稍后 `npm run publish:verify` 已从三个公共端统一回读 5.2.1。插件内置表单 Banner、后台任务、应用商城和模块配置最新 Skills，CodeBuddy、WorkBuddy、Codex 与 CLI bundle 版本和构建元数据保持一致。
- **v7.5.2 现代版全量编译与公共制品完成分层验收**：`Microi一键编译发布.sh` 按输入 `7.5.2、4、0、2、2、0、0` 完成官方资源三方同步、现代前端、串行压缩、后端 Release（0 个错误、9 条既有 xUnit 分析器建议）、发布目录守卫、Microi.net／Microi.AI 保护、iTdos MySQL／Redis／MongoDB 预检及独立 18080 liveness HTTP 200，最终复发版耗时 10 分 19 秒并以 exit 0 结束；19 个 NuGet 包已公开存在，同版本 409 被识别为不可变包而安全跳过。后端正式／测试镜像 `microi-api:v7.5.2` 与 `microi-api-dev:latest` 公开摘要均为 `sha256:015a06f09fc87b8d111038325f437dfab69d8676eee5b9356acc7c3ae53f91e6`，前端正式／测试镜像 `microi-web-dev:v7.5.2`、`microi-client-dev:v7.5.2` 均为 `sha256:fd65df2b86812137bdf6cb29da086ea56e4cb6b6ef0a94acf8c3bec5a42b4633`；本轮明确只生成现代版，没有 Chrome 49 legacy。发布后锁文件已释放、临时 18080 已关闭，共享 61500／61501 已恢复；以上证明源码、包、插件、官方应用与公共镜像可取，不等同于任一生产或客户节点已经拉取新版并重启。

## v7.5.1 - (2026-08-21 16:11)

- **五仓提交历史、远端跟踪引用与当前工作树按 v7.5.0 最终封版点完成去重审计**：以根仓 `d94eea013a272eea0ecc6b98edfa3997b8cad73e`、Microi.net `9a56f3633ff0c20f63a7b98306f51936d4c82748`、Microi.AI `8e35aa8e8e22d210c5489232119f5f3bd8c202b8`、Microi.VSCode `affa8864f5c1b7c1d0dcc70d75aa77b5f7d4d702` 和 WebOS `cf8b4703b51f2ddade779db4081e6d6ee18b6e9e` 为边界重新抓取 Gitee；五仓均为 ahead 0、behind 0，边界后没有新增提交、自动 merge 或需翻译的英文说明，本版本全部来自当前工作树净变更。写本节前根仓排除 `dist` 后有 177 个已跟踪差异文件、57 个新增文件（9,425 行新增/3,197 行删除），Microi.net 为版本文件与 2 个可信原子源码，Microi.AI 仅为版本文件，Microi.VSCode 为 32 个已跟踪差异文件与 6 个新增文件（5 个 SSO Skill 文件、1 个接口历史测试），WebOS 保持干净；应用包长 JSON、插件 bundle、哈希元数据和 `dist/bin/obj` 只按最终语义及同源性核对，不逐份冒充独立功能，也不为 WebOS 制造空提交。
- **表单引擎收敛为设计器直接配置、各入口复用的统一现代分组工作台**：`diy_table` 新增工作台模式、密度、分组标签位置、导航标题、动态统计、必填计数、HTML 标题/副标题/说明/页脚和记录选择器等物理字段，物理字段优先于旧 `FormPresentation` JSON，旧库缺字段或保留旧 JSON 时继续兼容。直达页、表格弹窗、抽屉和模块记录工作台统一复用 `diy-form-full.vue`、`DiyForm` 与同一套操作链，顶部记录选择、统计、搜索和更多操作合并为紧凑单行；弹窗/抽屉切换记录不再改路由或关闭原表格，模块级“默认打开第一条记录”迁移到 `sys_menu.OpenFirstRecord`，避免首次进入生成两个访问标签。
- **分组导航、字段布局、打印与移动端适配同步规范化**：分组卡片支持自定义总标题、数量单位、接口引擎动态统计、必填 Tag、图标、HTML 主副标题及说明，顶部/左侧导航遵循表单设计的标签位置并在窄屏折叠自适应；字段宽度既可拖动也可直接输入 1–24，字段说明在 Label 为 left 时贯穿标签与控件、为 right 时保持控件下方，日期与文本输入高度统一。通用“更多 → 打印”会展开全部分组、隐藏交互控件和记录面板，以白底全宽、分组分页和代码纯文本换行版式调用浏览器打印；重复的全局 `diy-form.scss` 收敛为单一基础样式源，并修复单分组弹层左侧空白、设计器渐进渲染、重型 Tab 延迟加载及异步状态清理。JSON 表格的动态 Select/SelectTree 列复用稳定字段上下文，异步数据源和已保存选项值不会因行列重新渲染而丢失。
- **SSO 身份联邦与平台登录完成“接口引擎编排、可信原子兜底”的统一架构**：官方 `app.microi.sso` 升至 v7.5.3，覆盖 OIDC、SAML2、CAS 和存量 Token 的双向联邦；连接投影、身份解析、绑定/JIT、角色与 Claim 映射、审计、登录完成、Secret 轮换及旧 Token 登录由 10 个 Managed 接口引擎编排，租户扩展保留 1 个 CreateIfMissing Hook。PC 统一通过 `/api/ApiEngine/Run?OsClient=` 调用业务能力，C# 仅保留协议报文、签名验签、证书与 Secret 隔离、PKCE、严格回调地址、一次性票据和 DiyToken 等可信原子；51 个原生 Controller 建立 Kernel、Bootstrap、Protocol、Managed、Operations 显式归属门禁。旧 URL Token 只读取窄化 capability，访问密钥会话严格服从 `_AccessKeyAllowedRoutes`，默认首页同样必须通过当前动态菜单权限校验。
- **短信注册登录和租户私密身份配置进一步失败关闭**：普通账号密码、图形验证码和 DiyToken 在基础应用未安装时仍可登录；短信证明由受信任的 `platform_auth_sms_login` 消费，证明有效期 2 分钟，SSO 登录票据有效期 90 秒且只能使用一次。首次注册使用带盐专用密码哈希，JIT 禁止下发平台管理员角色，客户端密钥轮换仅允许平台管理员执行且明文只返回一次；租户自有 Passkey、OAuth、短信模板与 Secret 迁入后端私密系统设置，只允许可信后端 V8/接口引擎读取，公开系统设置不再投影敏感值，私密设置弹层高度和滚动区域覆盖完整可视空间。
- **V8 运行限制统一改为正向 `V8Limit` 开关并保留旧库升级兼容**：表单和接口引擎统一以 `V8Limit=1` 启用 Jint 单次执行的超时、语句数、递归与分配限制；字段缺失、空值或 0 均表示默认不限制，旧 `V8Unlimited` 仅在新字段不存在时反向兼容。升级任务在版本门查询前幂等补齐物理字段与空值，MCP Manifest、建表、引擎保存、查询、VS Code 类型提示和官网文档同步采用正向语义，避免旧租户未安装最新应用包时启动或执行报错。
- **接口引擎修改历史升级为独立、幂等且可审计的子表**：新增 `mci_apiengine_change_history` 与唯一 `EntryKey`，MCP 和 VS Code 每次保存会追加版本、摘要、来源及代码记录，网络重试按 EntryKey 去重；旧多行 `ChangeHistory` 在资源建表后逐行迁移，旧字段继续作为滚动兼容回退，子表暂不可用时也不会阻断接口代码保存。MCP 的 `generate_system` 同时保留校验器完整 `{Code, Msg, Data}` 错误信封，并把动态对象与 `JValue` 行规范化后再验证，修复运行时读取 `.Val` 的异常；`plan/generate` 对菜单 JSON、ViewSchema、Actions、废弃 `DiyConfig` 和可执行前端字段统一执行失败关闭校验。
- **应用商城官方受管升级、旧库覆盖安装与稳定体验地址同时收口**：统一导入器升级到工作区 v2.2.1；仅可信 `api.itdos.com + iTdos` 官方平台包中 Application-owned Managed 资源允许覆盖升级，普通、社区、离线包和租户扩展继续执行 Base/Local/Incoming 三方保护，Tenant 与 CreateIfMissing 永不覆盖，官方数据库禁止安装应用的既有边界恢复。`InsertIfMissing` 只能更新声明的分类、说明与排序，禁止覆盖配置值或秘密；商城版本只在显式提交 Version 时变更，一键安装/更新平台应用不再把官方可覆盖资源误报为客户冲突。首页、列表与详情共用稳定 `ExperienceUrl` 解析器，清理缓存、租户、release/request/version 等临时参数，不再把不可变制品地址提升为正式体验入口；商城页同步压缩 Hero、筛选和操作密度，适配主题色及明暗模式。
- **菜单、左右树、聊天联系人和角色权限补齐通用体验边界**：左侧菜单支持历史向下展开与向右级联两种持久化模式；左右树 `DefaultExpandLevel` 未配置、非法或为 0 时全部折叠，正整数只展开到对应层级。聊天服务、通讯录、最近会话和历史记录增加 Name/Account 最小投影，显示优先级统一为“姓名 → 账号 → 未命名用户”，不再暴露内部 Id 或空联系人；角色表直连权限选择器改为远程检索、有限首屏和请求版本防迟到覆盖，菜单权限继续自动补齐祖先读取路径并遵守租户敏感表失败关闭规则。
- **Microi.UI、官网和 Skills 明确“清爽、清新、主题自适应”的视觉基线**：表单、商城、微应用和后台组件统一降低装饰密度，以清晰层级、留白、紧凑操作和可读对比为主；品牌色会派生交互色、边界、卡片与柔和表面令牌，同时适配任意主题色、浅色和深色模式。SSO、表单工作台、官方 Managed 资源、`V8Limit`、现代版构建、稳定体验地址、菜单/左右布局等中文官网文档与 64 项 Skills 同源更新。
- **Microi.VSCode、CLI 与多宿主插件发布 v5.2.0**：首次 `npm run publish` 在 `codex:test` 发现新增 `microi-sso` Skill 缺少 Codex 非阻塞自动更新约定，补齐唯一来源引用后以 `--no-bump` 原版本重试，生产构建、类型检查、CLI/插件/路由/工作区单源测试、64 项 Skills/177 个 VSIX 文件校验全部通过；`@microi.net/cli@5.2.0` 已发布，Open VSX 已显示 Published，Visual Studio Marketplace 上传任务已完成但公共索引仍处于 5.1.9 → 5.2.0 的传播窗口，没有因只读验证延迟重复升版。
- **前端发布默认仅生成现代浏览器制品**：`npm run build` 默认面向 Chrome/Edge 107+、Firefox 104+ 与 Safari 16+，不再自动生成 Chrome 49 legacy；需要兼容时必须显式使用 `npm run build:legacy`、`--with-legacy` 或对应发布变量。一键脚本新增最终 0/1 选择，现代模式拒绝 legacy 标记或残留文件、legacy 模式要求两套制品同时存在，内存预检只计算实际选中的阶段，避免无意义的双构建与峰值内存占用。
- **官方低代码资源与旧租户滚动升级边界同步发布**：表单引擎、模块引擎、SaaS 引擎、SSO 与应用商城官方包分别升至 v7.5.4、v7.5.1、v7.5.7、v7.5.3、v7.5.7，内嵌 `microi-platform-service` 升至 v1.6.9，应用商城基础升级号升至 6.4.5.0，资源同步基线、测试和哈希保持一致；物理字段升级只保留应用商城运行前必需的幂等兼容，未更新应用包的旧租户仍可依赖字段缺失、空值、旧 JSON 和旧历史字段回退正常启动，推荐安装/更新全部平台应用后启用完整新能力。
- **v7.5.1 现代版全量编译和公共制品完成验收**：`Microi一键编译发布.sh` 按输入 `7.5.1、4、0、2、2、0、0` 完成官方资源哈希校验、现代前端、串行压缩、后端 Release（0 个错误、9 条既有 xUnit 分析器建议）、发布目录 logs/PDB 守卫、Microi.net/Microi.AI DLL 混淆、依赖服务预检和独立 18080 liveness HTTP 200，用时 13 分 4 秒并以 exit 0 结束；19 个 NuGet 包全部上传。后端正式/测试镜像 `microi-api:latest/v7.5.1` 与 `microi-api-dev:latest` 摘要均为 `sha256:a1bb391381cd02cdfeec21a6db45b1bf7a38322123cd4ad9d5dc6c8ccceedfd4f`，前端正式/测试镜像 `microi-web-dev`、`microi-client-dev` 的 `latest/v7.5.1` 摘要均为 `sha256:710d94fa234538e496c9d28fcf26c186c11ed9c28942501ddef7a8f1aa4dcf560`；本轮明确未生成 Chrome 49 legacy。官网文档模式 6 另以 exit 0 完成 137 个 HTML 页、SEO 和主题构建检查，并将北京/杭州文档镜像推送为摘要 `sha256:d6b3ae61426c54d372fcc74d3786c0d1739e525848f4d380298345bcbbcb9b1c4`。以上证明源码、包、镜像与官网文档制品已发布，不等同于任一生产或客户节点已经拉取新版并重启。

## v7.5.0 - (2026-08-21 00:24)

- **五仓提交历史、当前工作树和自动合并按 v7.4.9 实际封版点完成去重审计**：以根仓 `16d275bd128f23d06435a1a7c6f2f277b8ac0473`、Microi.net `df8f07735465c466ee37a2eb93629c8c305e9b42`、Microi.AI `d388a3f4fc159d295d320526beb3071a6148f210`、Microi.VSCode `afbb89925f10ef077a687bc2200e7d643e08cb1f` 和 WebOS `cf8b4703b51f2ddade779db4081e6d6ee18b6e9e` 为边界重新抓取 Gitee 并核对本地、上游和最终提交树。最终推送前发现根仓新增 7 个可达团队提交，其中 `1932885d2`、`ed56b1b77`、`6a51a284f` 为 3 个真实功能提交，`533fc6e5e`、`3157394b7`、`0a3c29bcb`、`80a445fc4` 为 4 个分支同步/合并包装；按最终树去重为 35 个文件、1,716 行新增和 289 行删除，本地已无冲突快进到 `80a445fc4f8db0b62c8351c1cc5851fe3f5ec987`，没有遗漏的英文说明。其余四仓边界后无新增提交，首次审计时五仓均为 ahead 0、behind 0。写本节前根仓有 90 个已跟踪差异文件（2,837/1,082）和 8 个新增源码/测试文件（505 行），Microi.net 为 3 个文件（18/3），Microi.AI 为 2 个已跟踪文件（31/3）和 1 个新增测试文件（21 行），Microi.VSCode 为 18 个版本、Skills 与构建元数据文件（213/212），WebOS 工作树保持干净；应用包长 JSON、插件 bundle、哈希元数据及 `dist/bin/obj` 只按最终语义和同源性核对，不逐份冒充独立功能，也不为 WebOS 制造空提交。
- **v7.4.9 后并入的团队提交补齐 UniApp 业务查询、上传预览与表格批量选择**：跨表客户、通讯录人员角色等列表查询统一经接口引擎与可见筛选字段执行权限约束，客户/售后任务地图在首次请求即携带当前坐标范围，消息通讯录和首页通讯录可按角色筛选；模块与关联列表恢复模块设计中的卡片字段，标签/状态继续服从跨端移动卡片配置，小程序新增响应同时在外层和 `_RowModel.Id` 保留记录 Id。PC 文件/图片上传继续提交持久化 Path，同时用本地可预览地址即时显示私有资源；启用多选的表格支持点击普通行切换勾选状态，并排除操作列、选择框、链接、按钮、输入控件和文本选择等误触场景。上述 3 组能力按最终净差异归纳，4 个自动合并包装未重复记账。
- **团队 UniApp 代码合入后补齐可复现回归边界**：架构与原生控件检查统一归一化 CRLF/LF，并按当前共享卡片动作、跟进对象和独立滚动分页契约更新过时断言；默认与 xjy Profile 重新从单一配置生成，私有文件签名测试对齐 POST 请求体，避免把路径和权限上下文暴露在查询串。订单提交前事件会从请求上下文恢复订单 Id、缺失 Id 时失败关闭、无商品时明确归零金额，并保护零现价计算；本地同步的跨商家客户查重接口只向有客户菜单权限的登录用户返回脱敏受限卡片，2–3 字精确、4 字以上模糊，且不返回记录 Id、租户 Id、电话或坐标。最终 `npm run check:ui` 全链通过，包含 44/44 原生控件映射、跨表权限、上传、内容安全、登录会话、版本更新与分享策略；租户 V8 同步目录继续遵循工作区忽略规则，不把环境资源误当作公共框架源码。
- **白色主题从单一主色切换升级为“品牌色 + 可读交互色 + 表面层级”的完整契约**：白色、黄色等浅色 palette 会保留品牌倾向，同时派生可用于文字、图标、边框、按钮、骨架屏、启动页、登录方式和微应用的高对比交互色，暗色近黑/近白自定义色也会自动校正；微应用宿主实时下发 palette、前景色、强色、卡片/柔和表面、正文、次要文字和边界令牌，避免商城在白色主题下出现白字、白图标和不可辨识按钮。左侧菜单去掉菜单项、父级标题和展开项的成排描边/底线与上下不均阴影，仅保留轻表面和短侧标识；表单字段说明恢复鼠标与键盘 Tooltip，可在超长省略后显示完整内容。全局遮罩毛玻璃统一为正向 `sys_config.FormMaskBlur`，缺失或 `0/false` 默认关闭、显式 `1/true` 才开启，同时兼容旧 `DisableFormMaskBlur`；五种登录入口统一为显式 `DisableLogin*` 负向开关，并继续隔离 Passkey/OAuth 私钥等服务端配置。
- **应用商城详情、遮罩和微服务层级在框架与唯一平台服务源码中一起修复**：`microi-platform-service` 升至 v1.6.6，来源、详情和确认弹层改用浏览器原生 `<dialog>.showModal()` 顶层，完整弹层与 `::backdrop` 直接进入 body 顶层，不再受微服务 stacking context、主框架侧栏或宿主视口 z-index 限制；不支持原生顶层时才回退宿主全局遮罩，并显式区分 mask、promote 和 nativeTopLayer，未通过降低左侧菜单 z-index 规避。应用详情使用 DOMPurify 白名单净化后按真实 HTML 渲染，保留安全的标题、段落、列表、表格、图片、代码和官方 `mci-*` 排版，卡片摘要转换为纯文本，脚本、事件属性、表单、iframe 和危险链接均被拒绝；亮色主题下 Hero、页签、来源、卡片、按钮和详情富文本共同使用宿主可读令牌。
- **PC 聊天、SignalR 与平台通知收敛为紧凑、可观察且事件驱动的统一体验**：聊天弹层扩大为 `1120×720` 并限制在视口内，左侧列表固定紧凑宽度且禁止横向滚动，工具栏恢复表情入口，图片改用正确 Picture 图标并与文件、AI 模型选择器和帮助按钮垂直对齐；AI 助手统一使用顶栏同款机器人头像，AI 模型同时发送稳定 Key 与 Id，思考过程、流式分块和非流式完整答复都进入 Unicode/HTML 原子安全的打字机队列，失败会显示可见错误而不是留下永久“正在思考”。固定的“AI助手”和只读 `admin` 平台会话共享通知中心权威历史，角标规格统一，实时提示使用真正三角箭头；SignalR 每次握手读取最新 DiyToken，并携带 OsClient/设备标识，采用 `0/2/5/10/30` 秒有限退避、明确的连接/重连/暂停状态和手动恢复，服务端以可信 Token 身份覆盖客户端伪造的发送人字段。后台任务移除 3/5/10 秒整表轮询，只在启动、打开、重连、页面恢复或手动刷新时权威补读，创建时间显示完整日期时间。
- **低代码角色权限补齐菜单读取路径、表直连策略和保存时序的端到端边界**：选择任一菜单操作会自动给当前项、后代和整条祖先路径补齐 `Read`，避免有按钮权限却无法读取父级导航；超级管理员可在同一虚拟权限字段维护无菜单上下文的表直连权限，普通页面仍优先使用菜单权限，组件在保存前必须等待策略和表信息加载完成，加载失败则阻止提交。新增 `V8.Method.GetDirectTableGrantPolicies()` 只暴露服务端唯一维护的表名、授权模式和允许操作，`sys_role` 后端事务事件据此校验并分别持久化 `Menu`/`Table` 限制；SaaS 引擎包声明对应平台能力，现有角色委派、上下级、本人角色和平台敏感表的失败关闭规则不放宽。
- **模块引擎跨表 PageTabs 从整页路由替换升级为稳定入口宿主原位换表**：配置 `TargetSysMenuId` 后，运行时在同一个 `diy-table` 实例内重新加载目标模块的 `sys_menu / diy_table / diy_field` 与列表数据，只更新当前 URL 的 `Tab` 查询参数，面包屑、顶部访问标签、入口 Hero 和入口模块唯一一组 PageTabs 保持不变；隐藏的目标菜单仍必须具备当前角色权限，关联关系不会绕过菜单、表和行级授权。快速切换会主动取消旧请求并用模块上下文版本丢弃迟到响应，加载失败回滚原模块；首屏和切换期按元数据渲染与最终 Hero、指标、PageTabs、工具栏和列表同尺寸的主题化骨架屏，并在 `prefers-reduced-motion` 下关闭位移动效。模块/PageTabs 专项 17/17、真实 Edge 跨表原位切换 1/1 及现代版/Chrome 49 正式构建均通过。
- **AI 图片、音乐与视频的官网文档和 Skill 补齐同一套安全媒体契约**：明确 `MiniMax-M3` 只负责对话与意图路由，图片固定使用 `image-01`、纯音乐固定使用 `music-2.6`，视频继续使用受控任务句柄，不能把文本说明冒充媒体生成结果；图片/音乐请求使用稳定 RequestId、服务端白名单与共享幂等，供应商密钥、Base64、音频十六进制和原始视频任务 Id 不下发，成功内容进入当前租户 HDFS。浏览器按当前租户 `FileServer + FilePath` 解析附件，图片、音频、视频分别在原页使用 Element Plus 查看器、`audio` 和 `video` 控件预览，生成、持久化、可播放、下载和外部平台发布继续作为不同事实验收；新增官网媒体文档门禁 2/2 通过。
- **应用流式发布和 AI 聊天路由修复历史数据与并发时序边界**：应用文件发布只按确定性记录 Id 复用流式发布自有行，允许同一路径存在多个不可变归档版本，但仍拒绝重复的活动业务键，避免历史发布越多越容易命中非确定行。AI 普通问答、纯数字测试和指定复述不再因偶然命中 Schema 名称就误入 NL2SQL；只有明确数据表达或统一语义路由判定为 `data` 才进入 Schema 检索，订单数量、销售趋势、用户列表等数据问题继续保留快速路径。聊天服务补齐非流式模型最终内容、空结果和失败事件，历史记录晚到时与本地发送/流式气泡按稳定签名合并，不再清空用户刚看到的答复。
- **应用商城跨租户读取、官方包同步和失败重跑进一步可恢复且失败关闭**：统一导入器升至 v2.1.8、批量安装引擎升至 v1.2.3，商城模型、版本、统计和批量发现统一调用稳定的 `/api/ApiEngine/Run?OsClient=` 并在 JSON 中传 `ApiEngineKey`，不再依赖部分网关拓扑缺失的动态 `/apiengine/*` 路由；私有来源凭据、空响应有界重试、受管代码基线和安装写入不重放规则保持不变。官方资源三方同步将 `PackageInfo.Version` 视为交付元数据：结构合并时屏蔽双方独立升版，完成后恢复 base/local/remote 的最高正式版本，解决发布失败重跑的目标版本与官网独立补丁版本冲突，同时同一业务字段/代码行的真实冲突仍会阻止发布；SaaS/商城内置包和官网已同步为 v7.5.0 并完成哈希回读。
- **Microi.VSCode、CLI 与多宿主插件最终发布 v5.1.9 并完成同源升级**：首轮 `npm run publish` 完成生产构建、类型检查、测试、63 项 Skills/172 个 bundle 文件和 VSIX 内容校验，公开发布 5.1.8；Open VSX 首次上传遇到 503 后使用同版本恢复命令继续传播，没有因错误重试误增版本。并发任务收尾新增 PageTabs 与 AI 媒体 Skill 后再次机械重建多宿主 bundle，并按真实源码变化发布 5.1.9；`@microi.net/cli`、Visual Studio Marketplace 与 Open VSX 均已接受 5.1.9，Codex、CodeBuddy、WorkBuddy、DeepSeek Harness 的版本元数据、哈希和最终 Skills 保持同源。
- **v7.5.0 全量编译、分层回归和公共制品完成验收**：`Microi一键编译发布.sh` 按输入 `7.5.0、4、0、2、2、0` 在修复源码清洁门和应用包版本三方合并后完整重跑，用时 17 分 6 秒并以 `__MICROI_RELEASE_EXIT__=0` 收尾；现代前端、串行压缩、Chrome 49 legacy、后端 Release（0 个错误、9 条既有 xUnit 分析器建议）、发布目录守卫、Microi.net/Microi.AI DLL 混淆、依赖服务预检和独立 18080 liveness HTTP 200 均通过。客户端主题/聊天/通知/权限 51/51、商城导入与资源同步 92/92、后端资源流/权限/租户配置/AI 路由 145/145、平台服务构建及契约 3/3、UniApp `check:ui` 全链均通过；NuGet.org Flat Container 已逐包公开回读 v7.5.0 为 19/19。后端 `microi-api:latest/v7.5.0` 与 `microi-api-dev:latest` 摘要均为 `sha256:09da86bf038ce3ef4d52b13b3fd52313e204c4757d5b719ce8a73fe43efeb1ef`；拉取团队 PC 变更后的中间前端镜像摘要 `sha256:df5b0b4c84cb0505bb711bc46de4085c0b7a2e1342374091c8c3bf65c3a8dfe6` 已被最终 PageTabs 补发取代，最后一次同版本“只发布前端”用时 4 分 6 秒并以 `__MICROI_FRONTEND_LATE_EXIT__=0` 收尾，`microi-web-dev`、`microi-client-dev` 的 `latest/v7.5.0` 四个标签最终摘要均为 `sha256:6e862f5bb0b4c4da9ee418ad9312d7eccfef7bb5d60138c7a1c248e15deeaf6e`。发布锁、18080、61500 和 61501 监听均已清理。这些证据证明源码、插件、官方包、NuGet 和镜像可公开获取，不等同于任一生产/客户节点已经拉取新版并重启，也不冒充未执行的专用硬件验收。

## v7.4.9 - (2026-08-20 12:52)

- **五仓提交历史、当前工作树与自动合并按 v7.4.8 实际封版点完成去重审计**：以根仓 `13b2a49faa08ea5c1b564bb723658220742ff5a6`、Microi.net `740b6ececbcc0ec587ecd6177ee36f9b076f6509`、Microi.AI `239c9817cc6c69a602980d411f821b529ff7f9de`、Microi.VSCode `8bd2486ab07559ba855169afdb8edb0069082edd` 和 WebOS `cf8b4703b51f2ddade779db4081e6d6ee18b6e9e` 为边界抓取远端并核对提交树。边界后根仓只有 `112db44` 的列表筛选修复及其 `297cdde` 自动 merge 包装，按最终树去重后的历史净差异为 2 个文件、27 行新增；其余四仓没有新增提交，也没有遗漏的英文说明。写本节前根仓有 59 个非 `microi.mcp/dist` 已跟踪差异文件（1,214/292）和 3 个新增源码/测试文件（185 行），Microi.net 为 2 个文件（5/3），Microi.AI 为 1 个版本文件（3/3），Microi.VSCode 为 16 个版本、Skills 与构建元数据文件（224/204），WebOS 工作树保持干净；生成目录、插件 bundle、应用包长 JSON 与哈希元数据只按同源性和最终语义核对，不逐文件冒充独立功能，也不为 WebOS 制造空提交。
- **模块引擎 PC 复合列统一收敛为紧凑双行，并同步到 MCP、Skills 与官方应用包**：MCP 默认展示只生成“主字段 + 1 个次要行”，允许配置多个双行复合列，但一般场景不再生成三行；模块引擎、系统交付 Skills 和配置参考明确把三行限定为确有必要且经过视觉验收的例外。表单引擎、模块引擎、SaaS 引擎和应用商城官方包中已有 PC 复合列均清理为最多一个 `Lines` 项，并加入遍历全部官方包的回归门禁；四包版本分别更新至 v7.4.8、v7.4.8、v7.4.13、v7.4.14，发布前通过 `microi_itdos` 三方资源同步与哈希核对。
- **模块模板设计的数据源名称和跨端视图编辑密度完成修复**：“不显示列”等字段选择器固定保存字段 Id，并显示“字段 Label - 字段名（表说明 - 表名）”，不再把无法识别的 GUID 直接呈现给用户；拼音搜索改用 pinyin v3 的具名导入，避免模块检索值为空。跨端视图的统计指标和复合列描述项改为每个控件一个紧凑网格轨道，宽屏保持单行并允许横向滚动，窄容器才按容器查询分段，解决配置控件逐项换行、删除按钮异常拉宽和页面纵向过高的问题。
- **工作流审批页、抄送已读状态和“我的工作”统计统一为可验证的真实语义**：StartWork/DoWork 与 ViewWork 审阅上下文分别判断，处理或查看流程时隐藏普通保存、编辑、删除、草稿和自定义 CRUD 操作，只保留流程处理、流程历史、流程图与关闭；桌面端和移动端行为一致。新抄送明确写入 `IsRead=false`，打开记录后只标记当前收件人为已读并写入 `ReadTime`，历史缺少该字段的数据按已读兼容；“我的工作”角标精确汇总待办与未读抄送并分别返回我发起、我处理、抄送、相关统计，完整分页读取避免 500 条以上漏计。流程预览同时读取节点与连线完整包，前端 23/23、后端专项 14/14 回归通过。
- **应用商城受管接口升级兼容历史官方基线，同时继续保护租户本地代码**：统一导入器升至 v2.1.6，商城只读源请求支持有界重试，安装写入绝不自动重放；受管接口策略新增 `CompatibleBaseHashes`，只忽略自动生成的前导说明来判断可执行代码等价。旧安装缺少 ResourceState/BaseHash 时，仅对可信 iTdos 官方平台包读取精确历史发布快照恢复一次基线；未知客户修改仍整包冲突回滚，不会被官方包覆盖。商城发布器升至 v1.8.3，连续携带兼容基线并剔除当前重复哈希，导入器、商城安装、现代体验、模块资源和低代码角色部门契约测试全部通过。
- **v7.4.8 后的列表列筛选修复按最终净差异补记**：列头筛选容器恢复完整宽度并允许内部收缩，清空与筛选按钮使用受控的弹性宽度和零外边距，修复横向空间不足时右侧按钮被裁切的问题；SFC 回归同时验证清空、筛选操作仍存在，自动 merge 提交没有被重复归纳。
- **Microi.VSCode、CLI 与多宿主插件发布 v5.1.7 并完成三端公开回读**：`npm run publish` 完成生产构建、类型检查、测试、63 项 Skills/MCP bundle、VSIX 内容校验及上传；npm `@microi.net/cli@5.1.7`、Visual Studio Marketplace `Microi.v8-engine v5.1.7`、Open VSX `Microi.v8-engine v5.1.7` 已由 `npm run publish:verify` 从公共分发端同时确认，插件内置模块双行规范与工作流抄送规则同步更新。
- **v7.4.9 全量编译、公共制品与分层回归完成验收**：发布脚本按输入 `7.4.9、4、0、2、2、0` 完成官方资源检查、现代前端、串行压缩、Chrome 49 legacy、后端 Release（0 个错误、9 条既有 xUnit 分析器建议）、发布产物守卫、Microi.net/Microi.AI DLL 混淆、独立 18080 liveness HTTP 200 和 19 个 NuGet 推送；Docker Desktop 冷启动后首次拉取 MCR 基础镜像发生 TLS 超时，脚本已补充三次远端拉取重试与本地缓存兜底，并以模式 5 续跑成功。NuGet.org Flat Container 已逐包公开回读 v7.4.9 为 19/19；后端 `latest/v7.4.9/dev` 镜像摘要均为 `sha256:edb0327983cf6306693d043dbd0b0a32b30547170fb82a33e1984b0601679968`，前端 `microi-web-dev` 与 `microi-client-dev` 的 `latest/v7.4.9` 摘要均为 `sha256:7d19cd9f540ddb6cc769a4eb04b53e3ce20dc9aa1afab6075c9b4fe20dc41603`。MCP 124/124、前端 23/23、工作流后端 14/14 及升级包定向回归均通过；这些证据证明源码、公共包、镜像和插件可取，不等同于任一生产或客户节点已经拉取新版并重启。

## v7.4.8 - (2026-08-20 09:36)

- **五仓历史、当前工作树与自动合并按 v7.4.7 实际封版点完成去重审计**：以根仓 `a45fc90bb6c69f4a69cb8b9027e5b9e2707f0bbd`、Microi.net `f4a107c50af856a66494fdb4dc15fef13ca8b79f`、Microi.AI `e4c50d259ad8e8cba422a7ed864d9b897e40009d`、Microi.VSCode `46a0ca519f93a65812e70be43310fccde456295a` 和 WebOS `cf8b4703b51f2ddade779db4081e6d6ee18b6e9e` 为边界执行远端抓取、提交树和最终净差异核对。边界后根仓有 9 个可达提交，其中 `7e9ad32` 是已写入 v7.4.7 的封版提交，`3093cf5`、`9415806` 是自动合并包装；去掉这些重复层后，新增历史净差异为 31 个 UniApp 文件、1,952 行新增/123 行删除。Microi.net、Microi.AI、Microi.VSCode 各 1 个提交均为 v7.4.7 已记录的版本/发布同步，WebOS 无新增提交，五仓审计时相对上游均为 ahead 0、behind 0，也没有需翻译的英文提交说明。写本节前根仓有 60 个已跟踪差异文件（2,342/558）和 13 个新增源码/测试文件（1,138 行），Microi.net 为 1 个版本文件（3/3），Microi.AI 为 5 个已跟踪文件（91/7）和 2 个新增实现文件（544 行），Microi.VSCode 为 17 个版本、Skills 与构建元数据文件（224/209），WebOS 工作树保持干净；`dist/bin/obj`、插件 bundle、应用包 Base64 与生成元数据只按最终语义和哈希核对，不逐文件冒充独立功能，也不为 WebOS 制造空提交。
- **君驰维保 UniApp 的 6 个真实功能提交按最终树合并归纳**：PC 与小程序订单合同、售后任务客户信息以及商品/设备安装位置补齐定位；新增客户与售后任务地图，退出登录时清理账号会话缓存，并修复订单没有商品却保留金额的汇总边界；方案安装点位移除冗余字段、地图详细地址去掉重复城市；售后任务与任务设备列表按“当前账号 + 入口条件”恢复筛选、已加载记录和滚动位置；需求方案安装点位增加统一编辑能力。消息通讯录的角色筛选也已进入源码和契约测试，但原提交明确标注接口仍报错、功能未完成，本版本据实保留为待后续联调项，不把自动 merge 或未通过项写成已验收能力。
- **AI 对话页按主流助手交互重构为清爽工作中心**：左上模式切换、推荐提示词和底部输入器统一收窄层级，推荐问题由成排重卡片改为可点击文字行，模型、语义模式、推理强度和 Schema 等低频设置收进单一设置入口，发送区继续保留上传、模型路由和安全数据提示。AI 回复统一经过受限 Markdown 渲染与 DOMPurify 清洗，标题、列表、粗体、行内代码和链接显示为排版结果，不再把 `##`、`**` 等语法符号直接暴露给用户，同时拒绝事件属性和 `javascript:` 链接。
- **MiniMax 图片生成打通安全后端、租户 HDFS 与 Element Plus 原位预览**：新增 `image-01` 参数白名单、稳定 RequestId/指纹和 Redis 七日幂等状态，供应商密钥与 Base64 只停留在服务端，生成成功后验证 JPEG/PNG/WebP 魔数、租户上传限制并写入当前租户公有 HDFS；结果同时返回 `FilePath`、永久 URL、文件类型和大小。聊天历史与新结果都优先用系统设置 `FileServer` 拼接 `FilePath`，不再信任旧响应中的写死域名；点击图片使用 `el-image` 的 `preview-src-list`、Teleport 和遮罩关闭能力在当前页面放大，浏览器地址与标签数保持不变。
- **MiniMax 音乐能力使用专用 `music-2.6` 模型接入中转站并支持在线播放**：AI 页新增管理员可见的“AI音乐”语义模式，调用 `/api/Ai/GenerateMiniMaxMusic` 后把生成音频保存为会话附件，并以原生 `<audio controls>` 在聊天中直接播放。Microi.AI 新增音乐专用路由选择和中转站 `/v1/music_generation` 兼容端点，平台 API Key 鉴权、官方直连密钥隔离、HTTPS 主机白名单、额度记账、共享 Redis 幂等及结果不确定时禁止盲目重试均已闭环；文本模型 `MiniMax-M3` 只负责语义识别/普通对话，真正生成音乐使用 `music-2.6`。源码和中转协议已经交付，但当前官方直连账号对音乐接口返回 HTTP 410（供应商提示旧 Music API 不再向新用户开放），因此本版本不虚报真实出歌成功；待 MiniMax 为该账号开通当前音乐产品后无需再改前端播放器与中转协议。
- **“我的工作”回归表单引擎 + 模块引擎的真实配置事实源**：首页 PAGE10 只负责编排工作入口，`/mic-home-work-todo` 的运行数据由 1 个 `wf_work` 待办模块和 4 个 `wf_flow` 隐藏模块承载，五类模块分别配置真实 `SelectApi`、列表/搜索/隐藏/移动字段、10 条分页、跨模块 PageTabs、查看/处理/撤回/作废行按钮和待办批量审批；主模块菜单角标也继续按当前用户权限统计。主前端新增的 `V8.OpenWorkflowRecord` 与 `V8.BatchApproveWorkflowRecords` 只是模块按钮可复用的工作流原子能力，包含表单预热、过期打开令牌取消和仅审批未完成记录等保护，不再让每个“我的工作”页面复制一套定制表格逻辑。MCP 已回读 PAGE10、5 个 `sys_menu`、`wf_work/wf_flow` 绑定和字段/按钮配置，真实浏览器已在通用模块工作台逐一切换“我的待办、我发起的、我处理的、抄送我的、与我相关”并确认表单表格正常加载。
- **应用商城与 AI 应用入口收敛到一个平台服务和一个可审计源码根**：旧 `/mci-ai-app` 稳定重定向到 `/microi-store`，创建应用、返回商城、卡片和详情中的预览/开发入口统一由应用商城承载；Web、UniApp、MicroService 使用同一 `sys_microistore` 主表与分类，不再并行维护第二套 AI 应用列表。`microi-platform-service` 的正式源码根改由 `platform-service-release.json` 唯一指定，发布前要求源码子树无未提交修改，并逐文件核对源码清单、路由、4 个构建资产及两个内置包；SaaS 引擎包 v7.4.9 与应用商城包 v7.4.10 携带相同的 v1.6.4 `DatabaseOnly` 运行产物，允许新租户在 HDFS 尚不可用时启动商城/个人中心/系统设置，而普通应用上传、源码编辑和安装仍失败关闭。iTdos 在线文件运行时与数据库包的 4 个文件路径、大小和 SHA-256 已逐一一致；两种清单的聚合摘要算法不同，不再把摘要字符串不同误判为资产漂移。
- **AI 平台治理 v2.1.1 从教学看板接入当前租户正式事实**：总览只读汇总启用模型、微服务、成功发布应用和最近 7 天匿名化 Token 台账，不读取 ApiKey、私有源码、用户问题或回答正文；“预览并纳管”先生成 `PlanHash`，确认后以 `ExpectedPlanHash + IdempotencyKey` 只新增/同步 `auto:` 受管服务记录，客户同名记录、人工负责人、健康路径和健康状态均不覆盖，来源消失只停用不删除。官方应用已通过 `microi_itdos` 回读为 v2.1.1，文档把平台管理员、架构师、发布负责人、SRE 与交付工程师的使用路径和 AI 客服 v1→v2 灰度/告警/回滚证据链补齐。
- **复杂 Excel 模板导入升级为声明式 V8 桥与持久后台任务**：新增 `V8.OpenImportDialog`，页面按钮只声明 Sheet、固定单元格、列映射、数据起止行、业务 Key、文件/行数上限和后台并发参数；浏览器解析工作簿后仅提交精简行 JSON 与元数据，业务接口必须在后端再次校验。弹层复用平台统一大圆角 Dialog，显示后台任务的真实 Current/Total、状态和日志；无自定义参数时继续使用旧单行表头导入，避免破坏既有模块。
- **Microi.VSCode、CLI 与多宿主插件发布 v5.1.6 并完成三端公开回读**：`npm run publish` 完成生产构建、CLI 类型检查与测试、Codex 测试、Skills/MCP bundle、VSIX 内容校验和三端上传；`@microi.net/cli@5.1.6`、Visual Studio Marketplace `Microi.v8-engine v5.1.6`、Open VSX `Microi.v8-engine v5.1.6` 已由 `npm run publish:verify` 从公共分发端同时确认。内置 Skills 同步增加平台微服务唯一源码、数据库启动包边界、声明式 Excel 导入以及发布/共享服务保护规则。
- **v7.4.8 全量编译、专项回归和公共制品完成分层验收**：`Microi一键编译发布.sh` 按输入 `7.4.8、4、0、2、2、0` 用时 12 分 16 秒并以“全部完成”收尾，完成官方资源三方合并/哈希回读、现代前端、串行压缩、Chrome 49 legacy、后端 Release（0 个错误、9 条既有 xUnit 分析器建议）、发布产物守卫、DLL 加密和独立 18080 liveness HTTP 200。AI 图片源码契约 3/3、MiniMax 音乐支持 9/9、MiniMax 图片支持 11/11、平台微服务构建与发布契约 3/3 通过；真实 Edge 已确认 Markdown 排版、FileServer 图片、Element Plus 放大不新开标签和五类低代码工作流列表。NuGet.org Flat Container 已逐包公开回读 v7.4.8 为 19/19；`microi-api:latest/v7.4.8` 与 `microi-api-dev:latest` 摘要均为 `sha256:7fdc31bf88253c28440c8f8fbfdadcd7bc90507546a5b5e4d9ee24ce0929113d`，`microi-web-dev:latest/v7.4.8` 与 `microi-client-dev:latest/v7.4.8` 摘要均为 `sha256:83691bf25cd987c6498c6cc455e83278ea4a7a9e116ef1ce8d016a2eeb8170ca`。这些证据证明源码、插件、官方包、NuGet 和镜像可取，不等同于生产 API 节点已经拉取 v7.4.8、重启服务，也不等同于 MiniMax 已给当前账号开通音乐产品。

## v7.4.7 - (2026-08-20 02:07)

- **五仓历史、待提交工作树与自动合并按 v7.4.6 实际封版点完成去重审计**：逐仓执行远端抓取和最终树核对后，以根仓 `a45fc90bb6c69f4a69cb8b9027e5b9e2707f0bbd`、Microi.net `f4a107c50af856a66494fdb4dc15fef13ca8b79f`、Microi.AI `e4c50d259ad8e8cba422a7ed864d9b897e40009d`、Microi.VSCode `46a0ca519f93a65812e70be43310fccde456295a` 和 WebOS `cf8b4703b51f2ddade779db4081e6d6ee18b6e9e` 为边界，五仓边界后均无新增可达提交、英文提交说明、自动 merge 包装或遗漏推送，且相对各自上游均为 ahead 0、behind 0。写本节前根仓有 80 个已跟踪差异和 9 个新增源码／测试文件，原始净差异为 27,052 行新增、21,252 行删除；Microi.net 为 2 个文件、8/7，Microi.AI 为 1 个版本文件、3/3，Microi.VSCode 为 25 个文件、374/328，WebOS 工作树保持干净。SaaS 应用包的大段 JSON 重排、`microi.mcp/dist`、插件 bundle、Skills 元数据和版本清单只按最终语义与同源性核对，不逐文件冒充独立功能；WebOS 无变化不制造空提交。
- **角色管理和部门机构由定制页面迁移为可升级的低代码左右树表**：`/system/role`、`/system/dept` 菜单统一指向 `LeftTreeJoinRightForm`，SaaS 引擎应用包 v7.4.8 携带 `sys_role`、`sys_dept`、`sys_rolelimit` 表单元数据、35 个字段、两条左右树表配置和新增／修改／删除后端 V8 事务事件；角色权限明细采用不落物理列的虚拟 `DevComponent`，旧 `sysrole-manage.vue` 继续保留用于兼容。组织机构树使用真实树接口和正确的父子／叶子语义，初始即处于“全部”状态，首次或再次点击“全部”不会把右侧数据清空。
- **角色权限明细恢复完整权限闭环并消除重复界面元素**：定制组件不再嵌套第二层表单 Label，删除没有实际权限语义的“无详情、无搜索”，选择子菜单时自动勾选所有祖先菜单并只补齐祖先 `Read`，不会把子菜单增删改权限错误上卷；行更多重新提供“AI 数据权限”，可维护数据范围、业务表、模型、最大行数和原始 SQL 策略。菜单可见权限与接口引擎控制面权限继续分层：500 级“接口引擎专员”可获得菜单可见性，但 `sys_apiengine` 模型和平台控制面仍按既有 9999 级策略失败关闭，避免把菜单授权误当成超级管理权限。
- **左右树表筛选和初始化统一为单一 `_Where` 数据通道**：树未选中或关联值为空时不再生成 `ProjectId=""` 等假条件，历史 `_SearchEqual`、`_SearchCheckbox` 在请求前归一到 `_Where` 并移除空值，同字段已有显式条件时不再叠加重复条件；同一事件循环内由多个属性监听触发的初始化合并为一次，修复单击树节点时 `GetTableData`、`GetSysMenuModel`、`GetDiyTableModel`、`GetDiyFieldByDiyTables` 成对请求的问题。该修复同时覆盖君驰项目材料单等通用左右树表页面，而不是租户页面硬编码。
- **表单控件的值语义、Label 布局和字段说明完成一致性修复**：Switch 对数字、布尔值及历史字符串 `0/1/false/true` 做严格归一，避免字符串 `"0"` 被当作开启；字段显示名称优先完整展示，字段说明只占剩余空间并超长省略，Element Plus 提示可查看完整说明。字段显式配置的 `left/right/top` 对齐优先级高于代码编辑器、子表、富文本等重组件的历史默认 top，定制组件由宿主统一渲染 Label。二维码组件文档和 FormEngine Skill 明确区分外观配置与运行数据：`Config.Qrcode` 只控制样式，实际编码文本使用 `DataAppend.Code`，并补齐标题、字段行、颜色、下载文件名、旧字段别名和虚拟字段不入库示例。
- **接口引擎由负向 `V8Unlimited` 收敛为默认关闭的正向“V8 运行限制”**：新增持久版本门控的一次性升级，创建 `sys_apiengine.V8Limit`、隐藏旧字段元数据，并用一条整表 SQL 将 `V8Limit=0`、兼容字段 `V8Unlimited=1`；只有升级完全成功才推进后端版本，因此后续用户主动开启不会被再次还原。关闭时不施加单次 Jint 超时、语句、递归和分配预算，开启后才读取限制参数；常驻内存、取消、并发、嵌套、权限和数据库保护始终保留。Microi.Core、Microi.net、控制器、MCP、Manifest 和 VS Code 均使用正向语义，请求参数不能覆盖可信持久配置，旧 `V8Unlimited` 仅作为反向兼容别名并在冲突时告警。
- **应用商城、官方文档与 Skills 同步交付低代码页面和运行限制协议**：表单引擎／模块引擎应用包升至 v7.4.6，SaaS 引擎／应用商城包升至 v7.4.8，四包及 `.resource-sync-base` 都携带 `V8Limit` 物理列，SaaS 包额外携带角色／部门菜单、表单、字段和左右树表种子；官方包已通过 `microi_itdos` 发布并回读。表单引擎、左右布局、V8 接口配置／调试／表事件、菜单按钮、任务引擎和应用商城 Skills 与官网二维码、V8 服务端文档同步，普通租户通过商城安装／更新与新版后端一次性升级即可获得相同契约。
- **Microi.VSCode／CLI／多宿主插件发布 v5.1.5 并完成三端公开回读**：接口引擎创建、同步和类型提示统一显示“V8 运行限制”，默认关闭并传输 `V8Limit`；生产构建、CLI 类型检查与测试、Codex 测试、工作区单源检查、MCP／Skills bundle、VSIX 内容校验全部通过。npm `@microi.net/cli@5.1.5`、Visual Studio Marketplace `Microi.v8-engine v5.1.5`、Open VSX `Microi.v8-engine v5.1.5` 均已从公开分发端回读确认，内置 63 项 Skills 和 MCP／CLI bundle 同步更新。
- **v7.4.7 全量编译、专项回归和公共制品完成分层验收**：平台根版本、Microi.Client、Microi.net、Microi.AI 和 19 个服务端公共组件统一升至 v7.4.7；`Microi一键编译发布.sh` 按输入 `7.4.7、4、0、2、2、0` 用时 14 分 58 秒并以退出码 0 收尾，完成官方资源哈希一致性、现代前端、串行压缩、Chrome 49 legacy、后端 Release（0 个错误、9 条既有 xUnit 分析器建议）、发布产物守卫、Microi.net／Microi.AI 保护和独立 18080 liveness HTTP 200。NuGet.org Flat Container 已逐包公开回读 v7.4.7 为 19/19；后端固定镜像摘要为 `sha256:2f93992177f8c6d8fd9e3de0e4c61bc81f076464457a1fab01ffec9516f0d1bd`，前端固定镜像摘要为 `sha256:6e958241a1eefd44be34fa25a754bd724b74a6687a944c9c7b7cc289f4060dc9`。表单／左右树表／二维码／角色资源契约 22/22、MCP 类型检查与测试 124/124、后端一次性升级专项 6/6 通过；这些证据证明源码、公共包、镜像、插件和应用包可取，不等同于任一生产／客户节点已经拉取 v7.4.7、重启服务或完成真实业务验收。

## v7.4.6 - (2026-08-19 11:58)

- **五仓历史与待提交工作树按 v7.4.5 实际封版点完成去重审计**：以根仓 `865c8c6776fd86d7fabd4e19ef936da52459e3ae`、Microi.net `5d1ff8d5aa856fe5f609f0c58912f4e398ce1e29`、Microi.AI `67d7b092490fe2d87b9f5f280b4d98360c65a2c2`、Microi.VSCode `461a828a6d1db29b5332d141fa701c49c4441add` 和 WebOS `cf8b4703b51f2ddade779db4081e6d6ee18b6e9e` 为边界核对本地 HEAD、上游和最终树，五仓相对上游均为 ahead 0、behind 0，边界后没有新的提交／推送、英文提交说明、自动 merge 包装或需重复归纳的文件差异。写本节前根仓有 39 个已跟踪差异文件和 2 个新增文档／测试文件，已跟踪净差异为 1,598 行新增、184 行删除，新增文件合计 284 行；Microi.net、Microi.AI 各有 1 个版本文件，净差异均为 3/3；Microi.VSCode 有 11 个版本／构建元数据文件，净差异为 191/191；WebOS 工作树保持干净。`dist/bin/obj`、插件 bundle、Skills 元数据和版本清单只核对同源，不逐文件冒充独立功能，WebOS 无变化不制造空提交。
- **通知中心提供“安装／更新全部平台应用”的可恢复后台入口**：平台应用提醒同时识别未安装和版本过旧，非官方平台的超级管理员可在通知中心一次触发 `bulk-import-microi-store-packages` 持久化后台任务；提交前检查 Worker 状态，固定 `ApplicationType=Platform`、稳定并发键、幂等键、重试次数和最大尝试次数，成功后展示任务编号并切换到任务页跟踪。官方源、访问密钥会话和非超级管理员继续失败关闭，通知角标语义、简体中文、繁体中文、英文提示及源码契约同步更新。
- **应用商城升级器补齐平台微服务运行时完整性自愈**：基础应用完整性检查从菜单扩展到 `microi-platform-service`、`/marketplace` 页面和 `/micro-app/host` 入口，能够发现菜单绑定缺失、服务停用或指向错误、数据库内联 `index.html` 缺失以及页面路由绑定异常，并要求重新导入官方包恢复。导入前同时验证应用包中的 MicroService、仅数据库运行模式、启用路由、入口 Base64 与 SHA-256，在线资源必须携带后架构绑定恢复、包内绑定菜单、批量平台任务和运行时标记，否则拒绝使用旧包继续升级；反射回归覆盖完整包与畸形绑定，应用商城文档补充自动自愈和通知中心兜底的分层验收边界。
- **官网、README 与培训资料统一升级为企业级 AI 应用开发框架定位**：对外叙事由单一“开源 AI 低代码／节省 Token”收敛为“不只是开源 AI 低代码，更是企业级 AI 应用开发平台与框架”，明确可视化建模、V8 在线编程、.NET／Vue 专业源码和 AI Agent 共用一条可持续交付链；官网首页、登录页、SEO 描述、导航配置、README、培训大纲和文档模板同步。源码架构页新增中大型应用选型表，说明标准业务、差异化逻辑和底层深度扩展分别落在哪一层，并补充 Fusion Development、内部开发者平台等行业参考，避免把低代码与高代码描述成二选一。
- **官网首页按 MCI 设计契约重构为可访问的工程控制台叙事**：保留首屏 Microi AI Studio 对话区，在其后新增平台定位 Hero、事实证明、开发连续体地图、AI 协作层、三种开发模式、共享企业底座、多端输出和三组长期价值证明；中英文文案同源，亮暗主题、键盘焦点、390／768／1440 响应式布局、减少动画和减少透明度偏好均有明确降级。新增 `MCI-DESIGN.md` 作为首页视觉与内容契约，新增首页定位测试并纳入官网内容门禁，避免后续修改把 AI Studio、平台定位或三层开发路径拆散。
- **Microi.VSCode、CLI 与多宿主插件发布 v5.1.4 并完成三端公开回读**：`npm run publish` 完成生产构建、CLI 类型检查与测试、Codex 测试、工作区约定测试、MCP／Skills bundle 校验和 VSIX 内容校验；版本及构建元数据同步到 VS Code、CLI、Codex、CodeBuddy、WorkBuddy 和内置 Skills。npm `@microi.net/cli@5.1.4`、Visual Studio Marketplace `Microi.v8-engine v5.1.4`、Open VSX `Microi.v8-engine v5.1.4` 均已由 `npm run publish:verify` 从公开分发端回读确认。
- **v7.4.6 全量编译、定向回归与公共制品完成分层验收**：平台根版本、Microi.Client、Microi.net、Microi.AI 和 19 个服务端公共组件统一升至 v7.4.6；`Microi一键编译发布.sh` 按输入 `7.4.6、4、0、2、2、0` 用时 18 分 31 秒并以退出码 0 收尾，完成官方资源一致性、现代前端、串行压缩、Chrome 49 legacy、后端 Release（0 个错误、9 条既有 xUnit 分析器建议）、API 发布产物守卫、Microi.net／Microi.AI 保护和独立 18080 liveness HTTP 200。NuGet.org Flat Container 已逐包公开回读 v7.4.6 为 19/19；固定后端镜像摘要为 `sha256:a5ac39df9c9090a962779933271d694f8efc3d3a2e643a47c14d1f9af945e0b6`，固定前端镜像摘要为 `sha256:e5bd4451f990476a7f5a1da762458ef64979e74908d07fe8b8ed6680bd481156`。官网定位 3/3、客户端通知中心等契约 9/9、应用商城缓存与升级专项 23/23 通过；这些证据证明源码、公共包、镜像和插件可取，不等同于任一生产／客户节点已拉取 v7.4.6、重启服务或完成真实业务验收。

## v7.4.5 - (2026-08-19 07:45)

- **五仓历史、待提交工作树与自动合并按 v7.4.4 实际封版点完成去重审计**：逐仓执行 `fetch --all --prune` 后，以根仓 `dc20b7fdf54e67e733312105a137f5784206a8e3`、Microi.net `d758478c4c9a0e94f267efc5efde3f38b3d81fd0`、Microi.AI `f6d320e37eaa6b153a00b3b015edc1a884e03abd`、Microi.VSCode `a7b13fe07f46fbe73d8fdf39c5404fe8883cea77` 和 WebOS `cf8b4703b51f2ddade779db4081e6d6ee18b6e9e` 为边界；边界后没有新的可达提交、英文提交、自动 merge 包装或需重复归纳的文件差异。写本节前根仓有 47 个已跟踪差异文件和 5 个新增源码／测试文件，已跟踪净差异为 1,029 行新增、348 行删除，新增文件合计 234 行；Microi.net、Microi.AI 各有 1 个版本文件，净差异均为 3/3；Microi.VSCode 有 16 个已跟踪差异文件，净差异为 224/217；WebOS 工作树保持干净。根仓、Microi.net、Microi.AI、WebOS 相对各自上游均为 ahead 0、behind 0；Microi.VSCode 为 ahead 1、behind 0，该未推 v5.1.2 提交已在 v7.4.4 日志归纳，本次只随 v5.1.3 一并推送，不重复冒充新功能。`dist/bin/obj`、插件 bundle、应用包 Base64、Skills 元数据和版本清单只核对同源，不逐文件拆成独立更新；WebOS 无变化不制造空提交。
- **吾码官网应用流在上游 521／5xx 后停止自动死循环请求**：`ProductShowcase` 的可视哨兵、滚动兜底、窗口尺寸变化和 `IntersectionObserver` 在 `loadError` 或正在加载时都会停止继续取页，失败后只保留用户主动点击重试；修复本地 61503 对 `official_ai_apps` 失败响应反复自触发的问题，同时保留正常分页、推荐筛选和列表结束态。静态契约与浏览器故障注入覆盖“单次失败后不再自动重试、手动重试仍可恢复”的边界。
- **子租户 Logo 与官方品牌完成租户级隔离**：侧栏移除写死在容器底层的吾码官方 Logo，配置的租户 Logo 始终优先，只有 `OsClient=iTdos` 才允许使用官方缺省图；子租户资源加载失败时回退为当前租户标题首字，不再露出官方品牌。稳定图片节点、有限健康检查和同租户最多两次重载使主页与 `/microi-store` 路由切换保持同一品牌，修复乐闪购 Logo 与吾码 Logo 重叠、进入商城后被官方 Logo 替换的问题；本地连接生产 API 的 LSG Playwright 回归 1/1 通过。
- **索引管理改为以物理表结构为唯一候选来源**：自动索引推荐先读取真实列并按数据库类型过滤 text/blob/json/xml、空间类型和超长字符列，模块配置中不存在于物理表的搜索／排序字段不会进入候选；只有表中确实存在 `OsClient` 时才添加租户左前缀。`sys_apiengine` 不再因元数据中的 `OsClient`、`ApiKey` 等字段组合产生 6 个必失败索引，仍保留最多 6 个常见列表组合的优先级限制；真实页面回归覆盖自动添加成功且 `Failed=[]`，后端专项测试 9/9 通过。
- **表单字段说明由悬浮信息图标改为紧凑直显**：Label 左／右对齐时，字段说明以最多两行的灰色小字紧贴在 Label 与控件下方；Label 顶部对齐时，说明显示在 Label 右侧并使用单行省略及原文提示，避免窄列挤坏布局。移除原有 `i` 图标和 Tooltip 依赖，11px/15px 行高、30px 高度上限与紧凑间距兼顾可读性和同屏字段密度，相关源码契约和真实表单回归已覆盖三种对齐方向。
- **AI 平台治理收敛为一个入口和十页一体化工作台**：原父菜单改为唯一可见的“AI平台治理”并直接指向稳定 `/micro-app/ai-platform-studio/overview`，旧“AI平台治理工作台”和 40 个底层数据菜单继续隐藏保留安装、权限及兼容资源；门户、身份、用户组、配置、发布、服务、可观测、资产、协作和迁移等 40 项能力融入 10 个内部页面。新建门户等表单统一为大圆角、居中、可拖动且受视口边界约束的 Dialog，标题区包含图标、英文标识、大标题和副标题，右上关闭按钮使用可识别背景，内容区使用统一底色，底部关闭／保存为带图标大按钮；规范已同步到官方治理文档、`ui-design`／`ai-platform-governance` Skills 和 `MCI-DESIGN` 模板。
- **应用商城详情改为有界弹层与服务端版本分页**：详情遮罩进入宿主全局层并锁定 Body，玻璃效果覆盖完整框架；弹层默认居中，可拖动但始终保留至少 16px 可见边界，不再被拖出上下左右视口。历史版本接口新增 `_PageIndex`、5～20 的 `_PageSize` 与 `_Keyword`，当前版本固定单独返回，历史快照只查询当前页并返回 `DataCount`，详情页补齐搜索、翻页、页大小和加载／空状态，不再一次读取 500 条并在浏览器解析全部版本。应用商城资源包升至 v7.4.7、SaaS 引擎包升至 v7.4.6，平台内置微服务升至 CurrentVersion 20／v1.6.3。
- **官方应用商城传播与安装升级资源完成远端回读**：通过绑定 `https://api.itdos.com`、`OsClient=iTdos` 的 `microi_itdos` 回读，AI 平台治理已发布 v2.0.9，42 条相关菜单资源中仅 1 条“AI平台治理”可见，其余旧工作台和 40 条数据菜单均隐藏；平台内置微服务已发布 v1.6.3，运行清单中的 JS 为 278,936 字节、CSS 为 95,952 字节，商城版本接口正式端为 v1.1.1 并完整包含服务端分页／搜索逻辑。官方应用包已携带上述菜单、微服务、接口和兼容资源，其他租户通过商城安装或更新后可获得相同能力。
- **Microi.VSCode／CLI v5.1.3 完成三端构建、发布与公共回读**：版本和 Skills 同步到 VS Code、CLI、Codex、CodeBuddy、WorkBuddy 及插件清单；`npm run publish` 的生产构建、测试、VSIX 打包、npm 发布、Visual Studio Marketplace 上传和 Open VSX 发布全部成功。再次执行 `npm run publish:verify` 已确认 npm `@microi.net/cli@5.1.3`、Visual Studio Marketplace `Microi.v8-engine v5.1.3`、Open VSX `Microi.v8-engine v5.1.3` 三端均公开可取。
- **本地 API 以连续 10 分钟采样排除持续 CPU 高占用**：复用工作区受管的 61501 `dotnet run --launch-profile Microi.net.Api` 进程，602.3 秒内每 10 秒采样一次，共 60 组；按 16 逻辑处理器归一后的整机占用平均 1.40%、P95 2.50%、峰值 2.96%，高于 20% 的样本为 0，工作集由 1,038.6 MiB 降至 850.6 MiB、峰值 1,066.9 MiB。健康检查 60/60 返回 HTTP 200，平均 23.1ms、P95 32.8ms、峰值 120.8ms，因此本地没有复现持续高 CPU，也没有为未经复现的生产现象引入猜测性后端改动；`api.itdos.com` 容器的生产宿主、流量和数据库分析仍属于独立运维边界。
- **v7.4.5 全量编译与公共制品完成失败恢复和分层验收**：首次按输入 `7.4.5、4、0、2、2、0` 完成 19 个 NuGet 推送后，Docker 访问 MCR 遇到 TLS 握手超时；校验并预载官方 `mcr.microsoft.com/dotnet/aspnet:10.0` 的 linux/amd64 镜像后，复用同一 v7.4.5 以 `7.4.5、4、1、2、2、0` 跳过重复 NuGet 推送重试，7 分 34 秒、退出码 0 收尾。流程完成官方资源一致性、现代前端、压缩、Chrome 49 legacy、后端 Release（0 个错误、9 条既有 xUnit 分析器建议）、发布产物守卫、DLL 保护和独立 18080 liveness；NuGet.org Flat Container 已独立回读 v7.4.5 为 19/19。`microi-api:v7.4.5/latest` 与 `microi-api-dev:latest` 摘要为 `sha256:4d8bd2909a3881c87e4fbf67b7f337a5023c53a94da2d7abd06fc41437007235`，`microi-web-dev:v7.4.5/latest` 与 `microi-client-dev:v7.4.5/latest` 摘要为 `sha256:f3b02f05c9f96c00509503f7790a7c9ceaa113090edd24cd28ec18a01924a8ab`。关键 Node 契约 18/18、索引后端专项 9/9 通过；以上证明源码、公共包、镜像、插件和商城资源可取，不等同于任一生产节点已拉取 v7.4.5、重启实例或完成生产业务验收。

## v7.4.4 - (2026-08-19 03:11)

- **五仓历史、待提交工作树与自动合并按 v7.4.3 实际封版点完成去重审计**：逐仓执行 `fetch --all --prune` 后，以根仓 `79781472ddad81fb5217c2f25438dec3c9894dba`、Microi.net `f64bf5948c73cd2ae3a6d49b365d305039a192cd`、Microi.AI `6069066f85eee048478358c5cc5734de507dcc7e`、Microi.VSCode `a6f08330035dc590d3bcca8b3e8e3e2f0c362450` 和 WebOS `cf8b4703b51f2ddade779db4081e6d6ee18b6e9e` 为边界；审计时五仓相对各自上游均为 ahead 0、behind 0。根仓边界后的 5 个可达提交中，`5dfef5a48`、`74df59d36` 仅作为自动合并包装，3 个真实业务提交按最终树去重为 20 个文件、1,188 行新增、163 行删除。最终封版前根仓有 85 个已跟踪差异文件和 16 个新增源码／测试文件，已跟踪净差异为 6,976/699；Microi.net、Microi.AI、Microi.VSCode、WebOS 分别有 1、1、13、0 个状态项。`dist/bin/obj`、插件 bundle、Skills 元数据和版本清单只核对同源，不逐文件冒充独立功能；WebOS 无变化不制造空提交。
- **首屏动态路由、系统设置切换和平台外壳稳定性完成修复**：租户动态路由注册完成后会按当前 URL 重新匹配真实路由，不再被临时 404 捕获，直接打开或刷新 `/api-engine`、`/mci-full-test` 等菜单可正常进入；平台骨架只在外壳首次挂载前显示，从其它菜单进入系统设置不再触发全屏闪烁。侧栏 Logo 采用稳定节点、资源失败回退和重试机制，进入应用商城后不再消失；菜单悬停不再改变文字几何位置，减少导航时的抖动与卡顿感。
- **框架、表格和表单 Tabs 恢复 Element Plus 动画指示条并统一图标语义**：横向 Tab 使用主题色活动线及原生平移动画，左／右方向继续保留短线和小圆角，折叠／Tab 分组同时保留包裹字段的分组边界；配置过的 Font Awesome 图标可正确显示，图标与文字按同一基线垂直居中。没有配置图标时按序号从十组不同默认图标中稳定选取，避免全部页签使用同一个占位图标或问号，并覆盖框架多页签、模块页签、表单分组和右侧日志／评论／版本页签。
- **表单设计器字段配置弹层完成稳定拖动、居中和视觉统一**：新增统一 Dialog 运行时，以稳定标记和受控拖动保持大圆角、主题标题背景、毛玻璃遮罩及水平／垂直居中，修复拖动后瞬间退化成方角顶线弹层、部分控件无法拖动和首次打开闪过错误布局的问题。字段配置标题同时展示显示名称与字段名，并以小字补齐表中文名、物理表名、控件名称和字段说明；内容区改为单一背景，不再嵌套多余卡片，右上关闭按钮和标题垂直居中，底部取消／保存统一为带阴影的大号按钮。单行文本、页签分组、文件上传等真实配置弹层已纳入拖动与代码编辑回归。
- **字段独立配置、数据源性能与设计数据持久化系统性收口**：显式注册地址、颜色、图标、进度、二维码、评分等原生控件配置，双击字段只显示该控件独有参数，不再重复右侧已有的表名、字段名等通用属性。关联表单、数据源和接口选择器首次只取 20 条并支持防抖远程搜索，同时保留已选项；历史 Base64 SQL／V8 数据仅在确认为编码内容时解码，普通 SQL 不再被误转成乱码。字段复制保留有效配置并移除旧身份／运行态字段；值变更 V8 通过统一别名归一化写回物理 `Config`，修复 Autocomplete43 等控件保存后刷新代码丢失。
- **表单记录参数的前后端边界同步加固**：客户端统一把记录 Id 归一为有效标量，拒绝 `{}`、数组和其它错误结构；服务端 FormEngine 控制器对异常 RecordId 返回明确参数错误，不再让设计态关联表单请求携带 `Id:{}` 进入数据库并转成 500。缺少 `Component` 的 Id／系统字段按安全默认规则跳过无意义控件解析，避免打开全量测试表单时持续刷屏警告。
- **应用商城安装、版本读取和微服务绑定改为事务前完整校验与可恢复升级**：导入器在写入前预检包内及目标端重复菜单 URL、必需微服务应用包和页面路由，发现冲突即返回逐条失败详情并保持事务回滚；分块安装完成后重建 MicroService、页面和菜单绑定，恢复 `MicroServiceId/PageId`，避免更新到 98% 后 `/microi-store` 变成 `MICRO_APP_NOT_AVAILABLE`。商城源版本接口返回 HTTP 404 时兼容回退到当前可安装版本，详情元数据自动换行并消除横向滚动；应用资源包升至 v7.4.6，平台服务 CurrentVersion 19、BuildVersion v1.6.2，列表、模型、导入和批量安装接口同步升级。
- **系统日志队列和 MongoDB 故障隔离避免持续告警与内存膨胀**：主队列、溢出队列和紧急持久化 spool 均设置明确上限，未配置日志存储的租户直接跳过，失败租户按租户隔离并采用退避、节流告警和可恢复重放，不再拖累正常租户或无限输出“Microi日志批次持久化失败”。健康状态补充溢出、紧急落盘、丢弃和跳过计数；MongoDB 日志集合断路器按租户及月份隔离并保留租户大小写，相关缓存／升级回归覆盖队列满载、坏租户和恢复路径。
- **AI 工作台与自动合并带入的 UniApp 真实业务变更按最终树归纳**：AI 引擎页面降低大面积重色、边框和阴影，回归吾码清爽层级；全局加载页合并上下重复提示为一行随进度变化的状态文字。UniApp 的 3 个真实提交修复商城商品参数错乱，补齐“我的任务”字段、任务设备列表和地图展示，地图优先使用设备坐标并在缺失时回退客户坐标，同时绑定订单商品详情中的设备价格和滤芯价格；两个 merge 包装未重复计数。
- **Microi.VSCode／CLI v5.1.2 完成三端发布并修复 Windows 测试清理竞争**：Broker 生命周期测试先等待子进程退出，再对 `EBUSY/EPERM/ENOTEMPTY` 临时目录执行有界重试，仍未退出时才升级为强制终止，避免发布流程因工作区目录仍被占用而失败。首次 5.1.1 发布后，提交前门禁又发现 `v8-tcp-integration` Skill 尾部遗留空冲突标记；根仓权威 Skill 与插件副本同步清理、全量扫描归零后升至 5.1.2 重新发布。生产构建、CLI、Codex、Broker 和工作区单一来源测试均通过，npm `@microi.net/cli@5.1.2`、Visual Studio Marketplace `Microi.v8-engine v5.1.2`、Open VSX `Microi.v8-engine v5.1.2` 均已由各发布后台接收，公共端传播继续独立回读。
- **v7.4.4 全量编译与公共制品完成分层验收**：`Directory.Build.props`、Microi.Client、Microi.net、Microi.AI 和 19 个服务器公共组件统一升至 v7.4.4；`Microi一键编译发布.sh` 按输入 `7.4.4、4、0、2、2、0` 用时 13 分 35 秒并以退出码 0 收尾，完成官方资源一致性检查、现代前端、串行压缩、Chrome 49 legacy、后端 Release（0 个错误、9 条既有 xUnit 分析器建议）、API 发布产物守卫、Microi.net／Microi.AI 保护、依赖预检及独立 18080 liveness HTTP 200。NuGet.org Flat Container 已独立回读 v7.4.4 为 19/19；后端正式／测试标签摘要统一为 `sha256:eeb8e9772f6a1bcb09a018ca4b5175aabaccea15134016aae4ce7d1747082474`，前端 Web／Client 正式／测试标签摘要统一为 `sha256:c6e979bd4f8aea4bc2e679228bbc4ba8492716803c89850a6ec3cb1514d9def2`。发布锁、18080 和共享 61500／61501 监听均已清理；以上证明公共包、镜像和插件可取，不证明任何客户或生产节点已经拉取 v7.4.4、重启实例或完成真实业务验收。

## v7.4.3 - (2026-08-18 12:20)

- **五仓历史、待提交工作树与自动合并按 v7.4.2 实际封版点完成去重审计**：逐仓 `fetch --all --prune` 后，以根仓 `1d842bbc2d68e514072ecf4b9830e5fedc84b637`、Microi.net `5e06fe98c57772bd6156e4c46f48d5132a800057`、Microi.AI `75f5a0eb9aa16de3babd6efdab4e1f2d0e80ddfd`、Microi.VSCode `5727ffb9c3c5ba767b9f7f1faf079991525c3482` 和 WebOS `cf8b4703b51f2ddade779db4081e6d6ee18b6e9e` 为边界。根仓边界后的 9 个可达提交中，`280b80477`、`7fb26fdf6`、`e7709db3a` 只作为自动合并包装，最终树去重后为 32 个文件、3,057 行新增、426 行删除；其中英文提交 `feat: 完善表单工作台与平台安装体验` 已按中文功能归纳。写本节前根仓有 168 个已跟踪差异文件和 16 个新增源码／测试／资源文件，已跟踪净差异为 6,124/1,533；Microi.net、Microi.AI、Microi.VSCode、WebOS 分别有 4、3、27、0 个已跟踪差异文件及 0、1、2、0 个新增源码文件，已跟踪净差异分别为 146/42、6/3、608/354、0/0。审计时根仓相对 `origin/master` 为 ahead 2、behind 0，其余四仓均为 0/0；`dist/bin/obj`、插件 bundle、Skills 元数据、版本清单和测试 `TestResults` 只核对同源或排除生成物，不逐文件冒充独立功能，WebOS 无变化不制造空提交。
- **表格通用关键词搜索恢复清晰、等宽和亮暗主题一致的交互**：主搜索框左右操作按钮统一为 34px 固定宽度并消除多余内边距，刷新／搜索图标在明暗主题下使用可辨识的中性色和主色悬停反馈，输入文字与占位文字重新绑定主题前景色；不再出现关键词框比“名称、Key”等字段搜索更灰、更窄或两端按钮宽度失衡。客户端展示契约 12/12 和真实亮／暗主题 Playwright 1/1 通过，并保留两张验收截图。
- **吾码框架、表格、表单和右侧数据面板收敛为清爽的统一 Tabs 与卡片语言**：移除整条高饱和背景、满宽下划线和多余边框，横向／纵向 Tab 均使用轻量表面、柔和阴影和紧凑激活标记；左／右方向的激活线缩短并保留与侧栏菜单一致的小圆角。表格标题／副标题卡片与每个动态统计卡片彼此独立，数据日志、数据评论、数据版本增加真实数量角标；系统设置、员工表单、接口引擎等入口在亮色、暗色和主题色下共用同一套语义变量。
- **表单详情、加载、分组和字段设计器继续完成系统性修复**：字段搜索与当前记录刷新进入通用表单，分组支持四向导航、副标题和动态“x 项”统计；日期、单选、开关等 Label、控件和前后图标按同一基线对齐，字段间距更紧凑，数字步进按钮消除上下断层，必填错误在字段内部清晰显示，折叠分组按主题／视觉风格着色并为子表保留内边距。骨架屏移除成功态不存在的粗左线并按可视高度完整铺满；设计器新增显式控件配置注册表，Switch 等原生控件双击只展示自身配置，避免异步组件回退到 CollapseGroup／JsonTable／子表等错误面板，所有配置窗统一使用大圆角规范。FormEngine 后端按同一次请求并行返回日志、评论、版本 Counts，避免为角标重复请求。
- **Dialog、遮罩、系统 Logo、通知中心和首屏路由体验统一收口**：除 Drawer 外的业务弹层统一为可拖动大圆角 Dialog、无顶部装饰线；全屏遮罩使用顶层 Teleport 和滚动锁，默认毛玻璃，仅当全局或表级 `DisableFormMaskBlur=1` 时关闭。系统 Logo 采用稳定单节点和失败回退；首次进入系统设置不再因异步路由切换闪白，首屏 Loading 等待真实 `microi:app-ready` 并区分后端慢响应与失败。通知中心改为快速打开的 80% Dialog，Tab 内容延迟并行加载，右上角角标只汇总未读系统消息和未安装平台应用，平台消息直接在内容弹层阅读而不是跳转路由。
- **应用商城升级为同页完成发布、安装、历史版本和公私有多源联邦的完整工作台**：详情、来源管理、已安装应用、发布／离线包全部留在 `/#/microi-store`，旧 `microi-store-installed`／`microi-store-published` 菜单隐藏并回指统一入口；详情遮罩覆盖整个框架并锁定页面滚动，补齐预览图、分类筛选、标准分页／每页条数／页码跳转、已安装版本和彩色状态。应用新增 `IsPublic`，匿名只见公开应用，登录后可按所有权查看私有应用；每个来源显示含私有权限后的应用总数，安装时可从 `mci_data_version` 选择历史包版本。来源管理按 `ApiBase + OsClient` 自动发现系统标题和验证码，支持帐号密码登录，使用非 PC 长周期 DiyToken，并把跨源凭据加密保存在当前租户 `mci_system_setting`，代理调用不向浏览器泄露 Token；官方私钥运行态只显示“平台官方应用源”标记，不提供安装／更新自己。应用商城资源包升至 v7.4.2，包含 4 张表、140 个字段、4 个菜单和 19 个接口引擎；新增／升级列表、模型、版本接口，平台微服务 CurrentVersion 升至 18、BuildVersion 升至 v1.6.0。
- **租户公开配置与后端秘密形成单一、可验证的安全边界**：浏览器公开配置只投影 `sys_config` 物理字段，`mci_system_setting.IsPublic` 退役并强制全部后端私有；可信后端 V8 通过 `V8.SysConfig.ServerPrivateSettings` 读取秘密，不在根对象、匿名响应、日志或审计中暴露。`DisableAiAssistant`、`DisableFormMaskBlur` 和五项登录方式显隐迁移到 `sys_config` 物理字段并兼容旧数据，登录方式仍遵循“缺失／空值／1 显示，仅 0 隐藏”。SaaS、表单官方资源包、系统设置说明、服务端投影与安全回归保持同源。
- **SaaS 主租户启动诊断不再把数据库故障误报为 JWT 密钥为空**：Microi.net 在加载主租户配置时只对 Timeout、Socket、IO 和数据库连接类瞬时故障最多重试三次，非瞬时异常、结构不匹配和真实配置错误立即失败；主租户必须先成功挂载才进入 JWT 门禁。异常链保留可诊断的 inner exception，同时清洗数据库密码；系统监控识别新的启动数据库分类。新增启动诊断回归覆盖超时、重试、快速失败、密码脱敏和 JWT 顺序边界。
- **V8 新增面向设备、网关和工业协议的租户受控 TCP 客户端**：提供 `V8.Tcp.Send/SendAsync/SendAndReceive/SendAndReceiveAsync`，支持 Text、Hex、Base64、Byte 与 GB18030 编解码、连接／发送／接收超时、最大响应和 4 MiB 请求上限，结果可返回 RawBytes、Base64、Hex 与文本。能力接入 Jint 注册、客户端 Monaco／VS Code 类型提示、后端 V8 文档、`v8-tcp-integration` Skill 和 Microi.AI 向量资源，并以真实 loopback 测试覆盖同步／异步、分片读取、部分超时、截断、编码及大小限制。
- **AI/MCP/Skills 与模块生成规范同步本轮平台能力**：AI 新模块默认使用 80% Dialog，只有至少 36 个字段、两个以上子表或明确重型表单才建议 Drawer；背景任务分析器不会再把通过 `V8.OpenAppDialog` 打开的数据库备份等管理页误判为后台任务。Application Stream v3 在上传前校验 PascalCase 路由、规范路径、RoutePath 与 PageKey 唯一性；工作区规范强制 Vite 开发期优先 HMR，仅在全部工作完成后执行一次最终 `npm run build`，没有热更新时也只精确重启 61500 的 `npm run dev`。Microi.VSCode／CLI／Codex／CodeBuddy／WorkBuddy 同步新类型和 63 个 Skills，公开发布 v5.1.0，`@microi.net/cli`、Visual Studio Marketplace、Open VSX 三端均完成版本回读。
- **一键安装器支持安全复用客户现有 MySQL／MinIO，并归纳自动合并带入的 UniApp 真实业务变更**：安装时直接 Enter 仍默认新装服务；选择复用 MySQL 后会验证 5.7.x／8.0.x 精确版本、拒绝 MariaDB 和版本不符，目标库不存在时创建、为空时导入，检测到表／视图／过程／事件即停止，绝不覆盖非空库。复用 MinIO 会验证凭据、创建或复用私有／公有桶并回读匿名策略，客户密码／Secret 不在终端和失败摘要回显；本机服务通过 `host.docker.internal` host-gateway 访问，修复器保留外部连接而不重建客户服务。自动合并带入的 UniApp 最终树另包含项目方案快速新增安装点位、权限外客户查重、各 Tab 卡片展示规则和折叠面板完整分组初始化，重复 merge 包装未再次计数。
- **v7.4.3 全量编译、公共制品与传播回读完成**：`Directory.Build.props`、Microi.Client、Microi.net、Microi.AI 以及 19 个服务器公共组件统一升至 v7.4.3；`Microi一键编译发布.sh` 按输入 `7.4.3、4、0、2、2、0` 用时 13 分 09 秒并以 `__MICROI_RELEASE_EXIT__=0` 收尾，完成官方资源一致性检查、现代前端、串行压缩、Chrome 49 legacy、后端 Release（0 个错误、9 条既有 xUnit 分析器建议）、Microi.net.Api 发布产物守卫、Microi.net／Microi.AI 保护和独立 18080 liveness HTTP 200。NuGet.org 已回读 Microi.net／Microi.AI v7.4.3，19 个包均推送成功；后端 `latest/v7.4.3/dev-latest` 摘要统一为 `sha256:6958c2d992df93770f3973afef5b43b2a547830f9c3a6e9adb0ed335440b2b56d`，前端 Web／Client `latest/v7.4.3` 摘要统一为 `sha256:0af63036acd86d9d4430568a7f5981fcbe2403f19baaef855b85ddc566e1c71e4`。发布锁、18080 和共享 61500／61501 监听均已清理；以上证明公共包、镜像和插件可取，不证明任何客户或生产节点已经拉取 v7.4.3、重启实例或完成真实业务验收。

## v7.4.2 - (2026-08-17 05:16)

- **表单工作台与租户配置安全边界统一**：系统 Logo 改为稳定单节点并增加失败回退；表单遮罩和 AI 助手采用 `DisableFormMaskBlur`、`DisableAiAssistant` 负向开关，缺失/空值默认保留效果。现代表单补齐字段搜索、当前记录刷新、四向分组导航、副标题与动态“x 项”角标、紧凑控件/校验/折叠分组、完整骨架和设计器专项配置优先级。公开配置固定进入 `sys_config` 实体字段，`mci_system_setting` 只供后端并从 `V8.SysConfig.ServerPrivateSettings` 读取；平台微服务升级至 v1.5.10，SaaS 引擎包升级至 v7.2.16，表单引擎包升级至 v6.9.10。
- **五仓历史、待提交工作树与自动合并按 v7.4.1 实际封版点完成去重审计**：逐仓 `fetch --prune` 后，以根仓 `4a82a38fa00ea312c8b369e9aaaaae1f6ce9dc87`、Microi.net `deb7d8d54b80c2a10d2ab491c9f25a90c323c8a8`、Microi.AI `1c60fbd46cc7fbf51941a12c46594d85eb7e2e59`、Microi.VSCode `48465818c2e17b6053883eb714ef07294511d2af` 和 WebOS `cf8b4703b51f2ddade779db4081e6d6ee18b6e9e` 为边界，五仓均为 0 ahead／0 behind，边界后没有新增提交、英文提交说明、漏推记录、merge commit 或需要重复归纳的自动 merge 包装。写本节前根仓有 53 个已跟踪差异文件和 6 个新增源码／测试／资源脚本，已跟踪净差异为 4,191 行新增、542 行删除；Microi.net 有 3 个已跟踪文件和 1 个新增安全组件，已跟踪净差异为 294/3；Microi.AI、Microi.VSCode、WebOS 分别有 1、12、0 个状态项，已跟踪净差异分别为 3/3、271/271、0/0。`dist/bin/obj`、插件 bundle、Skills 元数据和版本清单只核对同源关系，不逐文件冒充独立功能；WebOS 没有历史或工作树差异，不制造空提交。
- **`diy-table` 移动端列表继续向 Microi.uniapp 的紧凑信息流收敛，同时完整保留动态业务能力**：统计卡、搜索区、卡片、标题、标签、字段、分隔线和操作区按窄屏重新压缩间距、高度与字号，选择框、编辑按钮、更多按钮和“查看详情 ›”在同一底栏垂直居中，44px 触控目标、两行省略和无横向溢出保持可用；选择、编辑、删除、还原、工作流以及 `_RowMoreBtnsOut/_RowMoreBtnsIn` 等动态 V8 行按钮继续走原权限和事件链，不因移动端视觉精简而丢失客户功能。卡片配置、图片解析、标题／底部标签和旧模块配置继续兼容，并补齐对应 SFC 与展示契约回归。
- **表单引擎新增可配置的通用 `FormWorkbench` 展现模式，并保留经典表格／表单入口**：`diy-form`、展示设计器、ViewSchema 运行时和状态／UI mixin 新增 `ViewSchema.List.Preset=FormWorkbench`，支持记录下拉切换、关键字搜索、分页、查看／编辑、新增、真实保存、表单 V8 事件和动态按钮；每个模块都可按需启用，而不是把系统设置写死成专用页面。系统设置官方配置采用 `SettingsCenter + Edit` 工作台，展示配置名称、系统标题和 API 地址等记录选择信息，允许返回经典列表，因此用户既能一眼浏览分组设置，也能明确当前正在编辑哪条配置。
- **应用商城升级为内置平台微服务承载的多源、可转发全能商城**：`/#/microi-store` 对应 `/marketplace` 页面，可在官方源、当前 `ApiBase + OsClient` 以及用户维护的其它上游源之间切换，支持 X 租户发布给 Y、Y 再发布给 Z 的链式分发，并提供应用发现、数据选择、包选择、安装、更新、重装、批量操作、离线包安装和来源管理；跨源凭据不透传，自安装冲突失败关闭。平台微服务 bundle 升至 v1.5.8、应用 CurrentVersion 升至 15，并由 `microi.routes.json` 确定性嵌入 `/create-empty-tenant`、`/app-store-data-selector`、`/app-package-selector`、`/offline-package-installer`、`/database-backup`、`/personal-settings`、`/system-settings`、`/marketplace` 八条完整路由。
- **系统设置、应用商城和官方资源包完成可升级交付**：新增幂等 `configure-modern-experiences.mjs`，安装或升级官方应用时为系统设置写入通用表单工作台展示配置、为应用商城写入平台微服务菜单配置；SaaS 引擎包升至 v7.2.15，包含 16 个接口引擎、2 个菜单、18 张表、664 个字段和 18 份 DDL，应用商城包升至 v7.3.19，包含 18 个接口引擎、4 个菜单、4 张表、139 个字段和 4 份 DDL。资源同步基线、正式包、升级包、物理列声明和现代体验配置测试保持同源，使独立部署及子租户通过应用商城更新后获得相同功能。
- **租户管理员初始凭据改为“所有权校验后一次性处理”，新密码全面使用不可逆专用哈希**：新增 PBKDF2-SHA256、210,000 次迭代、独立随机盐和固定时间校验的版本化密码格式，登录与修改密码同时兼容存量 DES／V8 编码，但数据库哈希不能被当作可复用客户端密码，也不能逆向显示。租户拥有者可在官网个人中心经认证、主租户和所有权检查后临时显示仍为旧可逆格式的管理员密码，或确认重置为密码学安全随机口令；重置使用分布式租约、原子写入、可信宿主会话吊销、60 秒前端清除和 `no-store` 响应，访问密钥会话、非拥有者和冲突请求全部拒绝，日志与审计不记录明文。
- **官网个人中心补齐安全的 Gitee 绑定迁移与应用详情体验**：当同一 Gitee 账号仍满足 Star 证据、原 Microi 账号没有 SaaS 租户且目标账号没有冲突绑定时，可经明确确认把绑定迁移到当前账号；原账号拥有租户、证据过期、并发建租户或身份冲突时失败关闭，迁移与租户创建共用分布式锁并以同批 FormEngine 更新保证一致性。个人中心同步加入管理员密码掩码、显示／隐藏、复制、随机重置和生命周期清除，中文／英文文案、响应式布局及应用详情页样式一并完善。
- **Application Asset Stream v3 与 AI 应用构建链补齐大文件可恢复发布契约**：MCP 的 `microi_upload_application_asset_stream` 统一进入可复用上传函数，完整携带路由／源码／运行时摘要、交付批次、请求指纹、门禁代次、应用与版本 fencing、行级 CAS 基线、活动／已提交版本和 dry-run 确认，拒绝符号链接、非普通文件和哈希不一致；v2 兼容路径继续保留。服务端新增 `microservice-page => mcimp-` 最终化投影记录类型并配套测试；AI 应用构建器升至 v1.6.9，同时修正跨域 MicroApp iframe 的 autoplay、fullscreen、gamepad 和 clipboard-write Feature Policy，导入包回归覆盖完整 Gitee 绑定恢复与多租户分发链。
- **Microi.VSCode／CLI／Codex 与多宿主插件公开发布 v5.0.9**：`npm run publish` 完成版本递增、生产构建、类型检查、Skills／MCP 组装和公开上传，`@microi.net/cli`、Visual Studio Marketplace、Open VSX 三端均已回读 v5.0.9；Codex、CodeBuddy、WorkBuddy 清单、构建元数据、内置 Application Asset Stream v3 MCP 和 Skills 版本保持同源。首次校验遇到 Visual Studio Marketplace 公共索引短暂仍为 v5.0.8 时没有重复上传不可变制品，待缓存传播后再次执行 `publish:verify` 即通过。
- **v7.4.2 全量编译、公共制品与传播回读完成**：`Directory.Build.props`、Microi.Client、Microi.net、Microi.AI 以及 19 个服务器公共组件统一升至 v7.4.2；`Microi一键编译发布.sh` 按输入 `7.4.2、4、0、2、2、0` 用时 12 分 20 秒并以 `__MICROI_RELEASE_EXIT__=0` 收尾，完成官方资源一致性检查、现代前端、串行压缩、Chrome 49 legacy、后端 Release（0 个错误、9 条既有 xUnit 分析器建议）、Microi.net.Api 发布产物守卫、Microi.net／Microi.AI 保护和独立 18080 liveness HTTP 200。NuGet.org Flat Container 已独立回读 v7.4.2 为 19/19；后端正式／版本／测试标签摘要统一为 `sha256:a4b972233ddfa0dbebaaf13a5d404c009815e17448ecaf133cbdd8b14eb12658`，前端 Web／Client 正式／版本标签摘要统一为 `sha256:ca4d6c91e75c1f79d21659242f14ca6b9de4e0577cf3ae535a9b78c5b852c852`。发布锁和 18080 监听已清理；以上证明公共包、镜像和插件可取，不证明任何客户或生产节点已经拉取 v7.4.2、重启实例或完成其真实业务页面验收。

## v7.4.1 - (2026-08-16 13:18)

- **五仓历史、待提交工作树与自动合并按 v7.4.0 实际封版点完成去重审计**：逐仓 `fetch --prune` 后，以根仓 `1c858196761dcf43a1f736349f9aebb3a2ec9293`、Microi.net `bfc4c95c965df7825c014135e37eec822522c69f`、Microi.AI `e6bca03ebaf09b8ac8a5f68cbf6d471d4d5125fe`、Microi.VSCode `a10fbddf9949d72756cd023422343ce42abe22e5` 和 WebOS `cf8b4703b51f2ddade779db4081e6d6ee18b6e9e` 为边界，五仓均为 0 ahead／0 behind，边界后没有新增提交、英文提交说明、漏推记录、merge commit 或需要重复归纳的自动 merge 包装。写本节前根仓有 51 个已跟踪差异文件和 3 个新增源码／测试／构建脚本，已跟踪净差异为 940 行新增、372 行删除；Microi.net、Microi.AI、Microi.VSCode、WebOS 分别有 5、1、16、0 个状态项，前三者已跟踪净差异分别为 18/15、3/3、215/211。`dist/bin/obj`、插件 bundle、Skills 元数据和版本清单只核对同源关系，不逐文件冒充独立功能；WebOS 没有历史或工作树差异，不制造空提交。
- **登录页五种登录方式改为“默认可见、仅显式关闭”并进入官方 SaaS 引擎包**：Passkey、身份验证器、Gitee、微信和 GitHub 在系统设置不存在、字段为空或值为 `1` 时全部显示，只有值为 `0`／`false` 才隐藏；能力尚未配置时保留入口并显示“未开启／待配置”，五项全部关闭时“登录方式”按钮仍保留，弹层给出明确空态。官方 SaaS 引擎资源由 v7.2.11 升至 v7.2.12，`mci_system_setting` 从 9 项增至 14 项，新增 `Login.Passkey.Display`、`Login.Authenticator.Display`、`Login.Gitee.Display`、`Login.WeChat.Display`、`Login.GitHub.Display` 五个公开布尔开关，默认值统一为 `1`；资源基线、安装包与升级包已同步，客户端契约 3/3 和包结构门禁通过。
- **`V8.SysConfig.PublicSettings` 从前后端协议中移除，权限边界改为根级字段投影**：浏览器端和匿名公共配置只把 `IsPublic=1` 且非敏感的系统设置平铺到 `V8.SysConfig` 根级，密码、Key、Secret、Token 及 `PwdV8` 不再因兼容结构泄露；后端接口引擎与后端 V8 事件仍可通过同一个根级 `V8.SysConfig[ConfigKey]` 读取当前租户全部启用设置，业务 Secret 在可信服务端解密后供程序逻辑使用，但不进入公共响应、日志或审计。配置快照按租户缓存 10 分钟，`mci_system_setting` 增删改会主动失效缓存，Redis 异常回退租户数据库；Microi.net 的表单入口、V8 运行时与调试会话同步使用该协议，服务端配置、敏感字段和跨租户边界定向回归 268/268 通过。
- **`diy-table` 移动端列表重排为紧凑、可扫读的卡片信息流**：统计区、搜索栏和卡片统一缩小外边距、内边距、圆角、头像、标题和字段间距；标题与标签同排，字段值支持两行截断，元信息和底部区域压缩为清晰层级。编辑、选择和更多操作由原来的大块双列按钮改为单行横向操作条，同时保留不小于 44px 的触控目标；“更多”使用图标化展示并保留无障碍名称，右侧增加明确的“查看详情 ›”，选择、编辑、批量处理、卡片图片和旧模块配置继续兼容。移动卡片与图片解析契约 5/5 通过。
- **MicroService 公共运行时与 AI 应用稳定入口完成可复现发布收口**：应用发布 `PublishTime` 统一为秒级 `yyyy-MM-dd HH:mm:ss`，避免写入 `varchar(25)` 时被带小数的数据库时间溢出；新增确定性平台服务 bundle 嵌入器，校验语义版本、256 个运行文件、5 MiB 上限和 SHA-256，只封装数据库运行资产而不夹带私有源码或源码 ZIP。平台服务 bundle 由 v1.5.6 升至 v1.5.7，应用当前版本由 13 升至 14，四项构建资产共 333264 字节，运行时摘要为 `d40a0a1feaba71d40578a62569ec1e84d22e3192b9f368ed63bf5027f424e7bc`；AI 应用构建器由 v1.6.4 升至 v1.6.7，官方商城包由 v7.3.16 升至 v7.3.18，稳定最新版入口改用全屏 iframe 指向 V3 当前版本解析器，历史版本地址继续保持不可变，Unity 桃源也通过稳定查询入口规避旧 CDN 重定向缓存。构建器与官网商城定向回归 11/11 通过。
- **Microi.VSCode／CLI／Codex 与多宿主插件公开发布 v5.0.8**：`npm run publish` 自动完成版本递增、生产构建、类型检查、CLI／Codex／共享 Broker／工作区单一规则测试、62 个 Skills 共 170 个文件组装、VSIX 与 npm 包校验；`@microi.net/cli`、Visual Studio Marketplace 和 Open VSX 上传成功，传播完成后 `npm run publish:verify` 已从三个公共端逐一回读 `5.0.8`。Codex、CodeBuddy、WorkBuddy 清单、bundle 元数据与根仓安全／SaaS／AI 应用／Unity／发布规范 Skills 保持同源。
- **v7.4.1 版本、定向回归与公共制品完成分层验收**：`Directory.Build.props`、Microi.Client、19 个服务器公共组件以及 Microi.net／Microi.AI 子仓统一升至 v7.4.1；客户端登录／移动卡片 8/8、商城／官网 11/11、服务端安全与租户边界 268/268 通过。`Microi一键编译发布.sh` 按输入 `7.4.1、4、0、2、2、0` 用时 8 分 12 秒并以退出码 0 收尾，完成官方升级资源哈希核对、现代前端、串行压缩、Chrome 49 legacy、后端 Release（0 个错误、9 条 xUnit 分析器建议）、Microi.net.Api 发布产物守卫、Microi.net／Microi.AI 保护、依赖预检及独立 18080 liveness HTTP 200；NuGet.org Flat Container 已独立回读 v7.4.1 为 19/19。后端 v7.4.1／测试标签摘要统一为 `sha256:d46a0150d97bbb78c731e61de720ee9828f4014a3bde01ed9aba936346c8fb14`，前端 Web／Client v7.4.1／测试标签摘要统一为 `sha256:1a6620d1977ba4376c3ebf673e8caceec54351deaa2286f7c6f95f136edc149a`；发布锁和 18080／61500／61501 监听均已清理。以上证明公共包、镜像和插件可取，不证明任何客户或生产节点已经拉取 v7.4.1、重启实例或完成其业务页面验收。

## v7.4.0 - (2026-08-15 21:15)

- **五仓历史、待提交工作树与自动合并按上一版实际封版点完成去重审计**：逐仓 `fetch --prune` 后，以 v7.3.9 实际提交的根仓 `1dd6fa7762ba072cabdd968d798f9a8e495fafe6`、Microi.net `8dee3c741570253e85199fcb3cf611a1e61eb141`、Microi.AI `418f5df472fd0c8a90b39505a73867f0d470d761`、Microi.VSCode `460269255e015da8623d8b906997ecfa78b84b9d` 和 WebOS `cf8b4703b51f2ddade779db4081e6d6ee18b6e9e` 为边界，五仓审计时均为 0 ahead／0 behind。边界后只有根仓存在 8 个可达提交，其中 6 个是 UniApp 真实业务提交，另外 2 个英文 `Merge branch master`／Gitee `!119` 只作为自动合并包装；按 `1dd6fa77..6a879ea1` 最终树去重后的净差异为 31 个文件、870 行新增、96 行删除，没有把合并时重复带入的 v7.3.9 平台变更再次计数。写本节前根仓有 91 个已跟踪状态项、4 个新增源码／测试／通用发布模板，已跟踪净差异为 91 个文件、597 行新增、448 行删除；Microi.net、Microi.AI、Microi.VSCode、WebOS 分别有 1、1、22、0 个状态项，VS Code 已跟踪净差异为 22 个文件、350 行新增、276 行删除。插件 bundle、Skills 元数据、版本清单、`dist/bin/obj` 等生成物只核对同源关系，不逐文件冒充独立功能；WebOS 无历史或源码差异，不制造空提交。
- **Microi.Client 内容 Loading 全面收口为保持几何的统一骨架系统**：新增全局 `v-mci-loading:<variant>` 指令和 `openMciLoading()` 服务，提供 `table/cards/form/detail/page/stats/list/tree/compact` 九种语义布局，并以引用计数维护并发服务、`aria-busy`、`aria-live`、宿主定位和全屏滚动锁。菜单路由守卫统一启动／结束页面骨架，90 ms 延迟显示与最短 180 ms 可见时间共同过滤瞬时闪烁；首页、`diy-table`、表单详情、统计卡、后台任务中心、工作流、AI、聊天、文件管理、3D/CAD/DWG、Page/Go/Print/Blueprint/StateMachine、系统管理、树和右侧面板等 46 个源码文件共 82 个挂载点已使用语义骨架。全源码静态扫描确认内容型 `v-loading`、`ElLoading.service` 与 `element-loading-*` 均为 0；第三方仍可能创建的 Element Plus Loading 也由不透明骨架样式兜底，不再显示截图中的半透明黑幕。
- **骨架颜色与平台亮色、暗色及租户自定义主题同源**：`theme-color.js` 从当前表面层级和品牌主色低饱和派生 `--mci-skeleton-surface/card/header/base/highlight/accent/border`，亮色、暗色、OpenClaw 和 12 套内置／自定义调色板均消费同一语义令牌；主色只用于低占比边缘与高光，内容表面保持不透明。桌面、窄屏和移动端会按几何重排，`prefers-reduced-motion` 自动关闭 shimmer；表格首屏／筛选重载使用完整表头行骨架，移动追加只在底部显示紧凑骨架，请求完成前不再抢先渲染“暂无数据”。`ui-design` 与 `microi-client-frontend` 权威 Skills 同步固化覆盖范围、主题令牌、空态、真实进度和浏览器验收边界，使其它吾码用户安装或更新插件后可继续遵循同一规范。
- **远程媒体加载与业务操作反馈不再混用黑幕或伪进度**：导航、AI、移动个人中心和用户管理头像改用圆形骨架；滑块验证码、私有图片、上传预览、表格图片／附件和文件验证码使用媒体或行内骨架，并在渲染层拦截历史 `loading.gif` 哨兵，避免把占位文件当真实网络资源请求。保存、提交、删除、登录验证等短操作继续保留按钮内 Loading，文件上传、后台任务等已有可信百分比继续显示真实进度，不用骨架伪造可量化进度。骨架／主题定向契约 12/12 通过，覆盖全源码遗留扫描、九种几何、路由／表格／表单、全部亮暗调色板、远程媒体、无障碍与减少动效；完整现代前端、逐文件压缩及 Chrome 49 legacy 构建也在正式发布流程中通过。
- **自动合并带入的 UniApp 六项真实业务修改按最终树一次性归纳**：跨端列表和卡片恢复按平台的旧式兼容配置排列字段，移动状态字段与卡片标题首字段遵循模块配置；客户新增／编辑页的关联列表与详情页统一为完整列表布局。列表与卡片“更多”动作新增受菜单、表和角色权限共同约束的删除入口，拜访打卡按当前用户与角色收紧数据范围，仅主管、总经理和管理员可查看全部记录；Token 过期改为单次登录导航，避免并发请求重复弹出登录页，并同步修正 xjy 租户菜单。相关提交说明均已转成中文语义归纳，两个自动 merge 提交及其中已在 v7.3.9 记录的平台文件不重复列为新功能。
- **Codex 自动打开脚本文件和多对话进程膨胀的根因在 v5.0.7 修复**：CLI 与 VS Code 初始化器现在识别 `Code.exe`、Cursor、Windsurf、Trae 和 Electron GUI 宿主，只要把 GUI 可执行文件作为 stdio `command`，便在 `.vscode/mcp.json`、`.cursor/mcp.json`、根 `.mcp.json` 及 `.codex/config.toml` 同步写入 `ELECTRON_RUN_AS_NODE=1`，避免 Windows 将 `mcp-codex-stdio-adapter.js` 当普通文件交给编辑器打开并额外拉起 GUI 进程树；Codex 继续只生成一个路由块并复用共享 Broker，不按 Profile 扩散真实 MCP。首次 `npm run publish` 自动升至 5.0.7 后，在任何外部上传前因活动 bundle 的 Windows `EPERM lstat` 中止；构建器随后改为复用带重试的原子运行时替换并新增禁止“先删后拷”的回归门禁，在同一版本使用 `--no-bump` 重试成功。生产构建、类型检查、CLI／Codex／Broker／非阻断更新与组装测试全部通过，`publish:verify` 已从 npm、Visual Studio Marketplace、Open VSX 三个公共端回读 5.0.7。
- **AI 门头文案、Codex／DSH 能力说明及发布资源保持一致**：官网 AI Studio、中文首页、AI 平台治理、V8 AI 开发文档和 v7.3.9 历史口径统一去掉容易造成重复限定的“典型业务场景”前缀，同时保留 10 倍目标的适用边界说明；Codex Skill 补充 DeepSeek Harness 共用配置、Token、MCP 与非阻断更新的边界。根仓内置 Skills 和 Microi.VSCode 插件副本通过同一构建源同步，元数据与 bundle 变化不重复计算为业务功能；新增的无凭据 `payload.json` 只是一份通用文章发布占位模板。
- **v7.4.0 公共制品发布完成，并与生产切换边界分开陈述**：`Microi一键编译发布.sh` 按输入 `7.4.0、4、0、2、2、0` 执行 12 分 44 秒并以 `__MICROI_RELEASE_EXIT__=0` 收尾；官网升级资源双端哈希一致，现代前端、串行压缩、Chrome 49 legacy、后端 Release（0 个错误、9 个 xUnit 分析器警告）、Microi.net.Api 发布产物守卫、Microi.net／Microi.AI 保护、iTdos MySQL／Redis／MongoDB 预检及独立 18080 liveness HTTP 200 全部通过。NuGet.org Flat Container 已独立回读 v7.4.0 为 19/19；阿里云后端 `latest/v7.4.0/dev-latest` 三个标签统一为 `sha256:1c0dcfd149544908e3e3ead6700d169afe8d1fc50a10c90bc3a5cd1277c6ec3a`，前端 Web／Client 的四个正式／测试标签统一为 `sha256:7c65c11c6aeade8307919cab9b7160574de7fdb286f740449869f93afc5d8dbd`。发布锁、18080、61500 和 61501 监听均已清理；以上证明公共包、镜像和插件可取，不证明任何客户或生产节点已经拉取 v7.4.0、重启实例、完成长时间 CPU 实测或通过业务页面验收。

## v7.3.9 - (2026-08-15 00:44)

- **五仓提交／推送历史与当前工作树按最终树完成去重审计**：以 v7.3.8 封版后的根仓 `5f83ae1b7ffa28829ecd110c52dcb9ae99168ec7`、Microi.net `0ea7a845a1d73d30783c3092ca9a1182fb592769`、Microi.AI `f045204eac60d1a5b93a4d469a2d9379bb9f42c0`、Microi.VSCode `28292021e51fb0ac358eccd0933f85ffba6fba67` 和 WebOS `cf8b4703b51f2ddade779db4081e6d6ee18b6e9e` 为边界逐仓执行 `fetch --prune`；审计时五仓均为 0 ahead／0 behind，边界后没有新增提交、漏推记录、英文提交说明、merge commit 或需要重复归纳的自动 merge 包装。写本节前根仓有 60 个已跟踪状态项和 1 个新增测试文件，净差异为 60 个文件、713 行新增、198 行删除；Microi.net、Microi.AI、Microi.VSCode、WebOS 分别有 1、1、14、0 个状态项。`microi.mcp/dist`、PNG、插件 bundle、Skills 元数据、版本清单及其它编译生成物只核对同源关系，不逐文件冒充独立功能；WebOS 无源码或历史差异，不制造空提交。
- **吾码 AI 平台门头定位统一为有边界、可验证的工程表达**：官网 AI Studio 首页原副标题、官方文档首页、根 README、AI 平台治理与 V8 AI 开发文档统一为“**相比传统 AI 开发：Token 更省 10 倍+、交付更快 10 倍+、20+ 成熟引擎开箱复用；深度融合 V8 引擎，业务逻辑无需编译发布。**”它把“速度”明确为完整交付，把“无需编译发布”限定为 V8 承载的业务逻辑，并明确实际结果仍取决于需求匹配度、模型、上下文质量、团队熟练度、定制深度和验收范围，避免把典型目标写成无条件承诺。<a href="https://blog.csdn.net/qq973702/article/details/163763831" target="_blank" rel="noopener noreferrer">门头文章《为什么 AI 开发的 Token 和交付时间，能同时降一个数量级？》</a>已接入各关键入口并强制新窗口打开，发布后直接回读为 HTTP 200、标题一致。
- **AI 平台架构图同步成为 v7.3.9 的可检索门头资产**：左上角标题新增 `v7.3.9` 圆角版本徽标，右上角四张价值卡收敛为“10×+ Token 更省、10×+ 典型交付更快、20+ 成熟引擎复用、在线生效／V8 无需编译发布”，图内继续只呈现 V8 引擎运行核心。SVG、1920×1080 PNG、3840×2160 PNG、README 机器可读能力索引和 SHA-256 Manifest 均由同一数据源重绘；253 个唯一能力标签、精确尺寸、文件体积与输出哈希 4/4 通过，并完成 1080p 原图人工检查，无文字截断、遮挡或版本漂移。
- **历史空 `sys_osclients` 主库获得最小、安全且多节点收敛的租户引导锚点**：JWT 签名密钥协调器完成物理结构修复后，仅在整张 `sys_osclients` 表完全无记录且宿主 `OsClient` 非空时，使用 `OsClient + OsClientType + OsClientNetwork` 规范化摘要生成确定性主键，以 `WHERE NOT EXISTS` 竞争插入一条启用主租户；密钥仍由每个节点独立生成 48 位密码学随机值，竞争失败、提交响应中断或并发主键冲突后只接受数据库中租户三参数完全匹配、状态有效且强密钥回读成功的权威行。表内已有任意启用、停用或删除记录时均禁止自动补租户，防止配置拼写错误静默创建新租户；后续继续复用既有密钥收敛流程，无法建立持久锚点则失败关闭。
- **Application Asset Stream v3 统一走可恢复传输，并可把已完成大资产原位登记为公共运行时**：MCP 对 v3 的大小文件一律使用持久化 multipart 会话，分片、完成、断线重连与已上传 Part 跳过采用同一状态机，不再依赖强制可恢复环境开关；旧 v2 仍保留 128 MiB 单请求上限和兼容路径。应用构建器新增 `RegisterResumablePublicDownload` 及兼容别名，在会话已经 `Prepared/Succeeded` 时严格校验租户、应用、版本、路径、大小、SHA-256、进度、存储范围和不可变目标根后登记公开 URL，不再次搬运或复制大对象，并把登记时间、路径和 URL 写回会话审计备注。构建器本地副本升至 v1.6.4，官方应用商城资源自动升至 v7.3.16；第一次定向测试发现包内元数据仍为 v1.6.3 后，使用三方同步器修正、发布、六项资源逐项哈希回读并二次零变化验证，最终构建器契约 7/7、MCP 全量 121/121 通过。
- **应用发布配额与普通交互上传彻底分仓，且如实记录发布中途到达的不可变制品边界**：第二轮发布的前端构建期间，4 个服务端源码／测试文件于 00:34:56 到达当前共享工作树；最终后端编译已将 `ApplicationPublishQuota` 独立 Redis 命名空间、ASCII scope 白名单和应用发布绝对日配额写入 Docker，避免受控应用发布挤占用户普通 `UploadQuota`，相关上传安全、v2/v3 资产流回归 134/134 通过。该变更晚于首轮 v7.3.9 NuGet 不可变包上传，NuGet.org 不允许同版本覆盖：公开 `Microi.Core 7.3.9` 解包确认尚无该 scope，而最终源码与 Docker 已包含；因此本项只宣称进入 Git 与最终 Docker，不虚构为已覆盖同版本 NuGet，后续 NuGet 补丁版本将自然承接。公开 `Microi.Upgrade 7.3.9` 则已解包确认两项可恢复登记函数都存在，仅内嵌引擎版本头仍为 v1.6.3；在线官方资源与最终 Docker 已统一为 v7.3.16／v1.6.4。
- **官网 AI 应用商城补齐推荐发现、稳定翻页和新窗口体验闭环**：新增紧跟“全部”的虚拟“推荐／Recommended”分类及可分享 `category=recommended` 查询状态，请求转换为独立 `Recommended=true`，不污染后端真实业务分类；单页由 12 提升到 24，并以接口回传 PageSize、总数和去重后已加载数判断下一页。IntersectionObserver、滚动／缩放 `requestAnimationFrame` 兜底、手动“加载更多”、进度与明确结束态共同防止大应用库在不同浏览器或缩放级别下停载；按钮补齐 hover、键盘焦点和无障碍状态。应用详情与“立即体验”均使用 `noopener,noreferrer` 新窗口打开，Unity 桃源使用公有桶稳定最新版入口，避免相对 v3 预览地址被错误解析到官网域名；推荐接口、列表加载、查询状态、详情新窗口与预览地址定向测试 18/18 通过。
- **Unity WebGL 构建与 Unity 6 输入兼容不再依赖单一工程假设**：`MicroiWebGLBuildUtility.Build` 新增可选 `WebGLCompressionFormat` 参数并保持 Gzip 默认值，调用方可按 CDN／服务器能力显式选择压缩格式；文档和内置 Skill 同步说明 Unity 6 Player 编译响应未引用 WebGL 模块时，应使用受 `link.xml` 保留的反射路径关闭浏览器键盘独占，并以真实键盘、数字、标点、Shift 与 IME 组合输入 E2E 验收，不能用编译成功代替输入行为通过。
- **Microi.VSCode／CLI／Codex 与多宿主插件发布 v5.0.6**：`npm run publish` 自动完成版本递增、生产构建、类型检查、测试、VSIX 与统一插件包重组，CLI、Codex、CodeBuddy、WorkBuddy 和内置 Unity Skills 同步使用 v5.0.6；npm `@microi.net/cli`、Visual Studio Marketplace 与 Open VSX 传播完成后，`npm run publish:verify` 已从三个公开端逐一回读 `5.0.6`，没有把上传命令成功或传播期旧缓存当成最终可安装。
- **v7.3.9 版本、定向回归、公共制品与运行边界完成分层验收**：Microi.Client、`Directory.Build.props`、根仓公共组件及 Microi.net／Microi.AI 子仓统一升级至 7.3.9；服务端租户密钥 42/42、应用发布配额与资产流 134/134、应用商城构建器 7/7、MCP 121/121、官网／架构图 18/18 均通过。`Microi一键编译发布.sh` 首轮按 `7.3.9、4、0、2、2、0` 成功后，针对资源元数据漂移复用同一版本完整重跑，最终轮 9 分 34 秒、退出码 0，完成现代前端、串行压缩、Chrome 49 legacy、后端 Release、受保护 DLL、独立 18080 liveness HTTP 200、19 个 NuGet 和前后端正式／测试 Docker；NuGet.org Flat Container 已直接回读 19/19。后端 `latest/v7.3.9/dev-latest` 三个标签统一为 `sha256:cf40694cf6291f0e35a9e37a8cc87bf06c8f9f6838054e6d68b37cec43858f81`，前端 Web／Client 的四个标签统一为 `sha256:0183500654e460617e7bc17f027ec2e3b466b83b8e123cd49b81e397a5083db5`；发布锁和 18080／61500／61501 监听均已清理。以上证明公共包、镜像、插件和文章可取，不证明任何客户或生产节点已经拉取 v7.3.9、重启实例或完成业务页面验收。

## v7.3.8 - (2026-08-14 14:33)

- **五仓提交／推送历史和当前工作树按最终树完成去重审计**：以 v7.3.7 日志封版后的根仓 `050e959c70e8cc299607af7d33f9a7e697c17239`、Microi.net `46dd2e598eac9f04b21d004a677591a744a8683d`、Microi.AI `c8927f6238a51b4767ff21c09da20666fc51c016`、Microi.VSCode `6bd19a8cc86f9118a6415868934f6fa2420f6662` 和 WebOS `cf8b4703b51f2ddade779db4081e6d6ee18b6e9e` 为边界逐仓执行 `fetch --prune`，审计时五仓均为 0 ahead／0 behind。边界后只有根仓 `84922a97`（完善 Unity v1.4 与应用商城大资产交付）、`1f303471`（忽略 Unity 本地素材缓存）、`29a5cc54`（英文 `chore` 说明已译为“整理 Unity 工程并排除私有 V8 源码”）3 个线性提交，没有 merge commit 或自动 merge 包装需要重复归纳。写本节前根仓有 43 个已跟踪差异和 1 个新增测试文件，Microi.net、Microi.AI、Microi.VSCode、WebOS 分别有 2、1、29、0 个状态项；VS Code 的 29 项中有 5 个新增 DSH 源码／测试文件。`dist/bin/obj`、VSIX／npm bundle、生成后的插件脚本和 Skills 元数据只核对同源性，不逐文件冒充独立功能；WebOS 没有源码或历史差异，不制造空提交。
- **旧租户 DiyToken 签名密钥从“发现不一致”升级为启动前可恢复且失败关闭的自愈流程**：JWT 协调器先按 MySQL、Oracle、SqlServer／SqlServer9 方言检查并补齐 `sys_osclients.AuthSecret`、`AuthSecretRotateVersion` 字符列，再以物理结构回读作为继续启动的门禁。某租户全部运行变体仍为空或弱密钥时，选择最早稳定记录作为引导锚点，生成 24 字节密码学随机强密钥，以旧值比较交换写入并回读并发胜者，然后复用既有多节点收敛协议同步 Product／Internal／Internet 变体；结构或最终密钥仍不可靠时拒绝签发 Token。Microi.net 子仓统一复用同一强密钥生成器并补充 `Generated`／`SchemaUpdated` 诊断，避免两套实现漂移；Microi.AI 子仓同步 v7.3.8 程序集、文件和 NuGet 版本。
- **MicroService 源码同步保留既有应用的真实运行类型**：`SyncMicroServiceSource` 在调用方省略 `ApplicationType` 时，不再把既有 Web 或 UniApp 应用误改成 MicroService，而是回读并保留当前规范类型；显式类型继续规范化，非法值继续失败关闭。新增 4 项契约回归，覆盖既有 Web／UniApp 保留、显式覆盖和非法类型拒绝，防止一次私有源码同步改变线上路由与构建协议。
- **应用商城的大资产交付收口为不可变公共运行时、严格安装和有界旧入口兼容**：Web／UniApp 协议 v3 可发布 `SharedPublicRuntime`，入口固定为当前租户、应用和版本下的 HTTPS 不可变地址，并同时绑定已提交 Manifest SHA-256；超大运行时不再重复经过 Base64、Jint ZIP 或目标租户 HDFS，普通自包含离线包仍保持原有路径。统一导入器由 v1.10.11 升至 v2.0.0，校验共享运行时的应用、版本、路径和摘要后才计入进度；发布器升至 v1.8.2，支持零菜单 AI 应用、纯共享运行时包和已提交版本 CAS 回钉，避免普通菜单编辑的自动增版污染不可变指针。构建器升至 v1.6.0，只允许为当前应用发布 4 个有界 legacy 重定向入口并按精确路径刷新 CDN，拒绝通配符、跨应用和非当前版本；官方应用商城包同步至 v7.3.14。首轮回归发现一个测试仍硬编码旧导入器版本，已同步为 v2.0.0 后复测通过，没有用删除断言掩盖版本漂移。
- **Unity 桃源 WebGL v1.4.4 与 UPM SDK v1.0.1 补齐真实输入、字幕和宿主安全边界**：桃源新增中英双语、与音频时间线同步的字幕，支持真实键盘中文、数字、标点、Shift 和 IME 组合输入，并关闭 Unity 对浏览器全部键盘事件的独占；零平台侧栏菜单、10 个宿主 API、Unity 6 浏览器兼容和公开下载说明同步更新。SDK 应用宿主上下文时，空 `ApiBase`、`OsClient`、`Did` 不再擦除调用方安全默认值，但退出／清理授权仍可显式生效。Unity 测试工程的 `Packages`／`ProjectSettings` 归位到 `Microi.Unity/TestProject~`，本地 `.asset-cache` 被 Git 排除；公开根仓同时移除三个租户私有 AI 应用的 V8／微服务源码，属于公私源代码边界清理，不代表删除目标租户已发布应用，也不把其中已生成的 `dist` 删除量计为功能更新。
- **Microi.VSCode／CLI／多宿主插件最终发布 v5.0.5，并新增 DeepSeek Harness 原生接入**：唯一 npm 包 `@microi.net/cli` 声明 `dsh.bundle.patch=./cordis.patch.yml`，复用官方 `@deepseek-ai/dsh-mcp-client` 和现有 `microi_codex` 路由器，不复制第二套连接、Token 或 Microi API。CLI 新增 `microi dsh install/status`，默认按 DSH 官方协议幂等安装并回读 `web`、`headless` profile 的精确依赖、安装版本、激活层和 bundle 文件；引导脚本按 `DSH_HOME/profile` 定位同版本包，拒绝越界 patch、本地伪包和版本漂移。非阻塞自动更新在检测到 DSH 时同步 bundle，DSH 不存在则安静跳过；Windows `EBUSY` 时可用当前版本隔离包预置后延迟全局替换，不终止当前 Codex Router、DSH 会话、MCP 或用户任务。README、CLI 文档、官网页、生成规则和安装 Skill 同步说明多宿主边界；版本由 v5.0.3 经已有 v5.0.4 继续补丁发布为 v5.0.5，npm、Visual Studio Marketplace、Open VSX 传播完成后均已公开回读 5.0.5。
- **版本元数据、定向回归和一键发布按最终源码完成验证**：Microi.Client、`Directory.Build.props`、根仓 17 个公共组件以及 Microi.net／Microi.AI 独立子仓统一升级至 v7.3.8。JWT 结构／密钥收敛、SaaS 配置和应用类型同步 .NET 定向测试 44/44，通过；应用商城构建、发布、导入和安装契约 80/80，通过；DSH 原生安装／状态及真实 MCP `tools/list` 2/2，通过；VS Code v5.0.5 的生产构建、CLI／Codex 组装和三端发布回读亦通过。
- **v7.3.8 公共制品发布完成，但不把公开可取等同于生产切换**：`Microi一键编译发布.sh` 按输入 `7.3.8、4、0、2、2、0` 运行约 13 分 35 秒并以退出码 0 完成官网资源预检、内存保护前端构建、Chrome 49 兼容产物、后端 Release、独立 18080 存活探测、受保护程序集、19 个 NuGet 及前后端 Docker 推送。NuGet.org Flat Container 已逐包直接回读 19/19；后端 `v7.3.8`、正式／测试 `latest` 三个标签统一为 `sha256:a83891253cf94c20eca940b58ed5d9ff42e6d1277b5c3ed4407b0fa08cbf7cde`，前端两个 `v7.3.8` 与两个 `latest` 标签统一为 `sha256:88d60466f28f87f3c8c369f01adcaedc222af0e09129314710dfc53919b1f23d`。发布锁、18080、61500 和 61501 监听均已清理；这些证据证明公共包和镜像可下载，不证明任何客户或生产节点已经拉取 v7.3.8、重启实例或完成业务页面验收。

## v7.3.7 - (2026-08-14 05:31)

- **五仓历史、待提交代码和自动合并按最终树去重封版**：以 v7.3.6 封版后的根仓 `81457b161df329cfb91a2570a40bf6aad5bbb3ed`、Microi.net `26ef78956f473b0a3bc8da43a4979a215791cd0b`、Microi.AI `aebbf67cd91798fa7d647ed24e06d6be0b1b0dae`、Microi.VSCode `261b6d90ccdd8c9ee8a4cccce7199494ab832801` 和 WebOS `cf8b4703b51f2ddade779db4081e6d6ee18b6e9e` 为边界执行 `fetch --prune`，五仓封版前均为 0 ahead／0 behind。边界后只有根仓 8 个真实业务提交和 2 层自动 merge 包装，合并后最终净差异为 `microi.uniapp` 22 个文件、1386 行新增、393 行删除，没有把 merge 提交再重复记录。写本节前根仓有 125 个状态项（93 个已跟踪、32 个未跟踪），其中 63 个 `dist` 生成物只核对同源性、2 个 `.asset-cache` 预览缓存明确不入版；Microi.net、Microi.AI、Microi.VSCode、WebOS 分别有 1、1、27、0 个状态项，前两者为 7.3.7 版本元数据，WebOS 不制造空提交。
- **UniApp 客户详情与拜访打卡从“能用”提升到完整业务闭环**：拜访对象类型、对象组件与当前用户选择现已按数据权限绑定，打卡记录保存经纬度并可回显地图，跟进时间与原生表单联动同步修正。客户详情的订单、设备、跟进等 Tab 改为原位完整列表，补齐订单金额和关联数量统计、独立滚动、分页加载与空态，不再依赖“查看更多”跳转；新增客户方案对比页和访问对象通用组件，同步增加当前用户、定位坐标、目标选择、订单金额、关联指标和 Tab 全列表契约检查。
- **PC 端本地运行目标支持可审计的 URL 级 `ApiBase` 与 `OsClient` 切换**：新增纯函数解析器，以当前 URL Query 为最高优先级，严格限定 HTTP(S) API 地址和安全租户标识，重复冲突、非法协议、含账密的 URL 和不同源误用均失败关闭。`main.js`、`diy.common.js` 与 `itdos.osclient.js` 统一使用该事实源，只在 `window.__MICROI_RUNTIME_ENDPOINT__` 暴露冻结的公开诊断信息，不包含 Token、密码或 Provider Key；本地运行文档、微服务宿主 Skills 与 Playwright CI 同步约束浏览器上下文隔离，测试覆盖显式覆盖、回退、非法值和冲突场景。
- **5GiB 应用资产与阿里云 OSS 大文件通道收口为可恢复、可定位的持久化流程**：Multipart 完成阶段改用可验证、可 Seek 且 `DeleteOnClose` 的临时文件组装，避免匿名管道的 `Broken pipe` 掩盖真实存储错误；阿里云 OSS 按 16MiB 有界 Provider Part 处理 5GiB 对象，大文件和不可 Seek 流走稳定直写通道，单操作上限收紧为 2 小时，ASP.NET 流使用真正异步读取。错误详情现在精确指出阶段、Part、偏移和恢复建议；完成请求的调用方 HTTP 连接中断不再取消已进入持久化的作业，进度、心跳与终态仍持续落库；针对性 .NET 回归覆盖组装流、大文件、取消和终态语义。
- **Application Asset Stream v3 的首发、多租户、跨数据库与管理员边界继续失败关闭**：产品总容量不再被旧版固定上限误拦，单文件大小仍必须落在 JavaScript 安全整数范围；首次发布允许空基线指针并保持并发前置条件，租户运行 Key 按规范大小写解析，出现大小写歧义则拒绝发布。路由快照纳入门禁，日期字段显式使用 `DbType.DateTime`，HTTP 边界将 `ExpectedCurrentVersion` 恢复为严格非负整数；平台管理权同时兼容 Level 999 和 9999。私有源同步只管理 `StorageScope=Private` 及兼容私有记录，不再误删 multipart 与审计控制行；MCP v3 完成回执可用匹配的 AppId 或 AppKey 证明身份，不再假设请求一定使用 AppKey。
- **MicroService 私有源码排除规则从任意 glob 收紧为安全目录前缀**：`.microi-micro-app.json` 的 `SourceExcludes` 只接受最多 100 条 `<relative-directory>/**`，拒绝根目录、文件类 glob、路径逃逸和版本库目录，默认忽略 `.codex-temp`。MCP 源码扫描、官网微应用文档、运行时交付 Skill 与同源测试已同步，既允许排除大型构建资产目录，又不会因过宽匹配静默丢失应发布源码。
- **VS Code／CLI／Codex 错版共存修复并以 v5.0.3 完整纠偏发布**：Codex Router 在存在有效安装包版本时，不再被父进程残留的 `MICROI_TOOL_VERSION` 覆盖，因此新插件不会误连旧共享 Broker；只有无包的稳定 CLI 启动才允许使用环境版本。首轮 `npm run publish` 已将当时完整源码发布为 5.0.2；官网／Skills 严格覆盖审计随后发现 `PersistMiniMaxVideoFile` 路由缺少 Skill 边界说明，补齐“只持久化当前用户已完成任务到当前租户 HDFS，禁止任意 URL 搬运”后，不覆盖旧包，而是将 CLI、VS Marketplace、Open VSX、55 条命令、62 个 Skills 和 170 个内置文件统一发布为 5.0.3。传播完成后，`npm run publish:verify` 已从 npm、Visual Studio Marketplace 和 Open VSX 三个公开分发端逐一回读到 5.0.3，没有把上传端成功或传播期旧缓存冒充最终可用。
- **部署器将容器自动更新从每日检查收紧为每 5 分钟滚动重启**：在线和离线安装脚本的 Watchtower 统一使用 `--interval 300 --rolling-restart`，避免新 `latest` 镜像需等待 24 小时才被拉取，同时减少前后端一起中断的窗口。脚本语法、版本头与参数契约检查均通过，`install-microi.sh` 版本头已升至 `v2026-08-14 05:20:59`。
- **v7.3.7 经过源码、定向回归、完整构建和公开制品四层验证**：Client 运行目标 5/5、微服务文档／Skills 同源 8/8、MCP 类型检查与 121/121、后端资产流六类定向测试 195/195、官网／Skills 覆盖审计和 6 个改动 Skill 格式验证全部通过；Microi.VSCode 生产构建、双类型检查、CLI／Codex／Broker 租约回归也全部通过。`Microi一键编译发布.sh` 按 `7.3.7、4、0、2、2、0` 在 13 分 28 秒内完成现代前端、逐文件压缩、Chrome 49 兼容版、后端 Release、受保护 DLL、独立 18080 liveness HTTP 200、19 个 NuGet 和前后端 Docker，并以 `__MICROI_RELEASE_EXIT__=0` 收尾；NuGet.org Flat Container 已独立回读 19/19。固定后端标签 `v7.3.7` 为 `sha256:669f31957b089b03b9a0490092d97f586cc558008d7d36787ce31d9b76116b03`，前端四个标签为 `sha256:a4d0a75d23cf74a3c4572c9e5263edf93bc185667e270b5a9baa883c4960aa94`。本轮发布后，并发的 Unity API 路由修正于 05:24 将两个可变后端 `latest` 标签推进为 `sha256:b7f7875265830ed58aba1f86b7d8fd85c470c6956d340d70edd8d8ff036c4ee9`；封版不回滚其他已发布的新补丁，因而 7 个镜像标签均可公开回读、其中 5/7 仍与本轮摘要逐字一致，固定 v7.3.7 制品未被改写。以上证明公共包与镜像可取，不等于生产节点已拉取新镜像或完成业务切换。

## v7.3.6 - (2026-08-13 21:44)

- **五仓提交、推送历史与工作树按最终树去重审计**：以 v7.3.3 封版后的根仓 `9dff76af42c3e3911203e78be72ea85a738354ee`、Microi.net `4fb11d3f3837fe8ec99703b320c4f3e315c3a531`、Microi.AI `ba467091d0905f62dd68d087406cb1e3ff163265`、Microi.VSCode `662fdd2fdfe68b15dd6d3528961c872f6e0a957d` 和 WebOS `cf8b4703b51f2ddade779db4081e6d6ee18b6e9e` 为边界执行 `fetch --prune`；五仓均为 0 ahead／0 behind，边界后没有新增提交、漏推记录、英文说明或自动 merge 包装，因而本节只归纳当前工作树的最终净差异一次。封版审计时根仓有 94 个已跟踪差异、9 个未跟踪源码／测试项，另有 1 个 `.asset-cache/` 预览缓存目录明确排除；Microi.net、Microi.AI、Microi.VSCode、WebOS 分别有 4、10、32、0 个状态项。`dist/bin/obj`、插件 bundle、Manifest 构建结果和版本清单只核对同源关系，不逐文件冒充独立功能，WebOS 不制造空提交。
- **Codex／VS Code／CLI 的 Token 自动恢复从“有凭据”推进到“可稳定自愈”**：共享 MCP Token 改为批量接纳并只持久化一次，避免多连接启动时为每个 Profile 重复拉起 Windows DPAPI 进程；工作区 DPAPI 调用增加 15 秒上限，凭据锁记录 PID，可立即回收进程已不存在的陈旧锁。CLI 新增 `microi auth repair`，只从当前工作区、当前 Windows 用户绑定的 DPAPI 密文保险库读取账号密码并重新登录，不接受命令行明文；Codex Broker 会把保险库路径和按工作区／租户派生的凭据 Key 传给唯一租约 MCP 子进程。真实 `microi auth repair` 与状态回读恢复成功，密码、Provider Key 和 Token 均未进入日志；这与后端 20 天 DiyToken 策略共同解决“后端更新后签名失效却无法无感续登”的反复故障，而不是单纯延长一个失效 Token。
- **Microi.VSCode、CLI、Codex、CodeBuddy、WorkBuddy 与 62 个内置 Skills 发布 v5.0.1**：首轮 `npm run publish` 已把鉴权自愈发布为 5.0.0；随后微应用生命周期与 multipart MCP 在同一工作树完成，项目自带 `cli:build` 把最新 Skills、129 个 MCP 工具和共享 Broker 重新组装到插件包，再按补丁版本发布 5.0.1，避免源码、文档与线上安装包漂移。完整 `npm test`、双 TypeScript 类型检查、CLI／Codex／自动更新／Broker 租约回归均通过；最终 `npm run publish:verify` 已分别从 npm、Visual Studio Marketplace 和 Open VSX 公开回读到 5.0.1，不把上传端成功或传播期旧缓存冒充三端可用。
- **MiniMax Token Plan 与 Hailuo 2.3 规格按当前官方事实重新校准**：画质优先默认由 `6 秒 / 768P` 调整为 `MiniMax-Hailuo-2.3` 的 `6 秒 / 1080P`，连续三个分镜只作为同一条约 18 秒母版的输入；`10 秒 / 768P` 仅是时长优先替代，两者不能叠加。Token Plan 由官方接口返回模型各自的当前窗口与周窗口，不再把前端本地计数冒充供应商权威配额；平台管理员可通过固定官方地址 `https://www.minimaxi.com/v1/token_plan/remains` 实时读取脱敏余量，Provider Key 只从服务端 `mic_ai` 路由读取。无法回读余量时 UI 失败关闭并禁止创建新分镜；Hailuo 2.3 API 没有 fps 参数，源片和母版只记录媒体探针实测值。本地 v7.3.4 端点已用当前工作区 DiyToken 真实回读：视频当前窗口 3/3 已用、周窗口 3/21 已用，当前窗口于北京时间 2026-08-14 00:00 重置，因此本轮没有再创建或额外计费生成新视频。
- **办公室男女对白成为 MiniMax 服务端受保护原子能力**：新增 `GenerateMiniMaxSpeech`，固定 `speech-2.8-hd`，女声 `female-tianmei`、男声 `male-qn-jingying`，对白文本、语速、音量、音调、情绪与 32kHz／128kbps 单声道 MP3 均由白名单约束。相同租户、用户和稳定 RequestId 先以共享 Redis 七天幂等占位；上游结果不确定时禁止重复扣减，成功音频在服务端校验 MP3 后直接写入当前租户 HDFS，供应商 Key 与原始十六进制不会返回浏览器。AI 内容应用新增 `AudioDialogue` 资产、生成按钮和受管接口，后台可统一试听／下载分镜、唯一母版、男女对白与纯音乐；视频画面本身仍是静音素材，最终作品必须混入对白、准确字幕和在人声下自动压低的配乐，禁止虚构模型原生带声或口型同步。
- **AI 视频质量门禁收口为一个连续剧情、一个帐号一条母版**：后台文案与应用策略固定三个 1080P 分镜共用人物设定、共享首帧、办公室、发型和整条母版只选择一次的服装，按“提出问题—共同排查—解决确认”构成男女主对话剧情；项目发布文档和已安装发布 Skill 同步固化约 25 岁成年女主、自然职业形象、T 恤与蓝色合身牛仔裤／不过度暴露短裙二选一，以及畸形脸手、塑料皮肤、换脸换装和身体比例失真的负面门禁。`VideoClip`、`AudioDialogue`、`AudioMusic` 均不得单独发布，只有通过 SHA-256、ffprobe 视频／音频流、分辨率、实际 fps、时长、编码器、字幕和响度审核的唯一 `VideoMaster` 才能进入支持视频的平台；标题继续强制纯文本，每帐号／母版哈希只允许一次正式任务。
- **委派账号管理支持精确授权，同时阻断角色越权**：`sys_user` 从平台只读资源改为按真实菜单／表的 `Read/List/Add/Edit/Delete` 权限授权，Import、Export、ByWhere 和其它无界操作仍要求平台管理员；JoinTables 不能再把平台表偷偷变成关联授权。后端会从租户主库重读操作者、目标用户和有效角色，普通账号管理员不能修改自己的角色、删除自己、管理同级外部角色／上级／管理员，也不能分配自己未持有的同级或更高角色；`Level` 只从有效角色重算，身份、租户、额度、许可、登录审计等服务端字段一律剥离。列表、详情和显式查询同时移除 `Pwd/PwdEncode`，角色目录仅投影可分配的低级角色与本人同级角色；角色增删改仍是平台管理员专属。
- **HDFS 大文件断点续传形成公开端到端闭环**：Aliyun OSS、MinIO 和 S3 实现带 `ContentLength`、取消令牌和目标流的 `CopyObjectToStream`，避免应用资产校验整体载入内存；阿里云 OSS 大对象写入按有界 Part 数自动计算块大小，失败时中止 Multipart Upload。应用资产 v3 以确定性会话、分布式续租锁、HDFS 暂存、逐块大小／SHA-256 回读和单调 `Current/Total` 支持同块幂等恢复；Controller 与 MCP 已公开 Initiate／Status／UploadPart／Complete，MCP 只发送原始 multipart 字节、自动跳过已持久化分片，完成对象后进入 `FinalizeApplicationStreamPublish` 的发布门禁、版本前置条件与指针提交。真实目录默认／硬上限为 20,000 文件和 20GB，5GiB 逻辑文件使用 16MiB 有界分片；旧节点 Base64 兼容仅允许 MicroService 的 256 文件／5MB 小型应急载荷，Web 等类型失败关闭。应用商城 v7.3.6 同步新增分片会话审计字段与“超大文件上传记录”只读菜单，升级器把审计菜单、状态能力和 7.3.6 包版本作为独立失败关闭门禁，不再误要求审计菜单复制商城三页签。
- **微应用单一缓存所有者、生命周期恢复和建租户反馈同步修正**：动态微应用菜单统一 `keepAlive=false`，由 `<micro-app keep-alive>` 独占子应用缓存；运行时名称使用租户、AppKey、菜单、fullPath、版本和入口的安全指纹，全局最多保留 5 个实例，只 LRU 淘汰最久未使用的 hidden 实例。关闭当前／其它／全部 Tab、访问记录淘汰、退出、Token 重置、角色或版本／入口变化都会按 fullPath 调用 `unmountApp(...,{destroy:true,clearData:true})` 精确销毁；aftershow 以 `forceSetData` 重发身份、权限、主题、路由和视口，afterhidden 停止看门狗，可见 DOM 异常只自动重建一次。官网 Gitee Star 失败提示保留已识别授权账号和服务端原因；官方消息通知应用结构补齐 `mic_msgset`、`mic_msg_event_log`、`wx_tpl_msg`、`wx_mp`、`wx_mini_program`，后两项不夹带数据集、Token 或 AppSecret。
- **v7.3.6 编译、回归与公共制品按最终源码完整纠偏发布**：账号／表单权限边界 143/143、MiniMax 规格与安全 22/22、V8 AI 可信身份 7/7、AI 内容应用 10/10、Client AI／微应用 21/21、MCP multipart 30/30、后端资产流／Schema／multipart 86/86、应用商城资源 79/79、官网／Skills 一致性 17/17 均通过，Microi.VSCode 完整测试、双类型检查、生产构建和 CLI／Codex 回归也全部通过。v7.3.5 构建后工作树继续完成上传审计资源、状态能力和升级门禁，NuGet 不可覆盖，因此按 `Directory.Build.props` 再 +1 到 v7.3.6；首次 v7.3.6 尝试在任何构建、NuGet 或 Docker 前被资源预检失败关闭，修正为只对三个商城导航菜单校验页签、独立校验审计菜单后再发布。最终 `Microi一键编译发布.sh` 按 `7.3.6、4、0、2、2、0` 在 14 分 32 秒内完成现代前端、逐文件压缩、756 个 legacy chunk、后端 Release、独立 18080 HTTP 200 冒烟、受保护 DLL、19 个 NuGet 和前后端正式／测试镜像，并以 `__MICROI_RELEASE_EXIT__=0` 收尾；NuGet.org Flat Container 已独立回读 19/19，Docker 匿名远端清单 7/7 一致。后端摘要为 `sha256:bc0c4f62389693f658f0e9747d3aa62cb2c7384932552befb30883d12fc80165`，前端摘要为 `sha256:279ee9572c7cb27c6151f7155b254da7033a54ec7ddc654ebe1781bc77f54f0a`，发布锁、发布 Bash 和 61500／61501 监听均已清理。构建后一次源文件等内容复写经制品特征核对，Promise 竞态修复、5 项 LRU 与 fullPath 精确销毁均已包含在公开前端镜像。以上证明公共制品可取，不等于生产 API 节点已拉取、`/#/mic-ai-engine` 已切到新后端或今天已经消耗 MiniMax 额度生成新片。

## v7.3.5 - (2026-08-13 21:10)

- **过渡制品由 v7.3.6 完整取代**：本版本已完成 19 个 NuGet 与前后端 Docker 发布，但构建之后同一工作树继续补齐应用资产上传审计菜单、MCP 状态能力和升级门禁，导致不可变的 v7.3.5 NuGet 不能代表最终源码。没有覆盖或伪造 v7.3.5 包；最终一致版本、完整功能说明、测试和制品摘要统一以 v7.3.6 为准。

## v7.3.4 - (2026-08-13 20:18)

- **过渡制品最终由 v7.3.6 完整取代**：本版本已完成 19 个 NuGet 与前后端 Docker 发布，但全量构建期间同一工作树仍在完成 multipart 公开完成入口和微应用五实例 LRU，导致不可变的 v7.3.4 NuGet 不能代表最终源码；随后 v7.3.5 又遇到上传审计与升级门禁的构建后补齐。没有覆盖或伪造 v7.3.4 包；最终一致版本、完整功能说明、测试和制品摘要统一以 v7.3.6 为准。

## v7.3.3 - (2026-08-13 15:00)

- **五仓提交、推送历史与工作树完成逐仓去重审计**：以 v7.3.2 封版后的根仓 `9ceb69f8025f29851e5ecd8cf933c2a875ca27e7`、Microi.net `a7c1a6b58b09ec37b36ce1bbebfbb54da6b74226`、Microi.AI `6d7b59a66bc146537b3cd47e750ea715d8ad252c`、Microi.VSCode `c0ddeaea724869b1616ed54b50a9722033f5d5af` 和 WebOS `cf8b4703b51f2ddade779db4081e6d6ee18b6e9e` 为边界逐仓 `fetch --prune`；五仓均为 0 ahead／0 behind，边界后没有新增提交、漏推记录、英文提交说明或自动 merge 包装，因而没有把同一改动重复归纳。写本节前根仓有 53 个已跟踪差异和 6 个新增源码／测试文件，Microi.net、Microi.AI、Microi.VSCode、WebOS 分别有 3、10、22、0 个状态项；`dist/bin/obj`、插件 bundle 和 Manifest 构建结果只核对协议与同源关系，不逐文件冒充功能，两个 `.asset-cache` 预览缓存不纳入版本，WebOS 也不制造空提交。
- **DiyToken 签名密钥按真实租户身份跨内外网入口收敛，后端更新不再随机踢掉 AI 助手**：同一 `OsClient` 的 Product／Internal／Internet 运行变体现在必须使用同一把 `AuthSecret`，启动加载和 API 接流前分别执行收敛与二次门禁。无显式轮换时保留最早出现的稳定强密钥，只有可信的唯一 `AuthSecretRotateVersion` 才选择最新轮换值；更新使用旧值比较交换、最多三次竞争重读并在最终回读不一致时失败关闭，避免后端更新、节点顺序变化或并行启动把“签名失效”误当成普通过期。VS Code 与 MCP 登录态默认有效期统一为 20 天，租户显式配置仍可覆盖，其它终端继续沿用原策略；回归测试覆盖稳定密钥、显式轮换、租户分组、默认时长和覆盖优先级。
- **VS Code、CLI、Codex 与 MCP 增加当前工作区加密凭据自动续登**：Windows 使用当前用户作用域 DPAPI 保存工作区凭据保险库，VS Code SecretStorage 作为主存储并镜像到带锁、原子替换和损坏失败关闭的工作区保险库；CLI／MCP 配置只接收保险库路径与用户名、密码键名，不写入明文账号密码。服务端 Token 失效时 MCP 会重新读取保险库、自动登录并原子更新连接 Token，敏感值不进入命令行、日志或错误输出；非 Windows 不允许退化为工作区明文。Microi.VSCode、CLI、Codex、CodeBuddy、WorkBuddy 与 bundled Skills 版本同步升至 v4.9.9，完整发布流程通过类型检查、自动更新／Broker／凭据测试和 62 个 Skills、170 个文件打包；npm、Visual Studio Marketplace、Open VSX 已分别公开回读为 4.9.9。
- **Microi.AI 补齐可信 MiniMax 视频中转、任务所有权与 HDFS 持久化闭环**：视频调用只从服务端 `mic_ai` 选择受保护的 MiniMax 直连节点或吾码中转站，浏览器和 V8 不能覆盖 Endpoint、ApiKey；新增兼容 `/v1/video_generation`、任务查询和文件读取入口，平台 APIKey 只标识中转用户，真实供应商密钥始终留在官方节点。创建请求强制稳定 `Idempotency-Key`，Redis 不可用即拒绝调用上游；任务／文件句柄按用户建立所有权映射，取消、超时或结果不确定时不会盲目重试。完成视频可由平台管理员转存当前租户 HDFS，下载大小、类型、文件路径与公有 CDN 均经服务端校验，临时 MiniMax 地址不会冒充永久资产。
- **MiniMax 纯音乐成为服务端安全原子能力并进入持久 Worker**：新增 `music-3.0`／`music-2.6` 无人声纯音乐生成，固定输出 44.1kHz、256kbps MP3；供应商响应在服务端校验十六进制与 MP3 签名后直接写入当前租户 HDFS，浏览器、V8 和业务表都不会接触供应商 Key 或原始音频。相同租户、用户、RequestId 使用七天幂等状态，HDFS 已写入而回执不确定时禁止重复扣减；AI 内容应用新增 `AudioMusic` 资产和每分钟一次的 `MciAiMusicWorker`，用数据库条件更新原子认领，不依赖 `setTimeout`、HTTP 请求寿命或伪进度承载长任务。
- **AI 引擎后台增加可追溯的视频创建、记录、预览与下载中心**：`/#/mic-ai-engine` 为平台管理员增加“AI视频”工作区，可按 MiniMax Hailuo 2.3 的当前 Token Plan 档位创建每日最多三段素材、轮询任务、把成品转存 HDFS、查看租户内创建记录、模型、时长、分辨率、任务状态和验片意见，并预览、下载或标记人工审核通过。界面明确当前订阅可用档为单段 6 秒、768P、固定 24fps；上游没有可调 FPS 参数，也不把剪辑后的 1080×1920 交付画布误写成 MiniMax 原生 1080P。
- **视频发布从“逐分镜重复提交”收口为“每帐号唯一带声母版”**：AI 内容应用把生成片段、音乐和最终成片分别建模为 `VideoClip`、`AudioMusic`、`VideoMaster`；分镜永远不能直接发布，质量门禁要求同一平台恰好一个审核通过的母版，并验证 `HasAudio=1`、SHA-256、ffprobe 分辨率／实际 FPS／时长／编码器、音轨与响度证据。发布标题会去除 HTML/XML，并在 `<p>`、`<topic>` 等标记进入标题时直接阻断；稳定幂等键纳入“内容 + 平台 + 帐号 + 内容模式 + 母版哈希”，每个帐号每版成片最多一次正式提交。认领与完成引擎同时改用 `DateNow/DateAdd`，修复 Jint 对 .NET 日期互操作的线上错误，并保留部分失败原因和 `needs_review` 边界。
- **真实带声母版和平台终态完成闭环回读**：三段 MiniMax Hailuo 2.3 分镜只作为源素材，已与 MiniMax Music-2.6 纯音乐剪成一条 16.834 秒、1080×1920 竖版、24fps 的 H.264 母版；文件含 AAC 48kHz 双声道音轨，综合响度 -18.16 LUFS，黑屏、长冻结和长静音扫描均为 0，SHA-256 为 `cc1fbb6c6b13f2cf3db2f876bcee6c2defe623e823fccf3a7309362ab94a50a4`。母版已写入 `static.itdos.com` CDN 并完成 HTTP 200、字节数和哈希一致回读；10 个视频帐号中搜狐号因分类接口 403 在提交前阻断，其余 9 个帐号各正式提交一次，终态为 5 个成功、4 个失败、0 个悬挂。CSDN、豆瓣、微信公众号当前帐号不支持视频协议而未发；成功任务未返回可靠公开 URL，因此只认定蚁小二任务终态，不把推测地址冒充公开页面回读。全程未使用蚁小二素材库容量，长期母版和音乐归档于吾码 HDFS。
- **版本、测试与 v7.3.3 公共制品完成可重复发布**：按 `Microi.Server/Directory.Build.props` 的 7.3.2 +1，将 Microi.Client、Microi.net、Microi.AI 与 19 个公共 NuGet 组件统一升至 v7.3.3。AI 内容应用构建测试 10/10、Client V8 AI 定向测试 5/5、MiniMax／AI 安全测试 21/21、Token 稳定签名测试 12/12 通过，Client 完整内存保护构建也已通过；缺少真实登录密码的 Playwright 用例按门禁跳过，不计作 UI E2E 成功。`Microi一键编译发布.sh` 按 `7.3.3、4、0、2、2、0` 完成现代／Chrome 49 前端、后端 Release、受保护 DLL、19 个 NuGet 和前后端正式／测试镜像发布；19/19 个包已从 NuGet.org Flat Container 独立回读，后端 `latest/v7.3.3` 标签统一为 `sha256:95d99c8c58559d7d338b8050f0610f3aa7307387c27840d4b5e1a6e1082e05b5`，前端 Web／Client 的 `latest/v7.3.3` 标签统一为 `sha256:2a93dc6aa562b3ca841a554861daa52e084d99911755b2b3dd1eeabe97a677bb`，发布锁与临时进程均已清理。以上证明公共制品可取，不等于生产节点已经拉取或重建。

## v7.3.2 - (2026-08-13 08:51)

- **五仓提交、推送历史与工作树完成逐仓去重审计**：以 v7.3.1 封版后的根仓 `1685ddd00a7004fac454666abfc066a8606a3e95`、Microi.net `d34c7288a78d7a30b7fc5380763dfa5d1b5a3594`、Microi.AI `d57593385236d9b2dcd6edd99ac3d2b7133e823e`、Microi.VSCode `c54d67bfa10263fdb9c8214043f55d34b6c6ba9c` 和 WebOS `cf8b4703b51f2ddade779db4081e6d6ee18b6e9e` 为边界逐仓 `fetch --prune`；五仓均为 0 ahead／0 behind，边界后没有新增提交、漏推记录、英文提交说明或自动 merge 包装，因此没有把同一改动重复归纳。写本节前根仓有 38 个已跟踪差异和 1 个新增交付测试，Microi.net、Microi.AI、Microi.VSCode、WebOS 分别有 2、1、10、0 个状态项；`dist/bin/obj`、商城资源基线和插件清单等生成内容只核对合同与同源关系，不逐文件冒充功能，两个既有 `.asset-cache` 预览缓存不纳入版本，WebOS 也不制造空提交。
- **后台任务恢复后的可见进度保持单调且仍以真实工作量为准**：进度计算同时接纳显式阶段进度、已完成单元和数据库持久化下限，在总量刻度不变时保留上一分片的 `Current`，避免子任务切片、断点续跑或阶段切换使百分比和已完成数倒退；可信 `_BackgroundTask` 上下文补齐 `Progress/Current/Total`，供应用安装器跨分片续传。运行中仍最高显示 99%，ETA 继续按已经提交的工作单元计算，完成态与失败态不会被虚假进度掩盖。
- **应用商城安装器补齐旧租户授权兼容、恢复闭环与失败关闭**：统一导入器由 v1.10.3 升至 v1.10.8，批量安装器由 v1.1.4 升至 v1.1.6，应用商城资源包由 v7.2.11 升至 v7.3.5。旧租户确有 `sys_rolelimit` 物理表、但缺少表单元数据时，安装器只对已确认的元数据错误使用参数化物理读写；其它异常继续失败关闭。权限时间戳改用数据库 `CURRENT_TIMESTAMP`，提交前后提升授权缓存版本，并在 Redis 失效失败时拒绝把不一致权限当成成功。断点会保存阶段进度和持久化进度下限，批量任务也会透传子任务检查点、失败阶段与对应恢复建议。
- **对象存储探测失败不再抢先阻断真实写入，权限错误给出可操作诊断**：上传前的对象存在性探测若因最小权限、代理或暂态错误失败，会换用带随机 ULID 后缀的防碰撞对象名并交由实际 PUT 判定写权限，不再在真正上传前误报失败；阿里云 OSS 诊断稳定返回错误类型、作用域、Bucket、对象、原始错误与处理建议，明确识别 403／Forbidden／AccessDenied。商城批量安装会把对象存储权限、配置和网络问题映射为对应恢复动作，不再统一提示为版本冲突。
- **内置 SaaS 引擎支持有界的 DatabaseOnly 构建资产交付**：SaaS 引擎应用包由 v7.2.10 升至 v7.2.11，源码策略改为 `NotIncluded`，构建策略改为 `DatabaseOnly`，保留 4 个、328207 字节的已构建运行资产并移除 23 个、481480 字节的源码；只有显式声明、MicroService 数据库存储、最多 256 个文件且总量不超过 5MB 的官方内置包才能绕过 HDFS。安装器把完整构建字节落库并拒绝伪造源码、越界文件或隐式降级，普通文件存储应用仍走原对象存储链路。
- **损坏的历史字段元数据不再拖垮整张表的更新**：Microi.net 的字段查找统一跳过 `Name` 为 null 或空字符串的历史／损坏 `diy_field` 行，再以不区分大小写的有效字段名匹配更新和查询条件；一个布局残留或异常元数据行不会再导致本表所有合法字段修改失败。新增反射回归测试覆盖“坏行与有效字段并存”的兼容场景，未改变现有 V8 调用协议。
- **平台与多宿主工具版本元数据分别准确递增**：按 `Microi.Server/Directory.Build.props` 的 7.3.1 +1，将 Microi.Client、Microi.net、Microi.AI 及 19 个 NuGet 公共组件统一升至 v7.3.2；Microi.VSCode、CLI、Codex、CodeBuddy、WorkBuddy 和 bundled Skills 清单由 v4.9.6 同步至 v4.9.8。插件侧 MCP／CLI／Codex 构建 Hash 均未变化，170 个 Skills 文件中只有版本标记内容变化，因此本轮如实记录为多宿主发布元数据同步，不虚构新的插件运行功能。
- **定向自动化与 v7.3.2 全量公共制品发布完成**：商城导入／安装合同测试 54/54、后台进度／HDFS／损坏字段元数据服务端测试 47/47 均为 0 失败。`Microi一键编译发布.sh` 按 `7.3.2、4、0、2、2、0` 完成现代前端、串行压缩、756/756 个 Chrome 49 legacy 分块、后端 Release、受保护 DLL 混淆、18080 启动探针、19 个 NuGet 包和前后端 Docker 推送，以 `__MICROI_RELEASE_EXIT__=0` 收尾；后端 0 个编译错误，5 条既有 xUnit 分析器建议不影响制品。19/19 个 v7.3.2 包已从 NuGet.org Flat Container 独立回读；后端 `latest/v7.3.2` 标签统一为 `sha256:a6d3cf24c4d455f4dbeb8a36734909d5014c5acf683ae2e1a9f8c091478a8620`，前端 `latest/v7.3.2` 标签统一为 `sha256:0fd5f98566f47e74890ecbda83487a8645bebe85201b71d0329faebae70f6af8`，发布锁和 18080 临时探针均已清理。以上证明公开制品可取，不等于生产节点已经拉取，也不把未执行的生产业务浏览器或硬件验收描述为通过。

## v7.3.1 - (2026-08-13 03:00)

- **五仓历史、工作树与自动合并去重审计**：以 v7.2.9 封版后的根仓 `25adbc3c938259e7a2b7d36cdde094f4faf80fc5`、Microi.net `2c5f5dd9247279620ffb80d83bda98ccdec51902`、Microi.AI `1b8197c76226128540264f2e62728dac8232ef5f`、Microi.VSCode `d6f71853a36752373f4be560b2a3d538529050d9` 和 WebOS `cf8b4703b51f2ddade779db4081e6d6ee18b6e9e` 为边界逐仓 `fetch --prune`，五仓均为 0 behind；根仓在写本节前有 2 个本地提交尚未推送，四个子仓均为 0 ahead。根仓边界后共审计 12 个提交：`5b72a5e0` 是 310 文件、17364 行新增／9590 行删除的平台更新；`26362547`、`0845447f`、`7e332397`、`ec5c09a7`、`289d1f23`、`1f817f2e`、`4b6d5309` 是外部 UniApp 业务提交；`1770d517`、`a76a6980`、`14ef2050`、`15a90481` 只是分支／PR 合并包装。外部提交相对平台主线的最终净变化为 45 个文件、1793 行新增／320 行删除，仅与大提交重叠 2 个 Skills 文件，因此按最终树归纳一次，不把自动 merge 当成四轮新功能。写本节前根仓有 25 个已跟踪差异和 2 个未跟踪项，Microi.net、Microi.AI、Microi.VSCode、WebOS 分别有 1、1、96+40、0 个状态项；其中用户表格回归测试和 VS Code 新源码／References 是交付内容，`.asset-cache`、`dist/bin/obj`、插件编译 bundle 与资源基线只作来源／同源校验，不逐文件冒充功能，WebOS 不制造空提交。
- **统一版本按已公开 v7.3.0 继续递增到 v7.3.1**：生产登录页和 `GetMicroiNetVersion` 已分别显示／返回 7.3.0，NuGet.org Flat Container 对 `Microi.Core`、`Microi.net`、`Microi.V8Engine` 也均确认 7.3.0 已存在；因此不能把当前未提交版本误判成“失败后可复用”，否则不可变 NuGet 包会跳过本轮 CPU 修复。按用户指定的 `Directory.Build.props` 当前版本 +1，本轮使用 7.3.1，并同步 Microi.Client、17 个根仓服务／公共包以及 Microi.net、Microi.AI 两个独立仓的 Version／AssemblyVersion／FileVersion；Microi.VSCode 继续使用本轮已经完成全套发布门禁的 v4.9.6，不把平台版本号混作插件版本号。
- **启动期多语言维护从多租户并发全表扫描改为集群级低扰动修复**：故障回溯确认，v7.2.9 后的多语言改动把原进程全局执行门拆成租户门，而默认 4 个后台 Worker 可同时运行多个租户任务；API 启动 2 分钟后又会为全部运行租户投递修复。旧路径会为每种目标语言把最多 50000 条 `diy_lang` 记录物化到托管堆后逐条判断，再刷新缓存、校准目录并执行最高 300000 行覆盖率复核，即使没有缺失译文也会重复消耗 CPU、数据库连接和约 1.5 GB 以上托管堆。现在同时增加进程级维护门和基于配置主租户 Redis 的集群统一租约，本机最多只允许 1 个多语言任务并为普通后台任务保留其余 Worker；启动修复按 UTC 日幂等，把所有目标语言合并为一次候选 SQL，只检查真实空值且每租户每天最多修复 200 条总候选，不再按“200 × 语言数”反复扫描。启动模式在没有修复时不预探测全部翻译服务、不刷新缓存，也不执行目录归一化和穷举覆盖率；人工全量初始化仍会纳入 Key／平台前缀占位候选并保留完整覆盖率、持久进度、租户权限和 fencing token，因而没有用“禁用多语言”换取表面降载。
- **两站故障现场证据指向共享源站启动／重启压力，而不是前端路由或文档静态文件损坏**：故障窗口内 `os.itdos.com` 返回 520 Origin Error，`microi.net` 返回 521 Origin Down；站点恢复后，API liveness／health、两个 HTML 入口及抽取的 12 个 JS/CSS 资产全部重新返回 HTTP 200。本机镜像缓存能确认此前已有 v7.3.0 `latest` 前后端镜像，但仓库不包含生产宿主 Compose／自动拉取配置，本轮也没有生产宿主日志权限，因此不会把“是哪一个 Watchtower／运维动作触发重启”写成已证实事实。可以确认的是，该故障时序与上述启动后多租户维护并发、旧实例约 2.5～3.2 GB RSS／1.55～1.76 GB 托管堆的现场读数一致；本版从源码上消除这一可复现的 CPU／堆放大器，并把“源站恢复”“静态资产可读”“API 健康”“真实业务浏览器验收”作为四层独立证据。发布镜像成功也不自动等于所有生产节点已经拉取，最终仍以公开域名回读为准。
- **PC 表格列设置成为按用户、租户和模块隔离的即时偏好**：操作列表头改用统一设置图标打开列面板，支持逐列显隐、全选、反选和恢复模块默认值；保存采用 450ms 防抖异步写入，不阻塞首次列表查询或正在进行的操作，同一账号在不同模块、租户之间不会串值。字段、审计列使用稳定 Key，布局／上传等不可见控件和 Id、CreateTime 等平台默认列不会重复进入用户选择；模块配置仍是权威默认值，用户偏好只作用于当前展示。固定操作列的表头、普通行、斑马纹和 hover 背景同步修正，新 SFC 合同测试与偏好纯函数测试覆盖作用域、即时应用、定时器清理和列宽行为。
- **Page Engine、Print Engine、表单设计器与代码编辑统一到可国际化的第一方 UI 栈**：新增 `platform-ui.js` 平台词条层以及 Page／Print 独立 i18n 适配，把设计器、渲染器、属性面板、预览、数据源和内置模板中的中文 UI 字面量迁移为中英文同源词条，并提供可重复执行的模板迁移脚本与覆盖测试。新增共享 Monaco 加载器、通用 `microi-code-editor` 和 Page HTML 编辑器，表单公式、子表回写、流程信息等编辑入口复用同一加载与主题边界；首选技术栈固定为 Element Plus + ECharts + Monaco，Naive UI／VChart 只留在懒加载 GoView 兼容区，避免普通页面重复打包整套 UI。页面／打印移动端与渲染器回归同时禁止示例业务数据混入正式视图，并验证 23 项模板国际化、UI 栈和用户列设置合同。
- **微应用宿主收口为单一滚动所有者并补齐移动工作台体验**：外层宿主只负责可用高度和边界，`<micro-app>` 作为兜底滚动容器，`overflow`、`overscroll-behavior` 与 `--micro-app-available-height` 避免宿主和子应用同时出现滚动条、虚拟列表被截断或内容区空白；真实可见子 DOM、首次挂载自愈和 KeepAlive 激活继续决定 Ready，而不是依赖可能丢失的生命周期事件。AI 助手、聊天、首页、消息、个人中心和工作台统一使用平台移动视觉、真实空态与接口状态，不携带演示记录；微应用友好路由、鉴权上下文、无 URL Token、稳定错误归属和单次重建均由合同与浏览器级用例覆盖。
- **V8/MCP 104 个后端动作建立默认拒绝的最小能力边界**：`V8EngineController` 增加不可绕过的控制器授权过滤器，每个公开动作必须且只能声明 `mcp:read`、`mcp:write`、`mcp:execute` 或 `mcp:admin`，新动作漏标会直接 403；数据库连接、外部 SQL、Playwright 上下文、角色权限、匿名接口和流式发布门等敏感操作固定要求 Admin。普通 DiyToken 仍需从当前租户主库复核实时平台管理员身份，访问密钥会话还必须携带显式最小 scope，请求 Header／Body 不能自报提权。MCP 的 OCR／文件翻译输入同时抽成独立安全模块：绝对普通文件与 Base64 二选一、拒绝符号链接、限制扩展名和 100MB／20MB 上限、计算 SHA-256、限制返回文本／文件大小，翻译结果只以 `wx` 创建新文件且不覆盖既有路径。
- **应用商城全部安装／更新与后台任务恢复信息变得可操作**：商城按钮在入队前回读 Worker 循环、schema readiness 和普通任务保留槽，不再把后台工作器未就绪伪装成“已开始”；批量接口把发现页、安装计划、当前索引、子安装检查点、已安装／已更新计数全部写入持久任务。每个子应用按稳定操作 Id 幂等执行，长安装以 `HasMore + Checkpoint` 分片继续，失败返回阶段、失败项、子任务详情和可恢复建议，前面已完成的应用在重试时不会重复副作用。Worker 的租户轮转、配置主租户槽、数据库资源耗尽自愈、租约续期和进度限频继续作为共享控制面，商城资源、内嵌引擎、共同基线和官网副本保持逐字节同源。
- **Microi.VSCode／CLI v4.9.6 增加永不阻断工作的自动更新与共享 Codex Broker**：VS Code 激活、Codex Router 启动和普通 CLI 命令只轻量投递 `microi update --background`；后台从 npm 官方 registry 检查并升级全局 CLI、Codex 插件与当前工作区 AI/MCP，断网、权限不足、`EBUSY/EPERM/EACCES` 或调度失败写入 `~/.microi/updater/status.json` 后延后重试，不结束当前扩展、MCP、Router 或任何用户进程。MCP 运行文件按版本落到 `~/.microi/runtime/versions/<version>`，完整写入后才原子切换 `current.json`，新进程使用新版本、旧进程自然结束；右下角延后提示统一进入持久输出日志。多个 Codex 任务不再各自常驻一套 MCP Server，而是由工作区级 Router 通过带心跳、独占租约、失主回收和空闲退出的共享 Broker 复用每个连接的一个子进程；Codex 配置收敛为单一 `microi_codex` 入口，但仍能路由全部连接和 129 个工具。
- **62 个 bundled Skills 完成渐进式披露与工作区规则单源治理**：长篇能力说明从入口 `SKILL.md` 拆到按任务路由的 `references/`，入口保留触发条件、强制边界和应读索引；新增 manifest、优化／刷新／验证脚本以及插件包同源检查，Microi.Client、UI、移动端、系统交付、Page、Playwright、V8 安全／HTTP／文件／工作流等重点 Skills 均同步。`workspace-conventions` 成为进度、资源保护、更新和临时文件规则的唯一事实源，AGENTS、CLI、Codex、CodeBuddy、WorkBuddy 只保留路由，不再复制容易漂移的长规则；自动去重测试会阻止规则重新散落。v4.9.6 完整测试已覆盖双 TypeScript 类型检查、生产 bundle、62 Skills／170 文件打包、129 工具握手、CLI 并发写入、自动更新故障注入、Broker 并发回收和文档单源。
- **官网首页与 AI 平台架构图升级为机器可校验的产品能力索引**：首页和 `ProductShowcase` 重新组织低代码、AI、MCP、应用商城、移动端和交付能力；AI 平台架构图以同一数据文件生成 SVG、1920×1080 PNG、3840×2160 PNG 和 manifest，锁定 V8 引擎唯一运行核心、能力节点、尺寸、源数据哈希与输出 SHA-256，避免手工改图造成文字或能力漂移。微应用和 VS Code／CLI 文档同步新的滚动、MCP、自动更新与运行时边界；`docs/en/public` 的重复静态资产按中文单源／统一翻译策略移除，中文官网资产仍完整保留。69 个中文可读页面、126 个多语言映射页面、链接、侧栏、开源仓库、亮暗色静态对比和架构图测试均通过；静态门禁不冒充真实浏览器视觉验收。
- **外部 UniApp 自动合并按最终业务差异归纳一次**：自定义 TabBar 把 AI 机器人稳定到右侧；微信小程序加入版本比较与更新提示，更新时保留 Token／授权等必要会话数据；列表卡片即使负责人为空也保留字段位并补充创建时间。客户跟进修复客户名称报错、详细地址剔除重复省市区、城市“全部”与级联赋值；拜访打卡补充时间、详情跳转、跟进／记录提示和客户新型下拉框，拜访对象类型当前只完成客户类，其余三类没有被日志夸大为已交付。详情编辑按钮统一位置，列表顶部重复新增按钮移除并保留底部入口；更新静态门禁统一 CRLF／LF 后再做语义判断，避免 Windows 工作区把已经位于登录守卫前的公开更新入口误报为回归。区域、打卡统计、卡片字段、按钮位置、更新状态和订单审批九组脚本全部通过，集福鲤 Profile 的微信小程序、H5 与 App 三端真实编译成功，微信产物质量门禁回读主包约 1121.4KB，未用“源码看起来正确”替代构建证据。

- **v7.3.1 全量制品完成可重复构建、发布与公共回读**：`Microi一键编译发布.sh` 严格按 `7.3.1、4、0、2、2、0` 执行，现代 Vite、逐文件串行压缩、Chrome 49 legacy、后端 Release、Microi.net.Api publish、受保护 DLL 混淆、依赖连通性和 18080 启动冒烟全部通过，成功轮以 `__MICROI_RELEASE_EXIT__=0` 收尾，耗时 12 分 30 秒；后端 0 个编译错误，5 条既有 xUnit 分析器建议不影响制品。Dos.Common、Dos.ORM、Microi.AI、Cache、Captcha、Core、HDFS、Job、MongoDB、MQ、MQTT、Microi.net、OCR、Office、SearchEngine、Spider、Upgrade、V8Engine、WeChat 共 19/19 个 v7.3.1 包已从 NuGet.org Flat Container 独立回读；阿里云后端 `latest/v7.3.1/dev-latest` 三个标签统一为 `sha256:d5346d75282c7a08a771306b515b3c7381818a13d9d7453a77731e7263f05cf7`，前端 Web／Client 的 `latest/v7.3.1` 四个标签统一为 `sha256:8c5686c8607f66a404c6a08420e0d3d25597bc7360addd6dedaac3e592521e52`；发布锁和 18080 临时探针均已清理。以上证明公共制品可取，不把镜像摘要代替生产节点实际拉取，线上生效仍以版本接口、实例切换、健康状态和真实浏览器回读为准。

## v7.2.9 - (2026-08-12 14:54)

- **五仓历史、工作树与自动合并去重审计**：以 v7.2.8 封版后的根仓 `66cb1e1ce39f7f300b2e7a20d74c419fcf4413d6`、Microi.net `c783b66d8232b4563f7a8c092491f5570b812f82`、Microi.AI `6ae18a3856395cef8e8df41241d87bb75451b7ef`、Microi.VSCode `0bf01312a29f62cae7bf8e8559d6b4ce393debfc` 和 WebOS `cf8b4703b51f2ddade779db4081e6d6ee18b6e9e` 为边界逐仓 `fetch --prune`，五仓均为 0 ahead／0 behind；从五个边界到写本节前没有新增提交、漏推记录、自动 merge 或需要翻译的英文提交说明，因此没有把分支合并包装重复归纳。写本节前根仓有 70 个已跟踪差异文件和 6 个未跟踪交付文件，Microi.net、Microi.AI、Microi.VSCode、WebOS 分别有 1、1、29、0 个状态项；商城资源基线、内嵌包、插件 bundle、`dist/bin/obj` 等生成物只核对同源和发布合同，不逐文件冒充新功能，WebOS 也不制造空提交。
- **一份存量 V8 同时兼容佳博 GP-M322 与 ZICOX CC4**：新增打印机 profile 与协议兼容层，设备名可自动识别 `gprinter-gp-m322`、`zicox-cc4` 和通用 TSPL，也可在连接页手工选择。GP-M322 路径不读取新增操作元数据，`createNew().getData()` 的 TSPL 继续逐字节原样发送；CC4 在首包写入前把同一套高层 TSC 调用整包转换为 CPCL，覆盖纸张、速度、浓度、间隙／黑标、方向、参考点、走纸、线条、方框、反相、文字、条码、二维码、位图和单次打印，无法安全映射的低层／特殊命令会以零写入失败关闭。`createNewESC()` 的 ESC/POS 始终原样发送，原有业务 V8 无需增加品牌分支；CC4 转换元数据不可枚举，因此业务必须直接传递构建器返回值，不能先做 `Array.from` 或 JSON 序列化。
- **蓝牙连接从单次 BLE 写入升级为应用级可恢复打印通道**：5+App 与 Web Bluetooth 都会枚举真实可写特征，Android CC4 在 BLE 不可用时可回退到系统已配对的 RFCOMM 通道 1／标准 SPP UUID；iOS 与浏览器继续只走 BLE，不虚构 Web SPP 能力。全站前端 V8 共享一个 `Print` 实例、连接快照、订阅器和串行发送队列，设备与型号同时持久化，页面恢复和意外断线按有界退避重连；新增 `getPrinterProfile`、`setPrinterProfile`、含传输／指令语言的连接状态，分包仍保持旧字段兼容且不会在整除包长时多写零字节尾包。发送成功只证明字节已写入 BLE 特征或 SPP 输出流，不等于打印机已经走纸或没有缺纸、字库、浓度与偏移问题。
- **蓝牙打印成为系统引擎、V8 编辑器与 Skills 的正式能力**：官网系统引擎新增“蓝牙打印机”菜单与独立专题，首页、打印引擎、V8 客户端、视觉路由和中文映射同步串起 TSPL／CPCL／ESC-POS、BLE／SPP、双型号对照、旧 V8 示例、CC4 转换白名单、平台限制、失败恢复与实机验收清单；打印引擎文档明确 `PageObj/PrintObj` 不能作为原生命令发送。`v8-frontend-events`、`print-engine`、`v8-utilities` 及 VS Code 内嵌 Skills 保持同源，V8 编辑器提示新增型号 API 和“直接传递 getData”约束。自动化已经证明协议转换、佳博原字节、分包、队列和连接状态，但本轮没有两台目标打印机在场，不能把这些断言描述为真实纸张／固件走纸验收。
- **MQTT 快速重连不再被旧会话的延迟断开误判为离线**：当前节点连接登记由单纯租户字符串升级为 `OsClient + ConnectionToken`，租户和令牌同时写入 MQTTnet `SessionItems`；Connected、订阅、发布和 Disconnected 优先读取当前会话租户，只有断开令牌仍等于当前登记令牌时才清理设备路由与接口引擎缓存。相同 ClientId 的新连接已经接管后，旧连接迟到的断开只记录 `StaleDisconnectIgnored`，不会删除新连接、写离线台账或触发错误的 Disconnected V8；跨租户 ClientId 冲突仍失败关闭。MQTT 官网与 Skill 同步补齐 TCP／TLS、QoS 0/1/2、Retain、Response Topic、七类事件、设备级引擎、Topic ACL、凭据、日志、容量、外部 Broker 与多节点边界，并明确内嵌 Broker 当前没有 WebSocket 监听、节点内会话不是集群全局事实源。
- **子租户可安全继承主租户 MongoDB 系统日志集群**：`DbMongoConnection` 加入可信运行时的共享基础设施继承，仅在内存中的租户配置快照继承，仍从前端 `V8.SysConfig`、普通表单和持久化子租户行隐藏；每个租户继续使用独立的 `sys_log_{lowerOsClient}` 数据库，不共享日志命名空间。系统日志写入、查询、Trace、生命周期、类型统计和接口调用计数统一通过同一个租户 Host 工厂，空连接串、空数据库名或运行态未刷新时返回包含主租户配置入口与刷新动作的可操作错误，不再以空值或底层驱动异常失败。SaaS 引擎商城资源同步补充 Mongo 字段提示与说明并升至 v7.2.10。
- **官方平台应用接口引擎升级既防降级也不覆盖更高官方版本**：官方商城发布器升至 v1.7.7；只有应用类型为 Platform 且发布者明确为官方／平台应用时，未显式指定所有权的 Managed 接口引擎才标记为 `Ownership=Platform`，`CreateIfMissing` 租户 Hook 继续归 Tenant。统一导入器升至 v1.10.3；只有从固定 `https://api.itdos.com + iTdos` 实时回读且商城元数据可信的官方平台包，才能把旧 `Application` 所有权迁移为 Platform，离线包、自定义商城源和自报发布者不能获得该权限。目标租户已有语义版本更高的平台引擎时保留当前版本并继续安装；同版本异哈希、无可比版本、普通应用修改、低版本本地修改、CreateIfMissing 反向接管或 Platform 所有权降级仍整包冲突回滚。应用商城资源、发布器／导入器基线、三份内嵌副本和 C# 启动门禁保持同源，应用商城资源包升至 v7.2.10。
- **MCP 表单布局默认值和旧节点兼容路径收口**：`microi_add_layout_field` 与批量布局工具为 `CollapseGroup` 默认显式写入 `FormWidth=24`，在用户未覆盖时补齐 `Config.CollapseGroup.ShowFieldCount=true`，并在写后同时回读宽度与嵌套 JSON；Divider／Tabs 等其它布局控件保持原默认。`microi_update_field` 仅在精确匹配旧 API 节点的 `Newtonsoft.Json.Linq.JValue.Val` 缺陷时，读取唯一字段与所属表、拒绝 TableId 不一致，再走有界 FormEngine 兼容更新、刷新 schema 缓存并逐字段回读；普通业务错误不会被当作可绕过的兼容写入。表单、布局和系统交付 Skills 同步纠正“CollapseGroup 不设整行宽度”的旧冲突规则，并要求先排查 `InFormV8/V8.FieldSet/hideField/HideFields` 后再判断运行表单渲染缺陷。
- **微应用宿主避免因丢失生命周期事件销毁健康页面**：首次挂载看门狗现在同时绑定 resolve generation 与本次 retry attempt，每 250ms 检查真实可见子 DOM；即使 `mounted/ready` 事件早于监听注册或在 KeepAlive 激活时丢失，只要页面已经可渲染就直接进入 Ready。只有持续 12 秒没有真实内容才执行一次自动销毁重建并给出稳定诊断，旧尝试的定时回调不能干扰新实例，从而避免微服务页面已经显示却又被误判超时刷新。
- **Unity 双端、多人在线与安装资料的交付说明补齐**：Unity 官方专题和 Skills 对齐“桃源云梦”v1.3.0 的 WebGL 在线入口与 Windows x64 安装包，补充随机昵称、2 秒心跳、10 秒共享数据库租约、角色插值、头顶名牌、文字／表情公屏、会话秘密哈希、限频和请求幂等，以及 4 张表、9 个接口、4 个运营菜单的交付口径；同时要求设定图与 Unity 实机证据分开、两个独立会话互验、强制断线后租约消失、安装包大小／SHA-256／安装卸载回读。本轮只更新官网和 Skills 的交付说明，没有执行 Unity Editor、Windows 安装器或公网双会话实测。Docker Compose 一键安装脚本版本时间同步到 2026-08-12 10:00:44，API 容器继续绑定安装租户，前端容器不再把该租户固化进环境变量，访问入口仍通过 `?OsClient=` 明确选择租户，避免镜像运行配置与访问上下文被错误锁死。
- **Microi.VSCode／CLI v4.9.5 强化 Windows 构建资产替换**：CLI 组装不再先删除目标 bundle 后直接复制，而是复制到唯一暂存路径，再以 rename 切换旧目标到备份、激活新文件并清理备份；EACCES、EBUSY、ENOTEMPTY、EPERM 使用有界指数退避，激活失败时恢复旧文件，避免杀毒软件、索引器或仍在退出的 Node 进程短暂占用文件后留下半包。扩展、CLI、Codex／CodeBuddy／WorkBuddy 清单、62 个 bundled Skills、MCP bundle 和构建元数据统一到 v4.9.5；新增故障注入测试覆盖暂时 EPERM 后成功与持续 EBUSY 后完整回滚。本轮只完成源码、bundle、测试与 Git 交付，没有把“测试中的 npm 授权流程”描述为真实发布 `@microi.net/cli@4.9.5`。
- **版本、定向回归与 v7.2.9 正式制品完成回读**：`Directory.Build.props`、Microi.Client、Microi.net、Microi.AI 以及 Dos.Common、Dos.ORM、Cache、Captcha、Core、HDFS、Job、MongoDB、MQ、MQTT、OCR、Office、SearchEngine、Spider、Upgrade、V8Engine、WeChat 等 19 个公共 NuGet 组件统一升至 v7.2.9；Microi.net 与 Microi.AI 子仓本轮只有版本元数据差异。蓝牙／微应用／商城资源 83/83、服务端 MQTT／Mongo／租户配置 52/52、MCP 类型检查与 116/116、MQTT Skill 覆盖、安装脚本 Bash 语法以及 Microi.VSCode 的完整测试、双类型检查、生产构建、129 工具握手和 CLI／Codex 插件回归均通过。首次发布因官方资源 Token 过期在任何构建／制品推送前失败，重新登录后继续同一个未提交版本；`Microi一键编译发布.sh` 随后按 `7.2.9、4、0、2、2、0` 在 12 分 46 秒内完整通过现代前端、串行压缩、762 个 Chrome 49 SystemJS chunk、后端 Release、DLL 保护、4 秒 HTTP 200 存活探针、19 个 NuGet 和前后端正式／测试镜像推送。NuGet 官方 Flat Container 已独立回读 19/19；后端 `microi-api:latest/v7.2.9` 与 `microi-api-dev:latest` 收敛为 `sha256:a64d7127a2e52a6c56c2c198f5ca0105d56c3e96087fa57fed46b61653f5f08d`，前端 `microi-web-dev:latest/v7.2.9` 与 `microi-client-dev:latest/v7.2.9` 收敛为 `sha256:4455d7e799195a213fd63d4ebd73243e2fc1e367e7dd8643a145699375d0f15e`，远端标签 7/7 全部回读一致。公共制品发布不等于客户或生产节点已经拉取重建，也不等于真实蓝牙打印机、Unity、浏览器业务或生产集群已经验收。

## v7.2.8 - (2026-08-12 04:37)

- **五仓历史、工作树与自动合并去重审计**：以 v7.2.7 封版后的根仓 `24e508b46e4093f3caeb44bde3722e5763efc1e2`、Microi.net `9e5cbc1e34957d69b8bd458bc7204256bf7f04c6`、Microi.AI `e17509a33dfc16db3c9cdbf2fda7b884c254f11d`、Microi.VSCode `7e04348b542dc2d608712add17b73767b8d43698` 和 WebOS `cf8b4703b51f2ddade779db4081e6d6ee18b6e9e` 为边界逐仓 `fetch --prune`，五仓均为 0 ahead／0 behind；从五个边界到写本节前没有新增提交、漏推记录、自动 merge 或需要翻译的英文提交说明，因此本版全部变化都来自当前工作树。写本节前根仓有 250 个已跟踪差异文件和 202 个未跟踪交付文件，Microi.net、Microi.AI、Microi.VSCode、WebOS 分别有 4、1、83、0 个状态项；`microi.apps` 到官方租户 `AI应用/{appKey}` 的同内容迁移只归纳一次，`dist`、插件 bundle、资源基线及其它编译文件只核对同源关系，不逐文件冒充新功能，WebOS 也不制造空提交。Unity 自动生成的 `Library/Temp/Logs/UserSettings` 已加入忽略规则，不进入版本库。
- **后台任务 Worker 在多租户、多节点和短暂故障下可自恢复**：执行租约按接口任务类型取 90～900 秒，空数据库长任务获得 15 分钟租约，Redis 续租遇到瞬时异常先以 5 秒间隔有限重试，真正丢失租约或连续失败才取消当前执行并交给持久队列恢复；续租线程收尾设 5 秒上限，终态不再被慢租户数据库回读长期占住全局槽位。Worker 为配置主租户保留可用执行槽，任务通知按用户／运行域合并、缓存投影异步清理，并新增循环心跳、异常监督重启、只读 readiness 快照和仅超级管理员可见的状态接口；数据库任务行、Redis 租约、稳定幂等键和 fencing token 继续是共享事实源，进程内状态只用于诊断与投影。
- **租户 JWT 签名根和事务后配置发布双重收口**：`AuthSecret`、`AuthSecretRotateVersion` 从 SaaS 表单隐藏、锁定并设为只读，FormEngine 新增／修改入口统一剥离对稳定签名材料的普通写入，只允许可信后端按 `BackendAuthSecretRotateVer` 触发轮换。租户配置刷新若处于数据库事务内，会注册提交后回调，只有事务真正提交才更新运行态与 Redis；回滚不会提前发布未提交密钥或留下节点间配置漂移。Microi.net 子仓同步保护新增／修改入口，并让 `ReloadSingleOsClient` 遵守同一提交后边界。
- **旧数据库启动前置字段由共享租约幂等自愈**：API 在 License、登录和 FormEngine 读取生成实体之前，先检查 `diy_table.V8Unlimited` 以及 `sys_apiengine` 的 StopHttp、Timeout、V8 限额和 Lock 等运行必需物理列；缺失时复用升级分布式租约执行只扩展不收缩的字段补齐，其它节点轮询物理不变量，超时或自愈后仍不完整就拒绝进入流量。该前置门禁解决旧空库在正常 HostedService 升级启动前已因缺列失败的问题，同时不把业务表、菜单或租户资源迁回 .NET 启动迁移。
- **空数据库发行升级为应用资源和运行痕迹零残留门禁**：后端递归对账应用包全部业务表归属，分批清除物理表、`diy_table/diy_field/sys_menu/sys_rolelimit` 元数据及对应 `diy_lang` 词条，并无条件清空后台任务、数据库备份、Gitee Star 审计、身份凭据／设备／TOTP、商城安装审计、租户额度日志、消息日志、分布式任务锁和微信消息配置等 13 类运行数据。平台核心表具有生成器与后端双重保护；发布校验返回残留表名、字段所属表、语言 Key 和逐表行数，任一应用资源、凭据或审计痕迹仍存在即失败关闭，不再只给无法定位的总数。
- **应用商城补齐管理员菜单权限并分离商城版本与运行版本**：统一导入器升至 v1.10.2，新建或恢复应用菜单时为每个系统管理员角色幂等新增／合并完整菜单和按钮权限，保留客户既有自定义权限，处理并发主键冲突后重新合并，并以数据库写后回读作为安装成功条件；运行时、下载资源、内嵌包和发布器共享 `ADMIN_MENU_PERMISSION_V1` 完整性门禁。AI 应用发布器升至 v1.7.6：无运行资产变化时允许只提升商城包版本，MicroService 的 AppVersion、VersionNo、BuildVersion 保持真实运行版本；严格 v3 发布仍要求已提交指针和包版本完全一致，避免为纯发行整理重复构建并污染 CDN 版本。
- **MQTT 历史 JoinForm 引用兼容真实接口引擎 Key**：设备级 `mci_mqtt_client.ApiEngineId` 与 SaaS `MqttApiEngine` 若保存的是历史 GUID／ULID 记录 Id，会先按当前租户解析 `sys_apiengine.ApiEngineKey` 再进入统一执行入口；已经保存 Key 的新配置保持零额外查询快速路径，解析失败仍交由原执行入口输出“不存在的数据”诊断，不把合法记录 Id 错当成 Key。
- **PC 表格、二维码和微应用宿主修复可见交互细节**：表格最后一个可见非固定业务列或最后一个可见审计列改用 `min-width` 吸收剩余空间，既保留配置列宽又消除操作列前的大块空白，固定列和表头单行布局不受影响。二维码控件恢复二维码卡片生成、预览、单个／批量 PNG 下载、安全文件名和颜色兼容，并在全局注册兼容旧表单／菜单 V8 的 `window.downloadQRCode`，不会覆盖租户自定义同名实现。微应用宿主增加 layout/paint containment 与样式隔离，防止子应用背景和通用样式污染吾码 Logo、主菜单与相邻页面。
- **MCP、VS Code 与 CLI 统一选择同租户最新 DiyToken**：typed／untyped Profile 产生多个同租户 Token 别名时，MCP 和 VS Code 按 JWT `MicroiTokenIssuedAt/iat` 选择签发时间更新者，修复旧精确别名覆盖刷新 Token 后持续签名失败；无法比较的旧 Token 仍以精确身份键优先，API 级历史别名只作无租户候选时的最后回退，禁止跨租户、跨服务器泄漏。`@microi.net/cli` 新增 `microi version --json`，VS Code、CLI、Codex／CodeBuddy／WorkBuddy 清单和 bundled Skills 统一为 v4.9.3；任何已加载 Microi Skill 的 Codex 任务都必须先从 npm 官方 registry 在线核对最新版，升级后强制执行 `ai init`、`doctor`、`codex status` 完整闭环，不能用本地状态冒充在线最新版。
- **三个官方 AI 应用收敛为服务器／租户唯一源码根**：删除 `microi.apps` 平行副本，将 AI 内容创作与发布、AI 平台治理、微信小程序内容安全的 Manifest、接口引擎、资源策略、测试、微服务源码和商城素材统一归档到 `Microi-V8-Engine/.../iTdos.Product.Internal/AI应用/{appKey}`，`.gitignore` 只对白名单官方应用开放，避免租户 Token 或其它私有源码被整体纳入。AI 内容运营继续以数据库唯一时段、质量门禁、租约和 fencing token 编排本机 yxer 连接器；微信内容安全继续采用 C# 最小协议网关、Managed 核心与 CreateIfMissing 租户 Hook。AI 平台治理运行／资源合同升至 v2.0.4，内部菜单只切换异步右侧内容区，局部骨架屏、主题变量和背景装饰限定在应用根容器；商城发行工程可独立升至 2.0.6，不重复发布未变化的 v2.0.4 运行资产。
- **Microi.Unity 成为独立 UPM SDK 与 WebGL 宿主能力**：新增面向 Unity 2022.3 LTS 的 `com.microi.unity` 包，提供 V8 接口引擎客户端、只驻留运行内存的 DiyToken、响应头 Token 并发安全轮换、WebGL `.jslib` 双向事件桥和通用构建入口；go-view Unity 组件可选注入当前 ApiBase、OsClient、DiyToken 与 Did，并在宿主收到轮换回调时按请求 Token 比较交换。Editor 工具箱提供有选择范围、预览、Undo 和 JSON 备份的相机点、Mesh 合并／恢复、贴图优化、场景诊断、Camera 深度及 WebGL 质量工具，不移动原 3D 项目的场景、模型或业务脚本。仓库同时保留可复现的 Packages／ProjectSettings，生成缓存不提交；该 SDK 不引入 `UnityEngine` 到 .NET 服务端，也不把游戏业务固化为 Controller。
- **官网、Skills 与下载资料同步新的交付边界**：新增“文件柜”和“Unity 3D 与 WebGL 集成”专题、独立视觉样式与真实示意图，首页、源码架构、3D/CAD/数据大屏、微服务内部路由、AI 平台治理和中文导航同步更新；Unity 文档明确源码、Editor、WebGL、浏览器、真实租户和商城是六层独立证据。新增 Unity Skill、Codex 路由 Skill，并让全部 Microi Skills 指向相同的在线版本门禁；旧 MySQL 5.6／SQL Server 2017 二进制空库附件从 Git 移除，资料目录改为列出 MySQL 5.7/8.0、SQL Server 2022、Oracle 19c、PostgreSQL 17、达梦 8、人大金仓等官方静态下载地址。
- **定向回归与同源门禁完成收口**：Microi.Client 二维码／表格／微应用 37/37、升级资源 93/93、空库清理 8/8、三个官方 AI 应用 39/39、MCP 类型检查与测试 112/112、服务端任务／租户配置／空库／MQTT／升级 110/110，共 399/399 通过；Microi.VSCode 的扩展与 CLI 双类型检查、生产构建、129 工具启动握手、并发／CLI／Codex 插件完整测试也全部通过。回归同时把“固定 Skills 数量”改成与规范源码目录逐项同源对账，并把 SaaS 商城资源版本的脆弱精确值断言改成合法最低版本门禁，从而兼容资源内容变化后发布器按规则自动升版，避免后续新增 Skill 或独立商城发行再次制造伪失败。
- **版本元数据与 v7.2.8 正式制品回读**：`Directory.Build.props`、Microi.Client、Microi.net、Microi.AI 以及 Dos.Common、Dos.ORM、Cache、Captcha、Core、HDFS、Job、MongoDB、MQ、MQTT、OCR、Office、SearchEngine、Spider、Upgrade、V8Engine、WeChat 等 19 个 NuGet 公共组件统一升至 v7.2.8；Microi.AI 子仓除版本外没有业务源码差异。`Microi一键编译发布.sh` 按 `7.2.8、4、0、2、2、0` 一次完整通过现代前端、串行压缩、Chrome 49 legacy、后端 Release、DLL 保护、HTTP 200 存活探针、19 个 NuGet 以及前后端 Docker 推送；后端 0 个编译错误，5 条既有 xUnit 分析器建议不影响制品。后端 `latest/v7.2.8/dev-latest` 摘要收敛为 `sha256:4057c8ce3c57d4b6b66c92c64254b13079f1437fdc28b0575e4e6a77cce22737`，前端 Web／Client 的 `latest/v7.2.8` 摘要收敛为 `sha256:776dc6a160ae9fd0549a266ca0a548f43f453b074e135b76daeb69cd72450cb3`；NuGet 官方 Flat Container 19/19 和 Docker 远端标签 7/7 均已独立回读。公共制品发布不等于客户或生产节点已经拉取重建，也不把本轮未执行的 Unity Editor／硬件、真实租户浏览器或生产业务验收描述为通过。

## v7.2.7 - (2026-08-11 07:41)

- **五仓历史、工作树与重复合并完整审计**：以 v7.2.6 封版后的根仓 `27d0e3e103774402d510dc5a4a4cd9f83de69198`、Microi.net `b36ea11e6553f1d76b77e48adf6ff1896c6395ff`、Microi.AI `be191d8a9dc09adb2f47c81c3a4abe3a66daf3bb`、Microi.VSCode `3bb42ccfcb8ce492b77c299b720f1f082438da82` 和 WebOS `cf8b4703b51f2ddade779db4081e6d6ee18b6e9e` 为边界逐仓 `fetch --prune`，五仓均为 0 ahead／0 behind；从各边界到写本节前都没有新增提交、推送遗漏、自动 merge 或需要翻译的英文提交说明，因此没有把合并包装重复归纳。写本节前，根仓有 63 个已跟踪差异文件和 1 个未跟踪测试文件，Microi.net、Microi.AI、Microi.VSCode、WebOS 分别为 1、1、13、0 个状态项；插件 bundle、商城资源基线和其它生成物只核对同源关系，不把编译代码逐文件冒充新功能，WebOS 也不制造空提交。
- **模块列表从“默认 CRUD”升级为可信的智能业务展示**：MCP Manifest、`microi_create_module` 与后端 `CreateModule` 在用户未显式配置时，都会根据当前租户真实 `diy_field` 元数据，为可见 Diy 模块生成只含 List／Card 的紧凑 Hero、业务副标题、真实动态指标、PC 复合主列和完整移动卡片；金额／数量优先使用字段聚合，没有合适数值字段时才使用真实 `DataCount/PageCount`，不会生成随机数或静态演示值。已配置模块的客户端兜底同样只使用后端汇总、筛选总数、本页加载数、本页数值求和或本页真实状态分布，并把局部口径明确标成“本页”；`EnableViewSchema` 继续只控制 Detail／Edit 自定义表单，不会误关 Hero、指标、复合列和移动卡片。
- **字段列宽、模块设计器与 AI 交付门禁统一收口**：MCP、Manifest 和后端新增相同的语义列宽推断，标题／名称／地址、日期／编码、数值、状态等字段使用稳定且更符合内容的宽度；控制器不再提前塞入固定 120px，PC 复合列的 `MinWidth` 优先于普通 `TableWidth` 和末列自适应。模块设计器固定为“模块标题与统计、PC 复合列、移动端卡片、自定义表单、高级 JSON”五个入口，标题与指标对比度同步提升。工作区 Skills、VS Code／CLI 生成的 AI 规则和插件内置 Skills 都加入同一视觉交付门禁，要求 AI 精调真实业务指标、去重字段区域并禁止 N+1 统计。
- **按钮与 PageTabs 的接口引擎选择器恢复可靠远程搜索**：模块引擎官方资源升至 v6.9.6，`PageTabs/MoreBtns/PageBtns/FormBtns/BatchSelectMoreBtns/ExportMoreBtns` 等 JsonTable 内嵌选择列统一通过同租户真实 `diy_field.Id` 代理数据源，使用带 `$Keyword$` 的有界远程 SQL 搜索并保持保存字段／显示字段一致；不再把内嵌列 Key 或列 Id 当作字段 Id，从根因上避免打开模块按钮设计时出现“DiyField 数据不存在”和同批数据源加载失败。
- **空数据库发行清理由表名扩展到完整应用资源所有权**：发行服务和官方 SaaS 引擎资源 v6.5.0／`admin_get_empty_database_sanitization_sql` v1.1.4 同步识别应用包声明的业务表、接口引擎、定时任务、Quartz 触发器、微服务、页面与菜单，并补齐 `mci_demo` 实测残留清理；表和接口优先读取轻量选择清单，菜单只查询具有 `StoreId` 的归属项并按被引用 Id 读取表定义，避免再次扫描大型包正文。平台保留范围改为精确白名单：`microi-platform-service`、`app.microi.*` 和 `microi-wechat-content-security`；被误标成 Platform 的普通业务应用仍会清理，资源 Key 必须通过安全字符门禁，准备与结果校验都对剩余表、接口、任务和菜单失败关闭。
- **MicroService 表单嵌入成为官网、表单文档与 Skills 的正式第四种入口**：在独立运行、菜单直开和 `V8.OpenAppDialog` 弹层之外，完整说明 `DevComponentPath` 通过页面 `LegacyComponentPaths` 匹配已发布 MicroService `RoutePath` 的协议；本地 Vue 组件存在时继续优先，不存在时由微应用组件宿主接管。文档补齐 `componentMode/componentData/permissionContext`、80～1600px 自动高度、`dev-component:resize` 与 `dev-component:event` 字段回写，以及 Add／Edit／View／只读、窄屏、有权／无权账号的验收边界；同一发布物由此覆盖独立、菜单、弹层和表单四种入口，但不会因复用宿主登录态而绕过 DiyToken、模块或数据权限。
- **官网图片预览与首页素材体验统一**：中文官网普通 Markdown 图片继续复用全局 Fancybox，平台首页 21 张原生 HTML 预览图和 MicroService 三张真实案例图分别组成可放大、缩放、拖动的共享图库；MicroService 案例桌面端稳定为等宽两列、窄屏回落单列，并新增自动化门禁防止重复引入第二套灯箱。根 README 直接嵌入 3840×2160 PNG，不再使用图片套链接的写法，使 Gitee 原生点击放大正常生效；官网仍保留可无限缩放的 SVG 架构图。
- **官方资源发布器兼容新版稳定 MCP 名称并保持失败关闭**：首轮 v7.2.7 发布实际发现 VS Code v4.9.0 会按连接身份生成 `microi_itdos_api_itdos_com`，旧发布器只接受精确名称 `microi_itdos`，导致官方资源同步在构建前中止。发布器现兼容 `microi_itdos_<稳定后缀>`，但仍强制唯一候选绑定 `https://api.itdos.com + OsClient=iTdos`；多个官方候选存在歧义时拒绝选择，旧精确名称只在同等候选中优先。新增回归锁定主机后缀、错误绑定、旧插件入口、官方读取、三方合并和发布后哈希回读，修复后七项官方资源及三份商城内嵌副本保持一致。
- **版本元数据和配套包同步**：`Directory.Build.props`、Microi.Client、Microi.net、Microi.AI 以及 Dos.Common、Dos.ORM、Cache、Captcha、Core、HDFS、Job、MongoDB、MQ、MQTT、OCR、Office、SearchEngine、Spider、Upgrade、V8Engine、WeChat 等 19 个 NuGet 公共组件统一升至 v7.2.7；Microi.net 与 Microi.AI 子仓本轮都只有版本元数据差异。AI 内容创作与发布 Manifest／构建清单由 v1.0.4 对齐到现有 v1.0.8，未把纯版本同步描述为新增业务逻辑；Microi.VSCode 仍为 v4.9.0，本轮提交的是模块视觉门禁源码、CLI 测试和同源 bundle／Skills 清单。
- **定向验证与 v7.2.7 正式制品回读**：模块展示客户端 14/14、空库／模块资源／官网资源同步 41/41、MCP 类型检查与 110/110、服务端空库／模块／身份资源 39/39，以及 Microi.VSCode 扩展与 CLI 双类型检查、CLI 回归全部通过。`Microi一键编译发布.sh` 按 `7.2.7、4、0、2、2、0` 在修复发布器后完整重跑成功，现代前端、串行压缩、Chrome 49 legacy、后端 Release、DLL 加密、19 个 NuGet 和前后端 Docker 推送全部完成；后端 0 个编译错误，5 条既有 xUnit 分析器建议不影响制品，发布产物启动探针在 5 秒内返回 HTTP 200。后端 `latest/v7.2.7/dev-latest` 摘要收敛为 `sha256:d0071d7ae4b2f444045a673430fd4ad4c2a5504ea3200c75737ad990fd5da1ef`，前端 Web／Client 的 `latest/v7.2.7` 摘要收敛为 `sha256:fbed01444bf94b51208c7f6aee3b24488ee0dae64c625bf0556783204045e077`；19/19 个 v7.2.7 包已从 NuGet 官方 Flat Container 完成公共版本回读。以上制品发布不等于客户或生产节点已经拉取重建，也不把本轮未执行的真实业务浏览器、硬件或生产部署验收描述为通过。

## v7.2.6 - (2026-08-11 00:42)

- **五仓历史、工作树与重复合并完整审计**：以 v7.2.5 更新日志提交后的根仓 `2fe0b04f423afa490539f3b56f5fd6370bfdc7fd`、Microi.net `0409a5850f9a394f8989d99e0d3c5c25e81ad159`、Microi.AI `7c6c3b2167247dabd52be36b81d2cc33a5688e0b`、Microi.VSCode `e5248a8acf40f5c8d30cf1d56203e2bf386a4425` 和 WebOS `cf8b4703b51f2ddade779db4081e6d6ee18b6e9e` 为封版边界逐仓 `fetch --prune`；其后只有根仓单父提交 `60c7988d` 将 README 中的架构图、.NET 10、许可证、Vue 和 QQ 群素材由 SVG 换成高清 PNG，其余四仓没有新增提交或漏推记录。五仓均没有自动 merge、冲突专属增量或需要翻译的英文提交说明，因此没有把合并包装重复归纳。写本节前，根仓共有 183 个状态项（94 个修改、36 个删除、53 个未跟踪），Microi.net 为 2、Microi.AI 为 1、Microi.VSCode 为 38，WebOS 为 0；`microi.mcp/dist`、微应用 `dist`、插件 bundle、清单和其它生成物只核对同源关系，不逐文件冒充新功能，WebOS 也没有制造空变更。
- **微服务源码同步告别人工分段与 Base64 搬运**：`microi_sync_microservice_source` 首选本地绝对路径 `directory`，由 MCP 进程直接扫描、排序、哈希和读取普通源码，模型上下文只接触清单；单个大文件保持完整，不再生成 `.sync-seg-*` 或 `sync-source-files.json`，也不再因 50KB 工具窗口反复切片。扫描会排除 `node_modules/dist/build/coverage`、缓存和版本库，拒绝符号链接、`.env` 等秘密文件以及失控的文件数／总量；确认写入前重新核对大小与 SHA-256，写后再验证服务器返回的文件数、总字节数和源码清单 Hash，旧 `sourceFiles` 仅保留受限兼容。
- **MicroService 独立运行、菜单直开和弹层路由共用一套可信上下文**：新脚手架在没有宿主 Token 时显示吾码帐号密码登录，并按租户 `EnableCaptcha` 动态决定验证码；嵌入后台时复用宿主 DiyToken，不把 Token 拼进 URL。菜单宿主、组件宿主、Dialog／Drawer 与 `V8.OpenAppDialog` 统一传递 `sysMenuId/moduleEngineKey/diyTableId`，表单和列表 V8 调用也继承当前或显式目标模块权限，解决“页面能打开但接口一直提示没权限”的上下文缺失。官方文档用数据库定时备份弹层、生产计数报工工作台和成品打包工作台三张真实案例说明独立地址、菜单指定路由和弹层指定路由；官网 AI 应用广场中大部分应用采用同一 MicroService 交付方式。
- **Manifest 的微服务菜单与主子表关系改为可移植声明并在后端二次门禁**：菜单可用 `microServiceKey + routePath` 解析当前租户真实服务／页面 Id，拒绝陈旧 Id、非法路由和不完整绑定；关系字段新增 `relation` 协议，`JoinForm` 只允许 1:1／N:1，`TableChild` 只允许 1:N，并强制真实外键、绑定目标子表且 `Display=0/AppDisplay=0/HasChild=0` 的隐藏子菜单，以及以 `(OsClient, 子表外键)` 开头的组合索引。计划、生成、单字段创建／更新和系统验收都调用同一 `V8McpFieldRelationGuard`，不能用低层工具绕过；解析发生在表、字段、菜单建立以后，不在跨环境 Manifest 中编造数据库 Id。
- **TableChild 在保持父级授权的同时恢复子表展示配置**：Microi.net 的 `FormEngineGetTableData` 只在父菜单和关系字段已通过服务端授权后，恢复经校验的隐藏子菜单作为展示上下文，读取子菜单列、搜索列、默认排序和统计配置；不会再次套用子菜单 `SqlWhere/SqlJoin/JoinTables` 作为授权条件，也不会接受跨租户、跨表或错误菜单身份。这样既修复嵌入子表列与筛选丢失，又不扩大父记录、子记录或模块权限边界。
...
<!-- mci-release-version-era:begin -->
<section class="mci-release-scope-banner mci-release-era-banner" aria-labelledby="mci-release-era-title">
  <div class="mci-release-scope-banner__glow" aria-hidden="true"></div>
  <div class="mci-release-scope-banner__mark" aria-hidden="true"><span>4.1</span><i></i><i></i><i></i></div>
  <div class="mci-release-scope-banner__content">
    <p class="mci-release-scope-banner__eyebrow">VERSION NUMBERING MILESTONE</p>
    <h2 id="mci-release-era-title">这里是前后端版本号合并的分水岭</h2>
    <p class="mci-release-scope-banner__lead">分水岭上方从 <strong>v4.1.0</strong> 起采用统一版本号；下方的更早时期由前端与后端各自发布，所以旧记录中版本号更高的一项，发布日期也可能反而更早。</p>
    <div class="mci-release-scope-banner__paths">
      <article>
        <span>合并后</span>
        <div><b>前后端统一版本</b><small>v4.1.0 及之后，前端、后端作为同一个 Microi吾码平台版本发布并统一记录。</small></div>
      </article>
      <article>
        <span>合并前</span>
        <div><b>前后端分别维护</b><small>旧记录保留原始版本号，并用“（前端）”“（后端）”说明标题版本号当时所属的版本线。</small></div>
      </article>
    </div>
    <p class="mci-release-scope-banner__note"><b>排序规则</b>：本页按发布时间从新到旧排列，页面底部保留平台最早期的更新历史。合并前的前端、后端版本号不能直接互相比较；括号标注的是标题版本号归属，不表示该条日志只包含这一端的改动。</p>
  </div>
</section>
<!-- mci-release-version-era:end -->

## v3.5.2（后端） - (2025-12-29)
...
## v2.4.5（后端） - (2025-10-16)

- 左右结构优化（commit b7dfd2a5）

## v2.4.1（后端） - (2025-10-15)
...
## v2.2.0（后端） - (2025-06-06)

- 优化了地图区域空间点击绘制，数据为空会报错。（commit 389854ad）

## v2.2.0（后端） - (2025-06-04)
...
## v2.1.6（后端） - (2025-05-30)

- 优化样式、更新文档（commit 18336617）

## v2.1.6（后端） - (2025-05-29)
...
## v2.1.5（后端） - (2025-03-31)

- 新增接口地址（commit 81764df2）

## v2.1.5（后端） - (2025-03-30)
...
## v2.1.4（后端） - (2025-03-13)

- 更新文档项目源码（commit 5f4f080d）

## v2.1.4（后端） - (2025-03-12)
...
## v2.0.3（后端） - (2024-11-11)

- 更新文档（commit 3437bff0）

## v2.0.3（后端） - (2024-11-06)

- 更新文档（commit 2e3b656f）

## v2.0.2（后端） - (2024-11-05)
...
## v2.0.1（后端） - (2024-11-03)

- 更新说明（commit 5536de1f）

## v2.0.1（后端） - (2024-11-02)

- 更新介绍（commit a1f8b19c）
- 更新文档（commit 70c604ba）

## v2.0.1（后端） - (2024-10-31)
