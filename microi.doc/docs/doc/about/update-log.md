...
- **运行中的子租户恢复任务支持原地热刷新且保持同一幂等轮次**：父任务用原 parent/child TaskId 重新投递官方工作器，优先处理 `jhyxdkj` 与 `lsg`，禁止创建重复安装任务；刷新检查点绑定 `startup-api-runtime-flags-v4`，如果热修发生在每 20 个租户的分片中途，则从首租户重启同一轮刷新，避免把 v3 与 v4 混合执行误标为完整成功。后端启动工作器缓存同时绑定权威源码 SHA-256 指纹，同一 API 进程内的官方热修可以即时生效。
...
- **官方应用商城与 SaaS 引擎包完成同源升级**：应用商城升至 v7.6.9，最终 SHA-256 为 `ccde7b8c6967bc8a1f0872a8e63ced9fcc5a234e7078bbb055121b24729408d62`；SaaS 引擎升至 v7.6.11，最终 SHA-256 为 `9dff6e904a64c7e1d454c0463d49ef555c802ee5d8d913c7e5e0ac37937c8118`。两包的本地升级资源、官方同步基线、结构化 ChangeLog 和内嵌工作器字节一致；该证据证明官方源具备后续安装能力，不等同于所有子租户已完成本轮恢复任务。
