...
- **Microi.VSCode、CLI 与多宿主插件发布 v5.1.8 并完成三端公开回读**：`npm run publish` 完成生产构建、类型检查、测试、63 项 Skills/172 个 bundle 文件和 VSIX 内容校验，发布 `@microi.net/cli@5.1.8` 与 Visual Studio Marketplace 5.1.8；Open VSX 首次上传遇到 503 后使用同版本恢复命令继续传播，没有错误递增为 5.1.9。最终 `npm run publish:verify` 已从 npm、Visual Studio Marketplace 和 Open VSX 三个公共分发端同时回读 5.1.8，Codex、CodeBuddy、WorkBuddy、DeepSeek Harness 的版本元数据与本轮 Skills 同步一致。
- **v7.5.0 全量编译、分层回归和公共制品完成验收**：`Microi一键编译发布.sh` 按输入 `7.5.0、4、0、2、2、0` 在修复源码清洁门和应用包版本三方合并后完整重跑，用时 17 分 6 秒并以 `__MICROI_RELEASE_EXIT__=0` 收尾；现代前端、串行压缩、Chrome 49 legacy、后端 Release（0 个错误、9 条既有 xUnit 分析器建议）、发布目录守卫、Microi.net/Microi.AI DLL 混淆、依赖服务预检和独立 18080 liveness HTTP 200 均通过。客户端主题/聊天/通知/权限 51/51、商城导入与资源同步 92/92、后端资源流/权限/租户配置/AI 路由 145/145、平台服务构建及契约 3/3、UniApp `check:ui` 全链均通过；NuGet.org Flat Container 已逐包公开回读 v7.5.0 为 19/19。后端 `microi-api:latest/v7.5.0` 与 `microi-api-dev:latest` 摘要均为 `sha256:09da86bf038ce3ef4d52b13b3fd52313e204c4757d5b719ce8a73fe43efeb1ef`；拉取团队 PC 变更后又以同版本执行“只发布前端”，现代、串行压缩和 Chrome 49 构建用时 8 分 24 秒，`microi-web-dev`、`microi-client-dev` 的 `latest/v7.5.0` 四个远端标签独立回读摘要均为 `sha256:df5b0b4c84cb0505bb711bc46de4085c0b7a2e1342374091c8c3bf65c3a8dfe6`。发布锁、18080、61500 和 61501 监听均已清理。这些证据证明源码、插件、官方包、NuGet 和镜像可公开获取，不等同于任一生产/客户节点已经拉取新版并重启，也不冒充本轮未执行的最终浏览器视觉或专用硬件验收。
