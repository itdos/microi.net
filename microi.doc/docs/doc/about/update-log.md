...
- **畅捷通回调新增租户显式启用的 V2 安全协议入口**：新增 `/api/Message/ReceiveV2?OsClient=...`，只从当前租户 `mci_system_setting` 读取并解密 AES Key 与最多 32 个 AppKey，校验租户、请求大小、密文／明文长度、应用标识和消息结构后，再调用固定的 Managed 接口 `platform-chanjet-callback-v2`；C# 只承载协议、密码学和可信上下文，业务写表与通知委托 CreateIfMissing 的 `platform-chanjet-callback-v2-hook`。官方 SaaS 引擎包升至 v7.8.26，首次安装的 Hook 使用带官方声明的安全空实现，租户后续个性化代码不会被升级覆盖；业务未成功时返回 503 让上游有界重试，响应不泄露凭据或内部异常。
