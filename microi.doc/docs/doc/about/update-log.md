...
- **运行中的子租户恢复任务切换到最新工作器后不再重复投递**：后台任务每个分片都会按 ApiEngineKey 获取最新接口源码，因此编排器 v1.2.7 将修订固定为 `startup-api-live-worker-v6-no-requeue`；v1.2.1–v1.2.6 遗留的 `RefreshBootstrap` 检查点只迁移回 `Monitor`，继续汇总原 `ChildTasks`，迁移区间明确禁止调用 `QueueChildTenantPlatformAppMaintenance`。这样既保留同一 API 进程内热修即时生效的能力，也避免历史幂等键格式变化后产生第二批安装任务；先前 v7.6.7–v7.6.12 的重新投递刷新方案已由 v7.6.13 收敛替代，不再作为最终执行路径。
...
- **官方应用商城与 SaaS 引擎包完成同源升级及 live 投影回读**：应用商城升至 v7.6.9，最终 SHA-256 为 `ccde7b8c6967bc8a1f0872a8e63ced9fcc5a234e7078bbb055121b24729408d62`；SaaS 引擎最终升至 v7.6.13，SHA-256 为 `6bfd5e6cf2fb77e0bad64ae8151844e49becbee1ec6fc5b5b10c85dee526ed75`。两包的本地升级资源、官网资源、官方同步基线、结构化 ChangeLog 和内嵌工作器字节一致，独立第二次 live 投影回读为 Managed 92、CreateIfMissing 8、总计 100；该证据证明官方源具备后续安装能力，不等同于所有子租户已完成本轮恢复任务。
...
- **源码、契约测试与 v7.6.5 现代版公共制品完成分层验收**：本轮针对性 C# 测试 64/64、v7.6.13 不重复投递迁移定向 C# 测试 27/27、应用商城与导入器资源契约 86/86 全部通过，Release 编译 0 错误、9 条既有 xUnit analyzer 建议。`Microi一键编译发布.sh` 按输入 `7.6.5、4、0、2、2、0、0` 完成现代前端、后端、独立 18080 liveness、DLL 加密、20 个 NuGet 与 7 个正式／测试 Docker 标签，耗时 10 分 37 秒并以 exit 0 结束；公共端回读 NuGet 20/20、Docker 7/7，后端摘要为 `sha256:cff80ebbafc9703122f9bdcfd449a6e634e83bde08e048fb7cac59b71e365905`，前端 Web／Client 摘要为 `sha256:514835ad7d2875070cb4aa4ea71b4f4ff7b8c4579c342a7b3118552c66abcde1`，只生成现代版、未生成 Chrome 49 legacy。v7.6.13 应用包修订通过官方应用源独立交付，不声称已进入先前不可变的 v7.6.5 NuGet／Docker 二进制；公共制品、官方应用源和源码测试分别成立，不把它扩写为所有客户生产节点已经拉取、重启或完成真实业务验收。
