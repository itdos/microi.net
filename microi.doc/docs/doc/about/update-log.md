...
- **流量观测按高并发场景实行“内存热窗口 + MongoDB 有界明细 + MySQL 固定桶汇总”**：热路径使用并发分钟桶、固定保留时间和接口／IP／用户／租户／内容类型基数上限，普通请求不逐条写 MySQL；仅大流量、上传下载或可疑请求进入现有异步日志队列并批量落 MongoDB，后台服务每 5 分钟只把各维度 Top 10 的统计结果幂等 Upsert 到 `mci_network_traffic_rollup`。单请求 1 MB、20 MB、100 MB 分级识别，匿名大文件上传、失败大响应等场景给出风险级别、原因与解决方案；持久化失败只写受限诊断且不阻断业务请求。应用包同步交付流量汇总、IP 规则、告警规则和观测设置等 4 张观测表及索引，兼顾诊断价值、租户隔离和写放大控制。
...
- **官方商城“系统日志/监控”独立升级为可完整迁移的 v7.0.9 应用**：官方源 `app.microi.sys-log` 已发布不可变版本 `01M0R3HV1KB47GH80VH0GW5Y7M`，内含 10 张表定义、204 个字段、13 条 DDL、431 个物理列、2 个 `Platform/Managed` 接口引擎，以及平台内置微服务 v1.8.7 的 37 个私有源码文件、4 个公有构建文件和 9 条路由；应用包不携带历史 `upload/` 路径。安装包与源代码包的清单、哈希、版本日志和可安装状态均从官方 `microi_itdos` 回读确认。该应用仍须在【应用商城】单独安装／更新，升级 v7.5.9 平台框架不会替用户自动改写已安装应用。
