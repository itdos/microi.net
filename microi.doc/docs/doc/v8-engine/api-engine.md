...
接口引擎开启【分布式锁】后，平台使用当前租户共享 Redis 协调多个 API/Worker 节点。锁 Key 自动包含 `OsClient`；未配置 `LockKey` 时按接口 Key 串行，配置 `LockKey` 时应填写请求参数的字段名，例如 `OrderId`，平台会用本次请求的 `OrderId` 值区分不同订单，避免所有订单排在同一队列。

| 调用方式 | 租约行为 | 适用场景 |
|---|---|---|
| 普通 HTTP 或普通 V8 调用 | `Timeout` 是固定单次租期，不自动续租 | 短事务、单次状态推进 |
| 平台可信持久后台任务 | `Timeout` 是单次 Redis 租期，持有期间自动续租；当前默认最长续租边界为 12 小时，显式配置得更长的单次租期不会被缩短 | 数据库还原、初始化、迁移等可靠长任务 |

`Timeout` 同时参与当前接口的执行预算配置；平台没有 `LockTimeout` 字段。普通调用必须把 `Timeout` 设置为大于正常执行时间的合理值。长任务不应为了防止锁过期而盲目设置数小时的单次租期，应通过 `RunBackground` 提交持久后台任务，由平台管理续租、取消、进度和终态。

::: warning 自动续租只信任平台后台执行上下文
自动续租要求服务端已经建立可信后台身份、任务 Id 和 fencing token。HTTP Body、Query 或普通 V8 调用自行传入 `_BackgroundTaskId`、`_TrustedServerInvocation`、`_CurrentUser`，都不能把本次请求升级为可信后台任务。
:::

平台为每次成功获取的锁生成唯一持有者令牌和单调递增 fencing token，续租与释放都必须确认当前仍是同一持有者。发生持有者令牌不匹配、锁已过期、Redis 所有权/续租确认失败，或者达到最长租约时，调用会失败关闭；不得把这类错误吞掉后返回成功，也不得让已经失去租约的旧执行者继续写入。

分布式锁只能减少并发，不能单独保证业务“只执行一次”。库存、扣款、积分、流水、迁移和批量导入还必须使用稳定幂等键、数据库唯一约束或条件更新、状态机、outbox/inbox；超过 10 分钟的任务仍要按检查点分片，并用 `_BackgroundTaskFencingToken` 拒绝旧执行者写入。SaaS ZIP 开库的页面进度和运维判断见 [SaaS 引擎的开库章节](../system-engine/saas-engine)。
