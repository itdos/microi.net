...
原生详情通过 `POST /api/FormEngine/GetFormRelatedData` 读取日志、评论和版本记录。
请求固定携带 `ParentFormEngineKey`、`ParentTableRowId`、`_SysMenuId`，
`RelatedType` 只接受 `Counts`、`DataLog`、`DataComment`、`DataVersion`。
租户和身份以当前 DiyToken 为准。

后端先校验父表菜单，再执行真实父记录读取及 `ServerDataV8`。
父记录不存在或数据过滤拒绝时，关联读取也拒绝；有父菜单权限不能代替父行授权。
通过后才按固定辅助表和“父表 Id + 父记录 Id”条件查询，普通角色不会因此获得辅助表直读权限。

| 返回能力 | 当前边界 |
| --- | --- |
| 日志 | 仅操作类型、时间、记录 Id，不返回历史 `Content`、自由文本标题或人员资料 |
| 版本 | 仅版本号、动作、时间、记录 Id，不返回历史 `Data`、备注或人员资料 |
| 评论 | 必须存在真实 `TableId` 绑定；不能仅用 `TableRowId` 猜测旧评论归属 |

`DataAppend.HistoryContentMode = 'MetadataOnly'` 表示只有历史元信息，前端禁用预览、对比和加载。
当前行经脱敏后可读，不等于其历史原始字段可读。
评论缺少父表绑定时，`Counts` 仍返回可用的日志和版本计数，但 `DataComment = null`，
并以 `DataAppend.DataCommentUnavailableReason/Message` 说明原因；评论详情返回明确失败。
界面显示“暂不可用”，不能显示成“0 条”。

这项兼容修复需要对应前后端版本。安全评论写入及旧评论归属处理是独立能力，
当前不能仅更新应用包便宣称已开放，也不会自动补猜或迁移旧评论。
...
原生查询的 `$CurrentUser.Id$`、`$CurrentUser.RoleIds$`、`$CurrentUser.Level$`
使用本次后端授权快照中的用户 Id、有效角色 Id 与有效级别。即使旧 Token 仍含已撤销角色，
只要标准权限变更已使快照失效，列表、Count、SUM 和导出的 SQL 就不会再按旧角色放行。
字段选项 SQL 同样遵循此规则；角色集合可继续用于 `IN ($CurrentUser.RoleIds$)`。

该替换只创建 SQL 局部副本，部门等其它兼容字段保留原语义，不修改 `V8.CurrentUser`。
浏览器不能提交授权快照或可信服务端标记；快照用户与当前身份不一致、用户已禁用时拒绝查询。
这需要更新后端二进制，单独更新菜单或应用包不能让旧后端获得此保护。

用户访问密钥继续使用其绑定的有效账号身份，并叠加原有 scope、路由与表范围限制。
它不是独立服务账号；所属账号不存在或停用时，不能借密钥中残留的角色或级别继续查询。

这项修复不改变查询数据源：只读副本延迟、父事务已有旧快照，以及 SQL 联查的业务授权表，
仍需分别验证其一致性。Count 和 SUM 早于数据过滤事件执行，不能只靠逐行脱敏保护统计范围。

#### 按表选择主库读取

安全关系等不能接受副本延迟的数据表，可在表单设计器“表单信息”中启用“主库读取”，
...
| `1`，没有显式事务 | 单条、列表行、Count/CountBatch、SUM、树计数和导出使用该表所属数据库的主库 |
| 已传 `DbTrans` | 继续读取原事务，包括本事务尚未提交的数据，不改变隔离级别 |
| 非法值或所需主库不可用 | 返回失败，不静默回退到副本 |

扩展库表选择它自身的主库，不会跳回租户基础库。`V8.DbRead` 的显式调用保持原义。
SQL 联查的授权关系如果位于父事务已有的 RR 快照中，仍可能读取旧关系；此配置只能解决
独立原生查询的副本延迟，不能代替当前身份复核、菜单范围或即时撤权设计。

部署顺序为支持 `FormEngineReadPrimaryV1` 的后端、包含该字段的正式表单引擎应用，
再配置需要的表。表单引擎 `v7.7.4` 候选提供可空物理列及设计器字段；包不会替换后端
二进制。MCP 应逐项回读 `ReadPrimary`，不能把保存 `Code=1` 代替落库验收。完整验收还应
覆盖普通身份的列表、计数、汇总、导出和撤权旧会话，并区分真实数据库测试与离线连接选择测试。
