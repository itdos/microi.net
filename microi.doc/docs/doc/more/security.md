...
- **管理员专用**：SaaS 配置、接口引擎、菜单/角色/权限、任务、数据源、MQ/MQTT、密钥、基础设施、AI 私有配置和安全审计等资源，通用客户端 FormEngine 对 `Level < 9999` 的全部操作硬拒绝。
- **只读委托**：`diy_table`、`diy_field`，以及工作流、微服务/应用商店、蓝图和微应用运行元数据。普通角色只有获得真实菜单或高级表 `Read` 权限后才能查询；新增、修改、删除、导入和导出仍要求 `Level >= 9999`。
- **按角色管理**：`mic_page`、`mic_print`、`sys_user`。登录用户按菜单或高级表中明确授予的 `Read`、`Add`、`Edit`、`Del` 操作；未授权仍返回 `NoAuth`。账号管理另受角色层级、自身权限不可提升及密码字段脱敏约束。
...
在 `/#/system/role` 的【表直连权限】中选择 `diy_table` 或 `diy_field`，仅勾选【查】并保存，
对应角色即可通过普通 FormEngine 查询，不必为单纯读取目录再编写接口引擎。例如：

```javascript
await V8.FormEngine.GetTableData('diy_table', {
  _SelectFields: ['Id', 'Name', 'Description'],
  _PageIndex: 1, _PageSize: 20
});
```

该授权包含表/字段配置及其中的事件源码，应仅授予可信角色；`_SelectFields` 是本次查询的选列，
不是字段权限边界。只需对外提供少量安全字段时，仍应使用固定投影的接口引擎。
本能力需要升级含此策略的后端，前端会读取服务端策略并显示【仅可授权查询】；
只更新前端或应用数据包不会解除旧后端限制，也不会自动恢复或新增角色授权。
...
SaaS 引擎的“JWT密钥状态”只读显示“已配置”或“待自动生成”，不显示密钥原文。新建或接入旧库租户后，保存/刷新配置由后端生成强随机 Key 并持久写入主控库 `sys_osclients.AuthSecret`；NULL、空字符串和旧弱值均可自动修复，多节点竞争时采用已经持久化的同一个 Key。重复保存、刷新和重启不会替换已有有效 Key。普通表单提交不能写入签名根；如需主动轮换，使用受控的“JWT密钥目标轮换版本”配置。
...
2. 需立即处理时，从**未被该 IP 封禁的管理网络**登录平台超级管理员，进入【系统日志 → 安全防护】找到该 IP 并解除；对应受保护接口是 `POST /apiengine/mci-system-observability-action`，请求体使用 `{ Action: "UnblockIp", Ip: "目标IP" }`。被封禁的同一出口无法调用自己的解封接口，这是预期的安全边界。
...
DiyToken 不是“随便生成的一段 JWT”，而是吾码为低代码、多租户和多终端场景建立的会话协议。它与 `sys_user`、角色、部门、菜单、表单权限、数据范围、接口引擎和共享 Redis 登录态一起构成完整体系：

- Token 固定绑定 `OsClient`、用户、终端类型和 `did`，跨租户不能复用。`did` 是设备标识，不是登录会话标识；同一设备上的两个小程序即使共用 `did`，也应保留各自登录态。客户端未提供 `did` 时，服务端为本次会话生成独立标识。
- 同一帐号可管理 PC、移动端、VS Code、MCP、访问密钥等多类终端；续签与单端退出只作用于当前登录会话，管理员可吊销指定会话或全部终端登录态。
- 每次请求同时校验签名与共享会话事实，支持 Token 轮换、并发续签、用户停用/角色变化后的服务端失效，不依赖单机粘性会话。
- 认证只确认“是谁”；菜单动作、表权限、字段元数据、部门/角色数据范围、平台控制面和接口引擎仍逐层在服务端授权。
- `V8.CurrentUser`、`V8.OsClient` 和可信后端调用上下文来自同一体系，接口引擎无需另造用户表或权限 Token。
- 浏览器访问密钥、SSO、OAuth、Passkey 和严格人脸都作为身份入口接入 DiyToken，不能绕过或另建第二套业务权限。

可配置的企业双向 OIDC、SAML2、CAS、账号映射、协议端点与迁移验收见 [SSO 身份联邦](./sso)。`diy_sso` 属于管理员专用平台表，匿名登录页只能读取服务端白名单投影，不能通过通用 FormEngine 获取端点、映射或 Secret 设置 Key。

ASP.NET Identity 是通用的 ASP.NET Core 用户管理框架，但不会自动理解吾码的租户路由、低代码菜单、表单动作、数据范围、V8、在线终端和访问密钥协议。整体替换会造成大量数据库、客户端、V8 和应用包兼容迁移，却不会天然增强这些平台能力。因此吾码保持 DiyToken 为单一会话与权限事实源；需要标准身份协议时采用适配/联合登录，验证成功后仍签发 DiyToken，而不是整体替换。

### 可逆业务秘密与显示明文规范

吾码允许对“业务上必须再次取回原文”的字段使用 `V8.EncryptHelper.DESEncode/DESDecode`，例如设备口令、受控第三方账号密码或客户明确要求可显示的业务字段。这是**可逆业务秘密**，与登录密码、支付签名私钥、JWT/AuthSecret 等基础设施机密不同。

统一开发规则：

1. 保存前只在后端接口引擎或后端表单事件中加密，数据库不保存明文；前端不得直接调用解密。
2. 列表、导出、搜索、日志、缓存、通知和普通详情一律不返回明文，默认显示掩码或“已配置”。
3. “显示明文”必须使用独立后端动作，重新校验当前 DiyToken、准确 `OsClient`、菜单/角色或平台管理员权限；高风险场景再要求 [Passkey/人脸步进验证](./identity-verification)。
4. 每次查看记录操作人、目标记录、用途和结果，不记录明文本身；响应使用 `Cache-Control: no-store`，页面离开、超时或失焦后重新隐藏。
5. 禁止匿名、访问密钥会话、通用 FormEngine 列表和可编辑普通 V8 获得批量解密能力。密钥不能硬编码到脚本、前端、日志或应用包。
6. DES 是现有兼容格式；新设计若要抵抗数据库泄露，应优先由可信后端使用带版本的现代认证加密和集中密钥管理，同时保留相同的授权显示协议。不能把“数据库中不是明文”描述为能够抵抗服务器所有者或代码执行者。

登录密码不能套用上述业务字段规则。登录密码的新存储应使用带盐、可调成本的专用密码哈希；只有存量 `PwdEncode=DES` 保留下面的管理员兼容查看能力。
...
Passkey、设备生物识别、严格人脸、改密步进验证和自定义 V8 敏感操作的完整用法见 [Passkey、设备生物识别与严格人脸验证](./identity-verification)。
