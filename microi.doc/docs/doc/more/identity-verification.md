...
H5、AI 应用等可复用 `send-sms-reg`（图形校验后发信）与 `platform_auth_sms_login`（验证码登录/首次注册）接口引擎，最终仍签发 DiyToken。SaaS 引擎 `v8.3.9` 将缺失的公开图形校验包装入口纳入 `Managed` 应用资源；只有发送器 `send_sms_reg` 并不代表注册依赖完整。

发送参数为 `Phone`、`_CaptchaId`、`_CaptchaValue`，租户取可信 `V8.OsClient`。图形验证码校验成功后以 Redis `HashDelete` 的原子返回值决定是否获得消费权，按当前租户的 IP 和手机号分别限流，再交给短信发送器；内部派发证明不返回客户端，绑定手机号、短时有效且只能消费一次。旧 `send_sms_reg` 公开地址保留，但同样执行图形校验，**旧调用未携带图形验证码时将被拒绝**，不能继续依赖历史无需校验的发送行为。

供应商凭据仍在当前租户【系统设置 → 安全与服务接入】中维护，不写入应用源码或包内。发送结果未知时先核对供应商/平台记录，不自动重复发信。模拟验证码、供应商响应和 DiyToken 签发仅用于隔离测试；模拟通过不能标记为真实运营商送达。
