...
::: tip 平台核心定位
**典型业务场景，相比传统 AI 开发：Token 更省 10 倍+、交付更快 10 倍+、20+ 成熟引擎开箱复用；深度融合 V8 引擎，业务逻辑无需编译发布。**
...
`ai-platform-studio` v2.0.4 应用包包含：
...
v2.0.4 不改变表、菜单、接口和任务资源合同。治理工作台内部菜单改由微服务自身管理路由，页面按需加载，骨架屏只出现在右侧内容区；左侧治理导航与顶部栏不再随页面切换重挂。主题变量、通用按钮规则和背景网格均限定在 `[data-mci-ui-root="ai-platform-studio"]` 根容器内，不能覆盖吾码主框架 Logo 或菜单样式。
