...
整站、复杂应用或需要多人和 AI 持续维护的项目，应在项目根目录维护 <code>MCI-DESIGN.md</code>。它不是灵感说明，也不是另一份脱离源码的色板，而是人和 AI 都能执行、检查和持续演进的界面契约。
...
token 要按用途命名，组件通过 <code>{路径}</code> 引用共享值；颜色透明度必须说明叠加底色。组件按适用场景定义 default、hover、focus、pressed、loading、empty、error、disabled、selected、success，不能只写默认态。
...
2. 颜色、长度、数字、布尔值和状态对象类型正确；所有 <code>{路径}</code> 引用存在且无循环。
