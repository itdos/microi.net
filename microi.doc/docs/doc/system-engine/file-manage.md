...
<div class="file-cabinet-connection-grid">
  <article>
    ...
    <ul>
      ...
      <li>登录后先检查 `mci_file_sync_capability`；探针按目标租户实时回查文件柜菜单并返回 `FileManagerSysMenuId`，旧端能力不足或菜单不明确时会失败关闭并提示升级。</li>
...
远程私有对象不会直接复用列表里的存储路径作为可访问 URL。客户端以 `ResourceKind=FileManagerObject` 提交能力探针返回的 `SysMenuId`，后端再次校验平台超级管理员 DiyToken、权威文件柜组件、租户根目录、大小写精确对象 Key 与对象存在性后，才签发短效代理地址。普通用户、访问密钥、硬编码其它租户菜单、前缀相似路径以及不存在的对象都会被拒绝。
