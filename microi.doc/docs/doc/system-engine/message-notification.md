...
同一条平台内部通知还会投影到右上角聊天系统的 `admin` 平台会话。通知中心和聊天会话共用 `mic_msg_event_log` 这一份权威历史与已读状态：聊天侧不另存一份通知，也不把 SignalR 事件当作历史记录。即使从未与任何用户聊天，最近联系人仍固定显示“AI助手”和“admin”；`admin` 会话只读，避免用户误以为平台通知是普通点对点消息。
