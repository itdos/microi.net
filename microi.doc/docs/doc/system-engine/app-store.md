...
- `SharedPublicRuntime` 复用已经提交的公有编译文件，不上传客户端源码，也不复制大体积素材。
  包内 `EntryUrl` 保持不可变且不带查询参数；安装器生成的 `PreviewUrl` 另带目标租户的
  `apiBase` 和 `OsClient`，不得直接继承发布方默认租户。目标系统设置的 `ApiBase` 必须有效，
  启动地址不包含 Token。导入器需达到 `v2.9.0`；旧租户先更新应用商城，再重新安装共享运行应用。
  验收须从安装后入口登录，并检查实际 HTTP 和 SignalR 使用目标租户，不能只检查安装任务成功。
