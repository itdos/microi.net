...
上述字段未配置或数据库值为 `NULL` 时不是“空白 Banner”。运行时会按真实 `diy_field` 智能选择：自动编号、
...
标题、副标题、图片和背景的**字段绑定**显式保存空字符串 `""` 时，表示不绑定该字段，
不能再次自动推断，也不能被旧配置别名覆盖。例如 `formBanner: { subtitleField: "" }`
表示不自动显示客户、类型或审计字段作为副标题；省略 `subtitleField` 才继续智能选择。
取消标题字段后仍保留表单名称作为标题，取消图片字段后仍使用默认图标。
这一空值语义需要包含该修复的 MCP 和平台前端；只更新其中一侧不能替代完整保存、回读与显示验收。
...
`microi_generate_system` 的最终验收及 `microi_validate_system` 也会按表回读这些配置，
缺失或与 Manifest 不一致时返回失败。写入成功或结构接口 `Code=1` 不表示整体验收通过，
还需检查 `Data.Passed`。旧库缺少上述物理列时，应完成平台正式升级，再配置并重新验收。
