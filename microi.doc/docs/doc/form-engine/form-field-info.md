...
接口引擎详情的评论通过可信入口提交。服务端校验父菜单、父表与父记录，固定作者和回复归属；
评论表使用 `ParentTableId + TableRowId` 绑定记录。旧评论缺少父表归属时不自动回填或猜测。
表单引擎应用 v7.7.5 补齐归属字段、评论复合索引及主库回读，需要配合支持该能力的前后端。

版本列表只读取版本号、动作和时间。代码管理表的有效平台管理员点击【预览】【查看差异】或
【加载】时，才按父表、记录及版本 Id 读取一条代码快照；【加载】后仍需正常保存才会生效。
代码快照仅覆盖其中包含的代码字段，不会清空名称、路由或其它未包含的配置。

打开、刷新或预览面板均不写入历史版本，也没有新增定时轮询或启动回填。
定时任务的运行时间更新继续使用原有条件更新，不触发日志、版本或表单事件；真实代码变化才生成版本。
