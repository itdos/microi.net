<template>
  ...
  >
    ...
    >
      ...
      <span class="mci-nuget-stats__metric" aria-live="polite">
...
const compactDownloads = computed(() => formatCompactDownloads(stats.value.totalDownloads, activeLocale.value))
const exactDownloads = computed(() => new Intl.NumberFormat(activeLocale.value).format(stats.value.totalDownloads))

const copy = computed(() => isEnglish.value ? {
  ariaLabel: 'Microi NuGet download statistics',
  eyebrow: 'LIVE NUGET DATA',
  title: 'Ranks first by downloads among publicly searchable .NET AI low-code platforms',
  description: 'Public packages owned by ITdos, aggregated live from the official NuGet V3 SearchQueryService.',
  downloads: 'downloads',
  liveSource: `${stats.value.packageCount} packages · official API live total`,
  fallbackSource: 'NuGet profile snapshot · official profile available',
  ...
  description: '实时汇总 ITdos 名下公开包的官方 totalDownloads。长期、可核验的开发者使用数据，可从侧面反映平台成熟度与开发者采用情况。',
  downloads: '累计下载',
  liveSource: `${stats.value.packageCount} 个公开包 · NuGet 官方 API 实时汇总`,
  fallbackSource: 'NuGet 官方主页快照 · 点击查看实时数据',
  methodNote: 'NuGet 各官方索引异步同步，数值可能短时略有差异；“位居首位”按可公开检索的同类平台下载量口径，不代表微软 / NuGet 官方评选。',
  linkTitle: '前往 NuGet 官方 ITdos 主页查看全部包与实时下载量'
})

const sourceText = computed(() => state.value === 'live' ? copy.value.liveSource : copy.value.fallbackSource)

onMounted(async () => {
  try {
    stats.value = await loadNugetOwnerStats()
    state.value = 'live'
  } catch {
