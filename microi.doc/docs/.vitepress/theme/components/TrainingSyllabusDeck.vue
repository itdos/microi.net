      ...
      <section v-for="(slide, index) in slideMeta" :key="slide.id" :id="`mci-training-${slide.id}`" class="mci-training-slide" :class="[`is-${slide.id}`, `is-kind-${slide.kind}`, { 'is-active': index === activeIndex }]" :aria-hidden="index === activeIndex ? 'false' : 'true'" :inert="index === activeIndex ? undefined : true" role="group" aria-roledescription="slide" :aria-label="`${index + 1} / ${slideMeta.length}，${slide.title}`">
        <div class="mci-training-slide__frame">
          <template v-if="slide.kind === 'cover'">
            <div class="mci-deck-cover-copy"><p class="mci-deck-eyebrow mci-deck-reveal">MICROI · ENTERPRISE TECHNICAL TRAINING</p><h1 class="mci-deck-cover-title mci-deck-reveal"><span>Microi吾码</span><strong>AI 开发框架</strong><em>技术培训大纲</em></h1><p class="mci-deck-cover-lead mci-deck-reveal">以功能点为路线，现场完成平台认知、引擎讲解、AI 开发与企业交付。</p><div class="mci-deck-cover-metrics mci-deck-reveal" aria-label="培训核心价值"><span><strong>10×+</strong> Token 更省*</span><span><strong>10×+</strong> AI 开发更快*</span><span><strong>20+</strong> 成熟引擎</span></div><p class="mci-deck-cover-proof mci-deck-reveal">成熟底座承接通用能力，让 AI 专注企业真正有差异的业务。<small>* 典型平台能力高复用场景，实际收益取决于需求与团队基线。</small></p><button class="mci-deck-start mci-deck-reveal mci-screen-only" type="button" @click="nextSlide">开始培训<svg viewBox="0 0 24 24" aria-hidden="true"><path d="m9 5 7 7-7 7"/></svg></button></div>
            <div class="mci-deck-core-visual mci-deck-reveal" aria-label="Microi 核心能力示意图"><div class="mci-deck-core-orbit is-outer"><span>20+ ENGINES</span><i></i><i></i></div><div class="mci-deck-core-orbit is-middle"><span>MCP + SKILLS</span><i></i><i></i></div><div class="mci-deck-core-orbit is-inner"><span>V8 RUNTIME</span><i></i></div><div class="mci-deck-core-mark"><img src="/icon.png" alt="Microi吾码"><strong>AI</strong><small>BUILD · RUN · DELIVER</small></div></div>
          </template>
          ...
          <template v-else-if="slide.kind === 'atlas'">
            <header class="mci-training-slide__heading mci-deck-reveal"><div><p class="mci-deck-eyebrow">05 · 20+ ENGINE ATLAS</p><h2>{{ slide.title }}</h2><p>{{ slide.summary }}</p></div><span>{{ padSlide(index) }}</span></header>
            <div class="mci-atlas-layout"><a class="mci-atlas-architecture mci-deck-reveal" href="/images/microi-ai-platform-architecture.svg" target="_blank" rel="noopener noreferrer" aria-label="新窗口打开 Microi吾码 AI 平台完整架构图"><picture><source srcset="/images/microi-ai-platform-architecture-3840x2160.png 3840w, /images/microi-ai-platform-architecture-1920x1080.png 1920w"><img src="/images/microi-ai-platform-architecture-1920x1080.png" alt="Microi吾码 AI 平台 20+ 引擎系统架构图"></picture><span>打开 SVG 高清架构图 ↗</span></a><div class="mci-atlas-directory"><article v-for="(group, groupIndex) in atlasGroups" :key="group.code" class="mci-atlas-domain mci-deck-reveal" :style="{ '--mci-deck-order': groupIndex }"><header><span>{{ group.code }}</span><strong>{{ group.title }}</strong></header><div><a v-for="entry in entriesInAtlasGroup(group)" :key="entry.id" :href="entry.href" target="_blank" rel="noopener noreferrer">{{ entry.nav }}<i>↗</i></a></div></article></div></div>
          </template>
          ...
          <template v-else-if="slide.kind === 'closing'">
            <div class="mci-deck-closing"><div class="mci-deck-closing__signal mci-deck-reveal"><i></i><img src="/icon.png" alt="Microi吾码"><span>READY TO BUILD</span></div><p class="mci-deck-eyebrow mci-deck-reveal">THANK YOU · {{ slideMeta.length }} / {{ slideMeta.length }}</p><h2 class="mci-deck-reveal">把 AI 的速度，<br><strong>变成企业交付力。</strong></h2><p class="mci-deck-reveal">从今天开始，少重复造轮子，多创造真正有差异的业务价值。</p><div class="mci-deck-outcomes mci-deck-reveal"><span>开源底座</span><i></i><span>20+ 引擎</span><i></i><span>MCP + Skills</span><i></i><span>全端交付</span><i></i><span>成熟案例</span></div><div class="mci-deck-closing__actions mci-screen-only mci-deck-reveal"><a href="/doc/getting-started/docker-run.html" target="_blank" rel="noopener noreferrer">立即开始使用</a><a href="/doc/" target="_blank" rel="noopener noreferrer">打开官方文档</a></div><small class="mci-deck-reveal">MICROI吾码 · OPEN-SOURCE AI DEVELOPMENT FRAMEWORK</small></div>
          </template>
        </div>
      </section>
    </main>
