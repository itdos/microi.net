...
-** プレーンテキスト **: テキストのさまざまなフォント、色、背景などのスタイルを設定できる、欠かせないキャラクター。
-** キー値対テキスト **: 印刷テンプレートの一般的なフォーマットは、上記のように設定されています。
-** 動的表 **: 非常に強力な動的表で、様々なシーンのニーズを実現でき、試してみるとわかる。
-** 静的フォーム **: 基本的なビジネスニーズを満たすことができます。
-** 二次元コード **: 基本的なニーズを満たすことができ、より良い体験感が必要な場合は、草料二次元コードを使って写真を作ることができる。
-** 写真 **: 門戸担当、人は衣装、馬は鞍、美しいテンプレートが欠かせない。
-** バーコード **: 基本的なニーズを満たすことができ、より良い体験が必要な場合は、草料二次元コードを使って写真を作ることができる。
-** データグラフ **: 'echart' グラフを設定できます。
-** 線 **: 実線、破線などがあり、基本的なニーズを満たし、スタイルが調整できる。
-** 基本図形 **: 円形、長方形などがあります。
-** 装丁線 **: 実は線の一種です。
-** ビジネスコンポーネント **: 実際のビジネスシーンに基づいて、よく使われるビジネステンプレートコンポーネントをプリセットすることで、時間を節約できます。
-** Html **:一般的には使われていません。主に問題を解決するために、できないテンプレートに遭遇したら、それがあることを忘れないでください。

## インテグレーション方式

### Npmパッケージ統合
-** メリット **: 拡張コンポーネントをカスタマイズして、自分のページを好きなようにデザインして、追加のビジネスロジックを書くことができます。
-** デメリット **: 要件は 'Vue2' やその他のフロントエンドフレームワークをサポートしていない 'vue3 viteオプティマスプラスフラッドarts' フレームワークに基づいている必要があります。
