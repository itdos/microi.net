...
## プロフィール
印刷エンジンは主に低コードプラットフォームの動的印刷タスクを解決しました。`iframe` `html`コンポーネントは、基本的にお客様の失礼な印刷要求を満たすことができます。
...
4. npmパッケージの依存関係
ここでは、すべてのサードパーティの依存パッケージですが、すべてのサードパーティのコンポーネントをインストールする必要はありません`npm`パッケージはいくつか内蔵されているので、次のように依存すればよい。
```js
...
```
5.完全demo
ここまでで基本的な前提条件が設定され、次は新規作成するだけです`vue`ページは、次のコードを参考にして走ることができます。簡単ではありませんか。
