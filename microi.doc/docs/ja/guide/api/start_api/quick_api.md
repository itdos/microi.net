クイックハンドこのチュートリアルでは、バックエンド・プロジェクトを迅速に実行できます。

準備作業- 使用 `Git` 工具从开源地址拉取最新代码 [microi.net](https://gitee.com/ITdos/microi.net)。
- 下载并安装 `.NET 9 SDK` ：[.NET 9 SDK](https://dotnet.microsoft.com/zh-cn/download)
- 要使项目跑起来，至少需要【数据库 + `Redis`】两个环境，缺一不可

【Visual Studio Code】を使用してソリューションを開く (推奨)- 下载并安装 `Vs Code`：[Vs Code](https://code.visualstudio.com)
...
【Visual Studio 2022】を使ってソリューションを開く- 下载并安装 `vs2022` ：[vs2022](https://visualstudio.microsoft.com/zh-hans)
...
環境配置に関する注意事項- 若没有 `MongoDB` 环境，则无法使用系统日志功能
