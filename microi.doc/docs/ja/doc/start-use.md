クイックスタート紹介* 不同于其它低代码平台的新建菜单、下一步、下一步、下一步即可创建好一个模块，Microi吾码面向开发者，采用传统开发思维流程来创建模块
...
方式1: 物理テーブルを作成する* 进入【表单引擎】模块，新增一条数据即会自动往数据库新建一张物理表
* 多数据库时可选择指定在哪个数据库创建表（不选择则是主库）

方式2: 物理テーブルを作成する* 进入【数据库管理工具（如Navicat）】新增一张物理表，并新增需要的字段
...
3、デザインフォーム* 查看在表单引擎中创建的数据，操作栏点击【设计】，即可进入表单设计器
* 每拖一个字段控件则会立即往数据库添加一个物理字段，之后的每次保存都是修改字段

4.メニュー (モジュール) を作成する* 进入【模块引擎】，新增一条数据，选择打开方式为【Diy】，选择刚刚创建的物理表，选择模板为【搜索+表格】或【搜索+卡片】（可扩展更多模板）
* 更多配置玩法见平台文档【模块引擎.md】

5、作成したばかりのメニューに入る* 进入菜单后即可看到增、删、改、查功能已全部就绪
