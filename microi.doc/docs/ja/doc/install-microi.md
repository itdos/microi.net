...
> * _ _ <Font color = "red">注意: 本番環境では、ワンクリックインストールスクリプトを推奨するのではなく、「mysqlプログラムdockerをネイティブにインストールする」という形でプラットフォームをインストールしますドキュメント [Docker配備](https://microi.net/doc/docker-run.html) を参照してください。</Font> _ _ _
