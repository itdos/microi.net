...
```
### MySqlのインストール
> * _ _ <Font color = "red"> サーバーパネルを使用してmysqlをネイティブにインストールすることをお勧めします。
> * _ _ <Font color = "red">注意: 宝塔パネルを使用してubuntu 24にネイティブにインストールされたmysql 8.0は、3306ポートを他のポートに変更しても起動できない可能性があります3306に戻ると起動し、一時的に解決策が見つからないので、3306をそのまま使用することをお勧めします
> * _ _ <Font color = "red"> データベースをインストールした後:</font _ _
> * 1、パネルのデータベースパフォーマンス構成を使用して、サーバが16GBの実行メモリである場合、最適化シナリオは8-16GBを推奨します
> * 2、設定ファイルに [mysqld] の下に【low er_case_table_names = 1】を追加します。
> * _ _ <Font color = "red">3、サーバーパネルのデータベース管理を使ってデータベースを復元してみます。ビューとビューの間にsqlが関連付けられていると、リストアに失敗します。データベースのリストアに失敗した場合は、Navicatのデータ転送機能を使用してデータベースのリストアを試みることができます (成功率は100% です) ビューとビューの間に関連sqlがある場合は、単一のビューを復元してください。
...
> * _ _ <Font color = "red"> 宝塔を使用してubuntu 24にネイティブにインストールされたmysql 8.0は、3306ポートを他のポートに変更しても起動できない可能性があります3306に戻ると起動し、一時的に解決策が見つからない
