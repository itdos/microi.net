# AI代書インターフェースエンジン
> * 使用`AI`Microi吾コードを素早く学習します。`V8接口引擎`使用法、および`数据库`アーキテクチャ、直接生成`接口引擎代码`、精度が高い`99%`左右
> * そうであれば`在线AI`、文章の中で言及した`.md`ファイル、`db.json`ファイルはオンラインでアップロードできます`AI`学習を行います。`几秒`勉強が終わります。
> * 注意: 会話の会話が多すぎると、コンテキストが失われる可能性があります。**# 复雑なビジネスロジックスを强く提案し、各インタフェスに新たな会话を** を追加します。AIの学習が非常に速いからです数秒で私のコードMD文書を学ぶことができます
> * 2回目のセッションは、1回目のセッションで生成されたものをそのまま使用できます`db.json`ファイルのため、複数のセッションでインタフェースを書くのと同じくらい効率的です
...
## AIが生成したインタフェースエンジンコード (一度のテストに成功し、100% 正確)
::: details AIが生成したインターフェイスエンジンコード
```js
/**
 * Microi吾码数据库结构获取接口引擎
 * 
...
## 【データベース構造の取得】を実行するインターフェイスエンジンコード
> * 上記のインタフェースエンジン要求を実行する前に`F12`を選択します`Data`の値で、完全なデータベース構造を取得できます。
> * ブロガーのプロジェクトの一つ`600多张表`、`1万个字段`データベース構造も`300多KB`ない`1秒钟`データベース構造を取得できます。フォーマット後の`db.json`ファイルはおおよそ`2M`左右

## AIにデータを移行するためのインタフェースエンジンを書いてもらいます
```
...
## ビジネスロジックを記述するプレフィックス
> *`为了防止有时出现的AI没关联到上下文，博主是每次写一个新的接口都会加上此前缀`
> * AIを`学习`あなたの経験はプログラミングしています。そうでなければ、AIが生成したコードは可能です`不会考虑那么多`
```
...
## AIが生成したインターフェイスエンジンコード (約2、3回の会話を経て、ビジネスロジックの記述を調整して成功し、100% 正確)
::: details AIが生成したインターフェイスエンジンコード
```js
/**
 * 批量排班接口引擎
...
## AIが生成したインターフェイスエンジンコード (ブロガーが問題にぶつかったら、AIはコンテキストを失う)
> *`解决方案`はい`新开一个会话`を選択します
>* **複雑なビジネスロジックを強くお勧めします。「インターフェイスごとに新しい会話」は、AIの学習が非常に速く、数秒で私のコードMD文書を学習できるからです**
::: details AIが生成したインターフェイスエンジンコード
```js
/**
 * 生产主计划引入接口引擎
...
## まとめ
> * 博主プロジェクトの実践を経て、説明するだけである`足够准确`,`AI`生成された`接口引擎代码`正解率はほぼ`100%`