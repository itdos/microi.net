...
## データソース設定
> * **関连テーピング**:joinテーブル、テーブルの別名を設定します

> * **クエリエ**:selectのフィールド

> * **列を表示しません**: 一部のidフィールドはselect后にテーブルに表示する必要はありません

> * _ * ソート可能な列 _ *: ソートできるフィールド

> * **デフォルテのソ-ト列**: デフォルトのマルチフィールドのソート

> * **検索可能な列**: 検索できるフィールドは何ですか?

> * **統計列**: 統計が必要なフィールド

> * **テ-ブル内編集**: テーブル内編集をオンにすると、編集可能な列も設定する必要があります

> * **Join関连**: 関連表を自由に作成する条件

> * **Where条件**: where条件を自由に作成し、権限制御を実現します

> * **インポートティエンプレ**: インポートテンプレートを作成してユーザーにダウンロードさせる
