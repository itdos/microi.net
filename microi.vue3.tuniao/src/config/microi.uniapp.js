...
=======
 * v1.8.7
>>>>>>> d1f03c73dfe508d85159b47a7881731d6f9a5e36
  【初始化】：
 ...
 */
export var Microi = {
	ApiBase: ApiBase || uni.getStorageSync('ApiBase') || 'https://api-china.itdos.com', // 'https://api-china.itdos.com', //接口前缀，前端写死
	OsClient: OsClient || uni.getStorageSync('OsClient') || 'iTdos',//'iTdos', //OsClient值，前端写死
	...
	PageUrlLogin : '/pages/login/login',
	Api: {
	...
	InitApi: function(){
		Microi.ApiBase = uni.getStorageSync('ApiBase') || ApiBase; // 'https://api-china.itdos.com', //接口前缀，前端写死
		Microi.OsClient = uni.getStorageSync('OsClient') || OsClient; //'iTdos', //OsClient值，前端写死
	},
	/**
	 * 平台初始化
	 * ）根据域名获取OsClient值
	...
	**/
	Run : async function(v8Code){// 
		try{
			...
			Microi.Tips('执行前端V8代码出现异常：' + e.message)
