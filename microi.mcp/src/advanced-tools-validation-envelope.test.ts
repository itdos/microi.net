...
async function validateBanner(rows: unknown, validation: unknown = { Code: 1, Data: { Passed: true, Errors: [], Warnings: [] } }, table = bannerTable, readError = false) {
  ...
  registerAdvancedTools({ tool(name: string, ...args: any[]) { handlers.set(name, args.at(-1)); } } as any, {
    ...
    getTableData: async (name: string, query: unknown) => { queries.push({ name, query }); if (readError) throw new Error('internal database details'); return { Code: 1, Data: rows }; },
...
test('generate_system uses the same persisted Banner gate after successful metadata writes', async () => {
  for (const persisted of [false, true]) {
    ...
    registerAdvancedTools({ tool(name: string, ...args: any[]) { handlers.set(name, args.at(-1)); } } as any, {
      ...
      updateTable: async (row: unknown) => { writes.push(row); return { Code: 1 }; },
    ...
    assert.equal(payload.results.some((step: any) => step.step === 'configureFormBanner' && step.response.Code === 1), true);
  }
});
