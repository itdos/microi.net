...
export function buildStoreApplicationBackgroundRequest(input: StoreApplicationTaskInput): {
  ApiEngineKey: string;
  Title: string;
  Param: JsonRecord;
  Options: JsonRecord;
...
function buildDefaultFormOpen(table: JsonRecord): { type: string; width: string } {
  ...
  if (explicitType) return { type: explicitType, width: explicitWidth || '80%' };
...
export function registerAdvancedTools(server: McpServer, client: MicroiClient, context: McpServerContext): void {
  ...
  server.tool('microi_list_database_backup_tenants', `List the enabled MySQL tenants eligible for database backup on the current backend runtime. The server strictly filters sys_osclients by its own OsClientType and OsClientNetwork and never returns connection strings. OsClient ${osClient}.`, {}, async () => (
    apiText('Database Backup Tenants', await client.listDatabaseBackupTenants())
  ));
  ...
  const registerStoreApplicationTool = (
    toolName: 'microi_install_store_application' | 'microi_update_store_application',
    operation: StoreApplicationOperation,
  ): void => {
    ...
    server.tool(
      ...
      async (input) => {
        ...
        return apiText(
          `${operationText}商城应用`,
          await client.runBackgroundApiEngine({
