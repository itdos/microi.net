...
import type {
  ...
  DbTable,
  DbField,
  PlaywrightContextData,
  PlaywrightEngineInfo,
  PlaywrightModuleInfo,
  TranslateTextResult,
...
import {
  buildMcpOcrResult,
  decodeMcpTranslatedFile,
  OCR_MAX_BASE64_CHARACTERS,
  prepareMcpOcrInput,
  prepareMcpTranslateFileInput,
  saveMcpTranslatedFile,
  TRANSLATE_INLINE_RESULT_BYTES,
  TRANSLATE_MAX_BASE64_CHARACTERS,
...
interface RemoteMicroServiceSourceBaseline {
  currentVersion: number;
  appVersion: string | null;
  sourceManifestHash: string | null;
}

interface RemoteMicroServiceSourceContext {
  application: Record<string, unknown>;
  capabilities: Record<string, unknown>;
  baseline: RemoteMicroServiceSourceBaseline;
  files: Array<Record<string, unknown>>;
}

function microServiceSourceIdentity(microService: Record<string, unknown>): string {
...
function parseRemoteMicroServiceSourceContext(result: ApiResponse): RemoteMicroServiceSourceContext {
  if (result.Code !== 1) {
...
function sameMicroServiceSourceBaseline(
  left: RemoteMicroServiceSourceBaseline,
  right: RemoteMicroServiceSourceBaseline,
): boolean {
...
function contextMatchesLocalMicroServiceSource(
  result: ApiResponse,
  manifest: LocalMicroServiceSourceManifest,
): boolean {
  if (result.Code !== 1) return false;
  const context = asJsonRecord(result.Data);
  if (String(context.SourceManifestHash || '') !== manifest.manifestHash) return false;
  const remoteFiles = Array.isArray(context.Files)
    ? context.Files.map(file => asJsonRecord(file))
    : [];
  if (remoteFiles.length !== manifest.files.length) return false;
...
export function buildMicroServiceSourceDeliveryBatchId(
  appIdOrKey: string,
  sourceManifestHash: string,
): string {
...
function validateMicroServiceSourceStageEvidence(
  result: ApiResponse,
  expected: {
    appIdOrKey: string;
    deliveryBatchId: string;
...
async function readRemoteMicroServiceSourceContext(
  client: MicroServiceSourceSyncClient,
  appIdOrKey: string,
): Promise<ApiResponse> {
...
function buildLegacyInlineSourceFiles(
  explicitFiles: Array<Record<string, unknown>>,
): { files: Array<Record<string, unknown>>; totalSize: number } {
  if (explicitFiles.length > LEGACY_MICRO_SERVICE_SOURCE_MAX_FILES) {
    throw new Error(
      `旧服务源码兼容通道最多允许 ${LEGACY_MICRO_SERVICE_SOURCE_MAX_FILES} 个文件；请升级后端并改传 directory。`,
    );
  }
  let totalSize = 0;
  const files = explicitFiles.map((file, index) => {
    const rawBase64 = getStringField(file, 'FileByteBase64', 'ContentBase64', 'Base64');
    if (!rawBase64) throw new Error(`sourceFiles[${index}] 缺少 FileByteBase64`);
    let base64 = rawBase64;
    if (/^data:/iu.test(base64)) {
      const dataUrl = base64.match(/^data:[^,\r\n]*;base64,([A-Za-z0-9+/=]*)$/iu);
      if (!dataUrl) throw new Error(`sourceFiles[${index}] 不是合法的 Base64 data URL`);
      base64 = dataUrl[1];
    }
    if (base64.length > LEGACY_MICRO_SERVICE_SOURCE_MAX_BASE64_CHARACTERS) {
      throw new Error(
        `sourceFiles[${index}] Base64 编码长度超过安全上限 `
    ...
    if (totalSize > LEGACY_MICRO_SERVICE_SOURCE_MAX_BYTES) {
      throw new Error(
        `旧服务源码兼容通道最多允许 ${LEGACY_MICRO_SERVICE_SOURCE_MAX_BYTES} bytes；请升级后端并改传 directory。`,
      );
    }
    return file;
  });
 ...
 */
export async function runMicroServiceSourceSync(
  client: MicroServiceSourceSyncClient,
  input: MicroServiceSourceSyncInput,
  ...
  try {
    ...
    if (!appIdOrKey) throw new Error('microService 必须包含 MsKey、MicroServiceKey 或 AppKey。');
    ...
    if (initialContextResult.Code === 2) {
      throw new Error(
        `在线 AI 应用不存在：${appIdOrKey}。请先调用 microi_create_microservice 创建应用，`
        + '源码同步不会在无法建立远端基线时隐式写入。',
      );
    }
    if (initialContextResult.Code !== 1) {
      throw new Error(
        `读取远端源码能力/基线失败，已拒绝 legacy 写入：`
    ...
    if (!localManifest || !supportsSourceStream) {
      if (localManifest
        ...
        throw new Error(
          `远端未暴露 SourceStreamPublish，旧兼容通道仅允许 ${LEGACY_MICRO_SERVICE_SOURCE_MAX_FILES} 个文件/`
      ...
      try {
        ...
        if (!localManifest) {
          throw new Error(
            '旧服务源码整包响应中断，sourceFiles 兼容调用没有可验证的本地完整清单；'
        ...
        if (!contextMatchesLocalMicroServiceSource(readback, localManifest)) {
          throw new Error(
            '旧服务源码整包响应中断，完整路径/大小/SHA-256/清单哈希回读未确认提交结果；'
            + '已停止且不会盲目重放。'
            + `原始错误：${error instanceof Error ? error.message : String(error)}`,
            { cause: error },
          );
    ...
    for (const file of localManifest.files) {
      ...
      if (recoveredFinalStateDuringStage) break;
    }
    ...
    if (!recoveredFinalStateDuringStage) {
      ...
      if (contextMatchesLocalMicroServiceSource(beforeFinalizeResult, localManifest)) {
        ...
        if (!sameMicroServiceSourceBaseline(beforeFinalize.baseline, baseline)) {
          throw new Error(
            '源码 finalize 前检测到 CurrentVersion/AppVersion/SourceManifestHash 基线漂移，'
    ...
    if (!recoveredFinalStateDuringStage) {
      const finalizeRequest: MicroServiceSourceFinalizeRequest = {
        AppIdOrKey: appIdOrKey,
        DeliveryBatchId: deliveryBatchId,
      ...
      try {
        ...
        if (!contextMatchesLocalMicroServiceSource(readback, localManifest)) {
          throw new Error(
            '私有源码 finalize 响应中断，完整清单回读未确认提交结果；已停止且不会盲目重放。'
            + `原始错误：${error instanceof Error ? error.message : String(error)}`,
            { cause: error },
          );
        }
...
export function createMcpServer(client: MicroiClient, context: McpServerContext): McpServer {
  ...
  server.tool(
    'microi_update_table',
    `Update a diy_table record for OsClient "${osClient}" (for example form layout, V8Limit, data log/comment/version switches, Description or IsTree). Only provided fields are patched. Automatically clears diy_table + diy_table_field_list Redis caches.`,
    {
  ...
  server.tool(
    ...
    async ({ microService, directory, sourceFiles, replace, confirmExecution }) => (
      runMicroServiceSourceSync(client, {
        microService,
        directory,
        sourceFiles,
        replace,
