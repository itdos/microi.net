...
test('secret tool discovers the existing settings facility, confirms writes and never returns values', async () => {
  ...
  const backend = {
    ...
    saveServerPrivateSecret: async (input: unknown) => { writes.push(input); return { Code: 1, Data: { Value: secret } }; },
  ...
  const call = async (params: object) => await client.callTool({ name: 'microi_codex', arguments: { action: 'microi_manage_server_private_secret', params } }) as CallToolResult;
  try {
    const blocked = await call({ action: 'Save', configKey: 'Integration.Changjet.AppSecret', value: secret });
    assert.equal(blocked.isError, true); assert.equal(writes.length, 0);
    ...
    assert.notEqual(saved.isError, true); assert.equal(writes.length, 1);
    ...
    const uncertain = await call({ action: 'Save', configKey: 'Integration.Changjet.AppSecret', value: secret,
      confirmExecution: 'SAVE:Integration.Changjet.AppSecret' });
    assert.equal(uncertain.isError, true); assert.doesNotMatch(JSON.stringify(uncertain), /test-secret-not-for-output/);
