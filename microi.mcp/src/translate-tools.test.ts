...
test('translation MCP tools expose the complete safe tenant-bound business surface', async () => {
  let textRequest: TranslateTextRequest | undefined;
  let fileRequest: TranslateFileRequest | undefined;
  let suggestionRequest: Record<string, string> | undefined;
  const audits: Array<{ action: string; target: string; content: string }> = [];
  const fakeClient = {
    translateText: async (input: TranslateTextRequest) => {
      textRequest = input;
      return {
        Code: 1,
        Msg: '',
    ...
    translateFile: async (input: TranslateFileRequest) => {
      fileRequest = input;
    ...
    suggestTranslation: async (input: Record<string, string>) => {
  ...
  try {
    ...
    assert.equal((textRequest as unknown as Record<string, unknown>).OsClient, undefined);
    ...
    for (const [action, params] of [
      ...
      const response = await client.callTool({ name: 'microi_codex', arguments: { action, params } }) as CallToolResult;
    ...
    assert.equal((fileRequest as unknown as Record<string, unknown>).Endpoint, undefined);
