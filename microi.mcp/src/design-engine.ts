...
function sanitizePageChartText(value: unknown): string {
...
function parseJsonish(value: unknown): { ok: boolean; value?: unknown; error?: string } {
  if (value === undefined || value === null || value === '') return { ok: false, error: 'JSON value is empty.' };
...
function mergeRecord(base: JsonRecord, patch: unknown): JsonRecord {
...
function pageCode(title: string): string {
  const suffix = Math.random().toString(36).slice(2, 8);
  const cleaned = title.replace(/[^\w\u4e00-\u9fa5]+/g, '_').replace(/^_+|_+$/g, '').slice(0, 24);
  return `page_${cleaned || 'ai'}_${suffix}`;
}

function param(sort: number, label: string, type: string, value: unknown, typeOptions?: JsonRecord): JsonRecord {
  const result: JsonRecord = { sort, label, type, value };
  if (typeOptions) result.typeOptions = typeOptions;
  return result;
}
...
function makeWidget(type: string, label: string, wrapperNumber: number, span: number, height: number, widgetParams: JsonRecord[]): JsonRecord {
  return {
    type,
    label,
    category: 0,
...
function makeWrapper(title: string, span: number, height: number, widgetListFactory: (wrapperNumber: number) => JsonRecord[], dark: boolean): JsonRecord {
...
function scenarioFromPrompt(prompt: string, theme?: string): string {
  const text = `${prompt} ${theme || ''}`.toLowerCase();
  if (/维保|维修|设备|工单|巡检|maintenance|service|ticket|asset/.test(text)) return 'maintenance';
  if (/商城|商品|库存|订单|门店|mall|shop|ecommerce|retail|sku/.test(text)) return 'mall';
  if (/财务|发票|收款|付款|finance|invoice|payment/.test(text)) return 'finance';
  if (/销售|客户|商机|营收|sales|customer|crm|revenue/.test(text)) return 'sales';
  return 'operations';
}

function scenarioData(scenario: string): {
  title: string;
  subtitle: string;
  stats: JsonRecord[];
  xAxis: string[];
  barSeries: JsonRecord[];
  lineSeries: JsonRecord[];
  pieData: JsonRecord[];
  tableHeaders: JsonRecord[];
...
function textElement(left: number, top: number, width: number, height: number, title: string, options: JsonRecord = {}): JsonRecord {
  return {
    options: {
      left,
      top,
...
function hline(left: number, top: number, width: number): JsonRecord {
  return {
...
function rect(left: number, top: number, width: number, height: number): JsonRecord {
  return {
...
function printScenario(prompt: string): string {
  return scenarioFromPrompt(prompt);
}

export function buildPrintTemplateDesign(input: PrintBuildInput): { pageObj: JsonRecord; printObj: JsonRecord } {
...
function unwrapPageCandidate(value: unknown): unknown {
  const parsed = parseJsonish(value);
  if (!parsed.ok) return value;
  let current = parsed.value;
  if (typeof current === 'string') {
    const again = parseJsonish(current);
    if (again.ok) current = again.value;
  }
  const record = asRecord(current);
  if (record.JsonObj !== undefined) return unwrapPageCandidate(record.JsonObj);
  if (record.jsonObj !== undefined) return unwrapPageCandidate(record.jsonObj);
  if (record.JsonStr !== undefined) return unwrapPageCandidate(record.JsonStr);
  if (record.jsonStr !== undefined) return unwrapPageCandidate(record.jsonStr);
  const formData = asRecord(record.formData ?? record.FormData);
  if (Object.keys(formData).length) {
    if (formData.JsonObj !== undefined) return unwrapPageCandidate(formData.JsonObj);
    if (formData.jsonObj !== undefined) return unwrapPageCandidate(formData.jsonObj);
    if (formData.formConfig !== undefined || formData.wrapperList !== undefined) return formData;
  }
  return current;
}

function normalizePageWidgetDataJson(widgetRecord: JsonRecord, path: string, warnings: string[]): void {
  const widgetType = getString(widgetRecord, 'type');
  if (!Array.isArray(widgetRecord.widgetParams) || widgetRecord.widgetParams.length === 0) return;

  const widgetParams = widgetRecord.widgetParams as unknown[];
  const titleIndexesByType: Record<string, number[]> = {
    bar: [5, 6],
    line: [5, 6],
    linebar: [5, 6],
    pie: [5, 6],
  ...
  for (const index of titleIndexesByType[widgetType] ?? []) {
    ...
    if (!Object.keys(titleParam).length || titleParam.value === undefined || titleParam.value === '') continue;
...
export function normalizePageJsonObj(value: unknown): NormalizeResult {
  const candidate = unwrapPageCandidate(value);
  const parsed = parseJsonish(candidate);
  if (!parsed.ok) return { ok: false, errors: [`JsonObj is not valid JSON: ${parsed.error}`], warnings: [] };
  const obj = asRecord(parsed.value);
  const errors: string[] = [];
  const warnings: string[] = [];

  if (!Object.keys(obj).length) errors.push('JsonObj must be an object.');
  if (obj.formConfig !== undefined && Object.keys(asRecord(obj.formConfig)).length === 0) warnings.push('formConfig is empty; defaults were applied.');
  if (obj.wrapperList !== undefined && !Array.isArray(obj.wrapperList)) errors.push('JsonObj.wrapperList must be an array.');

  const wrapperList = Array.isArray(obj.wrapperList) ? obj.wrapperList.map((item, index) => {
    const wrapper = asRecord(item);
    if (!getString(wrapper, 'type')) wrapper.type = 'pannel';
    if (!getString(wrapper, 'label')) wrapper.label = 'Card';
    if (wrapper.hidden === undefined) wrapper.hidden = false;
    ...
    if (!Array.isArray(wrapper.widgetList)) {
      ...
      wrapper.widgetList = wrapper.widgetList.map((widget, widgetIndex) => {
        ...
        if (!getString(widgetRecord, 'type')) errors.push(`wrapperList[${index}].widgetList[${widgetIndex}].type is required.`);
  ...
  if (!Array.isArray(obj.wrapperList)) warnings.push('wrapperList was missing and was set to an empty array.');
...
function unwrapPrintCandidate(value: unknown): unknown {
  const parsed = parseJsonish(value);
  if (!parsed.ok) return value;
  let current = parsed.value;
  const record = asRecord(current);
  if (record.PageObj !== undefined) current = record.PageObj;
  else if (record.pageObj !== undefined) current = record.pageObj;
  return current;
}

export function normalizePrintPageObj(value: unknown): NormalizeResult {
  const candidate = unwrapPrintCandidate(value);
  const parsed = parseJsonish(candidate);
  if (!parsed.ok) return { ok: false, errors: [`PageObj is not valid JSON: ${parsed.error}`], warnings: [] };
  const obj = asRecord(parsed.value);
  const errors: string[] = [];
  const warnings: string[] = [];
  if (!Array.isArray(obj.panels)) errors.push('PageObj.panels must be an array.');
  const panels = Array.isArray(obj.panels) ? obj.panels.map((panel, panelIndex) => {
    ...
    panelRecord.printElements = (panelRecord.printElements as unknown[]).map((element, elementIndex) => {
      const elementRecord = asRecord(element);
      const type = getString(asRecord(elementRecord.printElementType), 'type');
      if (!type) errors.push(`panels[${panelIndex}].printElements[${elementIndex}].printElementType.type is required.`);
  ...
  if (panels.length === 0) errors.push('PageObj.panels must contain at least one panel.');
  return {
    ok: errors.length === 0,
    value: { ...obj, panels },
...
export function normalizePrintObj(value: unknown): NormalizeResult {
  if (value === undefined || value === null || value === '') {
    return { ok: true, value: {}, json: '{}', errors: [], warnings: [] };
  }
  const parsed = parseJsonish(value);
  if (!parsed.ok) return { ok: false, errors: [`PrintObj is not valid JSON: ${parsed.error}`], warnings: [] };
  return { ok: true, value: parsed.value as JsonRecord, json: JSON.stringify(parsed.value), errors: [], warnings: [] };
}
...
export function pageDesignPayload(input: PageBuildInput): { title: string; number: string; desc: string; jsonObj: JsonRecord; jsonStr: string } {
...
export function printDesignPayload(input: PrintBuildInput): { title: string; number: string; desc: string; pageObj: JsonRecord; pageObjStr: string; printObj: JsonRecord; printObjStr: string; dataApi: string } {
