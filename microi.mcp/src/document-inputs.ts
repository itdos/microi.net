...
function decodeMcpOcrBase64(value: string): Buffer {
  ...
  if (normalized.length > OCR_MAX_BASE64_CHARACTERS) throw new Error('OCR 文件超过 MCP 100 MB 硬上限。');
...
export function prepareMcpOcrInput(input: {
  ...
  if (hasPath) {
    ...
    if (!localFileName || !path.extname(localFileName)) throw new Error('本地 OCR 文件必须使用受支持的扩展名。');
...
export function saveMcpTranslatedFile(outputFilePath: string, bytes: Buffer): string {
  const rawPath = String(outputFilePath || '').trim();
  const resolved = path.resolve(rawPath);
  if (!path.isAbsolute(rawPath)) throw new Error('outputFilePath 必须是绝对路径。');
