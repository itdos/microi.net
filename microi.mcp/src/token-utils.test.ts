...
function tokenWithIssuedAt(issuedAt: number, suffix: string): string {
  const encode = (value: object): string => Buffer.from(JSON.stringify(value)).toString('base64url');
...
test('selects a newer tenant token while retaining exact-key order for opaque ties', () => {
  ...
  assert.equal(
    selectPreferredAuthorizationTokenFromCandidates([staleExact, refreshedUntyped]),
    refreshedUntyped,
  );
  assert.equal(
    selectPreferredAuthorizationTokenFromCandidates(['opaque-exact', 'opaque-fallback']),
    'opaque-exact',
  );
});
