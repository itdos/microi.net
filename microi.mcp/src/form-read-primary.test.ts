...
test('actual createTable uses standard metadata update only when requested and preserves failure', async () => {
  for (const value of [undefined, null, 0, 1]) {
    const calls: { endpoint: string; data: any }[] = [];
    const fake = { config: { osClient: 'bound-tenant' },
      async post(endpoint: string, data: any) { calls.push({ endpoint, data }); return { Code: 1, Data: { TableId: 'server-table' } }; },
      updateTable: MicroiClient.prototype.updateTable,
    };
    const result = await MicroiClient.prototype.createTable.call(fake as any, 'Biz_Safe', '', { ReadPrimary: value });
    assert.equal(result.Code, 1); assert.equal(calls.length, value === undefined ? 1 : 2);
    assert.equal(Object.hasOwn(calls[0].data, 'ReadPrimary'), false);
    if (value !== undefined) assert.deepEqual(calls[1].data, { OsClient: 'bound-tenant', Id: 'server-table', ReadPrimary: value });
  }
  const fake = { config: { osClient: 'bound-tenant' }, post: async () => ({ Code: 1, Data: { TableId: 't' } }),
    updateTable: async () => ({ Code: 0, Msg: 'ReadPrimary 主库回读不一致' }) };
  assert.equal((await MicroiClient.prototype.createTable.call(fake as any, 'Biz_Safe', '', { ReadPrimary: 1 })).Code, 0);
});

test('registered validate tool rejects missing/mismatched ReadPrimary, including null vs zero', async () => {
  for (const expected of [null, 0, 1]) for (const actual of [undefined, null, 0, 1]) {
    ...
    registerAdvancedTools({ tool(name: string, ...args: any[]) { handlers.set(name, args.at(-1)); } } as any, {
      ...
      getTableData: async (_name: string, query: any) => { queries.push(query); return { Code: 1, Data: [{ Id: 't', Name: table.name,
