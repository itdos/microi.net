...
function result(response: ApiResponse): CallToolResult {
  ...
  const business=envelope?.Result && typeof envelope.Result==='object'
    ? envelope.Result as ApiResponse : response;
  return {content:[{type:'text',text:JSON.stringify(business,null,2)}],isError:business.Code!==1};
}
async function call(client:MicroiClient,action:string,input:Record<string,unknown>):Promise<CallToolResult>{
  try{return result(await client.executeEngine('mci-email',{...input,Action:action}));}
  catch{return {content:[{type:'text',text:'邮箱请求未得到确定回执。请检查连接；发信只能用同一草稿 Id 查询或重试，不能创建新草稿重复投递。'}],isError:true};}
}
function confirmation(actual:string|undefined,expected:string):CallToolResult|null{
...
export function registerEmailTools(server:McpServer,client:MicroiClient,context:McpServerContext):void{
 server.tool('microi_email_query',`Read the current user's email workspace through ApiEngine mci-email on tenant ${context.osClient}. Supports accounts/folders, paginated mail, body, attachment, connection test and durable sync status. Requires the installed email app and platform V8.Email. Account credentials are never returned.`,{
  ...
  if(['Read','Attachment'].includes(input.action)&&!input.id)return {content:[{type:'text',text:'该查询需要邮件 id。'}],isError:true};
  if(input.action==='TestConnection'&&!input.accountId)return {content:[{type:'text',text:'检测连接需要 accountId。'}],isError:true};
  if(input.action==='SyncStatus'&&!input.taskId)return {content:[{type:'text',text:'查询同步状态需要 taskId。'}],isError:true};
 ...
 server.tool('microi_email_manage','Manage an owned mailbox or draft through ApiEngine mci-email. SaveAccount reuses mic_email_server and its form events; credentials remain backend-encrypted. Sync queues durable work; reuse requestId. Flags and Move affect the remote mailbox. This tool never sends SMTP mail. Omit confirmExecution for a mutation-free preview.',{
  ...
  const preview=confirmation(input.confirmExecution,input.action+':'+target);if(preview)return preview;
  if(input.action==='SaveAccount'&&!input.account)return {content:[{type:'text',text:'SaveAccount 需要 account。'}],isError:true};
  if(input.action==='SaveDraft'&&(!input.accountId||!input.draft))return {content:[{type:'text',text:'SaveDraft 需要 accountId 和 draft。'}],isError:true};
  if(['Sync','Preferences'].includes(input.action)&&!input.accountId)return {content:[{type:'text',text:'该操作需要 accountId。'}],isError:true};
  if(input.action==='Sync'&&!input.requestId)return {content:[{type:'text',text:'同步需要稳定 requestId。'}],isError:true};
  if(['Flags','Move'].includes(input.action)&&!input.id)return {content:[{type:'text',text:'该操作需要邮件 id。'}],isError:true};
  // 审计只记录动作与内部标识，不记录授权码、邮件正文、地址或附件。
  try{await client.writeAuditLog('microi_email_manage',target,JSON.stringify({action:input.action}));}catch{}
 ...
 server.tool('microi_email_send','Send one previously saved email draft through ApiEngine mci-email. First read the draft and obtain the user’s authorization for its recipients and content. confirmExecution must exactly match draftId. Reuse the same draftId after an uncertain response: Sent is SMTP acceptance; Unknown must be checked in Sent folders before any new send intention.',{
  ...
  const preview=confirmation(input.confirmExecution,input.draftId);if(preview)return preview;
  try{await client.writeAuditLog('microi_email_send',input.draftId,JSON.stringify({action:'Send'}));}catch{}
