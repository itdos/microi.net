...
test('uploadApplicationAssetStream sends raw multipart bytes without Base64 materialization', async () => {
  ...
  try {
    globalThis.fetch = (async (_input: string | URL | Request, init?: RequestInit) => {
      ...
      assert.equal(
        init?.headers && (init.headers as Record<string, string>)['Content-Length'],
        String(captured.byteLength),
      );
...
test('uploadApplicationAssetStream falls back to native HTTP with the same raw multipart payload', async () => {
  ...
  try {
    ...
    assert.match(
      ...
      new RegExp(`filename="microi-asset-${crypto.createHash('sha256').update(raw).digest('hex').slice(0, 16)}\\.bin"`, 'u'),
    );
