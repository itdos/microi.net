...
function output(value:unknown,isError=false):CallToolResult{return {content:[{type:'text',text:JSON.stringify(value,null,2)}],isError};}
function unwrap(response:ApiResponse):ApiResponse {
 const envelope=response.Data as {Result?:ApiResponse}|undefined;
 return envelope?.Result&&typeof envelope.Result==='object'?envelope.Result:response;
}
async function execute(client:MicroiClient,key:string,params:Record<string,unknown>):Promise<ApiResponse>{return unwrap(await client.executeEngine(key,params));}
function failed(message:string):CallToolResult{return output({Code:0,Msg:message},true);}
function uncertain():CallToolResult{return failed('未取得确定回执。请先按原 Id / Key 查询已保存的数据；提醒保存和发布重试必须复用原 requestId，不能重新创建同一通知。');}
...
export function registerMessageNotificationTools(server:McpServer,client:MicroiClient,context:McpServerContext):void {
 server.tool('microi_get_notification_context',`读取当前租户 ${context.osClient} 的统一消息通知中心：能力、原 mic_msgset 业务配置、系统公告、接收对象、微信模板及分页投递记录。仅查询当前租户角色；Tenants/Editions 使用 AllAccounts 或 SuperAdmins，绝不读取其它平台的角色列表。本工具不会发送消息。`,{
  ...
  try {
   if(['BusinessRule','Reminder','History'].includes(input.action)&&!input.id)return failed('此查询需要明确的 id。');
   if(input.action==='Capabilities'){
    const reminders=await execute(client,reminderEngine,{Action:'Capabilities'});
    if(reminders.Code!==1)return output(reminders,true);
    const configuration=await execute(client,configEngine,{Action:'Capabilities'});
    if(configuration.Code!==1)return output(configuration,true);
   ...
   let key=configEngine,action:string=input.action;
   if(input.action==='BusinessRules')action='List';
   if(input.action==='BusinessRule')action='Get';
 ...
 server.tool('microi_configure_business_notification','通过 Managed 配置接口引擎校验或保存多渠道业务通知，继续存入原 mic_msgset 表，保留 Key、用户/角色、模板及通道适配器契约。ChannelApiEngineMap 只填写接口 Key，不得放入密钥。Validate 不写入、不发送；Save 的 confirmExecution 为 Save:<id 或 Key>，编辑还需 id 和查询得到的 expectedRevision。保存成功自动按 Id 回读。本工具不会调用 msg_event 或向外发送消息。',{
  ...
  if(input.id&&input.action==='Save'&&input.expectedRevision===undefined)return failed('编辑配置必须提供查询得到的 expectedRevision。');
  if(input.action==='Save'&&input.confirmExecution!==expected)return preview(expected,input.action,target);
  try{
   if(input.action==='Save')await client.writeAuditLog('microi_configure_business_notification',target,JSON.stringify({action:'Save'}));
   ...
   if(result.Code!==1||input.action!=='Save')return output(result,result.Code!==1);
   const saved=result.Data as {Rule?:{Id?:string}}|undefined;
   if(!saved?.Rule?.Id)return failed('保存响应缺少记录 Id，请按原 Key 查询回读。');
 ...
 server.tool('microi_manage_system_reminder','在统一消息通知应用中校验、保存、发布或撤回系统公告。Save 只保存草稿；只有通过官方 License 身份判断的服务可选择 Editions，主租户可选择 Tenants。AccountScope 使用 SuperAdmins 或 AllAccounts，由接收服务按本地可信身份裁剪。AfterServerRestart 要求接收协议 2，每个帐号在每次 API 启动批次中领取一次。只有用户授权对应内容和接收范围后才发布。结果不确定时复用原 requestId；confirmExecution 为 <action>:<id 或 requestId>。',{
  ...
  if(['Save','Publish'].includes(input.action)&&!input.requestId)return failed('保存和发布必须提供稳定 requestId，并在重试时复用。');
  const target=input.id||input.requestId||'new',expected=`${input.action}:${target}`;
  if(input.action!=='Validate'&&input.confirmExecution!==expected)return preview(expected,input.action,target);
  try{
   if(input.action!=='Validate')await client.writeAuditLog('microi_manage_system_reminder',target,JSON.stringify({action:input.action,requestId:input.requestId,expectedRevision:input.expectedRevision}));
   ...
   if(result.Code!==1||input.action==='Validate')return output(result,result.Code!==1);
   const resultData=result.Data as {Id?:string}|undefined,id=input.id||resultData?.Id;
   if(!id)return failed('操作响应缺少规则 Id，请按原请求查询回读。');
