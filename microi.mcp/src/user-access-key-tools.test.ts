...
import type { CreateUserAccessKeyInput, MicroiClient } from './microi-client.js';
...
test('current-user access-key tools require confirmation and expose plaintext only once', async () => {
  const created: CreateUserAccessKeyInput[] = [];
  ...
  const fakeClient = {
    ...
    createMyUserAccessKey: async (input: CreateUserAccessKeyInput) => {
      created.push(input);
      return {
        Code: 1,
        Data: {
    ...
    revokeMyUserAccessKey: async (id: string) => {
      revoked.push(id);
  ...
  try {
    ...
    const confirmation = JSON.parse(toolText(blockedCreate)) as {
      RequiredConfirmationSha256: string;
    ...
    assert.match(
      toolText(create),
      /\/\?OsClient=iTdos#\/access-login\?access_key=<AccessKey>&redirect=%2Fmic%2Fdata-dashboard%2Fpreview%2F01KK988A0YPHKAM8SF216917HX/,
    );
