...
test('job diagnostics and logs stay read-only and preserve backend failures and cursor bounds', async () => {
    const calls = [];
    const backend = {
        executeEngine: async (key, params) => {
            assert.equal(key, 'platform-schedule-job');
            calls.push(params);
    ...
    const call = (params) => client.callTool({ name: 'microi_codex', arguments: { action: 'microi_query_job_runtime', params } });
    try {
...
//# sourceMappingURL=job-runtime-tools.test.js.map