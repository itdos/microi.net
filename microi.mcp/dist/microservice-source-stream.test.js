...
import { buildLocalMicroServiceSourceManifest, buildMicroServiceSourceDeliveryBatchId, runMicroServiceSourceSync, } from './server.js';
...
function activeContext(manifest) {
...
test('source stream stages raw local paths under one batch and finalizes with all three CAS values', async () => {
    await withProject(async (directory, manifest) => {
        ...
        assert.equal(stages[0]?.DeliveryBatchId, buildMicroServiceSourceDeliveryBatchId('demo-source', manifest.manifestHash));
        ...
        assert.deepEqual(finalizeRequest?.Manifest.map(item => item.Path), manifest.files.map(item => item.relativePath));
...
test('old server falls back only for a bounded source directory', async () => {
    await withProject(async (directory, manifest) => {
        ...
        const files = legacyPayload?.sourceFiles;
...
//# sourceMappingURL=microservice-source-stream.test.js.map