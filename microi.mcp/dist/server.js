...
export function createMcpServer(client) {
    ...
    // Tool: 获取服务器状态
    // ========================
    server.tool('microi_get_status', 'Check connection status to the Microi backend server', {}, async () => {
        try {
    ...
    // Tool: 获取数据库表结构
    // ========================
    server.tool('microi_get_db_schema', 'Get database table structures from Microi platform. Returns table names, field names, types, labels, and descriptions. Use this to understand the data model before writing queries or API engines.', {
    ...
    // Tool: 列出接口引擎
    // ========================
    server.tool('microi_list_engines', 'List all API engines (接口引擎) on the Microi platform. Each engine is a JavaScript function that runs on the server with access to V8 APIs for database queries, HTTP calls, caching, etc.', {
    ...
    // Tool: 获取引擎源码
    // ========================
    server.tool('microi_get_engine_code', 'Get the JavaScript source code of a specific API engine by its key. Use this to understand existing engine logic before modifying or creating new engines.', {
    ...
    // Tool: 执行接口引擎
    // ========================
    server.tool('microi_run_engine', 'Execute an API engine on the Microi server and return the result. WARNING: This may have side effects (database writes, external API calls) depending on the engine logic. Use with caution.', {
    ...
    // Tool: 列出 V8 事件
    // ========================
    server.tool('microi_list_events', 'List V8 events (table events) on the Microi platform. Events are triggers that run before/after table operations like insert, update, delete, and form validation.', {
