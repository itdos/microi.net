...
import { generateMiniMaxMusic3Fallback, isRetiredMiniMaxMusicApi, } from './minimax-music3-fallback.js';
...
export function validateApplicationAssetV3FinalizeEvidence(result, expected) {
    const evidence = asJsonRecord(result.Data);
    ...
    if (expected.publishMode === 'stage') {
        ...
        if (verifying) {
            ...
            for (const [current, total] of [['VerifiedCount', 'TotalCount'], ['VerifiedBytes', 'TotalBytes']]) {
                const done = evidence[current];
                const all = evidence[total];
                if (!Number.isSafeInteger(done) || !Number.isSafeInteger(all) || done < 0 || all < 0 || done > all) {
                    throw new Error(`v3 stage ${current}/${total} 校验进度无效`);
                }
            }
            if (expected.assetCount !== undefined)
                requireStreamEvidenceNumber(evidence, 'TotalCount', expected.assetCount, context);
            if (expected.totalSize !== undefined)
                requireStreamEvidenceNumber(evidence, 'TotalBytes', expected.totalSize, context);
        }
 ...
 */
export async function tryLegacyMicroServiceStreamPublishFallback(client, manifest, input) {
...
function buildBrowserAccessKeyLoginUrlTemplates(osClient, redirectPath) {
 ...
 */
const TABLE_FIELD_SORT_COUNTER = new Map();
function nextSortFor(tableId) {
...
export function createMcpServer(client, context) {
    ...
    server.tool('microi_generate_minimax_speech', `Generate one original Chinese game voice line through the authenticated Microi AI engine for OsClient "${osClient}". The server fixes MiniMax speech-2.8-hd, approved system voices and mono MP3 output, keeps provider credentials private, persists the result to tenant HDFS, and uses RequestId for idempotency. This consumes an external AI quota, so confirmExecution must exactly equal requestId.`, {
    ...
    server.tool('microi_create_my_access_key', `Create one revocable, bounded access key for the current authenticated user on OsClient "${osClient}". This is not a permanent admin/MCP bypass: the backend stores only a SHA-256 hash, returns plaintext exactly once, and exchanges the key for a short scoped session. The MCP surface never creates permanent keys; omitting expiresAt uses the backend's bounded default (currently 90 days). Omit scopes for the minimum page:open + form:read permissions. For a browser kiosk, combine the one-time AccessKey with the Microi.Client frontend origin using /?OsClient=${encodeURIComponent(osClient)}#/access-login?access_key=<AccessKey>&redirect=<encodeURIComponent(redirectPath)>; do not use the API origin and do not append the key to the destination page. The first call is a dry confirmation step and returns RequiredConfirmationSha256; repeat the exact same payload with confirmExecution equal to that SHA-256.`, {
        ...
        try {
            ...
            const loginUrlTemplates = buildBrowserAccessKeyLoginUrlTemplates(osClient, confirmation.normalized.redirectPath);
    ...
    server.tool('microi_redis_list_keys', `SCAN Redis keys for OsClient "${osClient}" without using blocking KEYS. Returns type, TTL, memory estimate and an opaque next cursor.`, {
    ...
    server.tool('microi_redis_get_key', `Read one Redis key for OsClient "${osClient}". Supports String, Hash, List, Set, Sorted Set and Stream with bounded pagination.`, {
    ...
    server.tool('microi_redis_delete_keys', `Delete up to 500 Redis keys for OsClient "${osClient}". This is irreversible and requires confirmExecution="DELETE".`, {
    ...
    server.tool('microi_redis_replace_value', `Create or replace a Redis String/Hash/List/Set/Sorted Set for OsClient "${osClient}". Existing content is replaced; confirmExecution must equal the exact key or EXECUTE.`, {
        key: z.string().describe('Exact Redis key.'),
    ...
    server.tool('microi_redis_rename_key', `Rename a Redis key for OsClient "${osClient}" without overwriting an existing target. Requires confirmExecution equal to the new key or EXECUTE.`, {
    ...
    server.tool('microi_redis_set_ttl', `Set Redis TTL for OsClient "${osClient}". -1 persists, 0 deletes, positive values are seconds. Requires confirmExecution equal to the key or EXECUTE.`, {
        key: z.string().describe('Exact Redis key.'),
    ...
    // ========================
    server.tool('microi_seed_table_data', `Seed sample/demo rows into any low-code table for OsClient "${osClient}". Wraps V8.FormEngine.AddTableData. Use this for filling商品/订单/会员等样例数据。Each row will get Id/CreateTime/OsClient auto-filled by the platform.`, {
    ...
    // Tool: 上传平台文件
    // ========================
    server.tool('microi_upload_file_base64', `Upload a base64 file to Microi platform HDFS for OsClient "${osClient}". Use this for app images, posters, banners and other assets instead of third-party image URLs. Optionally writes the uploaded platform file path back to a low-code table field.`, {
    ...
    // Tool: 列出流程节点 V8 事件
    // ========================
    server.tool('microi_list_workflow_v8_events', `List workflow node V8 events from WF_Node for OsClient ${osClient}. WF_Line is returned only in the workflow package snapshot; executable route condition code is WF_Node.LineValueV8.`, {
    ...
    // Tool: 获取流程节点 V8 代码
    // ========================
    server.tool('microi_get_workflow_v8_code', `Get workflow node V8 JavaScript code from WF_Node by nodeId and event type (OsClient: ${osClient}).`, {
    ...
    // Tool: 保存流程节点 V8 代码
    // ========================
    server.tool('microi_save_workflow_v8_code', `Save workflow node V8 code into WF_Node for OsClient ${osClient}. Empty code clears the field without adding a generated header.`, {
        nodeId: z.string().describe('WF_Node.Id'),
        eventType: z.string().describe('WF_Node V8 field: StartV8 | EndV8 | StartV8Server | EndV8Server | LineValueV8 | AllowAddUserV8Code'),
    ...
    }, async ({ action, keyword, type, category, source, level, levelMin, searchMonth, pageIndex, pageSize, windowMinutes, windowSeconds, top, includeHost, includeDocker, traceId, incidentId, serviceName, apiEngineKey, name, lines, kind, status, dimensionType, rangeKey, hours, observedOsClient, transferAction, ip, userId, endpoint, poolTarget, operationId, }) => {
        try {
    ...
    // ========================
    server.tool('microi_query_mongodb_logs', `Query Microi MongoDB system logs (sys_log_<osClient>/log_yyyyMM) for OsClient ${osClient}. Use this after automated tests to inspect V8 errors, slow logs, workflow logs and platform guard logs.`, {
    ...
    // ========================
    server.tool('microi_write_mongodb_log', `Write a Microi MongoDB system log for OsClient ${osClient}. Useful for AI automated test milestones, reproduction markers, and repair verification notes.`, {
    ...
    server.tool('microi_add_layout_field', `Add a metadata-only form layout field for OsClient "${osClient}". Supports CollapseGroup, Divider and Tabs. The backend receives an empty physical Type and therefore must not create or alter a database column. Use this instead of microi_add_field for layout nodes. The operation is idempotent and verifies the saved metadata by readback.`, {
        ...
        try {
            ...
            const saved = unwrapList(readback.Data)
    ...
    server.tool('microi_bulk_apply_form_layout', `Read, plan or apply metadata-only form layout changes for OsClient "${osClient}". A dry-run may omit expectedFingerprint to obtain the current SHA-256 state; real writes require that fingerprint, preventing stale AI snapshots from overwriting another developer's changes. The tool may add CollapseGroup/Divider/Tabs metadata nodes, patch only Tab/Sort/Config/FormWidth/TableWidth on explicitly listed fields, and patch diy_table.Tabs. It verifies the final layout and proves table/field V8 code plus field Data remain unchanged. Re-running a partially completed batch is safe.`, {
        ...
        const readState = async (tableId) => {
            ...
            if (tableResponse.Code !== 1)
                throw new Error(tableResponse.Msg || '读取 diy_table 失败');
            const tableRows = unwrapList(tableResponse.Data);
            if (tableRows.length !== 1)
                throw new Error(tableRows.length ? 'TableId 命中多张表' : '未找到表');
            ...
            if (fieldResponse.Code !== 1)
                throw new Error(fieldResponse.Msg || '读取 diy_field 失败');
            return {
                table: tableRows[0],
                fields: unwrapList(fieldResponse.Data),
            };
        };
        ...
        const fingerprint = (state) => crypto
            .createHash('sha256')
            .update(JSON.stringify(canonicalState(state)), 'utf8')
            .digest('hex');
        const matchesExpectedJson = (actual, expected) => {
        ...
        const desiredLayoutMatches = (state, plan) => {
            if (plan.tableTabs !== undefined) {
                try {
                    if (!matchesExpectedJson(JSON.parse(String(state.table.Tabs || '[]')), JSON.parse(plan.tableTabs || '[]')))
                        return false;
                }
                catch {
                    return false;
            ...
            for (const patch of plan.fieldPatches || []) {
                ...
                if (!field)
                    return false;
                if (patch.tab !== undefined && String(field.Tab || '') !== patch.tab)
                    return false;
                if (patch.sort !== undefined && Number(field.Sort) !== patch.sort)
                    return false;
                if (patch.config !== undefined && String(field.Config || '') !== patch.config)
                    return false;
                if (patch.formWidth !== undefined && (field.FormWidth ?? null) !== patch.formWidth)
                    return false;
                if (patch.tableWidth !== undefined && Number(field.TableWidth) !== patch.tableWidth)
                    return false;
            }
            for (const layoutField of plan.layoutFields || []) {
                const field = state.fields.find(item => String(item.Name || '') === layoutField.name);
                if (!field || String(field.Component || '') !== layoutField.component)
                    return false;
            }
            return true;
        };
        const immutableState = (state, newNames) => ({
            tableEvents: {
                SubmitFormV8: state.table.SubmitFormV8 ?? '',
                SubmitBeforeServerV8: state.table.SubmitBeforeServerV8 ?? '',
                SubmitAfterServerV8: state.table.SubmitAfterServerV8 ?? '',
        ...
        for (const plan of plans) {
            try {
                ...
                for (const layoutField of plan.layoutFields || []) {
                    ...
                    if (addResult.Code !== 1)
                        throw new Error(`新增布局节点 ${layoutField.name} 失败：${addResult.Msg || ''}`);
                }
                if ((plan.fieldPatches || []).length) {
                    ...
                    if (updateResult.Code !== 1)
                        throw new Error(`批量更新字段失败：${updateResult.Msg || ''}`);
                }
                if (plan.tableTabs !== undefined) {
                    ...
                    if (updateTableResult.Code !== 1)
                        throw new Error(`更新表单 Tabs 失败：${updateTableResult.Msg || ''}`);
                }
                summary.updated++;
                ...
                if (plan.tableTabs !== undefined) {
                    ...
                    if (!matchesExpectedJson(actualTabs, expectedTabs))
                        throw new Error('回读 diy_table.Tabs 与计划不一致');
                }
                ...
                for (const fieldPatch of plan.fieldPatches || []) {
                    ...
                    if (!saved)
                        throw new Error(`回读未找到字段 ${fieldPatch.id}`);
                    if (fieldPatch.tab !== undefined && String(saved.Tab || '') !== fieldPatch.tab)
                        throw new Error(`字段 ${fieldPatch.id} Tab 回读不一致`);
                    if (fieldPatch.sort !== undefined && Number(saved.Sort) !== fieldPatch.sort)
                        throw new Error(`字段 ${fieldPatch.id} Sort 回读不一致`);
                    if (fieldPatch.config !== undefined && String(saved.Config || '') !== fieldPatch.config)
                        throw new Error(`字段 ${fieldPatch.id} Config 回读不一致`);
                    if (fieldPatch.formWidth !== undefined && (saved.FormWidth ?? null) !== fieldPatch.formWidth)
                        throw new Error(`字段 ${fieldPatch.id} FormWidth 回读不一致`);
                    if (fieldPatch.tableWidth !== undefined && Number(saved.TableWidth) !== fieldPatch.tableWidth)
                        throw new Error(`字段 ${fieldPatch.id} TableWidth 回读不一致`);
                }
                for (const layoutField of plan.layoutFields || []) {
    ...
    server.tool('microi_update_table', `Update a diy_table record for OsClient "${osClient}" (for example form layout, V8Limit, data log/comment/version switches, Description or IsTree). Only provided fields are patched. Automatically clears diy_table + diy_table_field_list Redis caches.`, {
        ...
        try {
            ...
            if (args.readPrimary !== undefined)
                patch.ReadPrimary = args.readPrimary;
            ...
            if (args.enableDataLog !== undefined)
                patch.EnableDataLog = args.enableDataLog === 1 ? 1 : 0;
            if (args.enableDataComment !== undefined)
            ...
            if (args.formBanner) {
                ...
                if (args.formBanner.titleField !== undefined)
                    patch.FormBannerTitleField = args.formBanner.titleField;
                if (args.formBanner.subtitleField !== undefined)
                    patch.FormBannerSubtitleField = args.formBanner.subtitleField;
                if (args.formBanner.imageField !== undefined)
                    patch.FormBannerImageField = args.formBanner.imageField;
                if (args.formBanner.icon !== undefined)
                    patch.FormBannerIcon = args.formBanner.icon;
                if (args.formBanner.backgroundField !== undefined)
                    patch.FormBannerBackgroundField = args.formBanner.backgroundField;
                if (args.formBanner.tagFields !== undefined)
                    patch.FormBannerTagFields = typeof args.formBanner.tagFields === 'string'
                        ? args.formBanner.tagFields
                        : JSON.stringify(args.formBanner.tagFields);
                if (args.formBanner.metrics !== undefined)
                    patch.FormBannerMetrics = typeof args.formBanner.metrics === 'string'
    ...
    // ========================
    server.tool('microi_run_engine_background', `Queue an API engine as a durable background task on Microi server (OsClient: ${osClient}). The task is visible in the notification center and survives process restarts. Reuse the same idempotencyKey after any timeout or uncertain response.`, {
        ...
        try {
            ...
            if (retryOnFailure !== undefined)
                options.RetryOnFailure = retryOnFailure;
    ...
    server.tool('microi_list_background_tasks', `List durable background tasks visible to the current authenticated Microi user (OsClient: ${osClient}), including progress, terminal status and failure details from the notification center.`, {}, async () => {
        try {
    ...
    server.tool('microi_cancel_background_tasks', `Request cancellation for one or more durable background tasks owned by the current authenticated Microi user (OsClient: ${osClient}). Running tasks stop cooperatively; pending tasks become canceled immediately.`, {
    ...
    // ========================
    server.tool('microi_configure_form_banner', `Configure the default-visible form Banner on diy_table for OsClient "${osClient}". Reads the real diy_field list, infers useful title/subtitle/image/tag/number fields when values are omitted, writes semantic diy_table fields, and verifies the saved configuration. Use after adding fields. Banner configuration never belongs to sys_menu.`, {
        tableId: z.string().optional().describe('Exact diy_table.Id. Provide tableId or tableName.'),
        ...
        try {
            ...
            const fields = unwrapList(fieldResponse.Data);
            ...
            if (!resolvedTableId || !resolvedTableName) {
                ...
                const tables = unwrapList(locateResponse.Data);
                if (tables.length !== 1) {
            ...
            const saved = unwrapList(readbackResponse.Data)[0];
    ...
    server.tool('microi_bulk_update_table_features', `Plan or apply data-log, data-comment and data-version switches to many diy_table records for OsClient "${osClient}". Each table is read immediately before writing, patched independently through UpdateTable, and verified by readback. Existing V8 events, Tabs and unrelated table metadata are never sent or overwritten. Re-running the same batch is safe and resumes by skipping already-matched rows.`, {
        ...
        const readTable = async (locator) => {
            ...
            if (response.Code !== 1)
                throw new Error(response.Msg || '读取 diy_table 失败');
            const rows = unwrapList(response.Data);
            if (rows.length !== 1)
                throw new Error(rows.length ? '定位到多张同名表，请改用 TableId' : '未找到表');
            return rows[0];
        };
        for (const locator of tables) {
            try {
                ...
                if (result.Code !== 1)
                    throw new Error(result.Msg || 'UpdateTable 返回失败');
                summary.updated++;
                ...
                const mismatches = Object.entries(desired)
                    ...
                    .map(([key]) => key);
                if (mismatches.length)
                    throw new Error(`回读不一致：${mismatches.join(', ')}`);
                summary.verified++;
            }
            catch (e) {
                summary.ok = false;
    ...
    server.tool('microi_bulk_apply_module_presentation', `Read, plan or apply presentation-only sys_menu changes for OsClient "${osClient}". The tool supports ViewSchema Hero/List/Card, statistics/mobile/card fields, menu badges and badge-only changes to existing buttons/PageTabs. It fingerprints the complete current module before writing, rejects stale snapshots, and proves routes, table binding, SQL/data permissions, API replacements and button V8/business actions remain unchanged. New PageTabs require an explicit per-plan opt-in.`, {
        ...
        const moduleState = (value, immutableOnly = false) => Object.fromEntries(Object.entries(value)
            .filter(([key]) => !key.startsWith('_'))
        ...
        const fingerprint = (value) => crypto
            ...
            .digest('hex');
        const presentationProjection = (value) => Object.fromEntries([...presentationFields]
            .filter((key) => value[key] !== undefined)
            .map((key) => [key, value[key]]));
        const stableButtonValue = (value) => {
            const parsed = parseJson(value);
            if (typeof parsed === 'string')
                return parsed.replace(/\r\n/gu, '\n').trim();
            if (Array.isArray(parsed))
                return parsed.map(stableButtonValue);
            if (!parsed || typeof parsed !== 'object')
                return parsed ?? null;
            return Object.fromEntries(Object.entries(parsed)
                .filter(([key]) => !key.startsWith('_'))
        ...
        const buttonBusinessSignature = (value, fieldName) => {
            const parsed = parseJson(value);
            if (!Array.isArray(parsed))
                return canonical([]);
            const clean = parsed.map((raw, index) => {
                ...
                if (normalized.Sort === undefined)
                    normalized.Sort = index * 10;
                if (normalized.IsVisible === undefined)
                    normalized.IsVisible = true;
                if (fieldName === 'MoreBtns' && normalized.ShowRow === undefined)
                    normalized.ShowRow = true;
                return normalized;
            });
            return JSON.stringify(clean.map(stableButtonValue));
        };
        const isEmptyButtonList = (value) => {
            const parsed = parseJson(value);
            return !Array.isArray(parsed) || parsed.length === 0;
        };
        const readModule = async (moduleId) => {
            ...
            try {
                ...
                for (const field of buttonFields) {
                    ...
                    if (field === 'PageTabs' && currentEmpty && !nextEmpty) {
                        if (!plan.allowCreatePageTabs)
                            throw new Error('新增 PageTabs 必须显式设置 allowCreatePageTabs=true');
                        continue;
                    }
                ...
                if (result.Code !== 1)
                    throw new Error(result.Msg || 'UpdateModule 返回失败');
                summary.updated++;
                ...
                for (const field of buttonFields) {
                    if (normalizedPatch[field] === undefined)
                        continue;
                    if (field === 'PageTabs' && isEmptyButtonList(before[field]) && plan.allowCreatePageTabs)
                        continue;
