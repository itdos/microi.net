...
import { buildPlan, manifestGuide, resolveManifestRelationConfig, validateManifestFieldRelations, } from './advanced-tools.js';
const validManifest = {
    tables: [
        {
            name: 'Biz_Order',
...
test('Manifest rejects a self-referencing JoinForm before any write', () => {
    ...
    const joinField = manifest.tables[0].fields[1];
...
test('Valid portable relations resolve current-tenant ids and override raw stale ids', () => {
    ...
    const childConfig = resolveManifestRelationConfig(validManifest.tables[0].fields[2], 'Biz_Order', tableIds, moduleIds);
...
test('Manifest schema teaches the portable relation contract and no JoinForm Name default', () => {
    ...
    const natural = guide.naturalFieldKeys;
    const examples = guide.relationExamples;
    assert.match(natural.fields.relation, /1:N/u);
    assert.ok(examples.joinForm);
    assert.ok(examples.tableChild);
});
//# sourceMappingURL=manifest-field-relations.test.js.map