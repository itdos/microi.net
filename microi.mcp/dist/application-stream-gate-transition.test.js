...
test('application stream gate transition is coordinate-bound, monotonic, and double-confirmed', async () => {
    const serverCalls = [];
    const fakeClient = {
        transitionApplicationStreamGate: async (request) => {
    ...
    try {
        ...
        const previewJson = JSON.parse(toolText(preview));
        ...
        const rollbackPreviewJson = JSON.parse(toolText(rollbackPreview));
        ...
        const v3OnlyPreviewJson = JSON.parse(toolText(v3OnlyPreview));
        ...
        for (const rejected of rejectedCases) {
            const callCountBefore = serverCalls.length;
...
//# sourceMappingURL=application-stream-gate-transition.test.js.map