...
test('microi_ocr_recognize requires consent and forwards only the safe tenant-bound contract', async () => {
    ...
    try {
        ...
        const forwarded = recognizeInput;
        assert.ok(forwarded);
        ...
        assert.equal(forwarded.OsClient, undefined);
...
test('prepareMcpOcrInput rejects ambiguous, relative and unsupported inputs before HTTP', () => {
    assert.throws(() => prepareMcpOcrInput({}), /必须且只能提供一个/u);
    assert.throws(() => prepareMcpOcrInput({
        filePath: 'relative.png',
        fileByteBase64: ONE_PIXEL_PNG_BASE64,
    }), /必须且只能提供一个/u);
    assert.throws(() => prepareMcpOcrInput({ filePath: 'relative.png' }), /绝对路径/u);
    assert.throws(() => prepareMcpOcrInput({
        fileByteBase64: ONE_PIXEL_PNG_BASE64,
        fileName: 'payload.exe',
    }), /仅支持 PDF/u);
});
//# sourceMappingURL=ocr-tools.test.js.map