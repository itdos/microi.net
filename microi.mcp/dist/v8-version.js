...
export function parseV8Version(value) {
    ...
    if (direct) {
        return `v${Number(direct[1])}.${Number(direct[2])}.${Number(direct[3])}`;
    }
...
export function incrementV8Version(version) {
    const parsed = toParsedVersion(version);
    if (!parsed) {
        return 'v1.0.0';
    }
...
function normalizeDescription(description) {
    ...
    if (!text) {
        return ['请补充该 V8 代码的完整功能说明。'];
    }
    return text.replace(/\r\n/g, '\n').split('\n').map(line => line.trim()).filter(Boolean);
}
function extractFunctionDescriptionFromHeader(header) {
...
function splitLeadingProtectedHeaders(code) {
    ...
    if (!isVersionHeader(first.comment)) {
        return { body: normalized };
    }
    const second = takeLeadingComment(first.rest);
...
function buildHeader(options, version, previousHeader) {
...
export function prepareV8VersionedCode(options) {
...
//# sourceMappingURL=v8-version.js.map