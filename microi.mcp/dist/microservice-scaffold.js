...
function fallbackMicroiSdk() {
    return [
        ...
        '  let config = { apiBase: "", osClient: "", token: "" }',
        '  const configure = (next = {}) => { config = { ...config, ...(next || {}) }; return client }',
        '  const getConfig = () => ({ ...config })',
        '  const setToken = (token) => { config.token = String(token || ""); return client }',
        ...
        '  }',
        '  const client = {',
        '    configure, getConfig, setToken, clearToken, post,',
        '    ApiEngine: { Run: (key, data = {}) => post("/api/ApiEngine/Run", { ApiEngineKey: key, ...(data || {}) }) },',
        '    FormEngine: { GetTableData: (table, data = {}) => post("/api/FormEngine/GetTableData", { FormEngineKey: table, ...(data || {}) }) },',
        '  }',
        '  return client',
        '}',
        'export function createMicroiV8() { return createClient() }',
...
function buildMicroiBridge() {
    return [
        "import V8, { createMicroiV8 } from './utils/microi.v8.js'",
        '',
        ...
        '}',
        '',
        'export function getMicroiContext() {',
        ...
        "    microRoute: data.microRoute || data.route?.microRoute || data.route?.microRoutePath || (window.microApp ? '' : getLocalRoutePath()),",
        '    route: data.route || {},',
        '  }',
        '}',
        '',
        ...
        '  const next = { apiBase: context.apiBase, osClient: context.osClient, token: context.token }',
        '  microiV8.configure?.(next)',
        '  if (context.token) microiV8.setToken?.(context.token)',
        '  V8.configure?.(next)',
        '  if (context.token) V8.setToken?.(context.token)',
        '  return microiV8',
        '}',
        '',
    ].join('\n');
...
function buildRoutesModule(routes) {
    ...
    return [
        ...
        '}',
        '',
        'export function findRoute(value) {',
...
function buildAppVue(name) {
    return [
        ...
        'const context = ref(getMicroiContext())',
        ...
        '  context.value = { ...getMicroiContext(), ...(data || {}) }',
        ...
        '}',
        '',
        'function setLocalRoute(routePath) {',
...
function buildContextPage(description) {
    return [
        ...
        'defineProps({ context: { type: Object, required: true }, route: { type: Object, required: true } })',
        '</script>',
        '',
    ].join('\n');
}
...
function buildFileContents(options, routes, buildVersion) {
    ...
    files.set('package.json', `${JSON.stringify({
        ...
        devDependencies: { '@vitejs/plugin-vue': '^5.2.1', vite: '^6.1.0' },
    }, null, 2)}\n`);
    files.set('index.html', '<!doctype html>\n<html lang="zh-CN">\n  <head>\n    <meta charset="UTF-8" />\n    <meta name="viewport" content="width=device-width, initial-scale=1.0" />\n    <title>Microi Vue 微服务测试</title>\n  </head>\n  <body>\n    <div id="app"></div>\n    <script type="module" src="/src/main.js"></script>\n  </body>\n</html>\n');
    ...
    files.set('src/microi.js', buildMicroiBridge());
    files.set('src/routes.js', buildRoutesModule(routes));
