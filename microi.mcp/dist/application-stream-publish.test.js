...
test('v3 durable verification exposes pending progress; manifest-only replay never uploads or finalizes', async () => {
    ...
    try {
        ...
        const input = {
            ...
            publishMode: 'stage', verificationOnly: true, protocolVersion: 3,
        ...
        const client = {
            ...
            finalizeApplicationStreamPublish: async (body) => {
                calls++;
                assert.equal(body.PublishMode, 'stage');
        ...
        assert.equal(receipt.PublishState, 'Verifying');
        assert.equal(receipt.uploadedCount, 0);
        assert.equal(receipt.stagedCount, 0);
        complete = true;
        const verified = parseToolJson(await runApplicationDirectoryStreamPublish(client, input));
        assert.equal(verified.Pending, false);
        assert.equal(verified.PublishState, 'ReleaseVerified');
        assert.equal(verified.uploadedCount, 0);
        assert.equal(calls, 2);
        complete = false;
        const expected = { ...resolveApplicationAssetStreamV3Contract(input, manifest.manifestHash), appIdOrKey: input.appIdOrKey,
            versionNo: input.versionNo, publishMode: 'stage', entryPath: 'index.html', encodedEntryPath: 'index.html',
