...
test('application stream limits reject files above 128 MiB and manifests above 1 GiB before upload', async () => {
    assert.throws(() => validateLocalApplicationAssetSize('assets/oversize.bin', 128 * 1024 * 1024 + 1, 128 * 1024 * 1024 + 1), /单文件超过硬上限 134217728 bytes.*上传前中止/u);
    assert.throws(() => validateLocalApplicationAssetSize('assets/final.bin', 1, 1024 * 1024 * 1024 + 1), /发布总大小超过上限 1073741824 bytes.*上传前中止/u);
    const root = createTempDirectory();
    let uploadCalls = 0;
    try {
