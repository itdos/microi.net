...
export function isRetiredMiniMaxMusicApi(result) {
...
function extractPredictionFileUrl(value) {
...
export async function generateMiniMaxMusic3Fallback(client, input) {
    ...
    try {
        ...
        if (!response.ok)
            throw new Error(`MiniMax-Music3 文件下载失败：HTTP ${response.status}`);
        const declaredLength = Number(response.headers.get('content-length') || 0);
        if (declaredLength > MAX_WAV_BYTES)
            throw new Error('MiniMax-Music3 音乐超过 MCP 32MiB 安全上限。');
        ...
        catch { /* another process never owns this exact handle */ }
    }
}
//# sourceMappingURL=minimax-music3-fallback.js.map