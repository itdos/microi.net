...
function rsaEncrypt(plainText) {
    const encrypted = crypto.publicEncrypt({ key: RSA_PUBLIC_KEY, padding: crypto.constants.RSA_PKCS1_PADDING }, Buffer.from(plainText, 'utf-8'));
    return encrypted.toString('base64');
}
function ok(label, value = undefined) {
    console.log(`✅ ${label}`);
    if (value !== undefined)
...
async function login() {
    ...
    if (devKey)
        headers['X-Microi-Dev-Key'] = devKey;
...
async function main() {
    try {
        ...
        // 1) initial list
        const list1 = await call('/api/V8Engine/ListBlueprints');
        if (list1.Code !== 1)
            fail('listBlueprints initial', list1);
        ...
        const save1 = await call('/api/V8Engine/SaveBlueprint', {
            Name: saveName,
            Code: 'smoke_test_bp',
        ...
        if (save2.Code !== 1)
            fail('saveBlueprint update', save2);
        ok('saveBlueprint update');
        // 8) validate (drift detection)
        const validation = await call('/api/V8Engine/ValidateBlueprint', { BlueprintId: blueprintId });
        if (validation.Code !== 1)
            fail('validateBlueprint', validation);
        ...
        if (del.Code !== 1)
            fail('deleteBlueprint', del);
        ok('deleteBlueprint');
        // 10) deleted no longer in list
        const list4 = await call('/api/V8Engine/ListBlueprints', { Keyword: saveName });
        const remainCount = Array.isArray(list4.Data) ? list4.Data.length : 0;
        if (remainCount !== 0)
            fail(`expected 0 after delete, got ${remainCount}`, list4);
...
main();
//# sourceMappingURL=blueprint-smoke-test.js.map