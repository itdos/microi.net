...
test('system observability MCP exposes bounded read catalog and confirmed IP governance', async () => {
    ...
    try {
        ...
        for (const action of ['Memory', 'MemoryIncidents']) {
            const result = await client.callTool({ name: 'microi_codex',
                arguments: { action: 'microi_query_system_observability', params: { action } } });
        ...
        const detail = await client.callTool({ name: 'microi_codex',
            arguments: { action: 'microi_query_system_observability', params: { action: 'MemoryIncident', incidentId } } });
        ...
        for (const invalid of [{}, { incidentId: '../../secrets' }]) {
            const result = await client.callTool({ name: 'microi_codex',
                arguments: { action: 'microi_query_system_observability', params: { action: 'MemoryIncident', ...invalid } } });
