...
test('MCP escalates to VS Code recovery when a refresh-issued token is immediately rejected', async () => {
    ...
    try {
        ...
        assert.equal(recoveryPayloads[0]?.failedTokenHash, crypto.createHash('sha256').update(refreshIssuedToken).digest('hex'), 'VS Code broker must receive the hash of the replacement token that was actually rejected');
        assert.equal(recoveryPayloads[1]?.failedTokenHash, crypto.createHash('sha256').update(extensionHeldToken).digest('hex'), 'a rejected extension-held token must trigger the broker relogin branch in the same bounded request');
        const storedTokens = JSON.parse(fs.readFileSync(tokenFilePath, 'utf8'));
...
test('updateEngineRuntimeLimit updates only V8Limit and verifies explicit true', async () => {
    ...
    try {
        ...
        assert.equal(result.Code, 1);
        assert.equal(result.Data.Verified, true);
        assert.equal(result.Data.V8Limit, 1);
...
test('updateModule verifies menu JSON after an uncertain write', async () => {
    ...
    try {
        ...
        const savedSchema = JSON.parse(String(storedModule.ViewSchema));
