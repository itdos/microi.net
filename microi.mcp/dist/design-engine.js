...
function mergePeriodSearchData(searchData) {
...
function withPeriodSearchData(dataJson) {
    return {
        ...dataJson,
        searchData: mergePeriodSearchData(dataJson.searchData),
    };
}
function enableWidgetPeriodParams(widgetType, widgetParams) {
    const indexesByType = {
        statistic: [18, 19],
        progress: [26, 27],
        bar: [1, 16],
...
function paramNumber(widgetParams, index, fallback) {
    return numberValue(asRecord(widgetParams[index]).value, fallback);
}
function paramEnabled(widgetParams, index) {
    return asRecord(widgetParams[index]).value === true;
}
function widgetDataLength(widgetRecord) {
    const widgetParams = Array.isArray(widgetRecord.widgetParams) ? widgetRecord.widgetParams : [];
    ...
    if (Array.isArray(record.bodyData))
        return record.bodyData.length;
    return 0;
}
function suggestedContentWidgetHeight(widgetRecord) {
...
function normalizeContentWidgetHeight(widgetRecord, path, warnings) {
    ...
    const wrapperList = Array.isArray(obj.wrapperList) ? obj.wrapperList.map((item, index) => {
        ...
        if (!Array.isArray(wrapper.widgetList)) {
            ...
            wrapper.widgetList = wrapper.widgetList.map((widget, widgetIndex) => {
                ...
                minContentWidgetHeight = Math.max(minContentWidgetHeight, normalizeContentWidgetHeight(widgetRecord, widgetPath, warnings));
                return widgetRecord;
            });
        }
