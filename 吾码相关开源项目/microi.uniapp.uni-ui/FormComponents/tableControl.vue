<template>
	<view class="table-page">
		<view class="uni-common-mb uni-common-pr" v-if="isOpenTable">
			<view class="search-box" :class="{ 'search-box-scrolled': isScrolled }">
				<view class="search-item uni-flex uni-common-mb-xs">
					<view class="search-input">
						<uni-easyinput prefixIcon="search" v-model="keyword" :value="keyword?keyword:''"
							placeholder="关键词" :styles="{borderColor: '#4179F7'}" @iconClick="ClickSearch"
							@blur="ClickSearch" @clear="onClear" />
					</view>
			...
			<uni-table border stripe :loading="loading" emptyText="暂无更多数据" :type="isMultipleSelect ? 'selection' : ''"
				@selection-change="selectionChange" :key="ParentIndex">
				<!-- 表头行 -->
				<uni-tr>
					<uni-th align="center" :width="60" v-if="isShowMoreBtn">操作</uni-th>
					<uni-th v-for="(item, index) in NewformFields" :key="index" align="center" :width="item.TableWidth">
				...
				<uni-tr v-for="(lsit, index) in NewChildList" :key="index">
						...
						<view class="table-btn">
							<view class="">
								<view v-for="(btn, btnIndex) in MoreBtns" :key="btnIndex">
									<button v-if="isShowBtn(btn,lsit, NewformFields, currentPermission, isType)"
										@click="goBtn(btn, lsit)" class="uni-common-mr table-item-btn" size="mini"
					...
					<block v-if="NewformFields.length > 0">
						<uni-td v-for="(item, index1) in NewformFields" :key="index1" align="left"
							:width="item.TableWidth" @click="goEdit(lsit.Id, 5, lsit)">
							<!-- 是否需要表内编辑 -->
							...
							<view>
								<view v-if="item.Component == 'ImgUpload'">
									<block v-for="(img, index2) in lsit[item.Name]" :key="img.Id">
										<image slot='cover' :src="img.url" class="item-Img"
											@click="previewImg(lsit[item.Name], index2)"></image>
									</block>
								</view>
								<uni-tooltip :content="String(lsit[item.Name])" placement="top" v-else>
									<view class="uni-ellipsis-3" :style="{ width: `${item.TableWidth}px` }">
										<view class="text-center" v-html="TmpEngineTable(lsit, NewformFields, item)">
										</view>
									</view>
								</uni-tooltip>
							</view>
		...
		<!-- 卡片方式 -->
		<view class="uni-common-mt" v-if="!isOpenTable && NewChildList.length > 0"
			:id="`card-${ParentdiyFormFields[ParentIndex].Name}`">
			<view class="work-list" v-for="(item, index) in NewChildList" :key="item.Id">
				<view class="work-item">
							...
							<template v-if="activeDropdown === index">
								<transition name="dropdown">
									<view class="type-dropdown">
										<view v-for="(btn, btnIndex) in MoreBtns" :key="btnIndex">
											<view
												v-if="isShowBtn(btn,item, NewformFields, currentPermission, ParentdiyFormFields[ParentIndex].Readonly ? 'view' : isType)"
					...
					<view class="work-item-content" @click="goEdit(null, 5, item)">
						<cardDetail :diyFormFields="NewformFields" :detail="item" :type="isType"
							:is="new Date().getTime()" :InTableEdit="InTableEdit" :InTableEditFields="InTableEditFields"
							:DiyTableId="DiyTableId" :Guid="Guid" :currenTotal="total" />
					</view>
				</view>
			</view>
		</view>
		...
		<view class="uni-common-mt"
			v-if="isType == 'edit' && currentPermission.includes('Add') && !ParentdiyFormFields[ParentIndex].Readonly ">
...
<script setup>
	...
	if (TableChildFkFieldName) {
		SearchEqual[TableChildFkFieldName] = childField ? (props.ParentFormData[childField] || '') : props.ParentFormData
			.Id
	}
	...
	const GetData = async () => {
		...
		if (DiyTableData.value && DiyTableData.value.ReportId && DiyTableData.value.DataSourceId) {
			...
			res = await Microi.FormEngine.GetTableData({
				_Keyword : keyword.value,
				ModuleEngineKey: props.currentFieldsConfig.TableChildSysMenuId || props.currentFieldsConfig
					.OpenTable.SysMenuId,
				_PageIndex: pageCurrent.value,
				_PageSize: pageSize.value,
				_SearchEqual: SearchEqual,
	...
	const getFormMenuData = async () => {
		...
		const res = await Microi.FormEngine.GetFormData({
			...
			Id: props.currentFieldsConfig.TableChildSysMenuId || props.currentFieldsConfig.OpenTable
				.SysMenuId
		})
		if (res.Code == 1) {
			Microi.HideLoading()
			...
			// 是否开启表内编辑
			InTableEdit.value = props.ParentdiyFormFields[props.ParentIndex].Readonly ? false : res.Data
				.InTableEdit
			// 表内可编辑字段
			...
			const formFields = await diyFormField({
	...
	onMounted(() => {
		nextTick(() => {
			setTimeout(async () => {
				...
				NewChildList.value = []
				if (TableChildFkFieldName) {
					SearchEqual[TableChildFkFieldName] = childField ? (props.ParentFormData[
						childField] || '') : props.ParentFormData.Id
				}
			...
			watch(() => props.ParentFormData, (newValue, oldValue) => {
				...
				if (newValue[childField] != oldChildField) {
					...
					if (TableChildFkFieldName) {
						SearchEqual[TableChildFkFieldName] = childField ? newValue[childField] :
							newValue.Id
					}
