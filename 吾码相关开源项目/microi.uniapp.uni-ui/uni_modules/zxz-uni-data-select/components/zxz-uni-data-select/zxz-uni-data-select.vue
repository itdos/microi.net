			...
			<view class="uni-select" :style="{height:multiple?'100%':' 35px'}"
				...
				<view class="uni-select__input-box" :style="{height:multiple?'100%':'35px'}" @click="toggleSelector">
					<view class="" style="display: flex;flex-wrap: wrap;width: 100%;" v-if="multiple&&current.length>0">
						...
						<input v-if="filterable&&!disabled" @input="inputChange" class="uni-select__input-text uni-select__input-placeholder"
							type="text" style="height: 52rpx;margin-left: 6px;width: auto;"
							placeholder="请输入" v-model="filterInput">
					</view>
					<view v-else-if="current&&current.length>0&&!showSelector" class="uni-select__input-text uni-select__input-placeholder">
						{{current}}
					</view>
					<input v-else-if="filterable&&showSelector" :focus="isFocus" @input="inputChange"
						:disabled="disabled" @click.stop="" class="uni-select__input-text uni-select__input-placeholder" type="text"
						style="position: absolute;z-index: 1;" :placeholder="placeholderOld"
						v-model="filterInput">
			...
			initDefVal() {
				...
				if (this.multiple) {
					...
					if (typeof defValue === 'object') {
						defValue = defValue[this.dataValue]
					}
					const def = this.mixinDatacomResData.find(item => item[this.dataValue] === defValue)
					// 如果def没查询到，则给插入一条defValue
					if (!def && defValue) {
						const defItem = {
							[this.dataKey]: defValue,
							[this.dataValue]: defValue,
						}
						this.mixinDatacomResData.push(defItem)
					}
					const defItem = this.mixinDatacomResData.find(item => item[this.dataValue] === defValue)
					this.current = defItem ? this.formatItemName(defItem) : ''
				}
			},
			/**
