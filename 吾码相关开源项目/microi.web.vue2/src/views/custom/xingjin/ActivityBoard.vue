    ...
    <div class="activity-board" v-loading="loading">
        <div class="board-header">
            <div class="header-left">
                <span class="title"> {{ projectDetail.XiangmuH }} - {{ projectDetail.XiangmuMC }} </span>
                <span class="status-wrapper" v-if="projectDetail.XiangmuJD"> 项目阶段：{{ projectDetail.XiangmuJD }} </span>
        ...
        <div class="board-content">
            <div v-if="gatewayList.length === 0" class="empty-state">
                <el-empty :image-size="120" description="暂无活动数据"> </el-empty>
            </div>
            <div v-for="(item, index) in gatewayList" :key="index">
                ...
                <div class="gateway-item">
                    <div class="gateway-header">
                        ...
                        <el-button type="text" @click="handleDeleteGateway(item, index)">
        ...
        <!-- APQP设置对话框 -->
        <el-dialog v-el-drag-dialog title="APQP活动设置" :visible.sync="apqpDialogVisible" width="600px" :append-to-body="true">
            <div class="apqp-settings">
                <div v-for="category in apqpCategories" :key="category.Id" class="category-section">
                    <div class="category-title">
                        <el-checkbox v-model="selectedCategories[category.Id]" @change="handleMainCategoryChange(category.Id)">
                    ...
                    <div v-if="category._Child && category._Child.length" class="sub-activities">
                        <div v-for="subActivity in category._Child" :key="subActivity.Id" class="sub-checkbox-item">
                            <el-checkbox v-model="selectedCategories[subActivity.Id]" @change="handleSubActivityChange(category.Id, subActivity.Id)">
        ...
        <!-- 设置时间对话框 -->
        <el-dialog v-el-drag-dialog title="设置活动时间" :visible.sync="timeDialogVisible" width="500px" :append-to-body="true" top="30vh">
            <div class="time-settings">
                ...
                <div class="dialog-item">
                    ...
                    <el-date-picker v-model="currentGateway.KaifaSJ" type="date" placeholder="选择开始日期" value-format="yyyy-MM-dd" style="width: 220px"> </el-date-picker>
                </div>
                <div class="dialog-item">
                    <span class="dialog-label">结束时间：</span>
                    <el-date-picker v-model="currentGateway.WanchengSJ" type="date" placeholder="选择结束日期" value-format="yyyy-MM-dd" style="width: 220px"> </el-date-picker>
                </div>
            </div>
            <span slot="footer" class="dialog-footer">
    ...
    methods: {
            ...
            this.apqpCategories.forEach((category) => {
                ...
                if (category._Child) {
                    ...
                    if (hasSelectedSubActivity && !isSelected) {
                        // 检查是否所有子活动都被选中
                        const allSubActivitiesSelected = category._Child.every((sub) => this.selectedCategories[sub.Id] === true);
        ...
        async generateApqpTasksPlanTime() {
            var res = await this.DiyCommon.FormEngine.GetTableData({
                ...
                _Where: [{ Name: "ProjectId", Value: this.projectId, Type: "Equal" }]
            });
            if (res.Code == 1) {
        ...
        calculateProjectStatus(data) {
            ...
            const isStagnant = data.every((item) => {
                ...
                // 只要有一个时间在3个月内，就不是呆滞
                return (!kaifa || kaifa < threeMonthsAgo) && (!wancheng || wancheng < threeMonthsAgo) && (!shangchuan || shangchuan < threeMonthsAgo);
            });
...
/* 使用与状态标签相同的颜色 */
/* R:风险 - risk */
.status-tooltip .tooltip-content .tooltip-item .tooltip-label.red-label {
    background-color: #f56c6c;
    color: white;
}
/* Y:关注 - attention */
.status-tooltip .tooltip-content .tooltip-item .tooltip-label.yellow-label {
    background-color: #e6a23c;
    color: white;
}
/* G:正常 - normal */
.status-tooltip .tooltip-content .tooltip-item .tooltip-label.green-label {
    background-color: #67c23a;
    color: white;
}
/* D:暂停 - pause */
.status-tooltip .tooltip-content .tooltip-item .tooltip-label.gray-label {
    background-color: #909399;
    color: white;
}
/* C:完成 - complete */
.status-tooltip .tooltip-content .tooltip-item .tooltip-label.blue-label {
    background-color: #409eff;
    color: white;
}
/* S:暂停 - stop */
.status-tooltip .tooltip-content .tooltip-item .tooltip-label.orange-label {
    background-color: #909399;
    color: white;
}
/* T:终止 - end */
.status-tooltip .tooltip-content .tooltip-item .tooltip-label.black-label {
    background-color: #909399;
    color: white;
}
