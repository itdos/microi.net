<template>
	<view v-if="!disabled" class="tn-image-upload-class tn-image-upload">
		<movable-area class="tn-image-upload__movable-area" :style="{
        height: movableAreaHeight
      }" @mouseenter="mouseEnterArea" @mouseleave="mouseLeaveArea">
			<block v-for="(item, index) in lists" :key="item.id">
				<movable-view class="tn-image-upload__movable-view" :style="{
            width: $t.string.getLengthUnitValue(width),
            height: $t.string.getLengthUnitValue(height),
            zIndex: item.zIndex,
            opacity: item.opacity,
            
          }" :x="item.x" :y="item.y" direction="all" :damping="40" :disabled="item.disabled"
					@change="movableChange($event, item)" @touchstart="movableStart(item)"
					@mousedown="movableStart(item)" @touchend="movableEnd(item)" @mouseup="movableEnd(item)"
					@longpress="movableLongPress(item)">
					<view class="tn-image-upload__item tn-image-upload__item-preview" :style="{
              width: $t.string.getLengthUnitValue(itemWidth, 'px'),
              height: $t.string.getLengthUnitValue(itemHeight, 'px'),
              transform: `scale(${item.scale})`
            }">
						<!-- 删除按钮 -->
						<view v-if="deleteable" class="tn-image-upload__item-preview__delete"
							@tap.stop="deleteItem(index)" :style="{
                borderTopColor: deleteBackgroundColor
              }">
							<view class="tn-image-upload__item-preview__delete--icon" :class="[`tn-icon-${deleteIcon}`]"
								:style="{
                  color: deleteColor
                }"></view>
						</view>
						<!-- 进度条 -->
						<tn-line-progress v-if="showProgress && item.data.progress > 0 && !item.data.error"
							class="tn-image-upload__item-preview__progress" :percent="item.data.progress"
							:showPercent="false" :round="false" :height="8"></tn-line-progress>
						<!-- 重试按钮 -->
						<view v-if="item.data.error" class="tn-image-upload__item-preview__error-btn"
							@tap.stop="retry(index)">点击重试</view>
						<!-- 图片信息 -->
						<image class="tn-image-upload__item-preview__image" :src="item.data.url || item.data.path"
							:mode="imageMode" @tap.stop="doPreviewImage(item.data.url || item.data.path, index)">
						</image>
					</view>
				</movable-view>
			</block>
			...
			<!-- 添加按钮 -->
			<view v-if="maxCount > lists.length" class="tn-image-upload__add" :style="{
          top: addBtn.y + 'px',
          left: addBtn.x + 'px',
          width: $t.string.getLengthUnitValue(itemWidth, 'px'),
          height: $t.string.getLengthUnitValue(itemHeight, 'px')
				...
				<!-- 添加按钮 -->
				<view class="tn-image-upload__item tn-image-upload__item-add" hover-class="tn-hover-class"
	...
	export default {
		...
		methods: {
			...
			async uploadFile(index = 0) {
				...
				this.uploading = true
				// 创建上传对象
				console.log('UploadThisName：', this.name)
			...
			doPreviewImage(url, index) {
				...
				const images = this.lists.sort((l1, l2) => {
					return l1.index - l2.index
				}).map(item => item.data.url || item.data.path)
				uni.previewImage({
					urls: images,
					current: url,
					success: () => {
					...
					this.fileList.forEach((item) => {
						...
						let tmp = this.lists.map(value => {
							...
							!tmp && this.lists.push(this.handleDragListItem({
								...
								error: false,
								progress : 100,
								//progress: (item.url.startsWith('http://tmp') || item.url.startsWith('wxfile://tmp')) ? 0 : 100 //by itdos.com
							}))
					})
