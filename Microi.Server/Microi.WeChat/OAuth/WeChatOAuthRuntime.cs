...
namespace Microi.net
{
    ...
    /// 
    /// </summary>
    public sealed class WeChatOAuthRuntime : IPlatformApiRuntime
    {
        ...
        public Task<object> ExecuteAsync(string action, JObject parameters)
        {
            ...
            {
                "BindSysUser" => BindSysUser(
                    parameters["authorization"]?.ToString(),
                    parameters["OsClient"]?.ToString(),
                    parameters["ReturnUrl"]?.ToString()),
                "UserInfoCallback" => UserInfoCallback(
                    parameters["code"]?.ToString(),
                    parameters["state"]?.ToString(),
                    parameters["OsClient"]?.ToString(),
                    parameters["o"]?.ToString()),
                _ => Task.FromResult<object>(new DosResult(0, null, "不支持的微信 OAuth 动作。"))
            };
        }

        private static object Unauthorized() => SignedResponse(401, string.Empty);

        private static object StatusCode(int statusCode) => SignedResponse(statusCode, string.Empty);

        private static object Content(string body) => SignedResponse(200, body ?? string.Empty);

        private static object Redirect(string location) => SignedResponse(
            302,
            string.Empty,
            new JObject { ["Location"] = location ?? string.Empty });

        private static DosResult SignedResponse(int statusCode, string body, JObject headers = null)
        {
            var response = new JObject
            {
                ...
                ["Headers"] = headers ?? new JObject { ["Cache-Control"] = "no-store" }
            };
...
return ''";
        ...
        /// <returns></returns>
        public async Task<object> BindSysUser(string authorization, string OsClient, string ReturnUrl)
        {
        ...
        /// <returns></returns>
        public async Task<object> UserInfoCallback(string code, string state, string OsClient, string o)
        {
