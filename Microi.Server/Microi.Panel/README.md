# Microi.Panel · 吾码服务器运维面板

独立的 .NET 10 + Vue 3 / Microi.UI 服务器面板，管理单台 Linux Docker 主机的服务、网站、证书、文件、备份、计划任务和吾码 API/Web 更新。运维页面、独立登录、SQLite 任务账本与日志均不依赖业务平台存活。

## v2.0.0

- 项目由 Microi.Ops 更名为 Microi.Panel；保留旧 `OPS_*` 配置、加密账本、平台运维日志来源及原部署清单。
- Docker 插件市场支持 Nginx、MySQL、SQL Server、PostgreSQL、Oracle、MinIO、Redis、MongoDB、翻译和 OCR，按版本与 CPU 架构选择。
- Nginx 静态网站、路径代理、配置预览与原子发布，PEM 证书、HTTP-01 申请与自动续期。
- 网站文件编辑、上传、回收与历史恢复；冷备份完整性检查、新数据卷恢复及定时任务。
- 独立安装器生成随机账号和 HTTPS 入口，默认 `/microi/panel`、端口 `61890`，无需先部署 API/Web。
- 吾码 UI 主题与弹窗行为保持统一；卡片、Tab 和导航不使用弧形强调侧框。

完整安装、共存、备份与迁移说明：[服务器运维面板](../../microi.doc/docs/doc/server-panel/overview.md)。安装脚本位于仓库根 `数据库、案例、文档、资料/install-microi-panel.sh`。旧 Ops 场景继续使用下方原平台部署清单说明；迁移时保持原挂载、端口和账号，只替换所选面板镜像，不重新初始化。
...
官方镜像：`registry.cn-hangzhou.aliyuncs.com/microios/microi-panel:v2.0.0`。独立安装使用 `install-microi-panel.sh`，生成 HTTPS 入口且无需 API/Web。平台一键安装器会复用已经安装的 Panel/Ops；只有尚无面板时，才在核心 API/Web 就绪后生成兼容运维编排。离线首次安装使用 Manual，在线原平台引导使用 Notify。
...
在吾码 SaaS 引擎 → 后端运行配置设置 `MicroiOpsUrl=https://ops.example.com`，安装/更新「SaaS引擎」「系统日志/监控」「应用商城」，即可通过系统引擎 → 服务器运维面板进入。平台管理员权限只控制入口显示与必要日志查询，不授予 Docker 权限；仍须登录独立运维账号。浏览器阻止跨站 iframe Cookie 时使用新窗口入口，推荐 API/Web/Panel 使用同一站点的不同 HTTPS 子域名。
...
docker build -f Microi.Server/Microi.Panel/Dockerfile -t microi-panel:acceptance .
$gate = '.tmp/panel-acceptance/current'
$image = 'microi-panel:acceptance'
./Microi.Server/Microi.Tests/run-tests.ps1 -Mode Panel -PanelStage Bind -PanelImage $image -ResultsDirectory $gate
./Microi.Server/Microi.Tests/run-tests.ps1 -Mode Panel -PanelStage Unit -PanelImage $image -ResultsDirectory $gate
./Microi.Server/Microi.Tests/run-tests.ps1 -Mode Panel -PanelStage Core -PanelImage $image -ResultsDirectory $gate
./Microi.Server/Microi.Tests/run-tests.ps1 -Mode Panel -PanelStage Acme -PanelImage $image -ResultsDirectory $gate
./Microi.Server/Microi.Tests/run-tests.ps1 -Mode Panel -PanelStage Legacy -PanelImage $image -ResultsDirectory $gate
./Microi.Server/Microi.Tests/run-tests.ps1 -Mode Panel -PanelStage Plugins -PanelImage $image -ResultsDirectory $gate -PanelPlugins 'nginx,mysql,postgresql,sqlserver,oracle,minio,redis,mongodb,translate'
# OCR 分片与前一轮顺序执行，先确保容器预算和主机安全余量充足。
./Microi.Server/Microi.Tests/run-tests.ps1 -Mode Panel -PanelStage Plugins -PanelImage $image -ResultsDirectory $gate -PanelPlugins 'ocr'
./Microi.Server/Microi.Tests/run-tests.ps1 -Mode Panel -PanelStage Linux -PanelImage $image -ResultsDirectory $gate -PanelLinuxEvidence '.tmp/panel-acceptance/linux-evidence.json'
./Microi.Server/Microi.Tests/run-tests.ps1 -Mode Panel -PanelStage Verify -PanelImage $image -ResultsDirectory $gate
```

门禁按轮次生成独立标签和目录，停止时逐项核对归属并保留数据卷。旧协议使用 61880–61886，核心检查使用 61980–61983，插件分片使用 61990/62000–62001，ACME 使用 62100–62103；端口必须空闲。真实平台登录默认复用已确认归属的 61500/61501，不自动重启共享服务。持有官方测试账号的发行机也可显式设置 `$env:PANEL_PLATFORM_API_BASE='https://api.itdos.com'`：前端继续使用 61500 的独立浏览器上下文，URL 明确携带 ApiBase 与 OsClient；远端证书先通过系统信任链验证，再固定指纹。测试入口只接受与既有授权凭据一致的官方目标。平台协议故障测试使用隔离 HTTPS 服务，真实登录与 MongoDB 回执另行验证；密码和 Token 不写入报告。

`Bind` 从当前源码重新构建并与候选镜像的实际 `/app` 文件比较。程序、页面、压缩资源与依赖逐字节相同；ASP.NET 自动清单的 `Last-Modified` 生成时间单独记录并核对真实文件时间，其余路由、Header、ETag 和完整性字段仍严格相同。各阶段回执绑定源码和镜像，`Verify` 要求全部阶段、实际插件目录的每个版本、保留卷重新安装及数据库族冷备份恢复均成功；缺失、失败、跳过、证据修改或源码漂移均停止发布。

Linux 证据必须来自两台带 `/var/lib/panel-acceptance-id` 标识的隔离主机，分别执行「Panel → 宝塔 → 1Panel」和「宝塔 → 1Panel → Panel」，端口互不冲突。使用 `Panel/panel-linux-install.e2e.py` 在每次实际安装后回读，在两台主机真正重启后再执行；反向顺序使用 `panel-linux-neighbors.e2e.py` 在 Panel 安装前、后及重启后比较第三方面板程序、配置、服务和 Docker 身份。配合 `panel-linux-browser.e2e.mjs` 验证三套真实登录页及吾码网站。第三方安装器按原厂签名/散列核对，不用模拟页面替代安装。

`linux-evidence.json` 是两个对象的数组，每个包含 `order`（`panel-baota-onepanel` 或 `baota-onepanel-panel`）、`report`（安装回读报告）、`browserReports`（安装后及重启后的两份浏览器报告）；反向顺序还包含 `neighborReport` 和 `upgradeReport`。后者由 `panel-linux-upgrade.e2e.py` 验证只读预检、离线升级、故障候选回退和恢复原镜像。路径指向本轮保留的 JSON 文件。隔离 Linux 必须使用稳定 IP 或域名；实验 DHCP 地址漂移不能通过改写面板凭据或跳过 TLS 验证掩盖。
