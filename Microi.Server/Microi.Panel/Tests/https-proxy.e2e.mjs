...
const ops=legacy.names.ops,proxy=legacy.names.proxy,label=legacy.label;
const docker=args=>execFileSync('docker',args,{encoding:'utf8',stdio:['ignore','pipe','pipe']}).trim();
function remove(name){let c;try{c=JSON.parse(docker(['inspect',name]))[0];}catch{return;}assert.equal(c.Config.Labels['io.microi.ops.test'],legacy.run);docker(['rm','-f',name]);}
function start(trusted=''){
 docker(['run','-d','--name',ops,'--label',label,'--memory','512m','--cpus','1','--env-file',resolve(work,'ops-test.env'),'--network',legacy.network,'-p','127.0.0.1:61880:8080',
 ...
 '-v',`${resolve(work,'data')}:/microi/ops/data`,'-v',`${resolve(work,'logs')}:/microi/logs/ops`,legacy.image]);
}
...
try{
 remove(proxy);docker(['run','-d','--name',proxy,'--label',label,'--memory','128m','--cpus','0.5','--network',legacy.network,'-p','127.0.0.1:61885:8443','-v',`${resolve(work,'fixture-tls')}:/fixture:ro`,'node:22-bookworm-slim','node','/fixture/proxy.mjs']);
 const ip=JSON.parse(docker(['inspect',proxy]))[0].NetworkSettings.Networks[legacy.network].IPAddress;
 remove(ops);start(ip);
