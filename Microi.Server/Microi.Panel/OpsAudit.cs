...
{
    [GeneratedRegex("(?i)\\binvalid authorization:\\s*(acme:\\s*error:\\s*[0-9]{3}\\s*::\\s*urn:ietf:params:acme:error:[a-z]+)")]
    private static partial Regex AcmeValidation();
    ...
    {
        // ACME uses 'authorization' for public domain validation, not an HTTP credential.
        // Normalize only the complete standard problem prefix; real headers still redact fully.
        var diagnostic = AcmeValidation().Replace(text ?? "", "ACME validation failed: $1");
        var quoted = JsonSecrets().Replace(diagnostic, "$1\"<redacted>\"");
