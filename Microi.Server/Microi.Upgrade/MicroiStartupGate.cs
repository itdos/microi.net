...
namespace Microi.net
{
    ...
    public sealed class MicroiStartupGate
    {
            ...
            CancellationToken cancellationToken = default)
        {
            ...
            using var progressScope = UpgradeProgress.EnterTenant(tenantName);
            UpgradeProgress.Configure(new[] { "启动配置与签名门禁", "启动前物理字段", "平台运行时接口闭包" });
            ...
            UpgradeProgress.Complete("启动配置与签名门禁");
            UpgradeProgress.WriteLine(
            ...
            CancellationToken cancellationToken = default)
        {
            ...
            using var progressScope = UpgradeProgress.EnsureTenant(tenantName);
            UpgradeProgress.Begin("启动前物理字段");
            UpgradeProgress.WriteLine($"Microi：【自动升级状态】【{tenantName}】【启动前物理字段】开始检查。");
            var physical = await _upgrade
                .EnsureRuntimePhysicalPrerequisitesAsync(mainTenant, cancellationToken)
                .ConfigureAwait(false);
            if (physical.Code != 1)
            {
                UpgradeProgress.WriteLine($"Microi：【自动升级状态】【{tenantName}】【启动前物理字段】失败：{physical.Msg}");
                return physical;
            }
            UpgradeProgress.Complete("启动前物理字段");
            UpgradeProgress.WriteLine($"Microi：【自动升级状态】【{tenantName}】【启动前物理字段】成功：{physical.Msg}");

            UpgradeProgress.Begin("平台运行时接口闭包");
            UpgradeProgress.WriteLine($"Microi：【自动升级状态】【{tenantName}】【平台运行时接口闭包】开始检查。");
            ...
            if (dependencies.Code != 1)
            {
                ...
                {
                    UpgradeProgress.Complete("平台运行时接口闭包");
                    UpgradeProgress.WriteLine(
                        $"Microi：【自动升级状态】【{tenantName}】【平台运行时接口闭包】官方应用源由签名应用源同步维护；当前差异不触发程序集基线反向覆盖。详情：{dependencies.Msg}");
                    UpgradeProgress.WriteLine($"Microi：【自动升级状态】【启动前门禁汇总】完成：租户数=1，成功=1，失败=0，主租户={tenantName}，官方应用源资源等待应用源同步，子租户转后台维护。");
                    return new DosResult(1, dependencies.Data,
                        "主租户是官方应用源，数据库物理门禁已通过；Managed 资源由官方应用源同步维护，子租户将在宿主启动后逐一维护。");
                }
                UpgradeProgress.WriteLine($"Microi：【自动升级状态】【{tenantName}】【平台运行时接口闭包】失败：{dependencies.Msg}");
                return dependencies;
            }
            UpgradeProgress.Complete("平台运行时接口闭包");
            UpgradeProgress.WriteLine($"Microi：【自动升级状态】【{tenantName}】【平台运行时接口闭包】成功：{dependencies.Msg}");
            UpgradeProgress.WriteLine($"Microi：【自动升级状态】【启动前门禁汇总】完成：租户数=1，成功=1，失败=0，主租户={tenantName}，子租户转后台维护。");
