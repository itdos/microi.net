...
test('application-store package embeds package-aware installation and immediate menu availability', () => {
  ...
  assert.equal(importerEngine?.Version, 'v2.7.1');
  ...
  for (const fieldName of ['RequiredPlatformCapabilities', 'Capabilities']) {
    ...
    assert.ok(capabilities.includes('ApiEngine:import-microi-store-package@v2.7.1'));
    ...
    assert.ok(!capabilities.some(item => /^ApiEngine:import-microi-store-package@(?!v2\.7\.1$)/.test(item)));
