...
test('application-store package embeds package-aware installation and immediate menu availability', () => {
  assert.equal(ensureMinimumPackageVersion({Version:packageModel.PackageInfo.Version}, 'v8.3.4'), packageModel.PackageInfo.Version);
  ...
  assert.equal(ensureMinimumPackageVersion({Version:importerEngine?.Version}, 'v2.8.12'), importerEngine?.Version);
  ...
  for (const fieldName of ['RequiredPlatformCapabilities', 'Capabilities']) {
    ...
    const minimumImporter = capabilities.find(c => c.startsWith('ApiEngine:import-microi-store-package@'))?.split('@')[1];
    assert.ok(minimumImporter);
    assert.equal(ensureMinimumPackageVersion({Version:minimumImporter}, 'v2.8.7'), minimumImporter);
    ...
    assert.equal(capabilities.filter(item => item.startsWith('ApiEngine:import-microi-store-package@')).length, 1, 'the importer capability must have one unambiguous minimum version');
