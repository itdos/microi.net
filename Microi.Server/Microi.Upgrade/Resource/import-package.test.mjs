...
test("background-task unique-index recovery preserves the authoritative row and rejects two active tasks", () => {
  ...
  assert.match(source, /Version: v2\.7\.3/);
});
...
test("trusted official packages may only add an idempotent Read baseline for all active roles", () => {
  ...
  assert.equal(embeddedImporter.Version, "v2.7.3");
...
test("application-store upgrade resources carry the canonical resumable importer", () => {
  ...
  assert.match(refreshSource, /versionNumber\s*<\s*2_007_003/);
  ...
  assert.match(refreshSource, /importerVersionNumber\s*<\s*2_007_003/);
