...
刷新脚本会拒绝低于 v1.5.0、缺少统一应用商城、原生菜单保护、源码校验或安装统计能力的导入器；也会拒绝低于 v1.2.0 的发布器和低于 v6.4.0 的商城基线，避免维护命令把已修复的本地资源降级。
