 ...
 * Version: v1.0.5
 * Function:
 * - 为模块顶部动态指标、左侧菜单数字角标、页面多 Tab 数字角标提供统一只读统计；支持单菜单与最多 200 个菜单的批量请求，只使用服务端白名单，优先调用最多 64 项、并发上限 4 的 FormEngine 批量计数原子能力，合并相同条件并使用用户级 10 秒 Redis 短缓存，支持 ForceRefresh 绕过缓存及旧 API 节点自动回退。
 */
