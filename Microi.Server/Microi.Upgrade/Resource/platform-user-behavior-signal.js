...
var beforeHook = V8.ApiEngine.Run("platform-runtime-custom-hook", {
    Stage: "BeforeUserBehaviorSignal",
    SourceApiEngineKey: "platform-user-behavior-signal",
    Action: action
});
if (!beforeHook || beforeHook.Code != 1) return beforeHook || { Code: 0, Msg: "用户行为个性化 Hook 未返回结果。" };

var result = V8.Method.TrackUserBehavior(p);
var afterHook = V8.ApiEngine.Run("platform-runtime-custom-hook", {
    Stage: "AfterUserBehaviorSignal",
    SourceApiEngineKey: "platform-user-behavior-signal",
    Action: action,
    ResultCode: result ? result.Code : 0
});
if (!afterHook || afterHook.Code != 1) return afterHook || { Code: 0, Msg: "用户行为个性化 Hook 未返回结果。" };
return result;
