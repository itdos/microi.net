using System;
using Microsoft.Extensions.DependencyInjection;

namespace Microi.net
{

    public static class MicroiCacheExtensions
    {
        public static IServiceCollection AddMicroiCache(this IServiceCollection services)
        {
            try
            {
                services.AddSingleton<IMicroiCache, MicroiCacheRedis>();
                services.AddSingleton<IMicroiCacheTenant, MicroiCacheTenant>();// 工厂
                services.AddSingleton<Microi.Cache.MicroiRedisManager>();
                services.AddSingleton<Microi.Cache.IMicroiRedisManager>(provider =>
                    provider.GetRequiredService<Microi.Cache.MicroiRedisManager>());
                services.AddSingleton<IMicroiCacheManagementRuntime>(provider =>
                    provider.GetRequiredService<Microi.Cache.MicroiRedisManager>());
                Console.WriteLine($"Microi：【✅成功】【{DateTime.Now:yyyy-MM-dd HH:mm:ss}】注入【分布式缓存】插件成功！");
                return services;
            }
            catch (Exception ex)
            {
                Console.WriteLine($"Microi：【❌Error】【{DateTime.Now:yyyy-MM-dd HH:mm:ss}】注入【分布式缓存】插件失败：{ex.Message}");
                return services;
            }
        }
    }
}
