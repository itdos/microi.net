...
namespace Dos.Common
{
    ...
    public class CacheHelper
    {
...
#else
        private static volatile MemoryCache cache = new MemoryCache(new MemoryCacheOptions());
#endif
        /// <summary>
        /// timeout 600秒
        /// </summary>
        ...
        public static void AddCacheSlidingExpiration(string cacheKey, object cacheValue, int timeout)
        {
            ...
            {
                cache.Set(cacheKey, cacheValue, TimeSpan.FromSeconds(timeout));
            }
        }
        ...
        public static void AddCacheFilesDependency(string cacheKey, object cacheValue, params string[] filenames)
        {
            AddCacheDependency(cacheKey, cacheValue, _timeOut);
        }
        ...
        /// <Param name="timeout">绝对过期时间 （单位：秒）</Param>
        /// <Param name="dep">缓存依赖</Param>
        public static void AddCacheDependency(string cacheKey, object cacheValue, int timeout)
        {
            if (string.IsNullOrEmpty(cacheKey))
            {
                return;
