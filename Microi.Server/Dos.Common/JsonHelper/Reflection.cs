    ...
    internal sealed class Reflection
    {
        ...
        public Dictionary<string, myPropInfo> Getproperties(Type type, string typename, bool customType)
        {
            ...
            else
            {
                ...
                foreach (PropertyInfo p in pr)
                {
                    ...
                    myPropInfo d = CreateMyProp(p.PropertyType, name, customType);
                    d.setter = Reflection.CreateSetMethod(type, p);
                    if (d.setter != null)
                        d.CanWrite = true;
                    d.getter = Reflection.CreateGetMethod(type, p);
                    //sd.Add(p.Name.ToLower(), d);
                    //sd.Add(name.ToLower(), d);
                    sd.Add(name, d);//by iTdos.com 2017-01-10
                }
                ...
                foreach (FieldInfo f in fi)
                {
                    ...
                    if (f.IsLiteral == false)
                    {
                        ...
                        sd.Add(f.Name, d);//by iTdos.com 2017-01-10
                    }
                }
        ...
        internal Getters[] GetGetters(Type type, bool ShowReadOnlyProperties, List<Type> IgnoreAttributes,bool UseFalseLiteral)//JSONParameters param)
        {
            ...
            foreach (PropertyInfo p in props)
            {
                ...
                var lcName = name;//2017-01-10 by iTdos.com
                #endregion
                if (g != null)
            ...
            foreach (var f in fi)
            {
                ...
                if (f.IsLiteral == false && UseFalseLiteral)
                {
                    ...
                    if (g != null)
                        ...
                        getters.Add(new Getters { Getter = g, Name = f.Name, lcName = f.Name });//2017-01-10 by iTdos.com
                }
            }
            val = getters.ToArray();
