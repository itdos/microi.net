...
try{
 page=await browser.newPage({ignoreHTTPSErrors:true,viewport:{width:1600,height:1100}});
 page.on('response',async r=>{if(r.url().includes('GetSysMenuStep')){try{const j=await r.json();diagnostics.menuResponse={code:j.Code,includesOps:JSON.stringify(j).includes('平台运维中心')};}catch{}}});page.on('pageerror',e=>diagnostics.errors.push(e.message));page.on('console',m=>{if(m.type()==='error'&&/GotoSystem error:|frame-ancestors|frame|CSP/i.test(m.text()))diagnostics.errors.push(m.text());});
 ...
 await expect(page.getByText('平台运维中心',{exact:true}).first()).toBeVisible({timeout:5000}).catch(async()=>{await system.click();});
 await page.getByText('平台运维中心',{exact:true}).first().click({timeout:30000});
 let entry;for(let i=0;i<60;i++){for(const f of page.frames()){if(await f.getByRole('heading',{name:'平台运维中心',exact:true}).count()){entry=f;break;}}if(entry)break;await new Promise(r=>setTimeout(r,500));}
 diagnostics.frames=page.frames().map(f=>{const u=new URL(f.url()||'about:blank');return{origin:u.origin,path:u.pathname};});
 assert(entry,'Platform MicroService frame missing');await expect(entry.getByRole('heading',{name:'平台运维中心',exact:true})).toBeVisible({timeout:30000});
 const temporary=entry.getByLabel('独立运维地址'),blank=entry.getByRole('link',{name:'在新窗口打开 ↗'});
 await expect(temporary.or(blank).first()).toBeVisible({timeout:30000});
 if(await temporary.isVisible()){await temporary.fill('http://localhost:61880');await entry.getByRole('button',{name:'打开运维中心',exact:true}).click();}
 await expect(blank).toHaveAttribute('target','_blank');await expect(blank).toHaveAttribute('rel','noopener noreferrer');
 const ops=entry.frameLocator('iframe[title="Microi.Ops 独立运维中心"]');await ops.getByLabel('运维账号').fill('ops-test',{timeout:25000});await ops.getByLabel('运维密码').fill(password);await ops.getByRole('button',{name:'登录运维中心',exact:true}).click();await expect(ops.getByRole('heading',{name:'运行总览',exact:true})).toBeVisible();
 const popupPromise=page.waitForEvent('popup');await blank.click();const popup=await popupPromise;await popup.waitForLoadState();await expect(popup.getByRole('heading',{name:'运行总览',exact:true})).toBeVisible();await popup.close();
 ...
 diagnostics.passed=['real shared frontend admin login','published system-engine MicroService route','nested Ops iframe independent login','target blank entry opens independently'];
