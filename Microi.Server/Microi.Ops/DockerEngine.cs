...
public sealed class DockerEngine : IDisposable
{
    ...
    public async Task<string> Logs(string name, CancellationToken ct)
    {
        await Negotiate(ct);
        using var limit = CancellationTokenSource.CreateLinkedTokenSource(ct); limit.CancelAfter(TimeSpan.FromSeconds(15));
        using var response = await http.GetAsync(prefix + "/containers/" + Uri.EscapeDataString(name) + "/logs?stdout=1&stderr=1&tail=120&timestamps=1", limit.Token);
        if (!response.IsSuccessStatusCode) throw new OpsException("无法读取容器日志。", 502);
