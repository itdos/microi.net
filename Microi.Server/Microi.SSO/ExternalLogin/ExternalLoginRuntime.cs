...
namespace Microi.net
{
    ...
    /// </summary>
    public sealed class ExternalLoginRuntime : IPlatformApiRuntime
    {
        ...
        public ExternalLoginRuntime(IHttpClientFactory httpClientFactory)
        {
        ...
        /// <summary>
        /// 公开动作由 platform-external-login 接口引擎确定；回调 HTML 使用受签名
        /// HTTP 响应契约返回，宿主无需保留 ExternalLoginController。
        /// </summary>
        public Task<object> ExecuteAsync(string action, JObject parameters)
        {
            ...
            {
                "Begin" => Begin(parameters.ToObject<BeginRequest>()),
                "Callback" => Callback(
                    parameters["code"]?.ToString(),
                    parameters["state"]?.ToString(),
                    parameters["OsClient"]?.ToString(),
                    parameters["Provider"]?.ToString()),
                "CompleteLogin" => CompleteLogin(parameters.ToObject<CompleteLoginRequest>()),
                "ListBindings" => ListBindings(),
                "RevokeBinding" => RevokeBinding(parameters.ToObject<BindingMutationRequest>()),
                _ => Task.FromResult<object>(new DosResult(0, null, "不支持的外部登录动作。"))
            };
        }
        ...
        public async Task<object> Begin(BeginRequest request)
        {
        ...
        public async Task<object> Callback(string code, string state, string OsClient, string Provider)
        {
        ...
        public async Task<object> CompleteLogin(CompleteLoginRequest request)
        {
            var osClientResult = await ResolveOsClientAsync(request?.OsClient, true).ConfigureAwait(false);
            if (osClientResult.Code != 1) return Json(osClientResult);
            var osClient = osClientResult.Data;
        ...
        public async Task<object> ListBindings()
        {
            var token = await RequireUserTokenAsync().ConfigureAwait(false);
            if (token.Code != 1) return Json(token);
        ...
        public async Task<object> RevokeBinding(BindingMutationRequest request)
        {
            var token = await RequireUserTokenAsync().ConfigureAwait(false);
            if (token.Code != 1) return Json(token);
        ...
        private object PopupResult(string targetOrigin, string provider, bool success, string message, string ticket)
        {
            ...
            return SignedHttpResponse(200, "text/html; charset=utf-8", html, new JObject
            {
        ...
        private static DosResult SignedHttpResponse(
            int statusCode,
            string contentType,
            string body,
            JObject headers)
