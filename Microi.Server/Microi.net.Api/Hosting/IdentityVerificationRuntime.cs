...
namespace Microi.net.Api
{
    ...
    /// </summary>
    public sealed class IdentityVerificationRuntime : IPlatformApiRuntime
    {
        ...
        public IdentityVerificationRuntime(IHttpClientFactory httpClientFactory)
        {
            _httpClientFactory = httpClientFactory;
        }

        private static HttpContext HttpContext => DiyHttpContext.Current;
        private static HttpRequest Request => HttpContext?.Request;
        private static HttpResponse Response => HttpContext?.Response;
        private static object Json(object value) => value;

        /// <summary>
        /// WebAuthn 依赖当前 .NET 宿主的 Fido2 验签实现，因此仅保留为无路由的宿主
        /// 原子；公开地址、匿名策略、动作白名单和租户扩展仍全部由接口引擎控制。
        /// </summary>
        public Task<object> ExecuteAsync(string action, JObject parameters)
        {
            parameters ??= new JObject();
            var cancellationToken = HttpContext?.RequestAborted ?? CancellationToken.None;
            return (action ?? string.Empty).Trim() switch
            {
                "GetCapabilities" => GetCapabilities(parameters.ToObject<BeginAuthenticationRequest>()),
                "BeginPasskeyRegistration" => BeginPasskeyRegistration(parameters.ToObject<BeginPasskeyRegistrationRequest>()),
                "CompletePasskeyRegistration" => CompletePasskeyRegistration(parameters.ToObject<CompletePasskeyRegistrationRequest>(), cancellationToken),
                "BeginPasskeyAuthentication" => BeginPasskeyAuthentication(parameters.ToObject<BeginAuthenticationRequest>()),
                "CompletePasskeyAuthentication" => CompletePasskeyAuthentication(parameters.ToObject<CompleteAuthenticationRequest>(), cancellationToken),
                "ListAuthenticators" => ListAuthenticators(),
                "RenameAuthenticator" => RenameAuthenticator(parameters.ToObject<AuthenticatorMutationRequest>()),
                "RevokeAuthenticator" => RevokeAuthenticator(parameters.ToObject<AuthenticatorMutationRequest>()),
                "UpdateAuthenticatorPolicy" => UpdateAuthenticatorPolicy(parameters.ToObject<AuthenticatorPolicyRequest>()),
                "BeginTotpEnrollment" => BeginTotpEnrollment(),
                "CompleteTotpEnrollment" => CompleteTotpEnrollment(parameters.ToObject<CompleteTotpEnrollmentRequest>()),
                "ListTotpAuthenticators" => ListTotpAuthenticators(),
                "RevokeTotpAuthenticator" => RevokeTotpAuthenticator(parameters.ToObject<AuthenticatorMutationRequest>()),
                "VerifyTotp" => VerifyTotp(parameters.ToObject<VerifyTotpRequest>()),
                "BeginFaceVerification" => BeginFaceVerification(parameters.ToObject<BeginFaceRequest>(), cancellationToken),
                "CompleteFaceVerification" => CompleteFaceVerification(parameters.ToObject<CompleteFaceRequest>(), cancellationToken),
                _ => Task.FromResult<object>(new DosResult(0, null, "不支持的强身份验证动作。"))
            };
        }
        ...
        public async Task<object> GetCapabilities(BeginAuthenticationRequest request)
        {
        ...
        public async Task<object> BeginPasskeyRegistration(BeginPasskeyRegistrationRequest request)
        {
        ...
        public async Task<object> CompletePasskeyRegistration(
        ...
        public async Task<object> BeginPasskeyAuthentication(BeginAuthenticationRequest request)
        {
            var purpose = "";
            try
            {
        ...
        public async Task<object> CompletePasskeyAuthentication(
        ...
        public async Task<object> ListAuthenticators()
        {
            var token = await RequireUserTokenAsync().ConfigureAwait(false);
            if (token.Code != 1) return Json(token);
        ...
        public async Task<object> RenameAuthenticator(AuthenticatorMutationRequest request)
        {
            var token = await RequireUserTokenAsync().ConfigureAwait(false);
            if (token.Code != 1) return Json(token);
        ...
        public async Task<object> RevokeAuthenticator(AuthenticatorMutationRequest request)
        {
            var token = await RequireUserTokenAsync().ConfigureAwait(false);
            if (token.Code != 1) return Json(token);
            var credential = await FindOwnedCredentialAsync(token.Data, request?.Id).ConfigureAwait(false);
        ...
        public async Task<object> UpdateAuthenticatorPolicy(AuthenticatorPolicyRequest request)
        {
            var token = await RequireUserTokenAsync().ConfigureAwait(false);
            if (token.Code != 1) return Json(token);
        ...
        public async Task<object> BeginTotpEnrollment()
        {
            var token = await RequireUserTokenAsync().ConfigureAwait(false);
            if (token.Code != 1) return Json(token);
        ...
        public async Task<object> CompleteTotpEnrollment(CompleteTotpEnrollmentRequest request)
        {
            var token = await RequireUserTokenAsync().ConfigureAwait(false);
            if (token.Code != 1) return Json(token);
        ...
        public async Task<object> ListTotpAuthenticators()
        {
            var token = await RequireUserTokenAsync().ConfigureAwait(false);
            if (token.Code != 1) return Json(token);
        ...
        public async Task<object> RevokeTotpAuthenticator(AuthenticatorMutationRequest request)
        {
            var token = await RequireUserTokenAsync().ConfigureAwait(false);
            if (token.Code != 1) return Json(token);
        ...
        public async Task<object> VerifyTotp(VerifyTotpRequest request)
        {
        ...
        /// </summary>
        public async Task<object> BeginFaceVerification(BeginFaceRequest request, CancellationToken cancellationToken)
        {
            try
            {
                var purpose = IdentityVerificationSecurity.NormalizePurpose(request?.Purpose ?? "Login");
        ...
        public async Task<object> CompleteFaceVerification(CompleteFaceRequest request, CancellationToken cancellationToken)
        {
            var osClientResult = await ResolveOsClientAsync(request?.OsClient, allowAnonymous: true).ConfigureAwait(false);
            if (osClientResult.Code != 1) return Json(osClientResult);
        ...
        private async Task<object> CreateDiyTokenLoginResultAsync(
            string osClient,
            JObject user,
            string clientType,
            string did,
