...
public class CacheAndUpgradeRegressionTests
{
    ...
    public void OfficialSsoBundle_IsEmbeddedAndCarriesManagedHookRuntimeContract()
    {
        ...
        var legacyCapabilities = Assert.Single(
            package["SysApiEngines"]!.Children<JObject>(),
            item => item["ApiEngineKey"]?.ToString() == "sso_legacy_capabilities");
        Assert.Equal("v1.0.4", legacyCapabilities["Version"]?.ToString());
        Assert.Contains("/api/SysUser/TokenLogin", legacyCapabilities["ApiV8Code"]?.ToString());
