    ...
    public partial class FormEngineExtend
    {
        ...
        public async Task<DosResult<dynamic>> AddDiyTable(dynamic dynamicParam, IMicroiDbTransaction _trans = null)
        {
            ...
            try
            {
                ...
                if (!(param["DataBaseId"]?.Value<string>()).DosIsNullOrWhiteSpace())
                {
                    dbInfo = DiyCommon.GetDbInfo(OsClientExtend.GetClientDataBase(osClientModel, param["DataBaseId"]?.Value<string>()).DbType);
                }
                IMicroiDbTransaction trans = _trans == null ? dbSession.BeginTransaction() : _trans;
                try
                {
                    if (trans.From<DiyTable>().Where(d => d.IsDeleted == 0 && d.Name == tableName).Count() > 0)
                    {
                        if (_trans == null)
                        {
                            trans.Rollback();
                    ...
                    try
                    {
                        ...
                        var result = MicroiEngine.ORM(dbInfo.DbType).AddDiyTable(new DbServiceParam()
                        {
                            ...
                            DbInfo = dbInfo,
                            DataBaseId = param["DataBaseId"]?.Value<string>(),
                            OsClient = osClient,
                ...
                if (dbSession != null)
                {
                    using (var trans = dbSession.BeginTransaction())
                    {
                        ...
                        if (model.IsTree == 1)
                        {
                            ...
                            if (!fieldListResult.Data.Any(d => d.Name.ToLower() == model.TreeParentField.ToLower()) || !fieldListResult.Data.Any(d => d.Name.ToLower() == model.TreeParentFields.ToLower()))
                            {
                                trans.Rollback();
        ...
        /// <param name="param"></param>
        /// <returns></returns>
        public async Task<DosResultList<DiyTable>> GetDiyTable(DiyTableParam param)
        {
            #region Check
            if (param.OsClient.DosIsNullOrWhiteSpace())
            {
            ...
            if (param.OsClient.DosIsNullOrWhiteSpace())
            {
                return new DosResultList<DiyTable>(0, null, DiyMessage.GetLang(param.OsClient, "ParamError", param._Lang) + "[GetTableTable]", null, new
                {
                    Param = param
                });
            }
            ...
            try
            {
                ...
                if (dbSession != null)
                {
                    ...
                    var list = fs.ToList();
                    return new DosResultList<DiyTable>(1, list, "", dataCount);
                }
                return new DosResultList<DiyTable>(0, null, "数据库参数错误！" + "[GetTableTable]", null, new
                {
                    Param = param
                });
            }
            catch (Exception ex)
            {
                ...
                return new DosResultList<DiyTable>(0, null, ex.Message + "[GetTableTable]", null, new
                {
                    Param = param
                });
            }
        ...
        public async Task<DosResult> UptDiyFieldList(DiyFieldParam param)
        {
            ...
            var newFieldList = JsonConvert.DeserializeObject<List<DiyField>>(param._FieldList);
            ...
            try
            {
                if (dbSession != null)
                {
                    using (var trans = dbSession.BeginTransaction())
                    {
                        ...
                        //    }
                        //}
                        //model = JsonConvert.DeserializeObject<DiyField>(JsonConvert.SerializeObject(modelJson));
        ...
        /// <param name="param"></param>
        /// <returns></returns>
        public async Task<DosResultList<DiyField>> GetDiyField(DiyFieldParam param)
        {
            #region Check
            if (param.TableId.DosIsNullOrWhiteSpace() && param.TableName.DosIsNullOrWhiteSpace())
            {
                return new DosResultList<DiyField>(0, null, DiyMessage.GetLang(param.OsClient, "ParamError", param._Lang));
            }
            if (param.OsClient.DosIsNullOrWhiteSpace())
            {
                param.OsClient = DiyTokenExtend.GetCurrentOsClient();
            ...
            if (param.OsClient.DosIsNullOrWhiteSpace())
            {
                return new DosResultList<DiyField>(0, null, DiyMessage.GetLang(param.OsClient, "OsClientNotNull", param._Lang));
            }
            #endregion
            try
            {
                ...
                if (dbSession != null)
                {
                    ...
                    //var diyTableModelResult = await GetFormDataAsync<DiyTable>(new
                    //{
                    //    FormEngineKey = "Diy_Table",
                    //    _Where = new List<DiyWhere>() { new DiyWhere() {
                    //                Name = "Id", Value = param.TableId, Type = "="
                    ...
                    //if (diyTableModelResult.Code != 1)
                    //{
                    //    return new DosResultList<DiyField>(0, null, diyTableModelResult.Msg);
                    //}
                    //var diyTableModel = diyTableModelResult.Data;

                    //oracle取不出来数据 --2024-03-26
                    //var diyTableModel = dbSession.FromSql("select * from diy_table where (Id=@TableId or Name=@TableName) and IsDeleted=0 ")
                    //                        .AddInParameter("TableId", DbType.String, param.TableId ?? "")
                    //                        .AddInParameter("TableName", DbType.String, param.TableName ?? "")
                    //                        .First<DiyTable>()
                    //                        ;
                    var _where = new Where<DiyTable>();
                    var idOrName = "";
                    _where.And(d => d.IsDeleted == 0);
                    if (!param.TableId.DosIsNullOrWhiteSpace())
                    {
                        _where.And(d => d.Id == param.TableId);
                        idOrName = param.TableId;
                    }
                    else if (!param.TableName.DosIsNullOrWhiteSpace())
                    {
                        _where.And(d => d.Name == param.TableName);
                        idOrName = param.TableName;
                    }
                    ...
                    //}
                    //else {
                    //    _where.And(d => d.IsDeleted == param.IsDeleted);
                    //}
                    ...
                    var result = new List<DiyField>();
                    var isCache = false;
                    if (param.IsDeleted == 0 || param.IsDeleted == null)
                    {
                        ////result = await DiyFieldCache.GetDiyFieldList(param.TableId, param.OsClient);
                        //result = null;// await DiyFieldCache.GetDiyFieldList(tableId, param.OsClient);
                        //if (result != null)
                        //{
                        //    isCache = true;
                        //}
                    ...
                    var where = new Where<DiyField>();
                    //where.And(d => d.OsClient == param.OsClient);
                    //where.And(d => d.TableId == param.TableId);
                    ...
                    if (param.IsDeleted == 0 && isCache != true)
                    {
                        //result = dbSession.From<DiyField>()
                    ...
                    if (param.Ids != null)
                    {
                        if (isCache)
                        {
                            result = result.Where(d => param.Ids.Contains(d.Id)).ToList();
                        }
                        else
                        {
                            where.And(d => d.Id.In(param.Ids));
                    ...
                    if (param._SelectFields != null && param._SelectFields.Any())
                    {
                        if (isCache)
                        {
                            result = result.Where(d => param._SelectFields.Contains(d.Name)).ToList();
                        }
                        else
                        {
                            where.And(d => d.Name.In(param._SelectFields));
                        }
                    }
                    if (param._OnlyRealField == true)
                    {
                        if (isCache)
                        {
                            result = result.Where(d => d.Type != "" && d.Type != null
                                        && !DiyCommon.NotRealField.Contains(d.Component)).ToList();
                        }
                        else
                        {
                            if (dbInfo.DbType == DatabaseType.Oracle)
                            {
                                where.And(d => d.Type != null
                                        && (d.Component.NotIn(DiyCommon.NotRealField) || d.Component == null));
                            }
                            else
                            {
                                where.And(d => d.Type != "" && d.Type != null
                                        && (d.Component.NotIn(DiyCommon.NotRealField) || d.Component == null));
                            }
                        }
                    }
                    ...
                    if (!param._Keyword.DosIsNullOrWhiteSpace())
                    {
                        if (isCache)
                        {
                            result = result.Where(d => d.Name.Like(param._Keyword) || d.Description.Like(param._Keyword)).ToList();
                        }
                        else
                        {
                            where.And(d => d.Name.Like(param._Keyword) || d.Description.Like(param._Keyword));
                    ...
                    if (!string.IsNullOrWhiteSpace(param._OrderBy))
                    {
                        ...
                        {
                            orderBy = orderBy && DiyField._.Sort.Asc;
                        }
                    }
                    else
                    {
                    ...
                    return new DosResultList<DiyField>(1, result, "", dataCount);
                }
                return new DosResultList<DiyField>(0, null, DiyMessage.GetLang(param.OsClient, "ParamError", param._Lang));
            }
            catch (Exception ex)
            {
                ...
                return new DosResultList<DiyField>(0, null, ex.Message);
            }
        }
        /// <summary>
        /// 必传：TableId
        ...
        /// <param name="param"></param>
        /// <returns></returns>
        public async Task<DosResultList<DiyField>> GetDiyFieldByDiyTables(DiyFieldParam param)
        {
            #region Check
            ...
            {
                return new DosResultList<DiyField>(1, new List<DiyField>());
            }
                ...
                if (dbSession != null)
                {
                    ...
                    if (!param.SysMenuId.DosIsNullOrWhiteSpace() || !param._ModuleEngineKey.DosIsNullOrWhiteSpace())
                    {
                        ...
                        if (sysMenuModel != null)
                        {
                            ...
                            {
                                var tables = JsonConvert.DeserializeObject<List<DiyTable>>(joinTables);
                    ...
                    {
                        return new DosResultList<DiyField>(0, null, "未找到任何表！");
                    }
                    //先获取 DiyTable的model
                    ...
                    foreach (var table in param.TableIds)
                    {
                        ...
                        {
                            return new DosResultList<DiyField>(0, null, getDiTableResult.Msg);
                        }
                    }
                    ...
                    return new DosResultList<DiyField>(1, result, "", result.Count);
                }
                return new DosResultList<DiyField>(0, null, DiyMessage.GetLang(param.OsClient, "ParamError", param._Lang));
            }
            catch (Exception ex)
        ...
        /// <param name="param"></param>
        /// <returns></returns>
        public async Task<DosResult<DiyField>> GetDiyFieldModel(DiyFieldParam param)
        {
            // var msg = "";
            if (param.Id.DosIsNullOrWhiteSpace()
                && param.TableId.DosIsNullOrWhiteSpace()
            ...
            {
                return new DosResult<DiyField>(0, null, DiyMessage.GetLang(param.OsClient, "ParamError", param._Lang));
            }
            ...
            {
                where.And(d => d.Id == param.Id);
                model = null; // await DiyFieldCache.GetDiyFieldModel(param.Id, param.OsClient);
                if (model != null)
                {
                    return new DosResult<DiyField>(1, model);
                }
            }
            else
            {
            ...
            if (model == null)
            {
                ...
                //DiyFieldCache.SetDiyFieldModel(model, param.OsClient);
            }
