...
{
    /// <summary>
    /// 音乐任务复用 mci_background_task 的持久队列、唯一键、租约和栅栏。
    /// HTTP 断开只结束等待，不取消已入队任务；列表/状态投影不返回原始提示词或供应商回执。
    /// </summary>
    public static class AiMusicBackgroundTaskService
    {
        public const string WorkerApiEngineKey = "__microi_native_ai_music__";
        public const string ProviderWorkerApiEngineKey = "__microi_native_ai_music_provider__";
        public static bool IsMusicWorker(string key) => string.Equals(key, WorkerApiEngineKey, StringComparison.OrdinalIgnoreCase)
        ...
        public static DosResult RuntimeUnavailable() => new DosResult(0,
            ...
            "当前节点缺少 AI 音乐持久任务运行时，请同步更新 Microi.Core、Microi.AI 和平台 API；尚未创建生成任务。");

        public static IReadOnlyCollection<string> ExcludeUnsupportedWorkers(
            IEnumerable<string> excludedKeys, bool musicRuntimeAvailable)
        {
            var excluded = new HashSet<string>(excludedKeys ?? Array.Empty<string>(), StringComparer.OrdinalIgnoreCase);
            if (!musicRuntimeAvailable) { excluded.Add(WorkerApiEngineKey); excluded.Add(ProviderWorkerApiEngineKey); }
        ...
        public static string BuildTaskIdempotencyKey(string osClient, string userId, string requestId)
        {
            using (var sha = SHA256.Create())
                return "ai-music:" + BitConverter.ToString(sha.ComputeHash(Encoding.UTF8.GetBytes(
                    MiniMaxMusicSupport.BuildIdempotencyKey(osClient, userId, requestId))))
                    .Replace("-", "").ToLowerInvariant();
        }

        public static DosResult Queue(string osClient, JObject currentUser,
            MiniMaxMusicGenerateParam request, bool requireDirectProvider = false)
        {
            var userId = currentUser?["Id"]?.ToString();
            if (string.IsNullOrWhiteSpace(osClient) || string.IsNullOrWhiteSpace(userId))
                return new DosResult(1001, null, "请先登录。");
            if (!MiniMaxMusicSupport.TryNormalize(request, out var normalized, out var error))
            ...
            try
            {
                ...
                // 只序列化白名单 DTO。参数已经过长度与时长上限校验，保存在服务端私有
                ...
                var task = BackgroundTaskService.StartApiEngine(osClient, userId, "AI 音乐生成",
                ...
                return persisted == null
                    ? new DosResult(0, new { Status = "Unavailable", TaskId = task.Id }, "音乐任务已提交，但持久状态暂不可读；请查询同一 TaskId。")
                    : ReadMatchingTask(osClient, userId, persisted, normalized.Fingerprint);
            }
            catch
            {
                return new DosResult(0, new { Status = "Unavailable" }, "音乐持久任务入队或回读失败；请保留 RequestId，未在 HTTP 请求中直接调用上游。");
            }
        }
        ...
        public static DosResult GetStatus(string osClient, string userId, string taskId)
        {
            ...
            if (!Guid.TryParseExact(taskId, "N", out _))
                return new DosResult(0, null, "音乐 TaskId 格式无效。");
            try
            {
                var summary = BackgroundTaskService.GetSummary(osClient, userId, taskId);
                if (summary == null || !IsMusicWorker(summary.ApiEngineKey))
                    return new DosResult(0, new { Status = "NotFound" }, "当前用户的音乐任务不存在。");
            ...
            {
                return new DosResult(0, new { Status = "Unavailable" }, "音乐任务状态暂不可读，请稍后查询同一任务。");
            }
        }

        private static bool HasStoredResult(string tenant, string userId, BackgroundTaskRecord task)
        {
            if (task == null || task.UserKey != userId || !IsMusicWorker(task.ApiEngineKey)) return false;
            var param = JObject.Parse(task.ParamJson ?? "{}");
            var request = param["Request"]?.ToObject<MiniMaxMusicGenerateParam>();
            if (!MiniMaxMusicSupport.TryNormalize(request, out var normalized, out _)
            ...
            var relayTask = cache.StringGet(MiniMaxMusicSupport.BuildIdempotencyKey(tenant, userId, normalized.RequestId) + ":relay-task");
            if (relayTask.HasValue && Guid.TryParseExact(relayTask.ToString(), "N", out _)) return true;
            var value = cache.StringGet(MiniMaxMusicSupport.BuildIdempotencyKey(tenant, userId, normalized.RequestId) + ":music3-event");
            if (!value.HasValue) return false;
            var saved = JObject.Parse(value.ToString());
            return saved["Fingerprint"]?.ToString() == normalized.Fingerprint && saved["TerminalFailure"]?.Value<bool>() != true
                && !string.IsNullOrWhiteSpace(saved["event_id"]?.ToString());
        }
        ...
        public static DosResult RecoverResult(string osClient, string userId, string taskId)
        {
            ...
            if (!Guid.TryParseExact(taskId, "N", out _)) return new DosResult(0, null, "音乐 TaskId 格式无效。");
            try
            {
                var task = BackgroundTaskStore.GetForUser(osClient, userId, taskId);
                if (!HasStoredResult(osClient, userId, task))
                    return new DosResult(0, null, "没有可恢复的供应商音乐节点；未重新生成音乐。");
            ...
            catch { return new DosResult(0, null, "音乐恢复状态暂不可确认，请查询同一任务。"); }
        }
        ...
        public static DosResult Project(BackgroundTaskSummary task, JObject result)
        {
            ...
            if (task.Status == "Succeeded" && result?["Code"]?.Value<int>() == 1
                && data["Permanent"]?.Value<bool>() == true && !string.IsNullOrWhiteSpace(data["FileUrl"]?.ToString()))
                return new DosResult(1, data, result["Msg"]?.ToString() ?? "音乐已生成并保存。");
            if (task.Status == "Pending" || task.Status == "Running" || task.Status == "Retrying")
            {
                data["Status"] = task.Status;
                data["PollAfterMs"] = 1500;
                return new DosResult(2, data, "音乐任务正在后台执行，请查询同一 TaskId。");
            }
            ...
            return new DosResult(0, data, result?["Msg"]?.ToString()
                ?? (string.IsNullOrWhiteSpace(task.Msg) ? "音乐任务未完成，未自动重复调用上游。" : task.Msg));
        }
    }
}
