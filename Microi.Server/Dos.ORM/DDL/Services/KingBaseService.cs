...
{
    /// <summary>
    /// 人大金仓数据库实现（兼容PostgreSQL语法）
    /// </summary>
    public class KingBaseService : IMicroiORM
    {
        ...
        public DosResult AddColumn(DbServiceParam param, DbTrans _trans = null)
        {
            try
            {
                ...
                if (!param.FieldLabel.DosIsNullOrWhiteSpace())
                {
                    ...
                    try { session.FromSql($"COMMENT ON COLUMN \"{param.TableName}\".\"{param.FieldName}\" IS '{comment}'").ExecuteNonQuery(); }
                    catch { }
                }
        ...
        public DosResult ChangeColumn(DbServiceParam param, DbTrans _trans = null)
        {
            try
            {
                ...
                {
                    var comment = param.FieldLabel.Replace("'", "''");
                    try { session.FromSql($"COMMENT ON COLUMN \"{param.TableName}\".\"{param.NewFieldName}\" IS '{comment}'").ExecuteNonQuery(); }
                    catch { }
                }
        ...
        public DosResultList<string> GetTables(DbServiceParam param)
        {
            var sql = @"SELECT tablename AS table_name
                        ...
                        WHERE schemaname NOT IN ('pg_catalog', 'information_schema', 'sys_catalog')
                        ORDER BY tablename";
        ...
        /// <summary>
        /// 转换MySQL类型为KingBase类型（兼容PostgreSQL）
        /// </summary>
