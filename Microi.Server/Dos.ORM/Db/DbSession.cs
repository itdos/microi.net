    ...
    public sealed class DbSession
    {
...
#else
                foreach (string key in db.DbProvider.CacheConfig.Entities.Keys)
#endif
                {
                    if (key.IndexOf('.') > 0)
                    {
...
#else
                            if (int.TryParse(db.DbProvider.CacheConfig.Entities[key], out expireSeconds))
#endif
                            {
...
#else
                                string tempFilePath = Path.Combine(AppDomain.CurrentDomain.BaseDirectory, db.DbProvider.CacheConfig.Entities[key]);
#endif
             ...
             where TEntity : Entity
        {
            ...
            else
            {
                ...
                //2023-04-02新增 --by anderson
                else if (Db.DbProvider.DatabaseType == DatabaseType.Oracle)
                {
                    if (tran == null)
                    {
                        ExecuteNonQuery(cmd);
