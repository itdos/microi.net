...
namespace Dos.ORM.SeedConversion
{
    ...
    internal sealed class SqlServerSeedSqlWriter : SeedSqlWriter
    {
        ...
        protected override void WriteHeader()
        {
            ...
            Output.WriteLine("SET ANSI_NULLS ON;");
            Output.WriteLine("SET ANSI_PADDING ON;");
            Output.WriteLine("SET ANSI_WARNINGS ON;");
            Output.WriteLine("SET ARITHABORT ON;");
            Output.WriteLine("SET CONCAT_NULL_YIELDS_NULL ON;");
            Output.WriteLine("SET QUOTED_IDENTIFIER ON;");
            Output.WriteLine("SET NUMERIC_ROUNDABORT OFF;");
            Output.WriteLine();
        }
        ...
        protected override void WriteIndexes(SeedTable table)
        {
            foreach (var index in table.Indexes)
            {
                ...
                var nullableFilterColumns = new List<string>();
                for (var columnIndex = 0; columnIndex < index.Columns.Count; columnIndex++)
                {
                    var column = index.Columns[columnIndex];
                    if (table.ColumnsByName.TryGetValue(column.Name, out var tableColumn)
                        && tableColumn.IsNullable)
                    {
                        // MySQL permits multiple rows whose UNIQUE key contains NULL.
                        // SQL Server permits only one such key unless the index excludes
                        // the nullable rows. Filter on the source column as computed
                        // prefix columns cannot be used in a filtered-index predicate.
                        nullableFilterColumns.Add(QuoteIdentifier(column.Name));
                    }
                    if (column.PrefixLength.HasValue)
                    {
                ...
                Output.Write(')');
                if (index.IsUnique && nullableFilterColumns.Count > 0)
                {
                    Output.Write(" WHERE ");
                    Output.Write(string.Join(
                        " IS NOT NULL AND ", nullableFilterColumns));
                    Output.Write(" IS NOT NULL");
                }
                Output.WriteLine(';');
            }
    ...
    internal sealed class PostgreSqlSeedSqlWriter : SeedSqlWriter
    {
        ...
        protected override string MapType(SeedColumnType type)
        {
            switch (type.Name)
            {
                ...
                case "bigint": return "BIGINT";
                // Microi's runtime metadata and V8/FormEngine predicates use
                // the cross-database 0/1 contract for MySQL BIT columns.
                // SMALLINT keeps that contract on PostgreSQL/Kingbase and
                // avoids boolean-vs-integer failures in legacy raw SQL.
