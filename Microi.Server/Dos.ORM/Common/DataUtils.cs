...
namespace Dos.ORM
{
    ...
    {
        public static List<string> OracleDefaultFieldNames = new List<string>() { "Unique", "Level", "Column", "Lock" };
        ...
        internal static string FormatSQL(string sql, char leftToken, char rightToken)
        {
            ...
            if (leftToken.ToString() == "\"")
            {
                left = "";
                right = "";
                //foreach (var item in OracleDefaultFieldNames)
                //{
                //    if (item.ToLower() == )
                //    {
