#region << 版 本 注 释 >>
/****************************************************
* 文 件 名：KingBaseProvider.cs
* Copyright(c) ITdos
* 创 建 人：Microi吾码
* 创建日期：2025
* 文件描述：人大金仓数据库驱动实现（Kdbndp）
* 备注：人大金仓基于PostgreSQL，语法高度相似
******************************************************/
#endregion
...
namespace Dos.ORM.KingBase
{
    /// <summary>
    /// 人大金仓 数据库提供程序实现
    /// </summary>
    public class KingBaseProvider : DbProvider
    {
        public KingBaseProvider(string connectionString)
            : base(connectionString, KdbndpFactory.Instance, '"', '"', ':')
        {
        }
        ...
        /// <summary>
        /// 创建分页查询（兼容PostgreSQL，使用 LIMIT...OFFSET 语法）
        /// </summary>
        ...
        public override void PrepareCommand(DbCommand cmd)
        {
            ...
            foreach (KdbndpParameter param in cmd.Parameters)
            {
                ...
                {
                    param.KdbndpDbType = KdbndpTypes.KdbndpDbType.Varchar;
                    param.Size = 36;
                    continue;
                }
                ...
                {
                    param.KdbndpDbType = KdbndpTypes.KdbndpDbType.Interval;
                    continue;
                }
                ...
                {
                    param.KdbndpDbType = KdbndpTypes.KdbndpDbType.Text;
                }

                // 处理二进制大数据
                if (param.DbType == DbType.Binary && ((byte[])value).Length > 8000)
                {
                    param.KdbndpDbType = KdbndpTypes.KdbndpDbType.Bytea;
                }
            }
        ...
        /// <summary>
        /// 处理 charindex 函数转换为 position 函数（兼容PostgreSQL）
        /// </summary>
