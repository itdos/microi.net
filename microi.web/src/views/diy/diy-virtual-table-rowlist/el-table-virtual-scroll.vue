<template>
    <div
        ...
        :class="[isExpanding ? 'is-expanding' : '', isHideAppend ? 'hide-append' : '', scrollPosition ? `is-scrolling-${scrollPosition}` : '', hasFixedRight ? 'has-custom-fixed-right' : '']"
    >
...
import { isScroller, getParentScroller, getScrollTop, getOffsetHeight, scrollToY, isEmpty, setMousewheelSlow, orderBy, getColumnById } from "./util";
...
export default {
    ...
    methods: {
        ...
        toggleRowsSelection(rows, selected) {
            ...
            if (toDeleteMap) {
                this.oldSelection = this.oldSelection.filter((row) => !(row[this.keyProp] in toDeleteMap));
            }
        ...
        hackSelection() {
            ...
            const unWatch = this.$watch(() => [this.elTable.fixedColumns, this.elTable.rightFixedColumns], onFixedColumnsChange, { immediate: true });
            this.unWatchs.push(unWatch);
        ...
        hackTableExpand() {
            ...
            store.toggleRowExpansion = (row, expanded) => {
                ...
                this.elTable.$emit(
                    "expand-change",
                    row,
                    this.listData.filter((row) => row.$v_expanded)
                );
