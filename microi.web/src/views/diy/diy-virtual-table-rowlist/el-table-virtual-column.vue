<template>
  <el-table-column v-bind="$attrs" v-on="$listeners" :type="getColumnType()" :class-name="getClassName">
    ...
    <template slot="header" slot-scope="scope">
      ...
      <template v-else>
        <!-- 多选类型-全选 -->
        <el-checkbox v-if="$attrs.type === 'selection'" :value="isCheckedAll" :indeterminate="isCheckedImn"
    ...
    <template slot-scope="scope">
      ...
      <template v-if="isTree">
        ...
        <!-- treeNodeKey 用来更新视图的 -->
        <div v-if="canExpand(scope.row) && treeNodeKey" class="el-table__expand-icon"
      ...
      <template v-else>
        <!-- 多选类型 -->
        <el-checkbox @click.native.stop v-if="$attrs.type === 'selection'" :value="scope.row.$v_checked || false"
          :disabled="getDisabled(scope)" @change="onCheckRow(scope, !scope.row.$v_checked)">
        </el-checkbox>
        <!-- 单选类型 -->
        <el-radio v-if="$attrs.type === 'radio'" :label="true" :value="virtualScroll.curRow === scope.row"
        ...
        <template v-else-if="scope.column.formatter">
          ...
          <el-table-virtual-column-formatter v-if="isVNode(getFormatterResult(scope))"
...
export default {
  ...
  methods: {
    ...
    onCheckAllRows(val) {
      ...
      if (this.selectable) {
        // 筛选出可选的行
        const selectableList = getAllNodes(list).filter((row, index) =>
