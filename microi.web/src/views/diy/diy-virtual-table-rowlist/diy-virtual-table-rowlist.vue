    ...
    <el-tabs id="table-rowlist-tabs" v-model="TableRowListActiveTab" @tab-click="tabClickRowList" :class="!IsPageTabs() ? 'table-rowlist-tabs tab-pane-hide' : 'table-rowlist-tabs box-card-top-tabs'">
      ...
      <template v-for="(tab, tabIndex) in SysMenuModel.PageTabs">
        <el-tab-pane v-if="tab.IsVisible" :key="TypeFieldName + 'page_tabs_' + tab.Id + tabIndex" :name="tab.Id" :lazy="true">
          <span slot="label" :style="{ color: TableRowListActiveTab !== tab.Id ? ' #171717 !important' : '' }">
            <i :class="DiyCommon.IsNull(tab.Icon) ? 'fas fa-list-ol marginRight5' : tab.Icon + ' marginRight5'" :style="{ color: TableRowListActiveTab !== tab.Id ? ' #171717 !important' : '' }" />
            <template>
              {{ generateTitle(tab.Name) }}
            </template>
          </span>
        ...
        <el-col :span="24">
          ...
          <el-card class="box-card box-card-table-row-list">
            <div v-if="IsTableChild() && TableChildField.Label" slot="header" class="clearfix">
              <span style="font-weight: bold"> <i class="mr-2 fas fa-table"></i>{{ $t(TableChildField.Label) }}</span>
            </div>
            <div v-if="PropsIsJoinTable && JoinTableField.Label" slot="header" class="clearfix">
              <span style="font-weight: bold"> <i class="mr-2 fas fa-table"></i>{{ $t(JoinTableField.Label) }}</span>
            </div>
            <!--DIY功能按钮 新版-->
            <div class="keyword-search">
              <div class="pull-left item-in" style="margin-right: 0px">
                <el-button
                  v-if="LimitAdd() && TableChildFormMode != 'View' && !TableChildField.Readonly && PropsIsJoinTable !== true"
                  :loading="BtnLoading"
                  type="primary"
                ...
                <template v-if="!IsTableChild() || (IsTableChild() && !TableChildField.Readonly)">
                  ...
                  <el-dropdown v-if="LimitExport() && !DiyCommon.IsNull(SysMenuModel.ExportMoreBtns) && SysMenuModel.ExportMoreBtns.length > 0" size="mini" trigger="click" style="margin-left: 10px">
                    <!-- {{ $t('Msg.Export') }} -->
                    <el-button> {{ $t("Msg.Export") }}<i class="el-icon-arrow-down el-icon--right" /> </el-button>
              ...
              <div class="pull-left item-in" v-if="IsPermission('NoSearch')">
                ...
                <el-input class="input-left-borderbg" style="margin-top: 1px" v-model="Keyword" @input="_debounce(GetDiyTableRow, { _PageIndex: 1 })" :placeholder="$t('Msg.Search')">
                  <el-button slot="append" icon="el-icon-search" @click="_debounce(GetDiyTableRow, { _PageIndex: 1 })"></el-button>
                </el-input>
              </div>
            ...
            <virtual-scroll ref="virtualScroll" :data="DiyTableRowList" :item-size="38" key-prop="Id" :throttleTime="16" @change="(renderData) => (virtualList = renderData)">
              <!--DIY表格-->
              <el-table
                ...
                :data="isTreeTypeData ? DiyTableRowList : virtualList"
                style="width: 100%"
              ...
              >
                <component :is="useComponentName" v-if="TableEnableBatch" type="selection" label="#" width="55" reserve-selection> </component>

                <!-- <el-table-column v-if="TableEnableBatch" type="selection" label="#" width="55"
                  :reserve-selection="true">
                </el-table-column> -->

                <component :is="useComponentName" type="index" :label="generateTitle('序号')" width="50" :index="indexMethod"> </component>
                <template>
                  <template v-for="(field, fieldIndex) in ShowDiyFieldList">
                    <component
                      ...
                      :label="generateTitle(field.Label)"
                      :width="elTableColWidth(field, fieldIndex, ShowDiyFieldList)"
                      :sortable="SortFieldIds.indexOf(field.Id) > -1 ? 'custom' : false"
                      :class-name="'column-' + field.Name"
                      show-overflow-tooltip
                    >
                      <template slot-scope="scope">
                        <template v-if="templateVhtml(field, scope)">
                        ...
                        <template v-else-if="NeedDiyTemplateFieldLst.indexOf(field.Component) > -1">
                          ...
                          <template v-else-if="field.Component == 'ImgUpload'">
                            <template>
                              <i class="far fa-images hand" :style="{ color: !DiyCommon.IsNull(scope.row[DiyCommon.IsNull(field.AsName) ? field.Name : field.AsName]) ? '#ff6c04' : '#ccc' }"></i>
                            </template>
                          </template>
                        </template>
                        ...
                        <template v-else>
                          ...
                          <template v-else-if="field.Component == 'Switch'">
                            <el-switch
                              :value="
                                scope.row[DiyCommon.IsNull(field.AsName) ? field.Name : field.AsName] == true || scope.row[DiyCommon.IsNull(field.AsName) ? field.Name : field.AsName] == 1
                                  ? true
                                  : false
                              "
                              :disabled="true"
                ...
                <component :is="useComponentName" fixed="right" :label="$t('Msg.Action')" class="row-last-op" :width="220 + MaxRowBtnsOut">
                  <template slot-scope="scope">
                    <template v-if="scope.Row && scope.Row._IsInTableAdd == true">
                      ...
                      <el-button size="mini" icon="el-icon-close" @click="CloseIntableAdd(scope.row)">
                        {{ $t("Msg.Close") }}
                      </el-button>
                    </template>
                    <template v-else>
                      ...
                      <template v-for="(btn, btnIndex) in scope.row._RowMoreBtnsOut">
                        ...
                        >
                          ...
                          <i :class="'more-btn mr-1 ' + (DiyCommon.IsNull(btn.Icon) ? 'far fa-check-circle' : btn.Icon)"></i>{{ $t(btn.Name) }}
                        </el-button>
                      </template>
                      <el-button v-if="IsPermission('NoDetail')" size="mini" icon="el-icon-tickets" class="marginRight10" @click="OpenDetail(scope.row, 'View')">
                        {{ $t("Msg.Detail") }}
                      </el-button>
                      <!--如果子表是只读，不显示编辑等按钮 2021-01-30 && TableChild!field.Readonly-->
                      <el-dropdown v-if="TableChildFormMode != 'View' && !TableChildField.Readonly" trigger="click">
                        <el-button> {{ $t("Msg.More") }}<i class="el-icon-arrow-down el-icon--right" /> </el-button>
                        <el-dropdown-menu slot="dropdown" class="table-more-btn">
                          <el-dropdown-item v-if="LimitEdit() && TableChildFormMode != 'View'" icon="el-icon-edit" @click.native="OpenDetail(scope.row, 'Edit')">
                            {{ $t("Msg.Edit") }}
                          </el-dropdown-item>
                          ...
                          <el-dropdown-item v-if="LimitDel() && TableChildFormMode != 'View'" icon="el-icon-delete" divided @click.native="DelDiyTableRow(scope.row)">
                            {{ $t("Msg.Delete") }}
                          </el-dropdown-item>
                        </el-dropdown-menu>
                      </el-dropdown>
    ...
    >
      <div slot="title">
        ...
        <div class="pull-right">
          ...
          <el-button
            v-if="LimitDel() && TableChildFormMode !== 'View' && FormMode != 'Add' && !TableChildField.Readonly && ShowDeleteBtn && OpenDiyFormWorkFlowType.WorkType != 'StartWork'"
            :loading="BtnLoading"
            type="danger"
            size="mini"
            icon="el-icon-delete"
                ...
                <el-timeline style="padding-left: 5px">
                  ...
                  >
                    ...
                    <div v-for="(log, i2) in item.Content" :key="'datalog_content_' + log.Name" style="background-color: #e8f4ff; margin-bottom: 5px; margin-top: 5px">
                      ...
                      >： 由 <span style="color: red">{{ log.OVal }}</span> 修改为 <span style="color: red">{{ log.NVal }}</span>
                      <!-- </el-tag>     -->
                    </div>
                  </el-timeline-item>
                </el-timeline>
      ...
      <div slot="title">
        ...
        <div class="pull-right">
          ...
          >
            ...
            <el-dropdown-menu slot="dropdown" class="form-submit-btns">
              <el-dropdown-item v-if="ShowFormBottomBtns.SaveAdd" :icon="BtnLoading ? 'el-icon-loading' : 'el-icon-s-help'" :disabled="BtnLoading" @click.native="SaveDiyTableCommon(false, 'Insert')">
                {{ FormMode == "Add" || FormMode == "Insert" ? $t("Msg.AddAdd") : $t("Msg.UptAdd") }}
              </el-dropdown-item>
              <el-dropdown-item
                v-if="ShowFormBottomBtns.SaveUpdate"
                :icon="BtnLoading ? 'el-icon-loading' : 'el-icon-s-help'"
                ...
                @click.native="SaveDiyTableCommon(false, 'Update')"
              >
                {{ FormMode == "Add" || FormMode == "Insert" ? $t("Msg.AddUpdate") : $t("Msg.UptUpdate") }}
              </el-dropdown-item>
              <el-dropdown-item v-if="ShowFormBottomBtns.SaveView" :icon="BtnLoading ? 'el-icon-loading' : 'el-icon-s-help'" :disabled="BtnLoading" @click.native="SaveDiyTableCommon(false, 'View')">
                {{ FormMode == "Add" || FormMode == "Insert" ? $t("Msg.AddView") : $t("Msg.UptView") }}
              </el-dropdown-item>
          ...
          <el-button
            v-if="LimitDel() && TableChildFormMode !== 'View' && FormMode != 'Add' && ShowDeleteBtn && OpenDiyFormWorkFlowType.WorkType != 'StartWork'"
            :loading="BtnLoading"
            type="danger"
            size="mini"
            icon="el-icon-delete"
          ...
          <el-tabs v-model="FormRightType">
            ...
            <el-tab-pane v-if="CurrentDiyTableModel.EnableDataLog" label="数据日志" name="DataLog">
              <div class="datalog-timeline">
                ...
                <div v-if="DataLogListLoading" style="height: 50px; line-height: 50px"><i class="el-icon-loading"></i> {{ $t("Msg.Loading") }}</div>
...
import diyTableRowlistJs from "./diy-table-rowlist-js";
...
export default {
  ...
  mixins: [diyTableRowlistJs],
  computed: {
    isTreeTypeData() {
      return this.DiyTableRowList && this.DiyTableRowList.length > 0 && this.DiyTableRowList[0].hasOwnProperty("_Child");
    },
    useComponentName() {
      return this.isTreeTypeData ? "el-table-column" : "virtual-column";
    },
  ...
  methods: {
    ...
    TableRowSelectionChange(val) {
      var self = this;
      // var uniqueSelection = [...new Set(val)];
      // console.log('TableRowSelectionChange', uniqueSelection);
      // self.TableMultipleSelection = uniqueSelection;

      this.$nextTick(() => {
        let Selection = this.$refs["virtualScroll"].getSelection();
        const flattenTree = (arr) =>
          _.reduce(
            arr,
            (acc, item) => {
              return acc.concat(item, flattenTree(_.get(item, "_Child", [])));
            },
            []
          ).filter((item) => item.$v_checked && !item._Child);

        console.log("TableRowSelectionChange", flattenTree(Selection));
        self.TableMultipleSelection = flattenTree(Selection);
      });
    },
    ...
    async tabClickRowList(tab) {
      ...
      // await self.RunPageTabV8Code(tab.V8Code);
    ...
    GetOpenTitle() {
      ...
      //  + ' - ' + title2
      const transTitle = (str) => {
        let _str = str;
        return _str
          .split("-")
          .map((item) => this.generateTitle(item))
          .join("-");
      };
      return title1 + (!self.DiyCommon.IsNull(title3) && title3 != title2 ? " - " + transTitle(title3) : "");
    },
    ...
    TableSetData(dataObj) {
      ...
      // this.$nextTick(()=>{
      //   let tableRefName = 'diy-table-' + this.TableId
