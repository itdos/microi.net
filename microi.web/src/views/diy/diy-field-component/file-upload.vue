...
<div>
    <el-upload
        ...
        :data="{Path: '/file', Limit : field.Config.FileUpload.Limit, Preview : false}"
        ...
        :before-upload="(file) => { return BeforeFileUpload(file, field)}"
        :on-success="(result, file, fileList) => {return FileUploadSuccess(result, file, fileList, field)}"
        :on-remove="(file, fileList) => {return FileUploadRemove(file, fileList, field)}"
        >
        <i class="el-icon-upload" />
        <div class="el-upload__text">将文件拖到此处，或<em>点击上传</em></div>
        <div
            slot="tip"
            class="el-upload__tip">{{ field.Config.FileUpload.Tips }}</div>
    </el-upload>
    <!--如果是单文件，并且不等于空，则直接显示下载路径-->
    <div v-if="!field.Config.FileUpload.Multiple 
                && !DiyCommon.IsNull(FormDiyTableModel[field.Name])" 
        style="">
        {{GetUploadPath(field)}}
        <i :class="FormDiyTableModel[field.Name] == '正在上传中...' 
                ? 'el-icon-loading mr-1' 
                : 'el-icon-document mr-1'"></i>
        <a class="mr-2"
            :href="FormDiyTableModel[field.Name + '_' + field.Name + '_RealPath']" target="_blank">
    ...
    <!--如果是多文件，并且不等于空，则显示列表-->
    <template v-else-if="field.Config.FileUpload.Multiple 
                && !DiyCommon.IsNull(FormDiyTableModel[field.Name])
        ...
        >
            ...
            width="35">
                <template slot-scope="scope">
                    <i :class="scope.row.State == 0 
                        ? 'el-icon-loading' : 'el-icon-document'"></i>
                </template>
            </el-table-column>
            <el-table-column>
                <template slot-scope="scope">
                    ...
                    <a class="fileupload-a"
                        :href="FormDiyTableModel[field.Name + '_' + scope.row.Id + '_RealPath']" target="_blank">
                        {{scope.row.Name}}
                    </a>
                </template>
            </el-table-column>
