...
-->
    <el-autocomplete v-if="field.Component == 'Autocomplete'" v-model="ModelValue"
        :clearable="TableInEdit ? false : true" :disabled="GetFieldReadOnly(field)"
        :placeholder="GetFieldPlaceholder(field)" :value-key="field.Config.SelectLabel" @focus="SelectField(field)"
        @change="(item) => { return CommonV8CodeChange(item, field) }"
        @blur="(item) => { return InputOnBlur(item, field) }" @input="(item) => { return InputInputEvent(item, field) }"
        @keyup.native="FieldOnKeyup($event, field)"
        :fetch-suggestions="(queryString, cb) => { return querySearchAsync(queryString, cb, field) }"
        @select="(item) => { return handleSelect(item, field) }">
        <i v-if="!DiyCommon.IsNull(field.Config.TextIcon) && field.Config.TextIconPosition == 'right'" slot="suffix"
            :class="field.Config.TextIcon" />
        <i v-if="!DiyCommon.IsNull(field.Config.TextIcon) && field.Config.TextIconPosition == 'left'" slot="prefix"
            :class="field.Config.TextIcon" />

        <template v-if="!DiyCommon.IsNull(field.Config.TextApend) && field.Config.TextApendPosition == 'left'"
            slot="prepend">{{ field.Config.TextApend }}</template>
        <template v-if="!DiyCommon.IsNull(field.Config.TextApend) && field.Config.TextApendPosition == 'right'"
            slot="append">{{ field.Config.TextApend }}</template>
    </el-autocomplete>
</template>
        ...
        querySearchAsync(queryString, cb, field) {
            ...
            if (field.Config.DataSourceSqlRemote === true) {
                ...
                if (field.Config.DataSource == "Sql") {
                    ...
                    self.DiyCommon.Post(apiGetDiyFieldSqlData, {
                        _FieldId: field.Id,
                ...
                if (field.Config.DataSource == "DataSource") {
                    ...
                    self.DiyCommon.Post(apiDataSourceUrl, {
                        _FieldId: field.Id,
                        _SqlParamValue: JSON.stringify({}),
                        _Keyword: queryString,
