              ...
              >
                <template slot-scope="scope">
                  <el-button
                    ...
                    class="marginRight5"
                    icon="el-icon-s-help"
                    @click="$router.push('/wf/flow-design/' + scope.row.Id)"
        ...
        <el-skeleton style="width: 100%" :loading="tableLoading" animated>
          ...
          >
            <el-card class="box-card card-data-animate no-padding">
              ...
              <div class="body">
                <span class="title"
                  ><i class="el-icon-s-platform"></i> {{ model.FlowName }}</span
                >
  ...
  methods: {
    ...
    GetDiyTableRow(initPageIndex) {
      ...
      self.DiyCommon.Post(
        self.DiyApi.GetDiyTableRow,
        {
          ...
          _Where: [
            { Name: "FlowName", Value: self.SearchModel.Keyword, Type: "Like" }
          ]
        },
        function (result) {
    ...
    DelDiyTableRow(m) {
      ...
      self.DiyCommon.OsConfirm(
        self.$t("Msg.ConfirmDelTo") + "【" + m.FlowName + "】？",
        function () {
          self.DiyCommon.Post(
