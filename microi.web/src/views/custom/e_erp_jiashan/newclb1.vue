  ...
  <div style="margin: 0px 20px" class="newclb">
    <div style="display: flex; flex-wrap: wrap">
      ...
      <div style="display: flex; margin: 0 20px 10px 0">
        ...
        <el-input placeholder="请输入床次" v-model="chuangci" clearable style="width: 120px"> </el-input>
      </div>
      <!-- 缸号 -->
      <div style="display: flex; margin: 0 20px 10px 0">
        <div class="el-input-group__prepend" style="color: black; font-size: smaller; height: 31px; width: 80px; height: 28px; display: flex; align-items: center">
      ...
      <div style="display: flex; margin: 0 20px 10px 0">
        ...
        <el-select v-model="gongxuMC" clearable placeholder="请选择工序" style="width: 120px">
          <el-option v-for="item in gongxuOptions" :key="item.value" :label="item.label" :value="item.value"> </el-option>
        </el-select>
      </div>
      <!-- 起始时间 -->
      <div style="display: flex; margin: 0 20px 10px 0">
        <div class="el-input-group__prepend" style="color: black; font-size: smaller; height: 31px; width: 80px; height: 28px; display: flex; align-items: center">起始时间</div>
        <el-date-picker v-model="Date_b" type="date" placeholder="选择起始日期" style="width: 120px"> </el-date-picker>
      </div>
      <!-- 结束时间 -->
      <div style="display: flex; margin: 0 20px 10px 0">
        <div class="el-input-group__prepend" style="color: black; font-size: smaller; height: 31px; width: 80px; height: 28px; display: flex; align-items: center">结束时间</div>
        <el-date-picker v-model="Date_e" type="date" placeholder="选择结束日期" style="width: 120px"> </el-date-picker>
      </div>
      <!-- 是否显示颜色 -->
      <div style="display: flex; margin: 0 20px 10px 0">
        <div class="el-tag el-tag--info el-tag--medium el-tag--light">显示颜色</div>
        <el-checkbox v-model="showColor" @change="changeColor" size="mini" border>显示颜色</el-checkbox>
      </div>
      <!-- 是否显示尺码 -->
      <div style="display: flex; margin: 0 20px 10px 0">
        <div class="el-tag el-tag--info el-tag--medium el-tag--light">显示尺码</div>
        <div>
          <el-checkbox v-model="showChima" @change="changeChima" size="mini" border>显示尺码</el-checkbox>
        </div>
      </div>
      <!-- 按钮 -->
    ...
    <div style="height: 80vh; position: relative">
      ...
      >
        ...
        <el-table-column label="缸号" prop="ganghao" width="80"></el-table-column>
        <el-table-column label="床次" prop="chuangci" width="80"></el-table-column>
      ...
      <el-table-column label="尺码">
        ...
        <el-table-column prop="gongxu" label="工序名称" align="center" width="150"></el-table-column>
        <!-- <el-table-column prop="amount1" label="裁剪数"></el-table-column> -->
        <el-table-column prop="chanliang" label="产量" sortable align="center"></el-table-column>
        <el-table-column v-if="showChima" prop="chima" label="尺码" sortable align="center"></el-table-column>
        <el-table-column v-if="showColor" prop="yanse" label="颜色" sortable align="center"></el-table-column>
        <el-table-column prop="xiugaiSL" label="完成数" sortable align="center" width="90"></el-table-column>
        ...
        <el-table-column label="完成进度">
          <template slot-scope="scope">
            ...
            <el-progress :text-inside="true" :stroke-width="22" :percentage="calculatePercentage(scope.row) > 100 ? 100 : calculatePercentage(scope.row)" :color="customColors"></el-progress>
          </template>
        </el-table-column>
      </el-table>
    </div>
    ...
    getProcess() {
      ...
      this.DiyCommon.Post(
        ...
        function (res) {
          ...
          // self.gongxuOptions = res.Data.map((item) => ({
          //   value: item.GongxuMC,
