...
export default {
    ...
    computed: {
        ...
        getModuleType() {
            return (type) => {
                const itemType = type.replace(/( |^)[a-z]/g, (L) =>
                    L.toUpperCase()
                );
    ...
    methods: {
        moudleListDisplay(item){
            ...
            if(item.type == 'Shangjia'
                &&
                (
                    self.GetCurrentUser.TenantId ||
                    self.CurrentShangjia
                )
            ){
                return false;
            }
        ...
        GotoListPage(item, type) {
            ...
            if (item.type == "Shangjia") {
                //商家
                self.$router.push(
                    "/shangjia?_SearchDateTime=" + _searchDateTime
                );
            } else if (item.type == "Dingdan") {
                //订单
                self.$router.push(
                    "/dingdan?_SearchDateTime=" + _searchDateTime
                );
            } else if (item.type == "Shangpin") {
                //商品
                self.$router.push(
                    "/shangpin?_SearchDateTime=" + _searchDateTime
                );
                ...
                //用户
                self.$router.push(
                    "/system-account?_SearchDateTime=" + _searchDateTime
                );
            }
        },
        // 获取信息
