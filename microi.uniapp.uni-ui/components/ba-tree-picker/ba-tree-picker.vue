	...
	export default {
		...
		methods: {
			...
			_confirm() { //多选
				...
				// console.log('_confirm', selectedList,this.treeList);
				let data = this.treeList.filter(v => selectedList.includes(v.id));
				this._hide()
				this.$emit("select-change", selectedList, selectedNames, data);
			},
			//格式化原数据（原数据为tree结构）
			...
			_onItemSelect(item, index) {
				...
				// console.log(item,this.treeList)
				if (!this.multiple) { //单选
					...
					let data = this.treeList.filter(v => v.id == item.id)[0];
					this._hide()
					this.$emit("select-change", selectedList, selectedNames, data);
					return
				}
