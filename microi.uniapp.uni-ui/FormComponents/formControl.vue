<template>
  <view class="w-full bg-white rounded-lg shadow-sm">
    ...
    <view class="w-full" v-if="formTabs[current] != '' && formTabs.length > 0">
      <scroll-view id="tab-bar" class="w-full py-2 overflow-x-auto whitespace-nowrap" :scroll-x="true" :show-scrollbar="false" :scroll-into-view="scrollIntoViewId">
    ...
    <view class="w-full pr-3 pt-3 " v-if="NewformFields.length > 0">
      ...
      :label-align="DiyTableData.FormLabelPosition == 'top' ? 'left' : 'right'">
        <block v-for="(item, index) in NewformFields" :key="index">
          <view v-if="(formTabs[current] == '' || item.Tab == formTabs[current]) && (item.AppVisible && item.Visible)" 
            ...
            <uni-forms-item v-else-if="item.Component == 'AutoNumber'" :label="item.Label"
              :required="item.NotEmpty == 1 ? true : false" :name="item.Name" >
              <uni-easyinput v-model="formData[item.Name]" :placeholder="item.Placeholder" :disabled="true" 
                />
            </uni-forms-item>
            <!-- 多行文本 -->
            ...
            <uni-forms-item v-else-if="item.Component == 'Checkbox'" :label="item.Label"
              :required="item.NotEmpty == 1 ? true : false" :name="item.Name" >
              <uni-data-checkbox v-model="formData[item.Name]" multiple :localdata="item && (item.Data || [])"
            ...
            <uni-forms-item v-else-if="item.Component == 'Map'" :label="item.Label"
              ...
              <view v-if="formData[item.Name]" class="mt-1 text-xs text-gray-700 bg-gray-50 p-1 rounded-md">
            ...
            <uni-forms-item v-else-if="item.Component.includes('Select')"
              ...
              <view class="flex items-center rounded-md px-2 py-1 text-sm bg-gray-50" v-if="item.Component == 'SelectTree'" @click="handleSelect(item)">
                ...
                <uni-icons :type="formData[item.Name] ? 'clear' : 'bottom'" size="18" color="rgb(192, 196, 204)" @click.stop="handleClearSelect(item)" class="cursor-pointer" />
              </view>
              <zxz-uni-data-select v-else
            ...
            <uni-forms-item v-else-if="item.Component == 'Address'" :label="item.Label"
              :required="item.NotEmpty == 1 ? true : false" :name="item.Name" >
              <view class="flex items-center rounded-md px-2 py-1 cursor-pointer text-sm bg-gray-50 border border-gray-300" @click="handleSelect(item)">
                <view class="flex-1 text-gray-700">
                  {{ formData[item.Name] ? formData[item.Name].join('/') : item.Placeholder }}
                </view>
                <uni-icons :type="formData[item.Name] ? 'clear' : 'bottom'" size="18" color="#999" @click.stop="handleClearSelect(item)" class="cursor-pointer" />
              </view>
            </uni-forms-item>
            <!-- 图片上传 -->
            ...
            <uni-forms-item v-else-if="item.Component == 'FileUpload'" :label="item.Label"
              ...
              <view class="rounded-lg hover:border-blue-500 transition-colors duration-200">
                <uni-file-picker v-model="formData[item.Name]" file-mediatype="all"
                  ...
                  :readonly="item.Readonly == 1 ? true : false" @select="upFileSelect($event, item)" />
              </view>
              <!-- #endif -->
              <!-- #ifdef MP-LARK -->
              <view class="rounded-lg hover:border-blue-500 transition-colors duration-200">
            ...
            <uni-forms-item v-else-if="item.Component == 'Rate'" :label="item.Label"
              :required="item.NotEmpty == 1 ? true : false" :name="item.Name" >
              <uni-rate v-model="formData[item.Name]" :disabled="item.Readonly == 1 ? true : false" disabledColor="rgb(255, 202, 62)"
                class="flex" size="18" />
            </uni-forms-item>
            <!-- 分割线 -->
            ...
            <!-- 按钮 -->
            <uni-forms-item v-else-if="item.Component == 'Button'" :required="item.NotEmpty == 1 ? true : false"
              :name="item.Name" >
              <button :type="item.Config.Button.Type" :size="item.Config.Button.Size" @click="handleButton(item)"
                ...
                <uni-icons v-if="item.Config.Button.Icon" :type="item.Config.Button.Icon" size="14" class="mr-1"></uni-icons>
                {{ item.Label }}
              </button>
            </uni-forms-item>
            <!-- 组织机构选择/级联选择 -->
                  ...
                  <uni-row class="w-full" :width="nvueWidth">
                    <uni-col>
                      <table-control :ref="setTableChildRef(index)" :currentFieldsConfig="item.Config"
                        :ParentFormData="formData" isType="edit" :ParentdiyFormFields="formFields" :ParentV8="GetV8()" :ParentIndex="index"
                        :TableSearchList="TableSearchList[item.Name]" :is="item.Name"
                        @handleEdit="handleEdit($event)" @click="handleClickChild(item, index)"/>
                    </uni-col>
                  </uni-row>
                </view>
            </view>
            ...
            <uni-forms-item v-else-if="item.Component == 'Switch'" :label="item.Label"
              :required="item.NotEmpty == 1 ? true : false" :name="item.Name" >
              <switch :checked="formData[item.Name] == 1 ? true : false" :disabled="item.Readonly == 1 ? true : false"
                @change="handleSwitch($event, item)" class="scale-75 origin-left" />
            </uni-forms-item>
            <!-- 表单V8模版引擎 -->
            ...
            <uni-forms-item v-else-if="item.Component == 'RichText'" :label="item.Label"
              ...
              <view class="editor-box">
                <sp-editor
                  :editorId="item.Name"
                  :read-only="item.Readonly == 1 ? true : false"
                  :toolbar-config="{
            ...
            <!-- 文本联想输入框 -->
            <uni-forms-item v-else-if="item.Component == 'Autocomplete'" :label="item.Label" :required="item.NotEmpty == 1 ? true : false" :name="item.Name">
              <uni-auto-complete
                v-model="formData[item.Name]"
    ...
    <uni-popup ref="popupSelect" type="bottom" border-radius="16px 16px 0 0">
     <select-control :currentModel="currentModel" :currentFieldsConfig="currentFieldsConfig" :isMultiSelect="isMultiSelect" :list="selectList[selectName]" @onSelectChange="onSelectChange" :key="new Date().getTime()" />
     </uni-popup>
    <!-- 新增/编辑子表弹窗 -->
    <uni-popup ref="popupTable" type="bottom" border-radius="16px 16px 0 0">
      <view class="w-full h-screen flex items-end">
        <view class="w-full bg-white rounded-t-2xl h-4/5 relative">
          ...
          <child-table v-if="showTableChild" :currentFieldsConfig="currentFieldsConfig" :ParentFormData="formData"
      ...
      <view class="w-full h-screen flex items-end">
        <view class="w-full bg-white rounded-t-2xl h-4/5 p-3 relative">
          ...
          <view class="h-[calc(100%-100px)] overflow-auto">
            <table-control :currentFieldsConfig="currentFieldsConfig"
                      :ParentFormData="formData" isType="edit" :ParentdiyFormFields="formFields" :ParentV8="GetV8()"
                      :TableSearchList="TableSearchList[currentModel.Name]" :TableSearchWhere="TableSearchWhere[currentModel.Name]"
                      :isOpenTable="true"
                       :key="new Date().getTime()"
                      />
          </view>
...
<script setup>
import { ref, reactive, inject, onMounted, watch, nextTick, provide, computed } from 'vue'
...
const upinImage = (tempFiles, editorCtx, id) => {
  ...
  tempFiles.forEach(async (file) => {
    const res = await Microi.Upload({'File': file.path }, function(){},{DataType: "form"})
...
defineExpose({ valiForm, formData });
