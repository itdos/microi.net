<template>
	<view class="work">
				...
				<block v-for="(tab, index) in formTabs" :key="tab.id">
					<view class="uni-tab-item" :id="'tab-' + index" :data-current="index" @click="onClickItem(index)">
						<text class="uni-tab-item-title"
							:class="current == index ? 'uni-tab-item-title-active' : ''">{{ typeof(tab) == 'string' ? tab : (tab.Name == 'none' ? '' : tab.Name) }}</text>
					</view>
				</block>
			</scroll-view>
		</view>
		<view v-for="(field, index) in diyFormFields" :key="field.Id">
			<view v-if="
          ((formTabs[current] == '' 
			|| field.Tab == formTabs[current]
			|| field.Tab == formTabs[current].Id
			|| field.Tab == formTabs[current].Name
			) 
			&&
            field.AppVisible &&
            field.Visible &&
            (!FieldsConfig.length ||
              FieldsConfig.some((item) => item.Id === field.Id))) ||
          type == 'list'
        ">
				<!-- 是否需要表内编辑 -->
				<view v-if="
            InTableEdit &&
            InTableEditFields.includes(field.Id) &&
            type == 'edit'
          ">
					<view class="uni-flex work-item">
						<view class="work-item-label" :class="{ 'text-red': field.NotEmpty }">{{ field.Label }}</view>
						<table-edit :field="field" :data="detail" :DiyTableId="DiyTableId"
							:diyFormFields="diyFormFields" :Guid="Guid" :currenTotal="currenTotal"
							:key="new Date().getTime()" />
					</view>
				</view>
				<template v-else>
					<!-- 如果是子表 -->
					<view v-if="field.Component == 'TableChild'" class="uni-common-mt-xs uni-common-mb-xs"
						:id="field.Name" :class="{ 'uni-flex': type == 'list' }">
						...
						<uni-row class="table-child" :width="730" v-else>
							<uni-col>
								...
								<table-control :ref="setTableChildRef(index)" :currentFieldsConfig="field.Config"
									:ParentFormData="detail" :isType="InTableEdit ? 'edit' : 'view'"
									:ParentdiyFormFields="diyFormFields" :ParentV8="GetV8()" :ParentIndex="index"
					...
					<view v-else-if="field.Component == 'ImgUpload'" class="uni-flex work-item">
						...
						<view class="uni-flex uni-flex-wrap">
							<block v-for="(item, index1) in detail[field.Name]" :key="item.Id">
								<image slot="cover" :src="item.url" class="item-Img"
									@click.stop="previewImg(detail[field.Name], index1)" />
							</block>
						</view>
					</view>
					<!-- 如果是文件 -->
					<view v-else-if="field.Component == 'FileUpload'" class="border-b border-gray-100 py-2">
						<view class="work-item-label">{{ field.Label }}</view>
						<view v-for="(item, index_file) in detail[field.Name]" :key="item.Id"
							class="border border-blue-500 p-2 rounded-md border-dashed mt-2"
					...
					<!-- 如果是开关 -->
					<view v-else-if="field.Component == 'Switch'"
						class="uni-flex items-center border-b border-gray-100 py-2">
						<view class="work-item-label">{{ field.Label }}</view>
						<switch :checked="detail[field.Name] == 1 ? true : false" disabled
							style="transform: scale(0.7)"></switch>
					</view>
					<!-- 如果是组织机构 -->
					<view class="work-item" v-else-if="
              field.Component == 'Department' || field.Component == 'Cascader'
            ">
						<view class="work-item-label">{{ field.Label }}</view>
						<m-cascader v-model="detail[field.Name]" :options="field.Data"
							:multiple="field.Config.Department.Multiple" :fieldProps="{
                value: field.Config.SelectSaveField || 'Id',
                label: field.Config.SelectLabel || 'Name',
                children: '_Child',
              }" placeholder="请选择" :save-full-path="field.Config.Department.EmitPath" :readonly="true"
							:show-border="false" />
					</view>
					<!-- 按钮模式 -->
                ...
                borderColor: '#ccc',
					...
					<view class="uni-common-mt" v-else-if="field.Component == 'Map' && type != 'list'">
						<view class="work-item-label uni-common-mb-xs">{{
              field.Label
            }}</view>
						<view v-if="detail[`${field.Name}_Lat`] && detail[`${field.Name}_Lng`]">
							<map :latitude="detail[`${field.Name}_Lat`]" :longitude="detail[`${field.Name}_Lng`]"
								:markers="[
                  {
					...
					<view class="work-item" v-else>
						...
						<view class="item-value uni-flex">
							...
							<!-- 复合文字 -->
							<view v-if="field.Config && field.Config.TextApend">{{
			...
			<view class="popup-box">
				<view class="popup-content uni-common-pb popup-content-child">
					...
					<child-table v-if="showTableChild" :currentFieldsConfig="currentFieldsConfig"
						:ParentFormData="detail" :isType="isChildType" :ChildFormId="ChildFormId"
						:ChildDiyTableId="ChildDiyTableId" :key="new Date().getTime()" @ChildSubmitOk="ChildSubmitOk"
						:isChildTable="true" />
				</view>
			</view>
		</uni-popup>
...
<script setup>
	...
	// 修复：使用computed来保持响应式，而不是直接修改props
	const diyFormFields = computed(() => {
		if (!props.diyFormFields || !props.diyFormFields.length) return []

		// 深拷贝避免直接修改props
		const fields = JSON.parse(JSON.stringify(props.diyFormFields))

		// 如果有表单分组配置，处理Tab属性
		if (props.DiyTableData && props.DiyTableData.Tabs) {
			const tabs = JSON.parse(props.DiyTableData.Tabs)
			const tabNames = tabs.map((item) => {
	...
	const tableChildRefs = computed(() => diyFormFields.value.map(() => ref(null))) // 创建一个引用数组

	// 动态设置ref的函数
	const setTableChildRef = (index) => {
		return (el) => {
			if (el) tableChildRefs.value[index].value = el // 将组件实例存储到ref数组中
		}
	}
	// 移除这段代码，因为已经在computed中处理了
	// 点击分组切换
	const onClickItem = (e) => {
		current.value = e
	...
	const handleClickChild = (item, index) => {
		...
		currentModel = item
		tableChild.value = tableChildRefs.value[index].value
	...
	onMounted(() => {
		// 初始化逻辑已经在computed中处理，这里可以添加其他初始化代码
	})
</script>
