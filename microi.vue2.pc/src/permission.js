...
router.beforeEach(async (to, from, next) => {
    ...
    for (let index = 0; index < diySsoArray.length; index++) {
        ...
        if ((token && token !== lastSsoToken) || (token && !DiyCommon.getToken())) {// && token != DiyCommon.getToken()
            ...
            var ssoApiResult = await DiyCommon.PostAsync(diySso.ClientSsoApi, {//'/api/SysUser/SsoPengrui'
                _token: token,
                Token: token,
