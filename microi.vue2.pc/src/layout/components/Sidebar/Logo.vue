<template>
    <div
        ...
        :style="{background :  SysConfig.MenuBg == 'Custom' && SysConfig.LogoBgColor ? SysConfig.LogoBgColor : '#fff'}"
    >
        <transition name="sidebarLogoFade">
            ...
            >
                ...
                <h1
                    ...
                    :style="{color : SysConfig.SysTitleColor ? SysConfig.SysTitleColor : '#000'}"
                >
                    {{
                        !DosCommon.IsNull(SysConfig.SysShortTitle)
                            ? SysConfig.SysShortTitle
                            : DosCommon.IsNull(ShortTitle)
                            ? WebTitle
                            : ShortTitle
                    }}
                </h1>
...
export default {
    ...
    methods: {
        GetSysLogo() {
            var self = this;
            if (!self.DosCommon.IsNull(self.SysConfig.SysLogo)) {
		...
		GetSysLogoHeight(){
			var self = this;
			if (!self.DosCommon.IsNull(self.SysConfig.SysLogoHeight)) {
                return self.SysConfig.SysLogoHeight + 'px';
            }
			return '40px';
		},
		IsDisplayShortTitle(){
            var self = this;
			if (!self.DosCommon.IsNull(self.SysConfig.SysLogoType)) {
		...
		GetSysLogoLink(){
			var self = this;
			if (!self.DosCommon.IsNull(self.SysConfig.SysLogoLink)) {
