...
var DiyOsClient = {
    ...
    OsClientInit: async function (init) {
        ...
        if (DiyOsClient.GetOsClientNotDomain() == "") {
            ...
            if (getOsClientByDomainResult.Code == 1 && !DosCommon.IsNull(getOsClientByDomainResult.Data)) {
                var osClient = getOsClientByDomainResult.Data.OsClient;
        ...
        if (!DosCommon.IsNull(sysConfig)) {
            ...
            //设置cdn相关
            if(!DosCommon.IsNull(sysConfig.FileServer)){
                store.commit('DiyStore/SetState', {
                    key : 'FileServer',
                    value : sysConfig.FileServer
                })
            }else{
            ...
            if(!DosCommon.IsNull(sysConfig.MediaServer)){
                store.commit('DiyStore/SetState', {
                    key : 'MediaServer',
            ...
            if (!DosCommon.IsNull(desktopBg)) {
                try {
                    desktopBg = JSON.parse(desktopBg)
                    if (!DosCommon.IsNull(desktopBg.BgVersion) &&
                        !DosCommon.IsNull(desktopBg.BgVersion[osClient]) &&
                        !DosCommon.IsNull(desktopBg.OsClient) &&
                        ...
                        // 设置主题色
                        if (!DosCommon.IsNull(desktopBg.Color)) {
            ...
            if (DosCommon.IsNull(localStorage.lang)) {
                DiyCommon.ChangeLang(DosCommon.IsNull(sysConfig.SysLang) ? 'cn' : sysConfig.SysLang, true)
            } else {
                DiyCommon.ChangeLang(localStorage.lang, true)
            }
            DiyCommon.SetWebTitle(DosCommon.IsNull(sysConfig.SysTitle) ? 'Loading...' : sysConfig.SysTitle)
            DiyCommon.SetShortTitle(DosCommon.IsNull(sysConfig.SysShortTitle) ? 'Loading...' : sysConfig.SysShortTitle)
            DiyCommon.SetClientCompany(DosCommon.IsNull(sysConfig.CompanyName) ? 'Loading...' : sysConfig.CompanyName)
            if (!DosCommon.IsNull(sysConfig.FaviconIco)) {
            ...
            if (DosCommon.IsNull(store.state.DiyStore.DesktopBg.ImgUrl) || init === true) {
                store.dispatch('DiyStore/SetDesktopBg', {
                    ...
                    LockImgUrl: DiyCommon.GetServerPath(DosCommon.IsNull(sysConfig.LoginBgImg) 
    ...
    GetApiBase: function () {
        ...
        if(!DosCommon.IsNull(localStorage.getItem('DiyApiBase'))){
    ...
    GetOsClientNotDomain: function () {
        ...
        if(!DosCommon.IsNull(localStorage.getItem('OsClient'))){
