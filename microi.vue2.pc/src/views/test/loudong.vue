...
<div class="input-group input-group-sm input-group-selects">
    ...
    <el-select class="custom-select" placeholder="" value-key="Id" v-model="Dong" :disabled="DosCommon.IsNull(XiaoquID)" @change="SelectDong">
        <el-option :value="dong" :label="dong.Name" v-for="(dong, i) in DongList" :key="'Dong_' + i">
        </el-option>
    </el-select>
    ...
    <el-select class="custom-select center" placeholder="" value-key="Id" v-model="Danyuan" :disabled="DosCommon.IsNull(Dong)" @change="SelectDanYuan">
        <el-option :label="item.DanyuanMC" :value="item" v-for="(item, i) in DanyuanData" :key="'dangyuan_' + i">
        </el-option>
    </el-select>
    <div class="input-group-append">
    ...
    <el-select class="custom-select center" placeholder="" value-key="Id" v-model="Lou" :disabled="DosCommon.IsNull(Danyuan)" @change="SelectLouCeng">
    ...
    <el-select class="custom-select center" value-key="Id" placeholder="" v-model="Fanghao" :disabled="DosCommon.IsNull(Lou)" @change="SelectFangHao">
...
export default {
    ...
    watch: {
        ...
        DataAppend: function (newVal, oldVal) {
            ...
            if (!self.DosCommon.IsNull(newVal)) {
                if(!self.DosCommon.IsNull(newVal.HourseDirId)){
                    //根据楼盘字段Id，重新获取所有楼栋
                    self.GetDong(newVal.HourseDirId);
                }
                if(!self.DosCommon.IsNull(newVal.Dong)){
                    self.Dong = newVal.Dong;
                }
                if(!self.DosCommon.IsNull(newVal.Danyuan)){
                    self.Danyuan = newVal.Danyuan;
                }
                if(!self.DosCommon.IsNull(newVal.Louceng)){
                    self.Lou = newVal.Louceng;
                }
                if(!self.DosCommon.IsNull(newVal.Fanghao)){
    ...
    methods: {
        ...
        GetDong(xiaoquId) {
            ...
            //改成模拟数据，不从接口获取
            self.DongList = [{Id : '1', Name : 'A'}, {Id : '2', Name : 'B'}];
        ...
        GetDanYuan() {
            var self = this;
            //改成模拟数据，不从接口获取
            self.DanyuanData = [{Id: '1', DanyuanMC : '1'}, {Id: '2', DanyuanMC : '2'}];
            // self.DiyCommon.GetDiyTableRow({
        ...
        GetLouCeng() {
            var self = this;
            //改成模拟数据，不从接口获取
            self.LouData = [{Id: '1', LoucengMC : '2'}, {Id: '2', LoucengMC : '1'}, {Id: '3', LoucengMC : '-1'}];
            // self.DiyCommon.GetDiyTableRow({
        ...
        GetFangHao() {
            var self = this;
            //改成模拟数据，不从接口获取
            self.FanghaoData = [{Id :'1', Fanghao : '101'}, {Id :'2', Fanghao : '102'}];
            // self.DiyCommon.GetDiyTableRow({
