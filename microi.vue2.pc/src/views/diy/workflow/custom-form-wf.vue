<template>
    <div class="custom-form-wf">
        ...
        <div class="pull-right container-right">
            <WFWorkHandler ref="refWfWorkHandler_3"
                ...
                @CallbackRecallWork="CallbackRecallWork"
    ...
    export default {
        ...
        methods: {
            ...
            /**
             * 撤回工作，其实不需要提交表单，无法修改表单。但需要执行节点开始V8、结束V8。
             * @param {*} param 
             */
            async CallbackRecallWork(param, callback){
                var self= this;
                try {
                    //-------第1步：在表单提交前，先执行节点开始v8。 
                    //此v8说明：
                    ...
                    //-------第2步：撤回工作
                    self.$refs.refWfWorkHandler_3.RecallWork({
                        FormData : v8Result.Form,
                    }, function(result){
                        if (result.Code == 1) {
                            self.$emit('CallbackWFSubmit', {Code : 1});
