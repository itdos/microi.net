    ...
    data.forEach(item => {
        try {
            ...
            if (isFater) {
                ...
                //如果没有下级，或只有一个下级
                if (
                    DosCommon.IsNull(item._Child) 
                    || item._Child.length == 0 
                    || (item._Child.length == 1 && (DosCommon.IsNull(item._Child[0]._Child) || item._Child[0]._Child.length == 0))
                    ) {
            ...
            //如果不是第一级
                ...
                //如果没有下级，或只有一个下级
                if (DosCommon.IsNull(item._Child)
                    || item._Child.length == 0 
                    ) {
