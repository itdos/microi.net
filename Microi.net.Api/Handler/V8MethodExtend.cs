...
public partial class V8MethodExtend : IV8MethodExtend
{
    ...
    {
        var methodInfo = GetType().GetMethod(methodName, BindingFlags.Public | BindingFlags.Instance);
        if (methodInfo != null)
        {
            return methodInfo.Invoke(this, args);
        }

        throw new ArgumentException($"V8.Method '{methodName}' not found.");
    }
    
    public string TestV8Extend(string testParam)
    {
        return "TestV8Extend：" + testParam;
    }
